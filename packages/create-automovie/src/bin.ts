 ...
 * @evidenceExclude requirements/agent-authoring/capability-discovery.md#agent-topic-document-discovery Topic routing is performed by the generated documentation and MCP host, not by this argv adapter.
 ...
 * @evidenceExclude requirements/agent-authoring/deterministic-precomputation.md#agent-precomputed-closed-basis The one-shot creator publishes scaffold bytes and never runs a generator, seals a dependency basis, refuses a stale result, or separates derived output from external provenance; the generated project's own scripts and its MCP compiler own that ledger.
 ...
 * @evidenceExclude requirements/agent-authoring/mcp-boundary.md#agent-mcp-contract-guidance Project creation publishes host configuration but does not answer project-context contract or technique queries.
 * @evidenceExclude requirements/agent-authoring/mcp-boundary.md#agent-mcp-provider-neutrality This local creator invokes no external provider and presents no external execution alternatives or provenance.
 * @evidenceExclude requirements/agent-authoring/mcp-boundary.md#agent-mcp-host-evidence The adapter produces no capture, analysis, diagnostic, or review receipt from an executed film target.
 * @evidenceExclude requirements/agent-authoring/mcp-boundary.md#agent-mcp-authoring-api-refusal Source authoring state is owned by the generated project and host; this one-shot creator exposes no MCP authoring API.
 * @evidenceExclude requirements/agent-authoring/mcp-boundary.md#agent-mcp-no-surprise-external-effects No knowledge or evidence request reaches this creator, so it owns no external-execution authorization boundary.
 * @evidenceExclude requirements/agent-authoring/mcp-boundary.md#agent-mcp-content-supply-refusal The creator publishes technique examples but does not serve completed assets in response to MCP requests.
