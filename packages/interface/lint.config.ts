import { type ITtscEvidenceGraphConfig, evidence } from "@ttsc/evidence";
import type { ITtscLintConfig } from "@ttsc/lint";

const H2_ONLY_CONTENT_DOCUMENTS = new Set([
  "specifications/editorial-render-and-delivery/delivery-package-provenance-and-publication.md",
  "specifications/editorial-render-and-delivery/delivery-profiles-time-and-picture.md",
  "specifications/editorial-render-and-delivery/delivery-validation-and-release-status.md",
  "specifications/editorial-render-and-delivery/editorial-audiovisual-continuity.md",
  "specifications/editorial-render-and-delivery/editorial-version-conform-and-validation.md",
  "specifications/editorial-render-and-delivery/rational-timeline-and-composition.md",
  "specifications/editorial-render-and-delivery/render-budget-identity-and-recovery.md",
  "specifications/editorial-render-and-delivery/render-encoding-and-validation.md",
  "specifications/editorial-render-and-delivery/render-products-visibility-and-color.md",
  "specifications/editorial-render-and-delivery/render-schedule-state-and-headless.md",
  "specifications/interior-space/boundaries-openings-and-circulation.md",
  "specifications/interior-space/deliverables-and-validation.md",
  "specifications/interior-space/elements-furnishing-and-clearance.md",
  "specifications/interior-space/lighting-acoustics-and-environment.md",
  "specifications/interior-space/materials-style-and-art.md",
  "specifications/interior-space/scope-and-host.md",
  "specifications/interior-space/services-wet-and-fluid.md",
  "specifications/interior-space/space-level-zone-topology.md",
  "specifications/interior-space/surface-assemblies.md",
]);

const contractClaim = (
  name: string,
  files: string[],
  requirements: string[],
  specifications: string[],
): ITtscEvidenceGraphConfig["claims"][number] => {
  const references = (documents: string[]) =>
    documents.map((file) => ({
      type: "markdown" as const,
      root: "../../docs",
      files: [file],
      symbol: file.endsWith("/README.md")
        ? (["h1", "h2", "h3"] as Array<"h1" | "h2" | "h3">)
        : H2_ONLY_CONTENT_DOCUMENTS.has(file)
          ? (["h2"] as Array<"h2">)
          : (["h3"] as Array<"h3">),
    }));

  return {
    name,
    type: "typescript",
    files,
    symbol: ["type", "function", "property"],
    reference: [...references(requirements), ...references(specifications)],
  };
};

/**
 * Public interface contracts answer only for exact documentary files that the
 * selected source modules collectively represent.
 */
const graph: ITtscEvidenceGraphConfig = {
  claims: [
    contractClaim(
      "analysis result and diagnostic schemas",
      [
        "src/analysis/IAutoMovieAnalysisReport.ts",
        "src/analysis/IAutoMovieAnalysisRun.ts",
      ],
      [
        "requirements/acceptance/README.md",
        "requirements/acceptance/criteria-and-observables.md",
        "requirements/acceptance/evidence-and-freshness.md",
        "requirements/acceptance/profiles-and-aggregation.md",
        "requirements/acceptance/uncertainty-and-partial-success.md",
        "requirements/diagnostics/README.md",
        "requirements/diagnostics/collection-fail-fast-and-determinism.md",
        "requirements/diagnostics/input-and-result-classification.md",
      ],
      [
        "specifications/review-and-acceptance/README.md",
        "specifications/review-and-acceptance/criteria-tolerance-and-comparison.md",
        "specifications/review-and-acceptance/evidence-freshness-and-completeness.md",
        "specifications/review-and-acceptance/profiles-aggregation-and-partial-results.md",
        "specifications/validation-and-diagnostics/README.md",
        "specifications/validation-and-diagnostics/classification-and-causality.md",
        "specifications/validation-and-diagnostics/collection-order-and-termination.md",
      ],
    ),
    contractClaim(
      "declared environment context schemas",
      ["src/analysis/IAutoMovieEnvironmentContext.ts"],
      [
        "requirements/building-exterior/README.md",
        "requirements/building-exterior/site-placement-and-orientation.md",
        "requirements/effects-and-simulation/README.md",
        "requirements/effects-and-simulation/environment-coupling.md",
        "requirements/lighting/README.md",
        "requirements/lighting/sun-sky-and-environment.md",
      ],
      [
        "specifications/building-envelope/README.md",
        "specifications/building-envelope/services-water-weather-and-site.md",
        "specifications/camera-light-and-visibility/README.md",
        "specifications/camera-light-and-visibility/light-source-photometry-and-environment.md",
      ],
    ),
    contractClaim(
      "built-space topology and envelope schemas",
      ["src/architecture/IAutoMovieBuiltEnvironment.ts"],
      [
        "requirements/building-exterior/README.md",
        "requirements/building-exterior/coordinates-and-shared-boundaries.md",
        "requirements/building-exterior/external-circulation-and-attached-elements.md",
        "requirements/building-exterior/facades-and-walls.md",
        "requirements/building-exterior/massing-area-and-height.md",
        "requirements/building-exterior/openings-and-fenestration.md",
        "requirements/building-exterior/scope-and-building-identity.md",
        "requirements/building-exterior/storeys-levels-and-heights.md",
        "requirements/building-exterior/structure-and-envelope.md",
        "requirements/building-exterior/validation-and-interior-consistency.md",
        "requirements/interior/README.md",
        "requirements/interior/acoustics-and-sound-boundaries.md",
        "requirements/interior/connections-and-circulation.md",
        "requirements/interior/doors-windows-and-openings.md",
        "requirements/interior/scope-and-host-boundary.md",
        "requirements/interior/spaces-and-occupancy.md",
        "requirements/interior/spatial-hierarchy-and-zones.md",
        "requirements/interior/surface-assemblies.md",
        "requirements/interior/walls-partitions-and-linings.md",
      ],
      [
        "specifications/building-envelope/README.md",
        "specifications/building-envelope/exterior-spaces-circulation-and-optics.md",
        "specifications/building-envelope/facade-roof-and-openings.md",
        "specifications/building-envelope/identity-scope-and-coordinates.md",
        "specifications/building-envelope/linked-interior-coordination.md",
        "specifications/building-envelope/massing-area-and-storeys.md",
        "specifications/building-envelope/structure-envelope-and-materials.md",
        "specifications/interior-space/README.md",
        "specifications/interior-space/boundaries-openings-and-circulation.md",
        "specifications/interior-space/lighting-acoustics-and-environment.md",
        "specifications/interior-space/scope-and-host.md",
        "specifications/interior-space/space-level-zone-topology.md",
        "specifications/interior-space/surface-assemblies.md",
      ],
    ),
    contractClaim(
      "design reference and lineage schemas",
      [
        "src/architecture/IAutoMovieDesignLineage.ts",
        "src/architecture/IAutoMovieDesignReference.ts",
      ],
      [
        "requirements/evidence-and-provenance/README.md",
        "requirements/evidence-and-provenance/entities-activities-agents-and-lineage.md",
        "requirements/evidence-and-provenance/generation-transformation-and-derivation.md",
        "requirements/evidence-and-provenance/observations-claims-and-human-judgments.md",
        "requirements/external-inputs/README.md",
        "requirements/external-inputs/source-selection-and-provider-neutrality.md",
        "requirements/production-design/README.md",
        "requirements/production-design/continuity-change-and-deliverables.md",
        "requirements/production-design/references-and-provenance.md",
      ],
      [
        "specifications/evidence-and-provenance/README.md",
        "specifications/evidence-and-provenance/entities-activities-agents-and-lineage.md",
        "specifications/evidence-and-provenance/generation-transformation-and-derivation.md",
        "specifications/evidence-and-provenance/observations-claims-and-human-judgments.md",
        "specifications/interchange-and-adoption/README.md",
        "specifications/interchange-and-adoption/intake-authority-and-routing.md",
        "specifications/narrative-and-intent/README.md",
        "specifications/narrative-and-intent/fidelity-references-and-provenance.md",
      ],
    ),
    contractClaim(
      "agent-authored stage and shot-program schemas",
      [
        "src/authoring/*.ts",
        "src/production/application/IAutoMovieGetGuideDocument.ts",
      ],
      [
        "requirements/agent-authoring/README.md",
        "requirements/agent-authoring/capability-discovery.md",
        "requirements/agent-authoring/partial-work.md",
        "requirements/agent-authoring/project-ownership.md",
        "requirements/agent-authoring/roles-and-authorities.md",
        "requirements/agent-authoring/source-owned-loop.md",
        "requirements/staging/README.md",
        "requirements/staging/scope-and-source-of-truth.md",
        "requirements/staging/shot-contracts-and-deliveries.md",
      ],
      [
        "specifications/authoring-and-authority/README.md",
        "specifications/authoring-and-authority/capability-and-content-boundary.md",
        "specifications/authoring-and-authority/delegation-and-decision-authority.md",
        "specifications/authoring-and-authority/partial-targets-and-atomic-results.md",
        "specifications/authoring-and-authority/source-authority-and-derivation.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/staging-events-coverage-and-validation.md",
      ],
    ),
    contractClaim(
      "shot, camera-intent, and interaction schemas",
      [
        "src/cinematics/AutoMovieGuidePass.ts",
        "src/cinematics/IAutoMovieInteractionEvent.ts",
        "src/cinematics/IAutoMoviePoseKeypoint.ts",
        "src/cinematics/IAutoMovieShot.ts",
      ],
      [
        "requirements/camera/README.md",
        "requirements/camera/axis-eyeline-and-screen-direction.md",
        "requirements/camera/framing-and-shot-size.md",
        "requirements/camera/position-and-movement.md",
        "requirements/staging/README.md",
        "requirements/staging/events-and-timing.md",
        "requirements/staging/shot-contracts-and-deliveries.md",
        "requirements/staging/visibility-and-readability.md",
      ],
      [
        "specifications/camera-light-and-visibility/README.md",
        "specifications/camera-light-and-visibility/camera-state-projection-and-gate.md",
        "specifications/camera-light-and-visibility/framing-axis-and-camera-path.md",
        "specifications/camera-light-and-visibility/visibility-and-image-space-observation.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/staging-events-coverage-and-validation.md",
      ],
    ),
    contractClaim(
      "sequence, trim, and transition schemas",
      [
        "src/cinematics/IAutoMovieSequence.ts",
        "src/cinematics/IAutoMovieTransition.ts",
        "src/cinematics/IAutoMovieTrim.ts",
      ],
      [
        "requirements/editorial/README.md",
        "requirements/editorial/clips-source-ranges-and-handles.md",
        "requirements/editorial/rational-time-and-ranges.md",
        "requirements/editorial/tracks-stacks-and-composition.md",
        "requirements/editorial/transitions-and-overlaps.md",
      ],
      [
        "specifications/editorial-render-and-delivery/README.md",
        "specifications/editorial-render-and-delivery/editorial-audiovisual-continuity.md",
        "specifications/editorial-render-and-delivery/editorial-version-conform-and-validation.md",
        "specifications/editorial-render-and-delivery/rational-timeline-and-composition.md",
      ],
    ),
    contractClaim(
      "frame-format and render-request schemas",
      [
        "src/cinematics/IAutoMovieRenderFrameFormat.ts",
        "src/cinematics/IAutoMovieRenderSpec.ts",
      ],
      [
        "requirements/delivery-and-accessibility/README.md",
        "requirements/delivery-and-accessibility/frame-rate-timebase-and-timecode.md",
        "requirements/delivery-and-accessibility/picture-color-and-image-sequences.md",
        "requirements/rendering/README.md",
        "requirements/rendering/frame-schedules-and-sampling.md",
        "requirements/rendering/passes-channels-and-products.md",
        "requirements/rendering/scope-and-artifact-identity.md",
      ],
      [
        "specifications/editorial-render-and-delivery/README.md",
        "specifications/editorial-render-and-delivery/delivery-profiles-time-and-picture.md",
        "specifications/editorial-render-and-delivery/render-products-visibility-and-color.md",
        "specifications/editorial-render-and-delivery/render-schedule-state-and-headless.md",
      ],
    ),
    contractClaim(
      "color value schemas",
      ["src/color/*.ts"],
      [
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/materials-and-textures.md",
        "requirements/lighting/README.md",
        "requirements/lighting/color-exposure-and-display-boundary.md",
        "requirements/rendering/README.md",
        "requirements/rendering/materials-lighting-and-color.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/model-geometry-and-surface-facts.md",
        "specifications/camera-light-and-visibility/README.md",
        "specifications/camera-light-and-visibility/light-transport-color-and-budget.md",
        "specifications/editorial-render-and-delivery/README.md",
        "specifications/editorial-render-and-delivery/render-products-visibility-and-color.md",
      ],
    ),
    contractClaim(
      "node identity schemas",
      [
        "src/core/AutoMovieNodeKind.ts",
        "src/core/IAutoMovieNamedId.ts",
        "src/core/IAutoMovieNode.ts",
      ],
      [
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/identity-and-instances.md",
        "requirements/asset-authoring/rig-and-state.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/identity-resources-and-lifecycle.md",
        "specifications/asset-and-representation/rig-deformation-and-state.md",
      ],
    ),
    contractClaim(
      "channel, curve, driver, and track schemas",
      [
        "src/core/AutoMovieChannelValueType.ts",
        "src/core/AutoMovieInterpolation.ts",
        "src/core/IAutoMovieChannel.ts",
        "src/core/IAutoMovieChannelLimit.ts",
        "src/core/IAutoMovieDrivenCurve.ts",
        "src/core/IAutoMovieDriver.ts",
        "src/core/IAutoMovieTrack.ts",
      ],
      [
        "requirements/motion/README.md",
        "requirements/motion/channels-controls-and-drivers.md",
        "requirements/motion/clips-keyframes-and-interpolation.md",
      ],
      [
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/motion-sampling-and-composition.md",
      ],
    ),
    contractClaim(
      "capability and profile declaration schemas",
      ["src/core/IAutoMovieCapability.ts", "src/core/IAutoMovieProfile.ts"],
      [
        "requirements/acceptance/README.md",
        "requirements/acceptance/profiles-and-aggregation.md",
        "requirements/agent-authoring/README.md",
        "requirements/agent-authoring/capability-discovery.md",
        "requirements/product/README.md",
        "requirements/product/authorability.md",
        "requirements/product/capability-and-content.md",
        "requirements/product/extensibility-and-compatibility.md",
      ],
      [
        "specifications/authoring-and-authority/README.md",
        "specifications/authoring-and-authority/capability-and-content-boundary.md",
        "specifications/review-and-acceptance/README.md",
        "specifications/review-and-acceptance/profiles-aggregation-and-partial-results.md",
      ],
    ),
    contractClaim(
      "architectural drawing and quantity schemas",
      ["src/drawing/*.ts"],
      [
        "requirements/building-exterior/README.md",
        "requirements/building-exterior/deliverables.md",
        "requirements/interior/README.md",
        "requirements/interior/deliverables-and-quantities.md",
        "requirements/interior/validation-and-iteration.md",
      ],
      [
        "specifications/building-envelope/README.md",
        "specifications/building-envelope/phases-deliverables-and-validation.md",
        "specifications/interior-space/README.md",
        "specifications/interior-space/deliverables-and-validation.md",
      ],
    ),
    contractClaim(
      "expression schemas",
      ["src/expression/*.ts"],
      [
        "requirements/actors/README.md",
        "requirements/actors/appearance-costume-and-attachments.md",
        "requirements/actors/pose-expression-and-gaze.md",
        "requirements/actors/representation-tiers-and-fidelity-boundary.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/rig-deformation-and-state.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/actor-identity-state-and-fidelity.md",
        "specifications/performance-motion-and-staging/rig-deformation-and-retargeting.md",
      ],
    ),
    contractClaim(
      "face parameter, template, and morph proxy schemas",
      ["src/face/*.ts"],
      [
        "requirements/actors/README.md",
        "requirements/actors/appearance-costume-and-attachments.md",
        "requirements/actors/pose-expression-and-gaze.md",
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/rig-and-state.md",
        "requirements/asset-authoring/validation.md",
        "requirements/product/README.md",
        "requirements/product/scope-and-exclusions.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/fidelity-and-validation.md",
        "specifications/asset-and-representation/rig-deformation-and-state.md",
        "specifications/authoring-and-authority/README.md",
        "specifications/authoring-and-authority/prototype-determinism-and-fidelity.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/actor-identity-state-and-fidelity.md",
      ],
    ),
    contractClaim(
      "fluid domain and water-feature schemas",
      ["src/fluid/*.ts"],
      [
        "requirements/effects-and-simulation/README.md",
        "requirements/effects-and-simulation/fluids-and-water.md",
        "requirements/interior/README.md",
        "requirements/interior/water-and-fluid-features.md",
        "requirements/map/README.md",
        "requirements/map/rivers-and-inland-water.md",
      ],
      [
        "specifications/interior-space/README.md",
        "specifications/interior-space/services-wet-and-fluid.md",
        "specifications/simulation-effects-and-sound/README.md",
        "specifications/simulation-effects-and-sound/fluids-water-and-world-coupling.md",
        "specifications/world-and-site/README.md",
        "specifications/world-and-site/hydrology-coast-and-groundwater.md",
      ],
    ),
    contractClaim(
      "coordinate and transform schemas",
      ["src/geometry/*.ts"],
      [
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/geometry.md",
        "requirements/asset-authoring/representations-bounds-and-lod.md",
        "requirements/external-inputs/README.md",
        "requirements/external-inputs/identity-coordinates-and-units.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/bounds-proxies-and-lod.md",
        "specifications/asset-and-representation/model-geometry-and-surface-facts.md",
        "specifications/interchange-and-adoption/README.md",
        "specifications/interchange-and-adoption/identity-coordinates-and-units.md",
      ],
    ),
    contractClaim(
      "script, action, target, and slate schemas",
      ["src/harness/*.ts"],
      [
        "requirements/actors/README.md",
        "requirements/actors/performance-and-story-binding.md",
        "requirements/camera/README.md",
        "requirements/camera/targets-focus-and-depth-boundary.md",
        "requirements/motion/README.md",
        "requirements/motion/object-motion-and-interaction.md",
        "requirements/motion/timing-and-semantic-events.md",
        "requirements/staging/README.md",
        "requirements/staging/events-and-timing.md",
        "requirements/staging/interactions-and-choreography.md",
        "requirements/staging/marks-zones-and-blocking.md",
        "requirements/story/README.md",
        "requirements/story/beats-and-causality.md",
        "requirements/story/dialogue-language-and-silence.md",
        "requirements/story/scenes-and-observable-action.md",
        "requirements/story/story-clock-and-state.md",
      ],
      [
        "specifications/camera-light-and-visibility/README.md",
        "specifications/camera-light-and-visibility/target-focus-exposure-and-sampling.md",
        "specifications/narrative-and-intent/README.md",
        "specifications/narrative-and-intent/dialogue-language-theme-and-meaning.md",
        "specifications/narrative-and-intent/events-causality-and-time.md",
        "specifications/narrative-and-intent/story-authority-and-hierarchy.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/kinematics-contact-and-interaction.md",
        "specifications/performance-motion-and-staging/staging-space-state-and-choreography.md",
      ],
    ),
    contractClaim(
      "material and surface-assembly schemas",
      ["src/material/*.ts"],
      [
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/materials-and-textures.md",
        "requirements/asset-authoring/patterns-and-procedural-composition.md",
        "requirements/building-exterior/README.md",
        "requirements/building-exterior/materials-and-assemblies.md",
        "requirements/interior/README.md",
        "requirements/interior/acoustics-and-sound-boundaries.md",
        "requirements/interior/materials-and-physical-properties.md",
        "requirements/interior/surface-assemblies.md",
        "requirements/sound/README.md",
        "requirements/sound/interior-acoustics.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/model-geometry-and-surface-facts.md",
        "specifications/building-envelope/README.md",
        "specifications/building-envelope/structure-envelope-and-materials.md",
        "specifications/interior-space/README.md",
        "specifications/interior-space/lighting-acoustics-and-environment.md",
        "specifications/interior-space/materials-style-and-art.md",
        "specifications/interior-space/surface-assemblies.md",
        "specifications/simulation-effects-and-sound/README.md",
        "specifications/simulation-effects-and-sound/ambience-music-spatial-and-acoustics.md",
      ],
    ),
    contractClaim(
      "model, mesh, geometry, and affordance schemas",
      ["src/model/*.ts"],
      [
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/external-assets.md",
        "requirements/asset-authoring/geometry.md",
        "requirements/asset-authoring/identity-and-instances.md",
        "requirements/asset-authoring/representations-bounds-and-lod.md",
        "requirements/asset-authoring/rig-and-state.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/bounds-proxies-and-lod.md",
        "specifications/asset-and-representation/identity-resources-and-lifecycle.md",
        "specifications/asset-and-representation/model-geometry-and-surface-facts.md",
        "specifications/asset-and-representation/rig-deformation-and-state.md",
      ],
    ),
    contractClaim(
      "motion clip, gait, and keyframe schemas",
      ["src/motion/*.ts"],
      [
        "requirements/motion/README.md",
        "requirements/motion/clips-keyframes-and-interpolation.md",
        "requirements/motion/layers-blends-and-transitions.md",
        "requirements/motion/procedural-motion-and-gaits.md",
        "requirements/motion/root-motion-and-trajectories.md",
        "requirements/motion/validation-and-determinism.md",
      ],
      [
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/kinematics-contact-and-interaction.md",
        "specifications/performance-motion-and-staging/motion-sampling-and-composition.md",
      ],
    ),
    contractClaim(
      "pose, skeleton, constraint, and attachment schemas",
      ["src/pose/*.ts", "src/skeleton/*.ts"],
      [
        "requirements/actors/README.md",
        "requirements/actors/appearance-costume-and-attachments.md",
        "requirements/actors/body-scale-and-landmarks.md",
        "requirements/actors/pose-expression-and-gaze.md",
        "requirements/actors/skeleton-rig-and-retargeting.md",
        "requirements/motion/README.md",
        "requirements/motion/constraints-and-inverse-kinematics.md",
        "requirements/motion/contact-weight-and-support.md",
        "requirements/motion/retargeting-and-scale.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/rig-deformation-and-state.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/actor-identity-state-and-fidelity.md",
        "specifications/performance-motion-and-staging/kinematics-contact-and-interaction.md",
        "specifications/performance-motion-and-staging/rig-deformation-and-retargeting.md",
      ],
    ),
    contractClaim(
      "asset manifest and external-adoption schemas",
      ["src/production/IAutoMovieAssetManifest.ts"],
      [
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/external-assets.md",
        "requirements/asset-authoring/generated-assets.md",
        "requirements/evidence-and-provenance/README.md",
        "requirements/evidence-and-provenance/canonical-digests-and-content-identity.md",
        "requirements/evidence-and-provenance/generation-transformation-and-derivation.md",
        "requirements/evidence-and-provenance/third-party-sources-rights-and-attribution.md",
        "requirements/external-inputs/README.md",
        "requirements/external-inputs/adoption-modes-and-composition.md",
        "requirements/external-inputs/conversion-receipts-and-determinism.md",
        "requirements/external-inputs/credentials-rights-and-provenance.md",
        "requirements/external-inputs/resource-closure-and-acquisition.md",
        "requirements/external-inputs/source-selection-and-provider-neutrality.md",
        "requirements/motion/README.md",
        "requirements/motion/external-motion-inputs.md",
        "requirements/motion/retargeting-and-scale.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/generated-assets-and-repaint-handoff.md",
        "specifications/asset-and-representation/identity-resources-and-lifecycle.md",
        "specifications/evidence-and-provenance/README.md",
        "specifications/evidence-and-provenance/canonical-digests-and-content-identity.md",
        "specifications/evidence-and-provenance/generation-transformation-and-derivation.md",
        "specifications/evidence-and-provenance/third-party-sources-rights-and-attribution.md",
        "specifications/interchange-and-adoption/README.md",
        "specifications/interchange-and-adoption/adoption-decisions-and-composition.md",
        "specifications/interchange-and-adoption/conversion-receipts-and-determinism.md",
        "specifications/interchange-and-adoption/media-inspection-boundaries.md",
        "specifications/interchange-and-adoption/provenance-rights-and-secrets.md",
        "specifications/interchange-and-adoption/resource-closure-and-acquisition.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/motion-sampling-and-composition.md",
        "specifications/performance-motion-and-staging/rig-deformation-and-retargeting.md",
      ],
    ),
    contractClaim(
      "legacy import inventory and rollback schemas",
      ["src/production/IAutoMovieLegacyImport.ts"],
      [
        "requirements/external-inputs/README.md",
        "requirements/external-inputs/adoption-modes-and-composition.md",
        "requirements/external-inputs/conversion-receipts-and-determinism.md",
        "requirements/external-inputs/unsupported-and-degradation.md",
        "requirements/operations-and-recovery/README.md",
        "requirements/operations-and-recovery/migration-and-compatibility.md",
      ],
      [
        "specifications/execution-and-recovery/README.md",
        "specifications/execution-and-recovery/portability-migration-and-compatibility.md",
        "specifications/interchange-and-adoption/README.md",
        "specifications/interchange-and-adoption/adoption-decisions-and-composition.md",
        "specifications/interchange-and-adoption/conversion-receipts-and-determinism.md",
        "specifications/interchange-and-adoption/support-degradation-and-refusal.md",
      ],
    ),
    contractClaim(
      "compiled-production and materialization schemas",
      ["src/production/IAutoMovieProductionCompiler.ts"],
      [
        "requirements/agent-authoring/README.md",
        "requirements/agent-authoring/partial-work.md",
        "requirements/agent-authoring/source-owned-loop.md",
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/generated-assets.md",
        "requirements/diagnostics/README.md",
        "requirements/diagnostics/identity-path-and-context.md",
        "requirements/diagnostics/input-and-result-classification.md",
        "requirements/delivery-and-accessibility/README.md",
        "requirements/delivery-and-accessibility/captions-subtitles-and-cues.md",
        "requirements/delivery-and-accessibility/integrity-provenance-and-authenticity.md",
        "requirements/delivery-and-accessibility/packages-manifests-and-dependencies.md",
        "requirements/editorial/README.md",
        "requirements/editorial/clips-source-ranges-and-handles.md",
        "requirements/editorial/picture-and-sound-edits.md",
        "requirements/editorial/rational-time-and-ranges.md",
        "requirements/editorial/transitions-and-overlaps.md",
        "requirements/effects-and-simulation/README.md",
        "requirements/effects-and-simulation/scope-and-simulation-tiers.md",
        "requirements/formations/README.md",
        "requirements/formations/hierarchies-and-units.md",
        "requirements/formations/layouts-and-slots.md",
        "requirements/motion/README.md",
        "requirements/motion/external-motion-inputs.md",
        "requirements/motion/retargeting-and-scale.md",
        "requirements/rendering/README.md",
        "requirements/rendering/encoding-and-multiplexing.md",
        "requirements/rendering/frame-identity-and-content-addressing.md",
        "requirements/rendering/scope-and-artifact-identity.md",
        "requirements/rendering/scene-lowering-and-runtime-state.md",
        "requirements/rendering/validation.md",
        "requirements/repaint/README.md",
        "requirements/repaint/identity-and-provenance.md",
        "requirements/repaint/sequence-continuity-and-publication.md",
        "requirements/review/README.md",
        "requirements/review/records-and-completeness.md",
        "requirements/review/reproducible-context.md",
        "requirements/staging/README.md",
        "requirements/staging/state-handoff-and-continuity.md",
        "requirements/story/README.md",
        "requirements/story/story-clock-and-state.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/generated-assets-and-repaint-handoff.md",
        "specifications/authoring-and-authority/README.md",
        "specifications/authoring-and-authority/partial-targets-and-atomic-results.md",
        "specifications/authoring-and-authority/source-authority-and-derivation.md",
        "specifications/editorial-render-and-delivery/README.md",
        "specifications/editorial-render-and-delivery/delivery-audio-text-and-localization.md",
        "specifications/editorial-render-and-delivery/delivery-package-provenance-and-publication.md",
        "specifications/editorial-render-and-delivery/delivery-validation-and-release-status.md",
        "specifications/editorial-render-and-delivery/editorial-audiovisual-continuity.md",
        "specifications/editorial-render-and-delivery/rational-timeline-and-composition.md",
        "specifications/editorial-render-and-delivery/render-encoding-and-validation.md",
        "specifications/editorial-render-and-delivery/render-products-visibility-and-color.md",
        "specifications/editorial-render-and-delivery/render-schedule-state-and-headless.md",
        "specifications/interchange-and-adoption/README.md",
        "specifications/interchange-and-adoption/adoption-decisions-and-composition.md",
        "specifications/interchange-and-adoption/conversion-receipts-and-determinism.md",
        "specifications/narrative-and-intent/README.md",
        "specifications/narrative-and-intent/events-causality-and-time.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/formation-identity-layout-and-terrain.md",
        "specifications/performance-motion-and-staging/motion-sampling-and-composition.md",
        "specifications/performance-motion-and-staging/rig-deformation-and-retargeting.md",
        "specifications/performance-motion-and-staging/staging-space-state-and-choreography.md",
        "specifications/simulation-effects-and-sound/README.md",
        "specifications/simulation-effects-and-sound/scope-tiers-and-identities.md",
        "specifications/review-and-acceptance/README.md",
        "specifications/review-and-acceptance/evidence-freshness-and-completeness.md",
        "specifications/review-and-acceptance/target-scope-and-context.md",
        "specifications/validation-and-diagnostics/README.md",
        "specifications/validation-and-diagnostics/classification-and-causality.md",
        "specifications/validation-and-diagnostics/diagnostic-identity-location-and-severity.md",
      ],
    ),
    contractClaim(
      "production intent, frame, light, budget, and deliverable schemas",
      ["src/production/IAutoMovieProductionDesign.ts"],
      [
        "requirements/acceptance/README.md",
        "requirements/acceptance/criteria-and-observables.md",
        "requirements/acceptance/profiles-and-aggregation.md",
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/identity-and-instances.md",
        "requirements/asset-authoring/representations-bounds-and-lod.md",
        "requirements/delivery-and-accessibility/README.md",
        "requirements/delivery-and-accessibility/captions-subtitles-and-cues.md",
        "requirements/delivery-and-accessibility/scope-and-profiles.md",
        "requirements/effects-and-simulation/README.md",
        "requirements/effects-and-simulation/budgets-and-bounded-work.md",
        "requirements/effects-and-simulation/clock-seek-and-determinism.md",
        "requirements/effects-and-simulation/environment-coupling.md",
        "requirements/effects-and-simulation/fire-smoke-and-atmosphere.md",
        "requirements/effects-and-simulation/particles-and-emission.md",
        "requirements/effects-and-simulation/scope-and-simulation-tiers.md",
        "requirements/formations/README.md",
        "requirements/formations/budgets-and-validation.md",
        "requirements/formations/hierarchies-and-units.md",
        "requirements/formations/layouts-and-slots.md",
        "requirements/formations/scope-and-identity.md",
        "requirements/lighting/README.md",
        "requirements/lighting/temporal-state-and-continuity.md",
        "requirements/map/README.md",
        "requirements/map/movement-and-visibility.md",
        "requirements/map/scope-and-coordinates.md",
        "requirements/map/terrain-and-landforms.md",
        "requirements/motion/README.md",
        "requirements/motion/external-motion-inputs.md",
        "requirements/motion/object-motion-and-interaction.md",
        "requirements/motion/timing-and-semantic-events.md",
        "requirements/production-design/README.md",
        "requirements/production-design/art-direction-and-visual-language.md",
        "requirements/production-design/budgets-and-feasibility.md",
        "requirements/production-design/scope-and-source-of-truth.md",
        "requirements/production-design/visual-delivery-and-fidelity-tiers.md",
        "requirements/rendering/README.md",
        "requirements/rendering/budgets.md",
        "requirements/review/README.md",
        "requirements/review/criteria-and-comparison.md",
        "requirements/review/scope-and-authority.md",
        "requirements/staging/README.md",
        "requirements/staging/events-and-timing.md",
        "requirements/staging/shot-contracts-and-deliveries.md",
        "requirements/staging/subjects-and-object-staging.md",
        "requirements/story/README.md",
        "requirements/story/logline-and-premise.md",
        "requirements/story/story-clock-and-state.md",
        "requirements/sound/README.md",
        "requirements/sound/interior-acoustics.md",
        "requirements/sound/spatialization-and-propagation.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/bounds-proxies-and-lod.md",
        "specifications/asset-and-representation/identity-resources-and-lifecycle.md",
        "specifications/camera-light-and-visibility/README.md",
        "specifications/camera-light-and-visibility/temporal-state-and-continuity.md",
        "specifications/editorial-render-and-delivery/README.md",
        "specifications/editorial-render-and-delivery/delivery-audio-text-and-localization.md",
        "specifications/editorial-render-and-delivery/delivery-profiles-time-and-picture.md",
        "specifications/editorial-render-and-delivery/render-budget-identity-and-recovery.md",
        "specifications/interchange-and-adoption/README.md",
        "specifications/interchange-and-adoption/adoption-decisions-and-composition.md",
        "specifications/narrative-and-intent/README.md",
        "specifications/narrative-and-intent/design-authority-and-visual-language.md",
        "specifications/narrative-and-intent/events-causality-and-time.md",
        "specifications/narrative-and-intent/story-authority-and-hierarchy.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/formation-identity-layout-and-terrain.md",
        "specifications/performance-motion-and-staging/formation-motion-resolution-and-budgets.md",
        "specifications/performance-motion-and-staging/motion-sampling-and-composition.md",
        "specifications/performance-motion-and-staging/staging-events-coverage-and-validation.md",
        "specifications/performance-motion-and-staging/staging-space-state-and-choreography.md",
        "specifications/review-and-acceptance/README.md",
        "specifications/review-and-acceptance/criteria-tolerance-and-comparison.md",
        "specifications/review-and-acceptance/profiles-aggregation-and-partial-results.md",
        "specifications/review-and-acceptance/target-scope-and-context.md",
        "specifications/simulation-effects-and-sound/README.md",
        "specifications/simulation-effects-and-sound/ambience-music-spatial-and-acoustics.md",
        "specifications/simulation-effects-and-sound/budget-admission.md",
        "specifications/simulation-effects-and-sound/clocks-ordering-seek-and-checkpoints.md",
        "specifications/simulation-effects-and-sound/particles-fire-and-atmosphere.md",
        "specifications/simulation-effects-and-sound/scope-tiers-and-identities.md",
        "specifications/world-and-site/README.md",
        "specifications/world-and-site/spatial-reference-and-identity.md",
        "specifications/world-and-site/terrain-ground-and-geology.md",
        "specifications/world-and-site/traversal-and-visibility.md",
      ],
    ),
    contractClaim(
      "production query and capture schemas",
      [
        "src/production/IAutoMovieProductionOracle.ts",
        "src/production/application/IAutoMovieCaptureFrame.ts",
      ],
      [
        "requirements/agent-authoring/README.md",
        "requirements/agent-authoring/mcp-boundary.md",
        "requirements/agent-authoring/partial-work.md",
        "requirements/rendering/README.md",
        "requirements/rendering/budgets.md",
        "requirements/rendering/passes-channels-and-products.md",
        "requirements/rendering/validation.md",
        "requirements/review/README.md",
        "requirements/review/reproducible-context.md",
      ],
      [
        "specifications/authoring-and-authority/README.md",
        "specifications/authoring-and-authority/knowledge-evidence-and-tool-boundary.md",
        "specifications/authoring-and-authority/partial-targets-and-atomic-results.md",
        "specifications/editorial-render-and-delivery/README.md",
        "specifications/editorial-render-and-delivery/render-budget-identity-and-recovery.md",
        "specifications/editorial-render-and-delivery/render-products-visibility-and-color.md",
        "specifications/editorial-render-and-delivery/render-schedule-state-and-headless.md",
        "specifications/review-and-acceptance/README.md",
        "specifications/review-and-acceptance/target-scope-and-context.md",
      ],
    ),
    contractClaim(
      "review evidence, finding, and verdict schemas",
      [
        "src/production/IAutoMovieProductionReview.ts",
        "src/production/application/IAutoMoviePrepareReview.ts",
        "src/production/application/IAutoMovieSubmitReview.ts",
      ],
      [
        "requirements/acceptance/README.md",
        "requirements/acceptance/evidence-and-freshness.md",
        "requirements/review/README.md",
        "requirements/review/annotations-findings-and-verdicts.md",
        "requirements/review/records-and-completeness.md",
      ],
      [
        "specifications/review-and-acceptance/README.md",
        "specifications/review-and-acceptance/evidence-freshness-and-completeness.md",
        "specifications/review-and-acceptance/observations-findings-and-defects.md",
      ],
    ),
    contractClaim(
      "repaint request and receipt schemas",
      [
        "src/production/IAutoMovieProductionRendition.ts",
        "src/production/application/IAutoMovieRepaintShot.ts",
      ],
      [
        "requirements/repaint/README.md",
        "requirements/repaint/eligibility-and-prerequisites.md",
        "requirements/repaint/identity-and-provenance.md",
        "requirements/repaint/prompts-controls-and-constraints.md",
        "requirements/repaint/providers-models-and-credentials.md",
        "requirements/repaint/retries-seeds-and-variation.md",
        "requirements/repaint/scope-and-user-choice.md",
        "requirements/repaint/source-frames-and-reference-locking.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/generated-assets-and-repaint-handoff.md",
        "specifications/authoring-and-authority/README.md",
        "specifications/authoring-and-authority/external-execution-and-provider-neutrality.md",
      ],
    ),
    contractClaim(
      "production sound event, dialogue, and analysis schemas",
      ["src/production/IAutoMovieProductionSound.ts"],
      [
        "requirements/sound/README.md",
        "requirements/sound/dialogue-voice-and-visemes.md",
        "requirements/sound/editing-synchronization-and-continuity.md",
        "requirements/sound/event-cues-and-timing.md",
        "requirements/sound/interior-acoustics.md",
        "requirements/sound/sources-and-external-assets.md",
        "requirements/sound/spatialization-and-propagation.md",
        "requirements/sound/validation-and-delivery.md",
      ],
      [
        "specifications/simulation-effects-and-sound/README.md",
        "specifications/simulation-effects-and-sound/ambience-music-spatial-and-acoustics.md",
        "specifications/simulation-effects-and-sound/mix-stems-loudness-and-av-join.md",
        "specifications/simulation-effects-and-sound/sound-sources-events-dialogue-and-foley.md",
        "specifications/simulation-effects-and-sound/validation-evidence-and-compatibility.md",
      ],
    ),
    contractClaim(
      "screenplay index and continuity schemas",
      ["src/production/IAutoMovieScreenplayIndex.ts"],
      [
        "requirements/story/README.md",
        "requirements/story/beats-and-causality.md",
        "requirements/story/coverage-and-acceptance.md",
        "requirements/story/revision-and-change-impact.md",
        "requirements/story/scenes-and-observable-action.md",
        "requirements/story/scope-and-source-of-truth.md",
        "requirements/story/story-clock-and-state.md",
        "requirements/story/treatment-and-sequences.md",
      ],
      [
        "specifications/narrative-and-intent/README.md",
        "specifications/narrative-and-intent/alternatives-revisions-and-compatibility.md",
        "specifications/narrative-and-intent/events-causality-and-time.md",
        "specifications/narrative-and-intent/scene-coverage-and-acceptance.md",
        "specifications/narrative-and-intent/story-authority-and-hierarchy.md",
      ],
    ),
    contractClaim(
      "render budget, inventory, target, mask, and report schemas",
      ["src/render/*.ts"],
      [
        "requirements/rendering/README.md",
        "requirements/rendering/budgets.md",
        "requirements/rendering/frame-identity-and-content-addressing.md",
        "requirements/rendering/geometry-visibility-and-culling.md",
        "requirements/rendering/materials-lighting-and-color.md",
        "requirements/rendering/passes-channels-and-products.md",
        "requirements/rendering/scope-and-artifact-identity.md",
        "requirements/rendering/validation.md",
      ],
      [
        "specifications/editorial-render-and-delivery/README.md",
        "specifications/editorial-render-and-delivery/render-budget-identity-and-recovery.md",
        "specifications/editorial-render-and-delivery/render-products-visibility-and-color.md",
        "specifications/editorial-render-and-delivery/render-schedule-state-and-headless.md",
      ],
    ),
    contractClaim(
      "camera scene-state schemas",
      ["src/scene/IAutoMovieCamera.ts"],
      [
        "requirements/camera/README.md",
        "requirements/camera/clipping-occlusion-and-spatial-constraints.md",
        "requirements/camera/position-and-movement.md",
        "requirements/camera/projection-lens-and-sensor.md",
        "requirements/camera/targets-focus-and-depth-boundary.md",
      ],
      [
        "specifications/camera-light-and-visibility/README.md",
        "specifications/camera-light-and-visibility/camera-state-projection-and-gate.md",
        "specifications/camera-light-and-visibility/target-focus-exposure-and-sampling.md",
      ],
    ),
    contractClaim(
      "light, fog, and environment-state schemas",
      [
        "src/scene/IAutoMovieFog.ts",
        "src/scene/IAutoMovieLight.ts",
        "src/scene/IAutoMovieProductionLighting.ts",
        "src/scene/IAutoMovieSceneEnvironment.ts",
      ],
      [
        "requirements/lighting/README.md",
        "requirements/lighting/practicals-and-local-lights.md",
        "requirements/lighting/shape-filters-and-linking.md",
        "requirements/lighting/sources-and-photometry.md",
        "requirements/lighting/sun-sky-and-environment.md",
        "requirements/lighting/temporal-state-and-continuity.md",
      ],
      [
        "specifications/camera-light-and-visibility/README.md",
        "specifications/camera-light-and-visibility/light-source-photometry-and-environment.md",
        "specifications/camera-light-and-visibility/practical-shaping-and-linking.md",
        "specifications/camera-light-and-visibility/temporal-state-and-continuity.md",
      ],
    ),
    contractClaim(
      "scene graph, space, and support-surface schemas",
      [
        "src/scene/AutoMovieSurfaceKind.ts",
        "src/scene/IAutoMovieScene.ts",
        "src/scene/IAutoMovieSceneNode.ts",
        "src/scene/IAutoMovieSpace.ts",
        "src/scene/IAutoMovieSurface.ts",
      ],
      [
        "requirements/asset-authoring/README.md",
        "requirements/asset-authoring/identity-and-instances.md",
        "requirements/interior/README.md",
        "requirements/interior/floors-and-raised-floors.md",
        "requirements/interior/spaces-and-occupancy.md",
        "requirements/map/README.md",
        "requirements/map/movement-and-visibility.md",
        "requirements/map/scope-and-coordinates.md",
      ],
      [
        "specifications/asset-and-representation/README.md",
        "specifications/asset-and-representation/identity-resources-and-lifecycle.md",
        "specifications/interior-space/README.md",
        "specifications/interior-space/space-level-zone-topology.md",
        "specifications/interior-space/surface-assemblies.md",
        "specifications/world-and-site/README.md",
        "specifications/world-and-site/spatial-reference-and-identity.md",
        "specifications/world-and-site/traversal-and-visibility.md",
      ],
    ),
    contractClaim(
      "building service-network and wet-zone schemas",
      ["src/service/*.ts"],
      [
        "requirements/building-exterior/README.md",
        "requirements/building-exterior/services-and-envelope-interfaces.md",
        "requirements/interior/README.md",
        "requirements/interior/services-and-environment.md",
        "requirements/interior/wet-areas-and-waterproofing.md",
      ],
      [
        "specifications/building-envelope/README.md",
        "specifications/building-envelope/services-water-weather-and-site.md",
        "specifications/interior-space/README.md",
        "specifications/interior-space/services-wet-and-fluid.md",
      ],
    ),
    contractClaim(
      "soft-body and moving-boundary schemas",
      [
        "src/soft/IAutoMovieSoftBodyDomain.ts",
        "src/soft/IAutoMovieSoftBodyState.ts",
        "src/soft/IAutoMovieSoftFurnishing.ts",
      ],
      [
        "requirements/actors/README.md",
        "requirements/actors/appearance-costume-and-attachments.md",
        "requirements/effects-and-simulation/README.md",
        "requirements/effects-and-simulation/soft-bodies-and-deformation.md",
        "requirements/interior/README.md",
        "requirements/interior/soft-materials-plants-and-deformation.md",
        "requirements/motion/README.md",
        "requirements/motion/secondary-motion.md",
      ],
      [
        "specifications/interior-space/README.md",
        "specifications/interior-space/elements-furnishing-and-clearance.md",
        "specifications/performance-motion-and-staging/README.md",
        "specifications/performance-motion-and-staging/actor-identity-state-and-fidelity.md",
        "specifications/simulation-effects-and-sound/README.md",
        "specifications/simulation-effects-and-sound/soft-bodies-and-deformation.md",
      ],
    ),
    contractClaim(
      "planting domain and installation schemas",
      [
        "src/soft/IAutoMoviePlantingDomain.ts",
        "src/soft/IAutoMoviePlantingInstallation.ts",
        "src/soft/IAutoMoviePlantingState.ts",
      ],
      [
        "requirements/interior/README.md",
        "requirements/interior/soft-materials-plants-and-deformation.md",
        "requirements/map/README.md",
        "requirements/map/vegetation-and-ecology.md",
      ],
      [
        "specifications/interior-space/README.md",
        "specifications/interior-space/elements-furnishing-and-clearance.md",
        "specifications/world-and-site/README.md",
        "specifications/world-and-site/ecology-weather-and-calendar.md",
      ],
    ),
    contractClaim(
      "constraint violation and validation-result schemas",
      [
        "src/validation/AutoMovieViolationKind.ts",
        "src/validation/IAutoMovieConstraintViolation.ts",
        "src/validation/IAutoMovieValidation.ts",
      ],
      [
        "requirements/acceptance/README.md",
        "requirements/acceptance/criteria-and-observables.md",
        "requirements/diagnostics/README.md",
        "requirements/diagnostics/identity-path-and-context.md",
        "requirements/diagnostics/input-and-result-classification.md",
      ],
      [
        "specifications/review-and-acceptance/README.md",
        "specifications/review-and-acceptance/criteria-tolerance-and-comparison.md",
        "specifications/validation-and-diagnostics/README.md",
        "specifications/validation-and-diagnostics/classification-and-causality.md",
        "specifications/validation-and-diagnostics/diagnostic-identity-location-and-severity.md",
      ],
    ),
    contractClaim(
      "shared humanoid capsule proxy schema",
      ["src/validation/IAutoMovieCapsuleProxy.ts"],
      [
        "requirements/effects-and-simulation/README.md",
        "requirements/effects-and-simulation/soft-bodies-and-deformation.md",
      ],
      [
        "specifications/simulation-effects-and-sound/README.md",
        "specifications/simulation-effects-and-sound/soft-bodies-and-deformation.md",
      ],
    ),
  ],
};

export default {
  extends: "../../config/lint.config.ts",
  plugins: { evidence },
  rules: {
    "evidence/documented": [
      "error",
      { symbol: ["type", "function", "property"] },
    ],
    "evidence/graph": ["error", graph],
    "evidence/todo": "error",
  },
} satisfies ITtscLintConfig;
