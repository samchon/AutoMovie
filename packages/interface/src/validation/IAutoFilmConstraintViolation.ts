import { AutoFilmViolationKind } from "./AutoFilmViolationKind";

/**
 * One deterministic constraint violation — the unit of feedback the harness
 * turns into a `// ❌` correction comment.
 *
 * This is autofilm's domain-level analogue of typia's `IValidation.IError`,
 * enriched with a {@link AutoFilmViolationKind} (so failures route to the right
 * tier/corrector) and a human-and-LLM-readable `expected` string. The engine
 * emits these from its ROM / physics / temporal verifiers; `@autofilm/agent`
 ...
 * @author Samchon
 */
export interface IAutoFilmConstraintViolation {
