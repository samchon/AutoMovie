...
export interface IautomovieConstraintViolation {
   ...
   * on (e.g. `"leftLowerArm flexion must be within [0, 150]째 (anatomical ROM),
   * but was 175"`).
   */
  expected: string;
   ...
   * unit (degrees for ROM): how far _past_ the limit it sat (an elbow at 175째
   * against a 150째 max ??`25`). Present only for numeric overshoots; lets a
   * corrector judge severity (a 2째 graze vs a 90째 break) and the `// ?? comment
