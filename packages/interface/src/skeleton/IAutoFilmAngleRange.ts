 ...
 * in degrees.
 *
 * This is the atom of autofilm's anatomical range-of-motion (ROM) model. The
 ...
 * correction. That is how autofilm makes "physically impossible poses are
 * structurally rejected" real, rather than hoping the LLM emits plausible
 * numbers.
 *
 * Sign convention is per-axis and documented on
 * {@link IAutoFilmJointConstraint}. `min <= max` is required; the engine treats
 * `min > max` as a malformed constraint.
 *
 * @author Samchon
 */
export interface IAutoFilmAngleRange {
