import { IMoticaAngleRange } from "./IMoticaAngleRange";

/**
 * The anatomical range of motion (ROM) of a single joint, decomposed into the
 * three clinical rotation axes.
 *
 * **This type is the heart of motica's differentiator.** A bounded blendshape
 ...
 * `IMoticaJointConstraint` to each bone and validating poses against it, motica
 * turns "is this pose anatomically possible?" into a deterministic verifier,
 * and lets the function-calling harness converge motion the same way it
 * converges JSON schemas: validate → `// ❌` → retry.
 *
 ...
 *   shoulder `[-90, 90]`.
 *
 * The numeric tables live in `@motica/engine` (sourced from goniometry norms),
 * not here — this type only describes their _shape_. Per-character overrides
 * are possible (a contortionist, a stylized non-human rig) by supplying a
 * different constraint on the bone.
 *
 ...
 * @author Samchon
 */
export interface IMoticaJointConstraint {
  /**
