 ...
 *
 * **This type is the heart of automovie's differentiator.** A bounded blendshape
 * vector prevents _out-of-range_ values, but nothing prevents a _physically
 * impossible_ joint angle (an elbow bending backward, a knee hyper-extending) ?? * which is exactly where raw LLM motion emission fails. By attaching a
 * `IautomovieJointConstraint` to each bone and validating poses against it,
 * automovie turns "is this pose anatomically possible?" into a deterministic
 ...
 *
 * **Axis & sign conventions** (right-handed, clinical neutral = 0째):
 *
...
export interface IautomovieJointConstraint {
   ...
   * combined swing (`2쨌acos(cos(flexion/2)쨌cos(abduction/2))`) so the pose
