 ...
 *
 * This enum is the backbone of motica's "named scalar" strategy. Because every
 * bone an LLM can address is a fixed identifier from this union, a hallucinated
 * or mistyped bone name is structurally impossible — the model picks from a
 * menu, it does not invent. The names map 1:1 onto `VRMHumanoid` so a validated
 * pose retargets cleanly onto _any_ VRM avatar (the whole point of a normalized
 * humanoid rig).
 *
 ...
 * where direct LLM emission gets brittle. motica handles hands as a separate,
 ...
 * @author Samchon
 */
export type MoticaHumanoidBone =
  // ── torso (4) ──
