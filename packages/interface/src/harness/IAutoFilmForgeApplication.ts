import { IAutoFilmModel } from "../model/IAutoFilmModel";
 ...
 *
 * Exposed to the model as `typia.llm.application<IAutoFilmForgeApplication>()`.
 * The single `process` method's `IProps` schema enforces the reasoning (the
 * JSDoc on each field is the prompt).
 *
 * @author Samchon
 */
export interface IAutoFilmForgeApplication {
  process(props: IAutoFilmForgeApplication.IProps): void;
}
export namespace IAutoFilmForgeApplication {
