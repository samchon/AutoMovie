import { IAutoFilmBeat, IAutoFilmCastMember } from "./IAutoFilmSlate";
 ...
 * `typia.llm.application<IAutoFilmScriptApplication>()`. The single `process`
 * method's `IProps` schema enforces the reasoning (the JSDoc on each field is
 * the prompt).
 *
 * @author Samchon
 */
export interface IAutoFilmScriptApplication {
  process(props: IAutoFilmScriptApplication.IProps): void;
}
export namespace IAutoFilmScriptApplication {
