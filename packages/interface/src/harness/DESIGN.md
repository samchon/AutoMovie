...
The question was: one giant schema that emits all motion at once, or a multi-stage macro?뭢icro pipeline (횪 la AutoBe), or a very thin motion schema the engine underpins, or both. The answer here is **both, layered** ??and that is the whole point:

- **Macro?뭢icro stages** give CoT compliance and let each step be validated before the next (AutoBe's waterfall+spiral). A film is decomposed the way a real production is: script ??staging ??blocking ??performance ??review.
...
```
   ...
   ?붴??????????????????????????????????????????섃봻
                        ...
                        ??                           ??
   ?뚢??????????????????????????????????????????먥봻
   ??5. REVIEW      IautomovieReviewApplication ?? render (M2) + validate (ROM/physics/timing) ??notes ??loop ?꾟뵍
   ?붴??????????????????????????????????????????섃봻                                                            ??
...
- **Spiral within a node:** PERFORMANCE?봕EVIEW iterate (write ??render ??validate ??correct) until the shot passes its gates or a retry budget runs out ??exactly AutoBe's correct-loop.
- **Fan-out** across beats: each beat's BLOCKING?뭁ERFORMANCE?뭃EVIEW is independent (a pipeline), so shots can be built/verified in parallel and only ASSEMBLE waits for all.
