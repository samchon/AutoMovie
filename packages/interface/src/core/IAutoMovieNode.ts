 ...
 * ({@link automovieNodeKind} `group`): its TRS moves the subtree, its local frame
 * is the rotation pivot, and a {@link IautomovieChannelLimit} on its rotation is
 ...
 * expressed as {@link IautomovieDriver}s, not multi-parenting, since the graph is
