...
import { AutoFilmNodeKind } from "./AutoFilmNodeKind";
 ...
 * ({@link AutoFilmNodeKind} `group`): its TRS moves the subtree, its local frame
 * is the rotation pivot, and a {@link IAutoFilmChannelLimit} on its rotation is
 * a group-level ROM — no separate grouping concept exists.
 *
 ...
 * expressed as {@link IAutoFilmDriver}s, not multi-parenting, since the graph is
 * a strict tree on export.
 *
 * @author Samchon
 */
export interface IAutoFilmNode {
  /**
