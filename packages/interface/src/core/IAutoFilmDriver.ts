 ...
 * layer glTF and USD deliberately omit (they bake the result); autofilm keeps
 ...
 * (A pure value limit is a {@link IAutoFilmChannelLimit}, not a driver —
 * computation and restriction are kept separate.)
 *
 * @author Samchon
 */
export type IAutoFilmDriver =
  | IAutoFilmCopyDriver
  | IAutoFilmAimDriver
  | IAutoFilmIKDriver
...
export interface IAutoFilmCopyDriver {
  /** Discriminator. */
  type: "copy";
...
export interface IAutoFilmAimDriver {
  /** Discriminator. */
  type: "aim";
  /** Node to orient. */
  owner: string;
...
export interface IAutoFilmIKDriver {
  /** Discriminator. */
  type: "ik";
  /** Bone chain, root → tip. */
  chain: string[];
...
export interface IAutoFilmIKPole {
...
export interface IAutoFilmParentDriver {
  /** Discriminator. */
  type: "parent";
  /** Node that follows. */
  owner: string;
 ...
 */
export interface IAutoFilmDrivenDriver {
  /** Discriminator. */
  type: "driven";
  /** Channel that receives the computed value. */
  output: IAutoFilmChannel;
  /** Channel read as the driver value. */
  source: IAutoFilmChannel;
 ...
 */
export interface IAutoFilmSpringDriver {
  /** Discriminator. */
  type: "spring";
