import { IMoticaVector3 } from "../geometry/IMoticaVector3";
 ...
 * layer glTF and USD deliberately omit (they bake the result); motica keeps it
 * live, which is what makes it an engine rather than a model holder.
 *
...
export interface IMoticaCopyDriver {
  /** Discriminator. */
  type: "copy";
...
export interface IMoticaAimDriver {
  /** Discriminator. */
  type: "aim";
  /** Node to orient. */
  owner: string;
...
export interface IMoticaIKDriver {
  /** Discriminator. */
  type: "ik";
  /** Bone chain, root → tip. */
  chain: string[];
...
export interface IMoticaIKPole {
...
export interface IMoticaParentDriver {
  /** Discriminator. */
  type: "parent";
  /** Node that follows. */
  owner: string;
...
/**
 * A driven relationship: one channel computed from another via a linear range
 * remap (the driven-key / range-map archetype). A finger-curl slider driving
 * three phalanx joints is three drivers reading the same source.
 */
export interface IMoticaDrivenDriver {
  /** Discriminator. */
  type: "driven";
  /** Channel that receives the computed value. */
  output: IMoticaChannel;
  /** Channel read as the driver value. */
  source: IMoticaChannel;
  /** Source value range `[in0, in1]` mapped onto `outRange`. */
 ...
 */
export interface IMoticaSpringDriver {
  /** Discriminator. */
  type: "spring";
