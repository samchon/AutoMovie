import { IMoticaClip } from "../core/IMoticaTrack";
 ...
 *
 * Time is local to the shot (origin 0, seconds); a {@link IMoticaSequence}
 * composes shots into a global timeline. The camera is a scene node, so its
 * move is an ordinary {@link IMoticaClip} of transform (and FOV) tracks — no
 * special camera-animation concept.
 *
 * @author Samchon
 */
export interface IMoticaShot {
  /** Stable id. */
  id: string;
   ...
   */
  cameraMotion: IMoticaClip | null;

  /** Per scene-node performances for this shot. */
  performances: IMoticaShotPerformance[];
...
export interface IMoticaShotPerformance {
