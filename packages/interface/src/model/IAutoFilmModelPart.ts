...
import { IAutoFilmGeometry } from "./IAutoFilmGeometry";
 ...
 * {@link IAutoFilmMeshSkin} (`attachedBone` is `null`); a _rigid_ part (a prop,
 * a sword, a primitive limb) is parented wholesale to one bone via
 * `attachedBone` and rides its transform.
 *
 * @author Samchon
 */
export interface IAutoFilmModelPart {
   ...
   */
  attachedBone: AutoFilmHumanoidBone | null;

  /**
   * Local transform of the part relative to its attachment (bone or model
   * root). `null` = identity.
   */
  transform: IAutoFilmTransform | null;
}
