...
/**
 * One limb's part in a gait cycle ({@link IautomovieGait}). The limbs differ only
...
export interface IautomovieGaitLimb {
   ...
   * (a hip, a shoulder) leaves this unset and swings `짹amplitude` about zero; a
   * limb that only bends one way needs a nonzero center to keep the whole swing
   * on the anatomical side. A knee, whose flexion ROM is `[0, 150]째` and cannot
   * hyperextend, walks with e.g. `{ neutral: 25, amplitude: 18 }` so its swing
   * stays in `[7, 43]째` instead of crossing zero ??the offset the ROM validator
