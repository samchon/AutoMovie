...
## Node entry (`@automovie/render/node`)

package root은 브라우저 번들이 그대로 가져갈 수 있다. Node 전용 edge는 `@automovie/render/node`만 가진다.

- `exportModelToGLB`: AutoMovie model AST를 glTF binary buffer로 직렬화한다. `@gltf-transform/core`는 호출 시점에 lazy import하므로, 이 entry를 다른 helper 때문에 가져온 script가 glTF serializer를 함께 적재하지 않는다.
