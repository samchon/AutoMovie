...
motica의 **재생·검수 표면**. `@motica/interface`의 모델·포즈·모션·씬을 `three.js`로 렌더하고, `@motica/engine`의 FK·보간으로 모션을 재생한다.
...
| 함수/클래스 | 역할 |
|---|---|
| `buildModel(model)` | `IMoticaModel` → `THREE.Group` + 본 맵 (프리미티브 테셀레이션·메쉬·스켈레톤·머티리얼) |
| `buildGeometry` / `buildMaterial` | 개별 형상·머티리얼 → three 객체 |
| `applyPose(modelObject, pose, skeleton)` | 엔진 FK로 본 쿼터니언 적용 |
| `buildScene(scene, getModelObject)` | `IMoticaScene` → `THREE.Scene` + 카메라·조명 |
| `MoticaPlayer` | 모션 클립을 시각 t에 샘플링(엔진)해 포즈 적용 — `update(seconds)` |
...
const player = new MoticaPlayer(modelObject, character.skeleton!, waveClip);
