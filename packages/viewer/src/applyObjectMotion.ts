 ...
 * node; a node with no object (or a non-node channel) is skipped.
 *
 ...
 * involved. Feeding a shot's `objectMotions` and `cameraMotion` through the same
 * helper keeps every demo's projectile/prop/camera on one code path.
 *
 * @author Samchon
 */
