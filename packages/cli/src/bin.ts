 ...
 * @evidence requirements/external-inputs/README.md#외부-입력-요구사항 Accepts the explicit local legacy-input channel without choosing a provider or replacing failed acquisition.
 * @evidence requirements/operations-and-recovery/README.md#운영과-복구-요구사항 Exposes render lifecycle, migration, status, retry, and cleanup actions while preserving their process outcomes.
 ...
 * @evidence specifications/execution-and-recovery/README.md#실행과-복구-시스템-계약 Preserves generated job, attempt, checkpoint, migration, retention, and refusal boundaries across dispatch.
 * @evidence specifications/interchange-and-adoption/README.md#interchange와-adoption-시스템-계약 Keeps imported legacy input outside project authority until explicit validation and application.
 ...
 * @evidence specifications/execution-and-recovery/portability-migration-and-compatibility.md#execution-cross-platform-portability Normalizes CLI paths and delegates migration through project-owned portable records on Windows and POSIX hosts.
 * @evidence specifications/execution-and-recovery/portability-migration-and-compatibility.md#execution-runtime-compatibility-evidence Returns the importer's explicit compatibility plan and validation outcome instead of assuming compatibility.
 ...
 * @evidenceExclude specifications/interchange-and-adoption/provenance-rights-and-secrets.md#interchange-rights-publication-gate The CLI dispatcher does not implement the interchange rights publication gate system responsibility; it only routes explicit local commands and preserves delegated outcomes.
 ...
 * @evidenceExclude requirements/external-inputs/credentials-rights-and-provenance.md#external-rights-license-conditions The CLI dispatcher does not implement the external rights license conditions requirement; it only routes explicit local commands and preserves delegated outcomes.
 ...
 * @evidence requirements/operations-and-recovery/contract-baseline.md#operations-contract-baseline-identity Reads the project's recorded contract baseline and renders the installed target baseline from the same language before any migration plan is made.
 * @evidence requirements/operations-and-recovery/contract-migration-plan.md#operations-contract-migration-plan Prints one canonical plan for dry-run and apply alike and stops on any unresolved conflict instead of continuing without a plan.
 * @evidence requirements/operations-and-recovery/contract-migration-publication.md#operations-contract-migration-publication Publishes creations, replacements and removals as project candidates only after the plan applied cleanly against the observed files.
 * @evidence specifications/execution-and-recovery/contract-baseline.md#execution-contract-baseline-identity Parses both baseline records as the portable identities the migration compares.
 * @evidence specifications/execution-and-recovery/contract-migration-plan.md#execution-contract-migration-plan Delegates planning and conflict classification to the evidence package and refuses to apply over a conflict.
 * @evidence specifications/execution-and-recovery/contract-migration-publication.md#execution-contract-migration-publication Routes the migration's successor files through the project candidate publication path rather than editing tracked files in place.
 */
