...
/**
 * One reason loaded builder-owned state cannot be treated as current.
 *
 ...
 *
 * @evidence requirements/evidence-and-provenance/completeness-freshness-and-refusal.md#evidence-dependency-based-current-status Computes current state from source, dependency, and builder identities.
...
export interface IAutoMovieProjectStateFreshness {
  ...
  /**
   * Fingerprint of the loaded builder-owned output, or null when absent.
   *
...
export interface IAutoMovieGeneratedProjectState {
  ...
  /** Strict builder-owned library index for the library arm. */
...
export interface IAutoMovieProjectState {
  ...
  design: IAutoMovieProductionDesignGraph;
  /**
   * Last builder-owned state, loaded independently from current design.
   *
...
/**
 *  Load current tracked design and last builder-owned output.
 *
 ...
 * and recomputes the current builder fingerprint without writing.
 *
 ...
 * @evidenceExclude requirements/diagnostics/input-and-result-classification.md#diagnostics-unsupported-state Unsupported capability classification belongs to the owning builder or tool, not this filesystem-state inspection.
...
export const loadAutoMovieProjectState = (
    ...
    for (const file of manifest.files) {
      ...
      if (actual !== file.digest) {
        problems.push({
          ...
          path: file.path,
          message: `Generated file digest is ${actual}, but the ownership manifest records ${file.digest}. Recompile instead of querying modified builder output.`,
        });
        continue;
      }
  ...
  if (manifest !== null && kind === "library") {
    const inspection = inspectAutoMovieLibraryProjectState({
      ...
      builder: manifest.builder.protocolVersion,
  ...
  if (
    ...
    problems.push({
      ...
      path: null,
      message:
        "Generated state lacks a digest-verified builder registry, production contract, or world contract. Recompile before querying it.",
    });
