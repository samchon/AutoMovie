...
nothing else, and it owes every item here.

## State the measured extent, and keep it apart from the declared one {#measured-extent}
...
Where a subject is bounded — a room, a bay, an enclosure — the document says
which surfaces belong to it and which belong to something else. A slab between
two storeys belongs to neither room alone, and saying so is the document's job
rather than the reader's inference.

A production that leaves this unstated does not read as unfinished. It reads as
finished, because every gate that counts elements is satisfied by an element
somebody else owns.

Sources: `BUILT_ENVIRONMENT` on the boundary record's `kind`, `spaces` and
realizing `elements`; the room schedule's declared enclosure gap.

## A placeholder in the final state is unmet work {#no-placeholder-final}

A primitive standing in for a subject is a legitimate step and an illegitimate
...
## Facts and constraints, not a schedule {#execution-neutrality}

Settings state what is true and what is not permitted. They do not assign a
subject to a scene, fix when something is revealed, or decide a shot. Those are
