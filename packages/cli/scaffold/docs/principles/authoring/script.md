...
A script scene answers for exactly one scenario unit and says which. This is
the join that makes an unrealized plan visible: a scenario nothing realizes is
one error naming that file, and a script scene realizing two hides which of
them the film actually shot.
...
Two citations, one for the immediate parent and one for the layer above it,
cost one line and catch a whole class of drift that neither citation alone
reports.
...
the shot has moved a decision out of the layer that can be reviewed as prose
and into one that can only be reviewed as pixels.
...
and an inserted one takes an alpha id. Renumbering makes every citation,
receipt and review that named the old number wrong at once, silently.
