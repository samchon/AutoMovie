...
This is the generated project's only documentation root. The principles directory contains per-file review questions. Common obligations are per-unit questions, while each specialist obligation file contains roles that its whole layer allocates. Both directories are shared targets: do not put production facts or evidence tags in them. Named authoring branches hold production hosts, while separately named production target families extend the graph as described below.
