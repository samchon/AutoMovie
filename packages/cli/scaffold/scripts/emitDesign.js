import { AUTOMOVIE_PRIMITIVE_ARCHETYPES, createAutoMovieArchetypeRegistry, } from "@automovie/archetypes";
import { AutoMovieProductionProject, findAutoMovieProjectRoot, } from "@automovie/mcp";
 ...
 */
const project = AutoMovieProductionProject.open(findAutoMovieProjectRoot(process.cwd()), config.productionId, createAutoMovieArchetypeRegistry(AUTOMOVIE_PRIMITIVE_ARCHETYPES));
/**
 ...
 */
const emit = (label, target, value, store) => {
    ...
    if (output.accepted === false)
        throw new Error(`Derived design "${label}" was refused: ${output.diagnostics
            .map((diagnostic) => diagnostic.message)
            .join(" ")}`);
    process.stdout.write(`${current === null ? "created" : "updated"} ${label}\n`);
};
const soloistRecipe = soloist.design();
emit("models/soloist.json", { kind: "model", id: soloistRecipe.id }, soloistRecipe, () => project.setModelRecipe(soloistRecipe));
const chorusHeroRecipe = chorusHero.design();
emit("models/chorus-hero.json", { kind: "model", id: chorusHeroRecipe.id }, chorusHeroRecipe, () => project.setModelRecipe(chorusHeroRecipe));
const chorusFormation = chorus.design();
emit("formations/chorus.json", { kind: "formation", id: chorusFormation.id }, chorusFormation, () => project.setFormationDesign(chorusFormation));
const world = plaza.design();
emit("world.json", { kind: "world" }, world, () => project.setWorldDesign(world));
//# sourceMappingURL=emitDesign.js.map