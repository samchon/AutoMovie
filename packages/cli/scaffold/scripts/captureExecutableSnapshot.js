...
const throwCaptureExecutableSnapshotFailure = (failure, descriptor, operation) => {
    try {
        fs.closeSync(descriptor);
    }
    catch (closeFailure) {
        throw new AggregateError([failure, closeFailure], `Capture executable snapshot ${operation} and descriptor cleanup failed.`);
    }
    throw failure;
};
/** Create private bytes and retain the exact descriptor opened with `wx+`. */
export const createCaptureExecutableSnapshot = (file, bytes) => {
    const namespacePath = path.resolve(file);
    const before = physicalDirectory(path.dirname(namespacePath), "capture executable directory");
    const descriptor = fs.openSync(namespacePath, "wx+");
    try {
        let position = 0;
        while (position < bytes.length) {
            const length = fs.writeSync(descriptor, bytes, position, bytes.length - position, position);
            if (length === 0)
        ...
        const directory = physicalDirectory(path.dirname(namespacePath), "capture executable directory");
        if (directory.path !== before.path ||
            directory.real !== before.real ||
            directory.identity !== before.identity)
            throw new Error(`Capture executable directory "${directory.path}" changed while created.`);
        ...
        if (resident.isSymbolicLink() ||
            ...
            throw new Error(`Capture executable "${namespacePath}" changed while created.`);
        const snapshot = {
            descriptor,
...
export const openCaptureExecutable = (file, maximumBytes = null) => {
    ...
    if (linked.isSymbolicLink() || linked.isFile() === false)
        throw new Error(`Capture executable "${namespacePath}" is not one physical file.`);
    ...
    try {
        ...
        if (opened.isFile() === false ||
            ...
            throw new Error(`Capture executable "${namespacePath}" changed before open.`);
        if (maximumBytes !== null && opened.size > BigInt(maximumBytes))
            throw new Error(`Capture executable "${namespacePath}" exceeds its maximum byte length.`);
        const digest = digestDescriptor(descriptor, maximumBytes);
        const completed = fs.fstatSync(descriptor, { bigint: true });
        const descriptorVersion = physicalVersion(opened);
        if (completed.isFile() === false ||
            physicalVersion(completed) !== descriptorVersion)
            throw new Error(`Capture executable "${namespacePath}" changed while hashed.`);
        const snapshot = {
            descriptor,
            descriptorVersion,
            digest,
...
export const assertCaptureExecutable = (expected) => {
    ...
    if (resident.isSymbolicLink() ||
        ...
        throw new Error(`Capture executable "${expected.path}" changed physical identity.`);
...
export const assertCaptureExecutableDescriptor = (expected) => {
    ...
    if (opened.isFile() === false ||
        ...
        throw new Error(`Capture executable "${expected.path}" changed open descriptor bytes.`);
};
/** Rehash the exact open descriptor when byte-for-byte publication requires it. */
export const assertCaptureExecutableBytes = (expected) => {
    assertCaptureExecutableDescriptor(expected);
    if (digestDescriptor(expected.descriptor, expected.maximumBytes) !==
        expected.digest)
        throw new Error(`Capture executable "${expected.path}" changed open descriptor bytes.`);
    const completed = fs.fstatSync(expected.descriptor, { bigint: true });
    if (completed.isFile() === false ||
        physicalVersion(completed) !== expected.descriptorVersion)
        throw new Error(`Capture executable "${expected.path}" changed while revalidated.`);
};
/** Verify that one atomic rename published the same open file at a new path. */
export const assertRelocatedCaptureExecutable = (expected, file) => {
    assertCaptureExecutableBytes(expected);
    const destination = path.resolve(file);
    const directory = physicalDirectory(path.dirname(destination), "capture executable directory");
    const resident = fs.lstatSync(destination, { bigint: true });
    if (directory.path !== expected.directory.path ||
        directory.real !== expected.directory.real ||
        directory.identity !== expected.directory.identity ||
        resident.isSymbolicLink() ||
        resident.isFile() === false ||
        physicalVersion(resident) !== expected.identity)
        throw new Error(`Capture executable "${expected.path}" was not relocated exactly.`);
};
/** Remove a private staged path only while it still names the captured inode. */
export const removeCaptureExecutableIfResident = (expected, file = expected.path) => {
    try {
        const destination = path.resolve(file);
        const directory = physicalDirectory(path.dirname(destination), "capture executable directory");
        const resident = fs.lstatSync(destination, { bigint: true });
...
export const closeCaptureExecutable = (snapshot) => fs.closeSync(snapshot.descriptor);
const digestDescriptor = (descriptor, maximumBytes = null) => {
    const hash = createHash("sha256");
    const chunk = Buffer.allocUnsafe(1024 * 1024);
    let position = 0;
    for (;;) {
        const capacity = maximumBytes === null
            ? chunk.length
...
const physicalFileIdentity = (status) => `${status.ino}`;
//# sourceMappingURL=captureExecutableSnapshot.js.map