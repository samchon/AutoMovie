...
Scenarios are production-capable initial scripts. They translate reviewed storyline units into physically executable action, reaction, movement, transition, and consequential exchange without choosing final audience expression or implementation.

## Staging blocks {#staging-blocks}

After its evidence comment, every H3 scene and H4 beat begins with executable progression rather than a planning summary. One paragraph carries one followable action beat or continuous process and breaks at a meaningful action, reaction, time, or spatial change. Consequential speech is separated by speaker and kept beside the action, interruption, silence, or response that changes its effect.
...
This item owns the initial script's readable block structure. `executable-progression` owns which actions occur, and the common substantive obligation owns completeness.
...
Every H3 establishes its time, place, participants or active forces, observational range, inherited physical and social pressure, and the condition that starts its first action or process. Its first H4 inherits those facts and identifies the narrower state that begins the beat. The opening blocks make both boundaries executable rather than listing a field sheet.

Review question: could the scene begin from the predecessor with every participant, resource, and active constraint in a known state?
...
Every H3 leaves an observable state that a successor scene can inherit, including any changed choice, relation, knowledge, resource, position, physical condition, emotion, or audience interpretation. Each H4 likewise leaves the local state its next sibling or scene exit inherits. A scene may return to its initial condition only when the repetition or refusal itself leaves an identifiable result.

Review question: what exact changed or deliberately unchanged state exists when the scene ends?
...
Every H3 orders its complete progression through concrete H4 actions, reactions, movements, and processes whose time and causal relations are legible. Where an objective operates, name who attempts what, the available resistance or limit, and the response. Observation, ritual, montage, environment, and non-human process need no artificial conflict, but still state what changes in what order. Movement, objects, resources, environment, bodily or mechanical limits, and required time obey settings.

Review question: could every stated action be enacted with existing settings capabilities without another physical or production decision?
...
Every H3 tracks what each relevant participant knows, believes, suspects, or misunderstands before and after the scene, while each H4 owns any local information delta it creates. New information travels through an authored carrier such as a document, observation, statement, silence, absence, or physical result. When the audience is ahead of or behind a participant, state the extent and duration of that difference. A production that claims no private mental state says so and limits knowledge claims to observable response.

Review question: by what observable carrier does every information change reach the participant or audience entitled to receive it?

This item owns information movement inside the scene. Narrative horizontal continuity owns preservation between scenes, and storyline information design owns audience disclosure order.
...
## Scenario boundary {#scenario-boundary}

Every scenario unit completes physical progression and dramatic legibility while leaving final audience wording, visual and aural selection, camera and lighting grammar, editorial mapping, exact shot-owned time, representation construction, and source interpolation to the contracts that own them.

Review question: which sentence would have to move downward because it chooses final presentation or implementation rather than executable scene progression?
