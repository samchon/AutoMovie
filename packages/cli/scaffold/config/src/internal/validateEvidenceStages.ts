...
import { hasEvidenceHosts } from "./requiresReview";
...
const requireReviewedParent = (
  ...
  if (hasEvidenceHosts(child) && parent !== "review")
    throw new Error(
...
export function validateEvidenceStages(
  ...
  if (
    props.kind === "brief" &&
    [props.storylines, props.scenarios, props.script].some(hasEvidenceHosts)
  )
    throw new Error(
  ...
  if (hasEvidenceHosts(props.briefs)) {
    if (hasEvidenceHosts(props.models))
      requireReviewedParent("briefs", props.briefs, "models", props.models);
    if (hasEvidenceHosts(props.motions))
      requireReviewedParent("briefs", props.briefs, "motions", props.motions);
  }
  requireReviewedParent(
    "modelSources",
    props.modelSources,
  ...
  if (hasEvidenceHosts(props.shots)) {
  ...
  if (hasEvidenceHosts(props.filmSources)) {
    requireReviewedParent(
      "filmSources",
      props.filmSources,
      "shots",
