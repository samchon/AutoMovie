 ...
 */
export class ChorusMember extends AutoMovieSubject {
    id = "chorus-hero";
    /**
     ...
     */
    height = soloist.height - ChorusMember.HEAD;
    /**
    ...
    design() {
        ...
        if (Math.abs(this.height - ChorusMember.SPECIFIED_HEIGHT) >
            ChorusMember.SCALE_TOLERANCE)
            throw new Error(`docs/characters/chorus.md states a member is ${ChorusMember.SPECIFIED_HEIGHT} m, a head shorter than the soloist, but this one is ${this.height} m. Correct the reference scale or the head, not this record.`);
        return {
            id: this.id,
            role: "performer",
            archetype: "stickman",
...
//# sourceMappingURL=chorusHero.js.map