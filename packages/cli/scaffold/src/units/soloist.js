...
export class Soloist extends AutoMovieSubject {
     ...
     */
    cue(context, props = { from: 0 }) {
        const skeleton = this.skeleton(context);
        const pose = (abduction) => ({
            skeleton,
            root: null,
            joints: [
                {
        ...
        const key = (time, abduction, easing) => ({
            time,
            pose: pose(abduction),
            expression: null,
            easing,
        ...
        return {
            ...
            keyframes: props.from >= this.cueAbduction
    ...
    skeleton(context) {
        ...
        if (model === undefined || model.skeleton === null)
            throw new Error(`The compiler-owned "${this.id}" stickman model must provide a skeleton.`);
    ...
    modelRef(context) {
        const model = context.runtimeModels[this.id];
        if (model === undefined)
            throw new Error(`The compiler-owned "${this.id}" runtime model must be available.`);
        return model.id;
    }
    /**
...
//# sourceMappingURL=soloist.js.map