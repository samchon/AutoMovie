...
const OPENING_CONTRACT = {
    ...
    camera: {
        intent: "A full-body three-quarter view proves silhouette and pose-pass wiring.",
...
const ANSWER_CONTRACT = {
    ...
    camera: {
        intent: "A second full-body view proves that film rendering and review cannot stop after the opening shot.",
        requiredSubjects: ["soloist"],
        maxOcclusionRatio: 0.05,
    },
    events: [
...
const buildCue = (context, openingAbduction) => {
    ...
    return {
        ...
        formationMotions: context.contract.participants.some((participant) => participant.kind === "formation" && participant.id === chorus.id)
            ...
            : [],
        effectCues: context.world.effectZones.some((zone) => zone.id === "plaza-haze") &&
...
export const opening = defineShot("opening", {
    ...
    build: (context) => buildCue(context, 0),
});
/**
...
export const answer = defineShot("answer", {
    ...
    build: (context) => buildCue(context, soloist.cueAbduction),
});
