...
Dialogue uses the local Kokoro ONNX/WASM adapter and caches each normalized line by content, model, voice, and inference settings, so changing one line invalidates only that line.
