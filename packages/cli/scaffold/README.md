...
Valid quarantine markers resolve private evidence relative to their proxy or final ownership tier and are inventoried as one GC candidate with combined reclaimable bytes. Apply removes the evidence first, its captured marker second, and the exact empty private container last; invalid marker content leaves private evidence outside automatic deletion, and duplicate physical references across tiers remain untouched for manual adjudication.
