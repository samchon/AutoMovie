...
Proxy finalization serializes the exact logical file inventory and bytes into one deterministic regular-file container, creates that candidate through a descriptor-bound `O_EXCL` write, and commits its inode to the content-addressed destination with a no-overwrite hard link. It never adopts a directory created across a `mkdir`/capture gap and never deletes a destination or post-link candidate successor; abandoned hidden candidates become ordinary exact-snapshot GC inputs. GC retains a current proxy container only after its cached captured bytes pass self-described manifest verification, so a pathname ABA cannot turn a valid generation into a removal decision.
