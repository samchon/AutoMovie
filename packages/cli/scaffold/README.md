...
Every frame PNG and encoded `chunk.mp4` is likewise reserved and written directly at its final pathname inside one captured UUID temporary tree. The tree identity is checked before and after every parent/file operation and immediately before publication; no temp-file rename or failure cleanup can replace or delete a child successor, and partial evidence remains available to exact abandoned recovery.
