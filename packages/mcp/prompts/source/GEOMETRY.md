...
The shear is invisible in the place an author looks first. A developed atlas is exactly equiareal, so it never thins the texels out: a slate on a spire keeps its area and its resolution the whole way up, and the frame shows no blur to explain what is wrong. What it loses is the angle between the two axes.
