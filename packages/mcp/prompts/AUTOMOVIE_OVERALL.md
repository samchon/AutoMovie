...
6. Ask the structural questions before asking for pixels. Read `SUBJECT_INSPECTION`: it describes what actually compiled, diffs one compiled artifact against another, and cuts section planes through a building whose outside hides its inside. Those queries are ordinary package APIs rather than MCP tools, and that guide says which run in a project script and which need a live renderer. An extent, a placement, or a missing member is a number there and only an impression in a frame. When the question is how a thing looks rather than what it measures, the same guide owns `inspectSubject`, which draws one compiled subject from an inspection-owned turntable and sections a named space on its own; it refuses until you have read `SUBJECT_INSPECTION`, and it refuses outright any subject it cannot frame. Those images sit outside the render root and discharge no shot review, so they are how you decide what to fix and never what a shot review accepts. What a sweep does publish is a viewpoint plan and one revision-bearing receipt per observation, which the review surface reads back, so a subject review now reports a real coverage state rather than an absence. Read `REVIEW_SUBJECT` for what each state means and what completing one takes.
7. Read `CAPTURE_FRAME`, then call `captureTurntable` for the complete review-required turntable of one asset, or `captureFrame` for one exact shot or asset view. `captured:false` is a refusal, not evidence. Read `VISUAL_CHANGE_REPORT` before comparing two existing digest catalogs; that comparison is neither a structural diff nor review evidence.
