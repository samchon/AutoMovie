...
- **Geometry has persistent rigs and derived motion.** A successful resident `perform` persists each actor's rig and clinical rest frames in `actors/<node>.json`, so rest/ambient `getReach` and `getResolvedPose` survive a reopen. Resident `commitScene` also remembers supplied model skeletons for the current session. Compiled motion remains derived: `commitShot` remembers its registry only in this application session, so a moving `getResolvedPose({ beat })` or bone-targeted `getReach({ beat, t })` needs that same-session registry or explicit context. `measureDistance` needs only the stored scene.
