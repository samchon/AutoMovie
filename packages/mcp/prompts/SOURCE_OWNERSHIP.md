...
The build context contains immutable compiler-generated `runtimeModels` and `formationSlots`. Source returns event sample times, its authored scene, sparse motions, and shot. It does not return model recipes, formation nodes, or subjective proof that it met the contract. AutoMovie adds compiler-owned data and derives the realization from current scene, pose, motion, camera, and contract predicates.
