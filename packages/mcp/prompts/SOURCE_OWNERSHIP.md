...
The build context contains immutable compiler-generated `runtimeModels` and compact `formationRuntime`. `engine.formationSlot` regenerates one exact representative without serializing the army. Source returns event sample times, its authored scene, sparse motions, optional bounded formation-level cues, and shot. It does not return model recipes, anonymous formation nodes, arbitrary per-member curves, or subjective proof that it met the contract. AutoMovie adds compiler-owned data and derives the realization from current scene, pose, motion, camera, and contract predicates.
