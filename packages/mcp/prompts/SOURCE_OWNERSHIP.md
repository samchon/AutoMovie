...
The build context contains immutable compiler-generated `runtimeModels` and compact `formationRuntime`. `engine.formationSlot` regenerates one exact representative without serializing the group. Source returns a registered thin actor/script/stage/blocking/performance program, event sample times, optional `enact` clips, optional bounded formation/effect cues, and optionally its own generated models, semantic props, structured built environments, the observation and lineage records those buildings cite, and the fluid, cloth, planting, and service-network domains it stages with the bindings that attach them. It does not return a finished scene or shot, model recipes, imported asset bytes, anonymous formation nodes, arbitrary per-member curves, or subjective proof that it met the contract. AutoMovie runs the engine film pipeline, adds compiler-owned data, and derives realization from current scene, pose, motion, camera, and contract predicates.
