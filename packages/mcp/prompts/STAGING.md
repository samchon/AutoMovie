...
- `getReach` tells you whether the actor's arm chain can actually reach the target from its placement, per arm, with the reach gap and the IK pose. "Actually" means the rig's range of motion too: `reachable` is `withinShell` (the arm spans the distance) AND an empty `romViolations` (the IK pose is one the joints can hold), which is exactly what `perform` will decide. A long arm and an impossible elbow both answer `reachable: false`, and the two fields tell you which.
