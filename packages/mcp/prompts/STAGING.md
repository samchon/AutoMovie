...
Geometry queries are resident-or-explicit. Pass `scene`/`context` for a stateless check, or omit them after `openProject`: `measureDistance` reads the committed scene from disk, while `getReach` and `getResolvedPose` read each actor's persisted rig and clinical rest frames from `actors/<node>.json`. `getResolvedPose({ beat })` and a bone-targeted `getReach({ beat, t })` also use the compiled motions remembered from resident `commitShot`. Those motion clips are session memory, not project files; after reopening, rest/ambient geometry still resolves, while a moving beat needs a same-session `commitShot` or explicit context.
