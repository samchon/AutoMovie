...
Use subject inspection after compilation to ask what one authored thing is and what it looks like, on its own, without staging a shot around it. Routes answer different halves of that. The numbers come from ordinary `@automovie/engine` queries over an `IAutoMovieSubjectArtifact`, which is one `IAutoMovieCompiledShotSource` paired with the revision you read it at. The pictures come from `inspectSubject`, the MCP tool this document gates.
...
This ground is divided, and no guide on it substitutes for another. This one answers what a compiled subject is, how two compiled artifacts differ, and what one subject looks like from an eye the inspection chose. `VISUAL_CHANGE_REPORT` answers which already-rendered views moved between two revisions, as digests. `REVIEW_SUBJECT` is where a verdict is produced, and no question answered here produces one.
...
The walk has three entries and needs no key you invented. `builtEnvironmentUnclaimedElements(environment)` names the elements nothing else lists, meaning a root of the hierarchy that carries no space of its own; the spaces no other space parents name everything under them; and each element's `/members` carries its child elements beside its placed parts, so one step down is always available. A transform-only group opens like anything else and reports null for its transform, its content bounds, its materials and its prototype, because it stages nothing itself: what it carries is the structure, which is the reason to open it. Walking that way, `ExampleBuilding` answers 30 of 30 elements from 2 index roots with nothing refused.
...
The queries above say what a subject is. A description will not tell you that it looks wrong, and the page that would show you is a page you cannot open. `inspectSubject` is that half: name a `shot` and a `subject` id, and the tool derives a viewpoint plan from the subject's own measured box, has the host draw every viewpoint in it, and returns the plan, each resolved pose, the artifact path and digest of each picture, and a coverage record over its own plan. The sweep also publishes what it saw: a `plan.json` naming the viewpoint population, written before the first picture is drawn, and beside each picture a receipt carrying that observation and the revision it was taken at. `prepareReview` reads that publication, so a sweep is what makes a subject review completable; `REVIEW_SUBJECT` owns what it then does and does not discharge.
...
- `review-subject-viewpoint-unsupported` means the subject carries no measured extent to frame, which is the honest answer for a formation. Report that range as unsupported, not as observed. Note that `prepareReview` returns this same code as a standing warning about something else entirely; `REVIEW_SUBJECT` says what it means there.
