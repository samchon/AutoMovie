...
The catalog is an input, not a camera generator. This function creates no view and renders nothing, so the entries are yours to assemble. Never hash a PNG yourself, because the digest already exists: take a delivery entry's from the `digest` of a review evidence frame, whose time and pass belong in the view identity you mint for it, take a directly captured entry's from the `outputDigest` of a `captureFrame` receipt, and take an inspection entry's from `inspectSubject`, whose answer carries a revision for the sweep and one `views` row per viewpoint holding that viewpoint's id and the digest of the picture drawn there. This API draws none of them and never merges an inspection sweep into delivery evidence.

A sweep maps onto a snapshot without translation: the compiled subject id is the `subject`, the viewpoint id is the `view`, and the sweep's own revision is the snapshot's. Ask for the same turntable on both sides. A viewpoint id is built from the azimuth count and the elevation rings, so changing either between revisions moves the ids, and a report that should have read `unchanged` reads `gone` and `new` instead while nothing about the production changed. The raster reaches the ids too, by one indirect path worth knowing: it sets the fit distance, the distance sets the angle a low ring is lifted to, and a ring that had to be lifted is therefore lifted differently. For a subject standing on the ground that is the ordinary case. `SUBJECT_INSPECTION` owns that tool and its rule.

That a sweep produces digests is a fact about pictures and not about review coverage. Whether an observation can discharge a subject's viewpoint obligation is a separate question, `REVIEW_SUBJECT` answers it, and comparing digests here neither asks it nor settles it.
