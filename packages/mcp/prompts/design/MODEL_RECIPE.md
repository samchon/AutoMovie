...
That holds while each surface is built at its own size, and it stops holding the moment one is reached by scaling another. A placement carries coordinates through untouched, so a scaled member keeps numbers that no longer measure it and its `"surface-metres"` declaration is false by the scale factor, uniformly for a uniform scale and per-face for a non-uniform one. Nothing reports it: `validateTextureScale` measures a binding against the part's own coordinate span and never reads the part's transform. Build a second member where the size differs rather than scaling one that carries a finish. `GEOMETRY` works the arithmetic.
