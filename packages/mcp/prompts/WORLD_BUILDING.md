...
An analysis run names a domain from the closed set `daylight`, `artificial-light`, `thermal`, `moisture`, `air`, `acoustic`, the design revision it read, its solver, its targets, and its metrics. A target carries its unit beside its value, because 300 lux and 300 candela are different requirements and a bare number would let one clear the other. Every metric comes back `meets`, `misses`, `untargeted` (a number exists but nobody said what good is), `unsupported` (no adapter), or `not-run` (an adapter exists but did not execute), and the last two carry a reason and the exact change that would produce a value.
...
Phases are a graph of prerequisites, not a line. Variants are alternatives preserved side by side over their base revisions, and decisions are the open and settled comparisons between them, so "we considered the other stair and rejected it" is a record rather than a memory. Every derived artifact stamps the revision it was computed from, which is what lets a drawing, a schedule, a take-off, or an analysis be caught reading a superseded design instead of quietly reporting last week.
...
All three carry declared budgets, produce a state digest, and are seekable to a step. Check the budget before the shot, compare the digest across machines, and never claim a simulated result from a domain you ran in a rest or static mode.
