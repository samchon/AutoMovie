...
To bind an external appearance, set `asset` to one exact glTF, GLB, or VRM path registered in `.automovie/assets.json`. Its typed `model` record must use one supported ingest profile (`gltf-static-v1`, `gltf-humanoid-v1`, or `vrm-humanoid-v1`), bind its own bytes as the first `hero` LOD, register every external buffer/image sidecar, and declare both deterministic proxies. Collision accepts `capsule-v1 { radius, height }` or `box-v1 { width, height, depth }`; measurement accepts `box-v1` or `humanoid-landmarks-v1 { height, shoulderWidth, hipWidth }`. Every value is finite and strictly positive. A proxy may instead cite a registered version-1 JSON proxy asset carrying the matching closed shape.

The compiler ingests and fingerprints the fixed model bytes, exposes the registered asset as the runtime model's final imported appearance, and keeps the proxy geometry for engine validation, collision, mass, distance, and projection semantics. The scaffold viewer loads the final mesh through the imported-model adapter for shots, films, turntables, and ROM-extremes review. A malformed model, unregistered sidecar, unsupported profile, missing rig root, or absent/invalid proxy is a compile error; there is no inferred mesh fallback.
