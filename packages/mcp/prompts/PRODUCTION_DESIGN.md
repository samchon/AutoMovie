...
Choose one stable id, title, logline, target runtime, frame clock, primitive-3D art grammar, and deliverable inventory. Runtime is the intended finished duration, not the duration of the current sample. Width, height, and fps become one clock-and-raster contract reused by compile, preview, review, captions, and render. Width and height are each bounded to 16,384 and their product must not exceed 16,777,216 pixels, because every required review frame must be capturable at the exact production raster.
