...
The queries above say what a subject is. A description will not tell you that it looks wrong, and the page that would show you is a page you cannot open. `inspectSubject` is that half: name a `shot` and a `subject` id, and the tool derives a viewpoint plan from the subject's own measured box, has the host draw every viewpoint in it, and returns the plan, each resolved pose, the artifact path and digest of each picture, and a coverage record over its own plan. That coverage is the tool's reading of its own sweep; what a subject review's worksheet reports is a separate question `REVIEW_SUBJECT` owns.
...
Do not carry a viewpoint id between the page and the tool. The page lays its turntable out against a fixed reference aspect and the tool against the raster its caller asked for, so one subject plans different id sets under the two. A viewpoint id is a property of the look that was taken, not of the subject, and it is comparable only against a plan built under the same rule. Subject ids do travel between them, which is what a finding should carry.
