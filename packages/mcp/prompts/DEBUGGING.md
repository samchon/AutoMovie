...
`review-subject-viewpoint-unsupported` is not the refusal it used to be, and misreading it costs a correction round. A viewpoint plan source now exists: `inspectSubject` derives a turntable from the subject's own measured extent, draws every viewpoint in it, and reports its own coverage. What it does not do is publish a record `prepareReview` can read, because the images it leaves behind state no compiled revision and no plan, so an image drawn before a recompile cannot be told from one drawn after and counting either would be a fabricated pass. Holding a coverage record that reads `reviewed` therefore does not move the one this surface folds. Read the diagnostic's own message, record what you inspected structurally, leave the viewpoint range explicitly unobserved, and spend no correction attempts on it.
