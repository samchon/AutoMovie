...
Three cases hold this corpus to the surfaces it teaches. `test_mcp_guide_corpus_closure` pins that the authored Markdown and the served names are one set and that `AUTOMOVIE_OVERALL` names every other served guide. `test_mcp_guide_snippet_compilation` compiles every `ts` example. `test_mcp_guide_surface_reachability` refuses a name a guide writes in call form that the sandbox does not publish, and a shot-source example carrying an import the gate would reject.

Nothing gates a guide's prose. Fixed lists once pinned four doctrine sentences, the five tool-name mentions, retired call vocabulary and retired guide filenames; `#1900` removed that class of check as repository shape rather than product behavior, and they are not coming back. The writing rules above are what holds them, and review is what enforces them.
