...
Preview paths include the target-local generated/viewer/capture identity, host-declared browser and graphics backend, and render-spec fingerprint. The manifest also retains the aggregate compile fingerprint as provenance and records every frame index, rational time, guide pass, path, digest, and dimensions. A change that can alter this target's pixels, renderer, frame, or manifest makes old visual review stale; an unrelated shot-source edit does not discard still-current evidence.
