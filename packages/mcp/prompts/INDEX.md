...
2. Read `EVIDENCE_GRAPH`, choose exactly one production kind, and activate only the layers that result requires. A `film` authors settings, storylines, scenarios, script, and the typed screenplay index in that order; read `SCREENPLAY_WRITING` for its craft. A `brief` authors settings and one bounded brief before its shots. A `library` may author settings, models, and motions without any timed-result prose. Every active rung cites its own declared predecessors, the graph refuses a rung that nothing below elaborates, and it refuses residual hosts for a disabled layer. Production kind records what is being delivered; it is never a shortcut around a narrative the result actually contains.
