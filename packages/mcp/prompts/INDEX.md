...
2. Read `EVIDENCE_GRAPH`, choose exactly one production kind, and activate only the layers that result requires. A `film` authors settings, storylines, scenarios, script, and the typed screenplay index in that order; read `SCREENPLAY_WRITING` for its craft. A `brief` authors settings and one bounded delivery/shot/observation hierarchy before its shots, including a simple short whose complete intent needs no independent narrative refinement. A `library` may author settings and design/source branches without any timed-result prose. Every active downstream rung accounts for its declared predecessors, and the graph refuses both an active empty layer and residual hosts for a disabled layer. Production kind records what is being delivered; it never hides a narrative-refinement layer the result actually needs.
