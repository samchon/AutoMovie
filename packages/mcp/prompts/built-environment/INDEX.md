...
The scaffold ships worked examples under `src/examples/`: a building assembled by loops, a physically-based finish and its image bindings, props declaring placement relations, a seeded instance set, an observed plan that is read rather than traced, and a renovation phased over identities the building already published. Read the one nearest your problem before writing a new pattern; they are examples of technique, not a content library to copy into a production. Two of them, `drawings.ts` and `services.ts`, demonstrate calls that belong to a project script rather than to shot source, so copy those into `scripts/` and not into `src/`; a later section says why.
...
## One environment per staged area, not one per production

A shot's contract carries the built environments its build function returns, and it carries them whole. So a production that authors **one** environment for everything gives every shot the entire building: measured on a `#1954` production, the front exterior elevation's generated contract was 155 KB and contained the courtyard's `offering` 242 times, its `fountain` 68 times, and its `basin` 14 times, and all twelve contracts were within a few bytes of each other. Adding furniture to one interior room then changed every contract, which staled 21 of 52 review targets for a change no exterior frame could show.

The lever is how many environments the production authors. A production that builds one environment per staged area — a wing, a courtyard, an elevation — hands each shot only the area it stages, and a change reaches the shots that face it and stops. Measured on a production that does: one new curtain wall moved 22 of 222 artifacts, and the only two shots whose pixels moved were the two that face it.

There is no way to take a subset of one environment after the fact. A valid environment is a graph — spaces, connectors, boundaries — and slicing it is not a supported operation. Visibility culling does not stand in for it either: a door is a portal and the exterior is a node, so an ordinary building's portal graph is fully connected and `autoMovieRoomVisibility` hides nothing on it. Decide the division while authoring, because it cannot be recovered later.
