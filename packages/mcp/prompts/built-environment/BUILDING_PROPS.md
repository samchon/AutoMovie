...
A body's box is not its body, and the placement answers know the difference. A shelf that is a back panel and two boards has a box spanning the floor to head height and is mostly air, so support and overlap are judged against the parts the element is drawn from rather than that box: the bearing face is the part top nearest the subject's underside among the parts its footprint covers, and two bodies meet only where their parts do. `builtEnvironmentElementPartBounds(` is the same reading in your own hands, one world box per drawn part. The box a bounds query reports is still the union, because that is what the element occupies; what changed is what a verdict is taken from.
