...
The coding agent writes the build function in `src`. It may use loops, helpers in the supported source boundary, explicit design seeds, compiler-provided `runtimeModels` and compact `formationRuntime`, and deterministic engine oracles. It returns `eventSamples`, its authored scene, sparse actor/object motions, optional bounded `formationMotions`, and shot. For every contract event, choose exactly one sample inside its declared window. Formation cues explicitly name a participating formation and compact translation/facing/spacing states; capability labels never create or authorize them. Do not return models, anonymous formation nodes, per-member curves, or a contract-compliance witness: the compiler owns and measures those facts.
