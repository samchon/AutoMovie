const HEADING = /^#{1,6}[ \t]+(SCN-[A-Za-z0-9-]+)[ \t]+(?:—|-|:)[ \t]+(.+?)[ \t]*$/u;
/**
 ...
 */
const comparableProse = (value) => value.split(/\s+/u).filter(Boolean).join(" ");
/**
 ...
 */
export const parseScreenplayProse = (content) => {
    const scenes = [];
    let current;
    let fence;
    let fenceLength = 0;
...
export const screenplayProseDiagnostics = (props) => {
    ...
    const read = (relativePath) => {
        ...
        if (content === null)
            refuse("screenplay-document-absent", `The screenplay index addresses "${relativePath}", which is not a readable project file. The index dangles from prose nobody can open. Restore the document or correct the path, then compile again.`, relativePath);
        return content;
    };
    // --- Treatment beats appear verbatim in the prose that promises them ------
    for (const sequence of screenplay.treatment.sequences) {
        const documentPath = sequence.path !== undefined && sequence.path.trim().length !== 0
            ? sequence.path
        ...
        for (const beat of sequence.beats)
            if (beat.text.trim().length !== 0 &&
                comparable.includes(comparableProse(beat.text)) === false)
                refuse("screenplay-beat-unwritten", `Treatment beat "${beat.id}" is indexed, but its exact prose is absent from "${documentPath}". The machine index would promise text the human document does not contain. Restore the exact prose or update both records, then compile again.`, documentPath);
    }
    // --- Scene headings ------------------------------------------------------
    const owned = new Map();
    ...
    const collect = (documentPath, only) => {
        const content = read(documentPath);
        if (content === null)
            return;
        for (const parsed of parseScreenplayProse(content)) {
            if (only !== undefined && parsed.id !== only)
                continue;
            if (only === undefined && owned.has(parsed.id))
                continue;
    ...
    for (const [id, documentPath] of owned)
        collect(documentPath, id);
    const declared = new Set(screenplay.screenplay.scenes.map((scene) => scene.id));
    for (const [id, entries] of headings) {
        const documentPath = headingPath.get(id);
        if (entries.length !== 1) {
            refuse("screenplay-heading-repeated", `Scene id "${id}" heads ${entries.length} sections of "${documentPath}". One stable id must occur on exactly one heading line, or a downstream citation cannot say which prose it cites. Keep one heading and compile again.`, documentPath);
            continue;
        }
        if (declared.has(id) === false)
            refuse("screenplay-heading-unindexed", `"${documentPath}" heads a scene "${id}" the index does not declare. Downstream records cannot cite an unindexed scene, so this prose is unreachable. Add it to the index or remove the heading, then compile again.`, documentPath);
    }
    for (const scene of screenplay.screenplay.scenes) {
        const documentPath = owned.get(scene.id) ?? screenplay.screenplay.path;
        if (documents.get(documentPath) === null)
            continue;
        const entries = headings.get(scene.id) ?? [];
        if (entries.length === 0) {
            refuse("screenplay-heading-absent", `Scene "${scene.id}" is indexed, but "${documentPath}" heads no such scene. Downstream citations dangle from the human screenplay. Restore the heading and compile again.`, documentPath);
            continue;
        }
        if (entries.length !== 1)
            continue;
        ...
        if (entry.title !== scene.title)
            refuse("screenplay-heading-retitled", `Scene "${scene.id}" is titled "${scene.title}" in the index and "${entry.title}" in "${documentPath}". Human prose and machine identity have diverged, so neither can be trusted to name the other. Make the titles exact and compile again.`, documentPath);
        if (scene.status === "active" && entry.body.trim().length === 0)
            refuse("screenplay-scene-unwritten", `Active scene "${scene.id}" has a heading in "${documentPath}" and no prose beneath it. A heading-only ledger entry cannot satisfy authored dramatic work. Write the scene and compile again.`, documentPath);
    }
    return diagnostics;
};
//# sourceMappingURL=screenplayProseDiagnostics.js.map