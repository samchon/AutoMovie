...
export const muxProductionFeatureMp4 = (props) => {
    ...
    if (videoProbe.kind !== "video" ||
        audioProbe.kind !== "audio" ||
        videoProbe.runtimeSeconds !== audioProbe.runtimeSeconds)
        throw new Error("Feature mux requires byte sources with exactly equal track runtimes.");
    ...
    trimProductionAudioPresentation({
        ...
        presentationSamples: Math.round(audioProbe.runtimeSeconds * audioProbe.sampleRate),
    });
...
export const conformProductionRenditionVideoMp4 = (props) => {
    const { parsed, first, description, sampleDuration, mediaDuration } = productionRenditionVideoPlan(props);
    const output = createFile();
    output.init({
    ...
    for (const clip of parsed) {
        for (const sample of clip.samples) {
            ...
            const ctsFrame = (sample.cts - clip.presentationStart) / clip.sampleDuration;
            output.addSample(trackId, Uint8Array.from(clip.bytes.subarray(sample.offset, sample.offset + sample.size)), {
    ...
    if (frame !== props.timeline.totalFrames)
        throw new Error("Repaint clip samples do not cover the exact current film timeline.");
    const bytes = new Uint8Array(output.getBuffer().buffer);
    ...
    if (probe.kind !== "video" ||
        ...
        throw new Error("Conformed repaint video failed exact parser verification.");
    return bytes;
};
/**
...
export const assertProductionRenditionClipDelivery = (props) => {
    ...
    if (clip.probe.width !== props.width ||
        ...
        throw new Error(`Repaint clip "${props.shot}" does not match its exact raster, rational frame clock, frame count, and runtime contract.`);
};
...
export const assertProductionFeatureUsesRenditionClips = (props) => {
    ...
    const samples = track === undefined ? [] : actual.file.getTrackSamplesInfo(track.id);
    if (track === undefined ||
        ...
        throw new Error("Feature video presentation does not match the selected repaint timeline.");
    let index = 0;
    for (const clip of plan.parsed) {
        ...
        for (const source of clip.samples) {
            ...
            const ctsFrame = (source.cts - clip.presentationStart) / clip.sampleDuration;
            const sourceBytes = clip.bytes.subarray(source.offset, source.offset + source.size);
            const actualBytes = props.feature.subarray(actualSample.offset, actualSample.offset + actualSample.size);
            if (actualSample.duration !== plan.sampleDuration ||
                ...
                sameSampleDescription(sampleDescription(actualSample), plan.description) === false ||
                actualBytes.length !== sourceBytes.length ||
                actualBytes.some((value, offset) => value !== sourceBytes[offset]))
                throw new Error(`Feature video sample ${index} differs from the selected repaint timeline:\n${JSON.stringify({
                    timing: {
                        actual: {
                    ...
                    sampleDescriptionMatches: sameSampleDescription(sampleDescription(actualSample), plan.description),
                    payload: {
                        actualBytes: actualBytes.length,
                        expectedBytes: sourceBytes.length,
                        firstDifferingActualByte: actualBytes.findIndex((value, byte) => value !== sourceBytes[byte]),
                    },
                }, null, 2)}`);
            ++index;
        }
...
export const assertProductionFeatureUsesRenditionVideo = (props) => {
    ...
    if (actualTrack === undefined || expectedTrack === undefined)
        throw new Error("Rendition delivery proof requires one feature video track.");
    ...
    if (actualTrack.timescale !== expectedTrack.timescale ||
        ...
        sameSampleDescription(sampleDescription(actualSamples[0]), sampleDescription(expectedSamples[0])) === false)
        throw new Error("Feature video track does not match the selected rendition track.");
    for (let index = 0; index < actualSamples.length; ++index) {
        const left = actualSamples[index];
        const right = expectedSamples[index];
        const leftBytes = props.feature.subarray(left.offset, left.offset + left.size);
        const rightBytes = props.renditionVideo.subarray(right.offset, right.offset + right.size);
        if (sameSampleTiming(left, right) === false ||
            sameSampleDescription(sampleDescription(left), sampleDescription(right)) === false ||
            leftBytes.length !== rightBytes.length ||
            leftBytes.some((value, offset) => value !== rightBytes[offset]))
            throw new Error(`Feature video sample ${index} differs from the selected rendition.`);
    }
};
const productionRenditionVideoPlan = (props) => {
    const parsed = props.timeline.segments.map((segment) => {
        if (segment.transitionIn.kind !== "cut" ||
            segment.transitionOut.kind !== "cut")
            throw new Error(`Repainted feature delivery currently requires cut-only editing; shot "${segment.shot}" declares a transition.`);
        const bytes = props.clips.get(segment.shot);
        if (bytes === undefined)
            throw new Error(`Repainted feature delivery is missing the current clip for shot "${segment.shot}".`);
        const clip = parseProductionRenditionClip(bytes, segment.shot);
        if (segment.sourceInFrame !== 0 ||
            ...
            throw new Error(`Repainted feature delivery requires one full-shot ${props.timeline.fps}fps clip for segment "${segment.shot}"; partial trims and mismatched media are not representable yet.`);
        return { segment, ...clip };
    });
    const first = parsed[0];
    if (first === undefined)
        throw new Error("Repainted feature delivery requires a non-empty timeline.");
    const description = sampleDescription(first.samples[0]);
    let nextFrame = 0;
    for (const clip of parsed) {
        if (clip.segment.startFrame !== nextFrame)
            throw new Error(`Repaint clip "${clip.segment.shot}" does not begin at the next continuous film frame.`);
        if (clip.probe.width !== first.probe.width ||
            clip.probe.height !== first.probe.height ||
            sameSampleDescription(description, sampleDescription(clip.samples[0])) === false)
            throw new Error(`Repaint clip "${clip.segment.shot}" changes dimensions or H.264 decoder configuration within the feature.`);
        nextFrame = clip.segment.endFrame;
    }
    if (nextFrame !== props.timeline.totalFrames)
        throw new Error("Repaint clips do not cover one continuous current film timeline.");
    ...
    if (Number.isSafeInteger(mediaDuration) === false)
        throw new Error("Repainted feature duration exceeds the exact MP4 clock range.");
    return { parsed, first, description, sampleDuration, mediaDuration };
};
const parseProductionRenditionClip = (bytes, shot) => {
    const probe = probeProductionVideoMp4(bytes);
    const mp4 = parseMp4(bytes);
    const track = mp4.movie.videoTracks[0];
    const samples = track === undefined ? [] : mp4.file.getTrackSamplesInfo(track.id);
    if (probe.kind !== "video" ||
        track === undefined ||
        samples.length !== probe.frameCount ||
        samples.length === 0)
        throw new Error(`Repaint clip "${shot}" has no exact parser-owned H.264 sample inventory.`);
    const sampleDuration = samples[0].duration;
    const presentationStart = samples.reduce((minimum, sample) => Math.min(minimum, sample.cts), samples[0].cts);
    ...
    if (samples[0].is_sync !== true ||
        ...
        samples.some((sample, index) => Number.isSafeInteger(sample.dts) === false ||
            Number.isSafeInteger(sample.cts) === false ||
            sample.dts !== index * sampleDuration ||
            Number.isSafeInteger((sample.cts - presentationStart) / sampleDuration) === false ||
        ...
        throw new Error(`Repaint clip "${shot}" must start independently, use one decoder configuration, and expose one complete untransformed rational-clock presentation.`);
    return {
        bytes,
        probe,
        track,
...
const neutralTrackPresentation = (track, presentationStart) => {
    const identityMatrix = [65_536, 0, 0, 0, 65_536, 0, 0, 0, 1_073_741_824];
    if (Array.from(track.matrix).some((value, index) => value !== identityMatrix[index]) ||
        Number.isSafeInteger(track.movie_timescale) === false ||
        Number.isSafeInteger(track.movie_duration) === false ||
        track.movie_timescale <= 0 ||
        equalRational(track.movie_duration, track.movie_timescale, track.duration, track.timescale) === false)
        return false;
    const edits = track.edits ?? [];
    return ((edits.length === 0 && presentationStart === 0) ||
        (edits.length === 1 &&
            edits[0].media_time === presentationStart &&
            edits[0].media_rate_integer === 1 &&
            edits[0].media_rate_fraction === 0 &&
            equalRational(edits[0].segment_duration, track.movie_timescale, track.duration, track.timescale)));
};
const sameTrackPresentation = (left, right) => Array.from(left.matrix).every((value, index) => value === Array.from(right.matrix)[index]) && JSON.stringify(left.edits ?? []) === JSON.stringify(right.edits ?? []);
const equalRational = (leftNumerator, leftDenominator, rightNumerator, rightDenominator) => [leftNumerator, leftDenominator, rightNumerator, rightDenominator].every(Number.isSafeInteger) &&
    BigInt(leftNumerator) * BigInt(rightDenominator) ===
        BigInt(rightNumerator) * BigInt(leftDenominator);
const sameSampleTiming = (left, right) => left.duration === right.duration &&
    left.cts === right.cts &&
    left.dts === right.dts &&
    sameSampleFlags(left, right);
const sameSampleFlags = (left, right) => left.is_sync === right.is_sync &&
...
const canonicalSampleDependsOn = (sample) => sample.is_sync === true && sample.depends_on === 0 ? 2 : sample.depends_on;
/** Bounded parser-visible flag evidence for one failed sample comparison. */
const sampleFlagRecord = (sample) => ({
...
const serializeDescriptionBox = (box) => {
    const stream = new DataStream();
    box.write(stream);
    return new Uint8Array(stream.buffer);
};
const sameSampleDescription = (left, right) => left.type === right.type &&
...
const copyTrack = (props) => {
    ...
    for (const sample of samples)
        props.output.addSample(id, Uint8Array.from(props.bytes.subarray(sample.offset, sample.offset + sample.size)), sampleOptions(sample));
    return id;
};
...
//# sourceMappingURL=muxProductionFeatureMp4.js.map