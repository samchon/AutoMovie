import { compileDefinedShot, defineShot, formationSlotPosition, heightAt, makeActorSynthesizer, placeFormationSlot, realizeShotContract, sampleFormationMotion, sampleFormationSlotMotion, validateModel, validateMotion, validateShotArtifact, } from "@automovie/engine";
...
import { AutoMovieProductionInputRaceError, AutoMovieProductionSourcePathError, } from "./AutoMovieProductionProject";
import { AUTOMOVIE_COMPILE_FINGERPRINT_PROTOCOL, canonicalAutoMovieJsonBytes, compareCodeUnits, digestAutoMovieBytes, encodeAutoMoviePathSegment, fingerprintAutoMovieFields, normalizeAutoMovieSource, } from "./contentIdentity";
import { readAutoMovieFilmTimeline } from "./filmTimeline";
import { AUTOMOVIE_SANDBOX_ENGINE_EXPORTS, isProjectSourceSpecifier, linkProductionSource, } from "./linkProductionSource";
import { materializeCompiledFormationInventory, materializeCompiledInstanceSetInventory, materializeCompiledShot, materializeProductionModels, } from "./materializeProduction";
...
export const AUTOMOVIE_PRODUCTION_COMPILER_VERSION = require(path.join(__dirname, "..", "..", "package.json")).version;
/**
...
export class AutoMovieProductionCompiler {
    project;
    reviewQueue;
    constructor(project, reviewQueue = () => ({
        entries: [],
    })) {
    ...
    run(input, materialize) {
        ...
        const diagnostics = [
            ...
            ...validateAutoMovieProductionGraph(graph, this.project.productionId, archetypes),
        ];
        const designReady = diagnostics.every((diagnostic) => diagnostic.category !== "error");
        ...
        const readLinkedSource = (relative) => Buffer.from(normalizeAutoMovieSource(this.project.readSource(relative))).toString("utf8");
        const sourceFields = [];
        const contentFields = [];
        let contentInputs;
        let declaredAssets = [];
        ...
        if (input.scope !== "design")
            try {
                ...
                const assetInventory = compilerAssetInventory(projectManifest.assetManifest, contentInputs, graph.production?.id ?? this.project.productionId, graph, archetypes);
        ...
        let formationRuntime = {};
        let instanceSetRuntime = {};
        let filmSource = null;
        let filmSourceDigest = null;
        if (input.scope !== "design" && designReady) {
            runtimeModels = new Map(materializeProductionModels(graph.models, externalModels, archetypes));
            formationRuntime = materializeCompiledFormationInventory(graph.formations, graph.models, externalModels, graph.world.surfaces, archetypes);
            instanceSetRuntime = materializeCompiledInstanceSetInventory(graph.world, graph.models, externalModels, archetypes);
        }
        const shotSources = new Map();
        for (const [id, contract] of graph.shots) {
            ...
            try {
                ...
                diagnostics.push(sourcePathDiagnostic(id, contract.source.module, error));
                sourceFields.push({
                    role: `source:${id}`,
                    kind: "absent",
                    payload: new Uint8Array(),
        ...
        if (input.scope === "design")
            ...
            try {
                filmSource = normalizeAutoMovieSource(this.project.readSource(FILM_SOURCE_PATH));
        ...
        if (input.scope !== "design" && designReady) {
            ...
            for (const entry of shotCompileOrder(graph.shots, filmEditSource?.value ?? null)) {
                const normalized = shotSources.get(entry.id);
                let closing = null;
                if (normalized !== undefined) {
                    const previous = previousVideo !== null &&
                        entry.placement !== null &&
                        fullHardCutBoundary(previousVideo, entry, graph.production.frameFormat.fps)
                    ...
                    if (result.value !== null) {
                        ...
                        if (postDiagnostics.every((diagnostic) => diagnostic.category !== "error"))
            ...
            diagnostics.push(...storySyncDiagnostics({
                acceptance: graph.acceptance,
                contracts: graph.shots,
                realizations,
            }));
        }
        let compiledFilm = null;
        if (filmContext !== null && filmEditSource !== null) {
            ...
            if (film.value !== null) {
                const useDiagnostics = validateCompiledAssetUses(graph.production.id, assetRecords, film.value.edit);
                diagnostics.push(...useDiagnostics);
                if (useDiagnostics.length === 0)
                    compiledFilm = film.value;
            }
        }
        const inputFingerprint = productionCompilerInputFingerprint(this.project.productionId, graph, sourceFields, contentFields);
        const filmArtifacts = compiledFilm === null || filmSourceDigest === null
            ? null
            : materializeFilmArtifacts(compiledFilm, filmSourceDigest, inputFingerprint);
        const inputCurrent = () => `${this.project.revision()}\0${currentAutoMovieProductionCompilerInputFingerprint(this.project, input.scope)}\0${this.project.revision()}` ===
            `${inputRevision}\0${inputFingerprint}\0${inputRevision}`;
        const files = input.scope === "design"
            ? null
            : materializeGeneratedFiles(this.project.productionId, graph, runtimeModels, compiled, realizations, filmArtifacts, inputFingerprint);
        const entries = files === null
            ? []
            : [...files]
                .map(([file, bytes]) => ({
                path: file,
        ...
        if (manifest !== null)
            diagnostics.push(...this.generatedOwnershipDiagnostics(manifest, materialize));
        ...
        const reviewSnapshot = contentInputs === undefined
            ? undefined
            : {
        ...
        const reviews = diagnostics.some((diagnostic) => diagnostic.code === "content-input-unsafe") || input.scope === "design"
            ? { entries: [] }
            : this.reviewQueue(statusForReview(), reviewSnapshot);
        const screenplay = this.project.screenplayIndex();
        diagnostics.push(...screenplayResidencyDiagnostics({ contracts: graph.shots, screenplay }), ...screenplayLedgerDiagnostics({
            acceptance: graph.acceptance,
            contracts: graph.shots,
            screenplay,
        ...
        if (input.scope !== "design")
            diagnostics.push(...screenplayCoverageDiagnostics({
                acceptance: graph.acceptance,
                contracts: graph.shots,
                realizations,
                scope: input.scope,
        ...
        if (input.scope === "final")
            diagnostics.push(...finalDeliverableDiagnostics(this.project, graph.production, inputFingerprint, reviews));
        diagnostics.sort(compareDiagnostics);
        const inputRaceFailure = (message) => {
            diagnostics.push({
        ...
        let revision;
        try {
            revision = this.project.commitGenerated(sourceFiles, sourceManifest, inputCurrent, inputRevision);
        }
        catch (error) {
            if (error instanceof AutoMovieProductionInputRaceError === false)
                throw error;
    ...
    generatedOwnershipDiagnostics(expected, repairDeclaredFiles) {
        const manifest = this.project.generatedManifest();
        const generatedManifestPath = normalizeSlash(path.relative(this.project.root, this.project.trackedStatePath("generated-manifest.json")));
        const diagnostics = [];
        const expectedByPath = new Map(expected.files.map((file) => [normalizeSlash(file.path), file]));
        const declaredByPath = new Map((manifest?.files ?? []).map((file) => [normalizeSlash(file.path), file]));
        if (manifest === null)
            diagnostics.push({
        ...
        for (const file of listFiles(this.project.generatedRoot())) {
            const relative = normalizeSlash(path.relative(this.project.generatedRoot(), file));
        ...
        for (const entry of expected.files) {
            ...
            let actual = null;
            try {
                actual = digestAutoMovieBytes(this.project.readGeneratedFile(entry.path));
            }
            catch (error) {
        ...
        if (manifest !== null &&
            Buffer.from(canonicalAutoMovieJsonBytes(manifest)).equals(Buffer.from(canonicalAutoMovieJsonBytes(expected))) === false)
            diagnostics.push({
...
const resolvedFilmFrame = (time, fps) => {
 ...
 */
const fullHardCutBoundary = (previous, current, fps) => {
    ...
    const previousDuration = resolvedFilmFrame({ seconds: previous.contract.durationSeconds }, fps);
    return (current.placementIndex === previous.placementIndex + 1 &&
...
const compileShotSource = (props) => {
    const program = compileDeterministicSource({
        ...
        validate: (input) => typia.validateEquals(input),
    });
    ...
    const runtime = actorRuntimeOf(program.value, props.context.runtimeModels, `shot:${props.id}`, props.path);
    ...
    const clipById = new Map((program.value.clips ?? []).map((clip) => [clip.id, clip]));
    const referenceSynthesizer = makeActorSynthesizer(runtime.actors, runtime.nodes);
    const compiled = compileDefinedShot({
        shot,
        context: undefined,
        runtime: {
            synthesize: (action, actor, previous) => action.verb === "enact"
    ...
    return {
        value: {
            ...
            formationSlotMotions: structuredClone(program.value.formationSlotMotions ?? []),
...
const contractOfRegistration = (contract) => {
    const { id: _id, source: _source, ...registration } = contract;
    return registration;
};
/** Bind a thin program's actor facts to compiler-owned runtime models. */
const actorRuntimeOf = (program, runtimeModels, target = `shot:${program.blocking.beat}`, sourcePath = null) => {
    const diagnostics = [];
    const actors = new Map();
    const models = new Map();
    const stageActors = new Map(program.stage.actors.map((actor) => [actor.node, actor]));
    program.actors.forEach((actor, index) => {
        ...
        const gaitNames = new Set();
        const gaits = model?.profiles
    ...
    const nodes = new Map([
        ...program.stage.actors.map((actor) => [actor.node, actor.position]),
        ...(program.stage.set ?? []).map((piece) => [piece.node, piece.position]),
        ...program.stage.cameras.map((camera) => [camera.node, camera.position]),
    ]);
...
const compileDeterministicSource = (props) => {
    ...
    const imported = linked.modules.filter((module) => module.path !== props.path);
    for (const module of imported)
        diagnostics.push(...inspectSource(props.target, module.path, module.source));
    if (diagnostics.some((diagnostic) => diagnostic.category === "error"))
        return { value: null, diagnostics };
    const transpiledImports = [];
    for (const module of imported) {
    ...
    try {
        ...
        for (const module of transpiledImports)
            new vm.Script(`__automovieDefine(${JSON.stringify(module.path)}, ${JSON.stringify(module.imports)}, (module, exports, require) => {\n${module.output}\n});`, { filename: module.path }).runInContext(sandbox, { timeout: 1_000 });
        new vm.Script(transpiled.output, {
            filename: props.path,
        }).runInContext(sandbox, { timeout: 1_000 });
        sandbox.__automovieExportName = props.exportName;
        new vm.Script(`globalThis.__automovieExportValid =
        typeof module.exports[__automovieExportName]?.build === "function";
       delete globalThis.__automovieExportName;`, { filename: `${props.path}#export` }).runInContext(sandbox, { timeout: 1_000 });
        ...
        if (props.registration !== undefined) {
            sandbox.__automovieExportName = props.exportName;
            new vm.Script(`globalThis.__automovieRegistrationJson = (() => {
         ...
         delete globalThis.__automovieExportName;`, { filename: `${props.path}#registration` }).runInContext(sandbox, { timeout: 1_000 });
            const registrationJson = sandbox.__automovieRegistrationJson;
            const registration = typeof registrationJson === "string"
                ? JSON.parse(registrationJson)
                : null;
            const contractMatches = registration !== null &&
                typeof registration.contract === "object" &&
                registration.contract !== null &&
                Buffer.from(canonicalAutoMovieJsonBytes(registration.contract)).equals(Buffer.from(canonicalAutoMovieJsonBytes(props.registration.contract)));
        ...
        const resultJson = sandbox.__automovieResultJson;
        const value = typeof resultJson === "string" ? JSON.parse(resultJson) : undefined;
 ...
 */
const compileFilmEditSource = (props) => compileDeterministicSource({
    target: "film",
    label: "film edit",
...
const compileFilmSource = (props) => {
    ...
    const targetFrames = frameTime({ seconds: props.context.production.targetRuntimeSeconds }, fps, "production target runtime", diagnostics);
    if (edit.id !== props.context.production.id)
        diagnostics.push(filmDiagnostic("film-id-mismatch", `Film id "${edit.id}" differs from production id "${props.context.production.id}". Return the current production id from ${FILM_SOURCE_PATH}.`));
    const omitted = new Set();
    for (const omission of edit.omissions) {
        if (omission.shot.trim().length === 0 ||
            omission.reason.trim().length === 0 ||
            omitted.has(omission.shot))
            diagnostics.push(filmDiagnostic("film-shot-accounting-invalid", `Omission "${omission.shot}" must name one unique current shot with a non-blank reason.`));
        else
            omitted.add(omission.shot);
        if (props.contracts.has(omission.shot) === false)
            diagnostics.push(filmDiagnostic("film-shot-unknown", `Omission "${omission.shot}" is not a current shot contract. Remove it or restore that contract.`));
    }
    const used = new Set();
    const segments = [];
    for (const placement of edit.tracks.video) {
        const contract = props.contracts.get(placement.shot);
        if (placement.shot.trim().length === 0 ||
            used.has(placement.shot) ||
            omitted.has(placement.shot))
            diagnostics.push(filmDiagnostic("film-shot-accounting-invalid", `Video shot "${placement.shot}" must appear once and cannot also be omitted.`));
        else
            used.add(placement.shot);
        if (contract === undefined) {
            diagnostics.push(filmDiagnostic("film-shot-unknown", `Video shot "${placement.shot}" is not a current shot contract.`));
            continue;
        }
        if (props.compiled.has(placement.shot) === false ||
            props.realizations.has(placement.shot) === false)
            diagnostics.push(filmDiagnostic("film-shot-not-compiled", `Shot "${placement.shot}" has no current compiled source and realization. Correct that shot before compiling the film.`));
        const sourceInFrame = frameTime(placement.sourceIn, fps, `${placement.shot} sourceIn`, diagnostics);
        const sourceOutFrame = frameTime(placement.sourceOut, fps, `${placement.shot} sourceOut`, diagnostics);
        const startFrame = frameTime(placement.start, fps, `${placement.shot} global start`, diagnostics);
        const headHandleFrames = frameTime(placement.handles.head, fps, `${placement.shot} head handle`, diagnostics);
        const tailHandleFrames = frameTime(placement.handles.tail, fps, `${placement.shot} tail handle`, diagnostics);
        const transitionIn = normalizeFilmTransition(placement.transitionIn, fps, `${placement.shot} transitionIn`, diagnostics);
        const transitionOut = normalizeFilmTransition(placement.transitionOut, fps, `${placement.shot} transitionOut`, diagnostics);
        const shotFrames = frameTime({ seconds: contract.durationSeconds }, fps, `${placement.shot} contract duration`, diagnostics);
        if (sourceInFrame === null ||
            sourceOutFrame === null ||
            startFrame === null ||
        ...
        if (sourceOutFrame <= sourceInFrame ||
            ...
            diagnostics.push(filmDiagnostic("film-source-range-invalid", `Shot "${placement.shot}" source range ${sourceInFrame}..${sourceOutFrame} and handles ${headHandleFrames}/${tailHandleFrames} must fit its ${shotFrames}-frame contract.`));
        segments.push({
            shot: placement.shot,
            sourceInFrame,
            sourceOutFrame,
    ...
    for (const shot of props.contracts.keys())
        if (used.has(shot) === false && omitted.has(shot) === false)
            diagnostics.push(filmDiagnostic("film-shot-unaccounted", `Shot "${shot}" is neither placed nor explicitly omitted. Account for every current narrative shot.`));
    ...
    if (targetFrames !== null && totalFrames !== targetFrames)
        diagnostics.push({
            ...filmDiagnostic("film-runtime-mismatch", `Film timeline ends at frame ${totalFrames}, but production target runtime is frame ${targetFrames}.${props.scope === "source"
    ...
    const audio = normalizeAudioCues(edit, props.context.assets, fps, totalFrames, diagnostics);
    const captions = normalizeCaptionCues(edit, fps, totalFrames, diagnostics);
    const effects = normalizeEffectCues(edit, props.context.effectZones.map((zone) => zone.id), fps, totalFrames, diagnostics);
    return {
...
const filmDiagnostic = (code, message) => ({
    code,
    category: "error",
    phase: "compile",
    target: "film",
...
const frameTime = (value, fps, label, diagnostics) => {
    ...
    if (Number.isFinite(raw) === false ||
        ...
        rounded < 0 ||
        Math.abs(raw - rounded) > Number.EPSILON * 64 * Math.max(1, Math.abs(raw))) {
        diagnostics.push(filmDiagnostic("film-time-off-grid", `${label} does not resolve to one non-negative safe production frame at ${fps} fps. Use an exact frame or frame-grid second.`));
        return null;
    }
    return rounded;
};
const normalizeFilmTransition = (transition, fps, label, diagnostics) => {
    if (transition.kind === "cut")
        return { kind: "cut" };
    const durationFrames = frameTime(transition.duration, fps, `${label} duration`, diagnostics);
    if (durationFrames === null)
        return null;
    if (durationFrames === 0) {
        diagnostics.push(filmDiagnostic("film-transition-invalid", `${label} ${transition.kind} duration must be at least one frame.`));
        return null;
    }
    return { kind: transition.kind, durationFrames };
};
const transitionDuration = (transition) => ("durationFrames" in transition ? transition.durationFrames : 0);
const validateVideoTimeline = (segments, props, fps, diagnostics) => {
    if (segments.length === 0) {
        diagnostics.push(filmDiagnostic("film-video-empty", "The finished film must contain at least one current video placement."));
        return;
    }
    if (segments[0].startFrame !== 0)
        diagnostics.push(filmDiagnostic("film-global-order-invalid", `The first video placement starts at frame ${segments[0].startFrame}; it must start at frame 0.`));
    for (let index = 0; index < segments.length; ++index) {
        const segment = segments[index];
        if ((index === 0 && segment.transitionIn.kind === "dissolve") ||
            (index === segments.length - 1 &&
                segment.transitionOut.kind === "dissolve"))
            diagnostics.push(filmDiagnostic("film-transition-invalid", `Shot "${segment.shot}" cannot dissolve beyond the beginning or end of the film.`));
        for (const [side, transition, handle] of [
            ...
            if (transition.kind !== "cut" &&
                ...
                diagnostics.push(filmDiagnostic("film-transition-handle-missing", `Shot "${segment.shot}" ${side} ${transition.kind} needs ${transitionDuration(transition)} frames, but only ${handle} transition-handle frames are declared.`));
        if (index === 0)
            continue;
        const previous = segments[index - 1];
        if (previous.transitionOut.kind !== segment.transitionIn.kind ||
            transitionDuration(previous.transitionOut) !==
                transitionDuration(segment.transitionIn))
            diagnostics.push(filmDiagnostic("film-transition-mismatch", `Transition between "${previous.shot}" and "${segment.shot}" must have identical outgoing and incoming kind/duration.`));
        const overlap = previous.transitionOut.kind === "dissolve"
            ? transitionDuration(previous.transitionOut)
            : 0;
        const expectedStart = previous.endFrame - overlap;
        if (segment.startFrame !== expectedStart)
            diagnostics.push(filmDiagnostic("film-global-order-invalid", `Shot "${segment.shot}" starts at frame ${segment.startFrame}; transition law requires frame ${expectedStart}. Arbitrary gaps and overlaps are forbidden.`));
        validateStateContinuity(previous, segment, props, fps, diagnostics);
    }
};
const validateStateContinuity = (previous, current, props, fps, diagnostics) => {
    ...
    if (previous.sourceOutFrame !== previousFrames ||
        current.sourceInFrame !== 0) {
        if (previousContract.closing.length !== 0 ||
            currentContract.opening.length !== 0)
            diagnostics.push(filmDiagnostic("film-state-handoff-unverifiable", `Trimmed boundary "${previous.shot}" -> "${current.shot}" cannot use contract edge-state continuity. Author full contract edges or remove edge-state claims.`));
        return;
    }
    ...
    if (Buffer.from(canonicalAutoMovieJsonBytes(closing)).equals(Buffer.from(canonicalAutoMovieJsonBytes(opening))) === false)
        diagnostics.push(filmDiagnostic("film-state-handoff-mismatch", `Closing state of "${previous.shot}" does not equal opening state of "${current.shot}". An untrimmed cut hands one measured state across, so both edges must claim it; leave both unclaimed when the cut is a scene break rather than a continuous handoff.`));
};
const normalizeAudioCues = (edit, assets, fps, totalFrames, diagnostics) => {
    const output = [];
    const ids = new Set();
    let priorStart = -1;
    for (const cue of edit.tracks.audio) {
        const sourceDurationFrames = frameTime(cue.sourceDuration, fps, `${cue.id} audio source duration`, diagnostics);
        const sourceOffsetFrame = frameTime(cue.sourceOffset, fps, `${cue.id} audio source offset`, diagnostics);
        const startFrame = frameTime(cue.start, fps, `${cue.id} audio start`, diagnostics);
        const durationFrames = frameTime(cue.duration, fps, `${cue.id} audio duration`, diagnostics);
        const fadeInFrames = frameTime(cue.fadeIn, fps, `${cue.id} audio fadeIn`, diagnostics);
        const fadeOutFrames = frameTime(cue.fadeOut, fps, `${cue.id} audio fadeOut`, diagnostics);
        if (sourceDurationFrames === null ||
        ...
        if (cue.id.trim().length === 0 ||
            ...
            diagnostics.push(filmDiagnostic("film-audio-cue-invalid", `Audio cue "${cue.id}" must be unique, ordered, in film/source range, reference a present declared asset, use fades within duration, and set gain from 0 through 4.`));
        ids.add(cue.id);
...
const normalizeCaptionCues = (edit, fps, totalFrames, diagnostics) => {
    const output = [];
    const ids = new Set();
    let priorEnd = 0;
    for (const cue of edit.tracks.captions) {
        const startFrame = frameTime(cue.start, fps, `${cue.id} caption start`, diagnostics);
        const endFrame = frameTime(cue.end, fps, `${cue.id} caption end`, diagnostics);
        ...
        if (cue.id.trim().length === 0 ||
            ...
            diagnostics.push(filmDiagnostic("film-caption-cue-invalid", `Caption cue "${cue.id}" must be unique, non-overlapping, in range, plain non-blank text, and use a non-blank language/speaker identity.`));
        ids.add(cue.id);
        priorEnd = endFrame;
        output.push({
            id: cue.id,
...
const normalizeEffectCues = (edit, zones, fps, totalFrames, diagnostics) => {
    const output = [];
    const ids = new Set();
    let priorStart = -1;
    for (const cue of edit.tracks.effects) {
        const startFrame = frameTime(cue.start, fps, `${cue.id} effect start`, diagnostics);
        const durationFrames = frameTime(cue.duration, fps, `${cue.id} effect duration`, diagnostics);
        ...
        if (cue.id.trim().length === 0 ||
            ...
            cue.intensity > 1 ||
            startFrame < priorStart)
            diagnostics.push(filmDiagnostic("film-effect-cue-invalid", `Effect cue "${cue.id}" must be unique, ordered, in range, reference a registered world zone, and use intensity from 0 through 1.`));
        ids.add(cue.id);
        priorStart = startFrame;
        output.push({
            id: cue.id,
 ...
 */
const containsTemplateSentinel = (source) => {
    ...
    const identifier = (character) => character.toLowerCase() !== character.toUpperCase() ||
        (character >= "0" && character <= "9") ||
        character === "_" ||
        character === "$";
    const boundary = (character) => character === undefined || identifier(character) === false;
    for (let index = source.indexOf(TEMPLATE_SENTINEL); index >= 0; index = source.indexOf(TEMPLATE_SENTINEL, index + 1))
        if (boundary(source[index - 1]) &&
            boundary(source[index + TEMPLATE_SENTINEL.length]))
            return true;
    return false;
};
const inspectSource = (target, sourcePath, source) => {
    ...
    const visit = (node) => {
        ...
        if (ts.isPropertyAccessExpression(node)) {
            ...
            if ((expression === "Math" && name === "random") ||
...
const importDeclarationHasRuntimeBinding = (declaration) => {
 ...
 */
const isLinkableImport = (declaration) => {
    ...
    if (clause === undefined ||
        ...
        return false;
    const runtime = clause.namedBindings.elements.filter((element) => element.isTypeOnly === false);
    if (runtime.length === 0)
        return false;
    if (isProjectSourceSpecifier(specifier))
        return true;
    if (specifier !== "@automovie/engine")
        return false;
    return runtime.every((element) => AUTOMOVIE_SANDBOX_ENGINE_EXPORTS.has(element.propertyName?.text ?? element.name.text));
};
...
const validateCompiledShot = (contract, value) => {
    const id = contract.id;
    const diagnostics = [];
    if (value.shot.id !== id)
        diagnostics.push(engineDiagnostic(id, "shot.id", `must equal contract id "${id}"`));
    if (value.shot.duration !== contract.durationSeconds)
        diagnostics.push(engineDiagnostic(id, "shot.duration", `must equal contract duration ${contract.durationSeconds}`));
    appendValidation(diagnostics, id, validateSceneArtifact(value.scene, value.models));
    const motionIds = new Set(value.motions.map((motion) => motion.id));
    appendValidation(diagnostics, id, validateShotArtifact(value.shot, value.scene, motionIds));
    ...
    const skeletons = new Map(value.models.flatMap((model) => model.skeleton === null
        ? []
        : [[model.skeleton.id, model.skeleton]]));
    for (const motion of value.motions) {
        const skeleton = skeletons.get(motion.skeleton);
        if (skeleton === undefined)
            diagnostics.push(engineDiagnostic(id, `motion:${motion.id}`, `references missing skeleton "${motion.skeleton}"`));
        else
 ...
 * is measured once.
 */
const formationGroundMembers = (formation) => {
    const remembered = formationGroundMemberCache.get(formation);
    if (remembered !== undefined)
        return remembered;
    const looks = Array.from({ length: FORMATION_GROUND_SUPPORT_DIRECTIONS }, (_, index) => {
        const radians = (2 * Math.PI * index) / FORMATION_GROUND_SUPPORT_DIRECTIONS;
        return { x: Math.cos(radians), z: Math.sin(radians) };
    });
    const furthest = looks.map(() => Number.NEGATIVE_INFINITY);
    const outermost = looks.map(() => null);
 ...
 */
const formationGroundSampleTimes = (cue, radius) => {
    ...
    const spread = Math.max(Math.abs(cue.from.spacingScale.lateral), Math.abs(cue.to.spacingScale.lateral), Math.abs(cue.from.spacingScale.depth), Math.abs(cue.to.spacingScale.depth));
    const reach = Math.hypot(cue.to.translation.x - cue.from.translation.x, cue.to.translation.z - cue.from.translation.z) +
        ((Math.abs(cue.to.facingOffsetDeg - cue.from.facingOffsetDeg) * Math.PI) /
            180) *
            radius *
            spread +
        Math.max(Math.abs(cue.to.spacingScale.lateral - cue.from.spacingScale.lateral), Math.abs(cue.to.spacingScale.depth - cue.from.spacingScale.depth)) *
            radius;
    const steps = Math.min(FORMATION_GROUND_SAMPLE_LIMIT, Math.ceil((2 * reach) / FORMATION_GROUND_SAMPLE_METRES));
    const span = cue.end - cue.start;
    return [
        cue.start,
        ...Array.from({ length: Math.max(0, steps - 1) }, (_, index) => cue.start + (span * (index + 1)) / steps),
        cue.end,
    ];
};
/**
 ...
 */
const formationSlotGroundSampleTimes = (cue) => {
    const reach = Math.hypot(cue.to.offset.x - cue.from.offset.x, cue.to.offset.z - cue.from.offset.z);
    const steps = Math.min(FORMATION_GROUND_SAMPLE_LIMIT, Math.ceil((2 * reach) / FORMATION_GROUND_SAMPLE_METRES));
    const span = cue.end - cue.start;
    return [
        cue.start,
 ...
 */
const formationGroundEscape = (space, place) => {
 ...
 */
export const validateAutoMovieFormationGround = (contract, value) => {
    ...
    for (const formation of value.formations) {
        ...
        const members = new Map(formationGroundMembers(formation).map((member) => [member.slot, member]));
        ...
        const radius = Math.max(0, ...[...members.values()].map((member) => Math.hypot(member.point.x - formation.anchor.x, member.point.z - formation.anchor.z)));
        const times = [
            ...new Set([
        ...
        for (const time of [...(resting ? [null] : []), ...times]) {
            const motion = time === null ? null : sampleFormationMotion(own, formation.id, time);
            for (const member of members.values()) {
                ...
                const placed = placeFormationSlot({
                    ...
                    unit: motion,
                    member: sampleFormationSlotMotion(ownSlots, formation.id, member.slot, time),
                });
        ...
        if (escape === null)
            continue;
        diagnostics.push(engineDiagnostic(contract.id, `formation:${formation.id}`, 
        ...
        `must stand on the space this shot staged, but ${escape.time === null
            ? "a member of it stands at"
            : `at ${round(escape.time)}s its cue takes a member of it to`} (${round(escape.place.x)}, ${round(escape.place.z)}) ${escape.carried === null
 ...
 */
export const autoMovieModelColumns = (model) => {
 ...
 */
const axialBoneHeights = (skeleton) => {
    const heights = new Map();
    const axial = (skeleton?.bones ?? []).filter((bone) => bone.rest.translation.x === 0 &&
 ...
 */
const axialPartColumn = (part, heights) => {
    ...
    const radius = Math.min(solid.across * Math.abs(scale.x), solid.deep * Math.abs(scale.z));
 ...
 * measure.
 */
const axialPartHeight = (part, heights) => {
 ...
 * stood on.
 */
const vertical = (value) => value.x === 0 && value.z === 0;
/**
...
const formationOverlapMembers = (formation) => {
    const remembered = formationOverlapMemberCache.get(formation);
    if (remembered !== undefined)
        return remembered;
    const members = Array.from({ length: Math.min(formation.count, FORMATION_OVERLAP_MEMBER_LIMIT) }, (_, slot) => ({ slot, point: formationSlotPosition(formation, slot) }));
 ...
 */
const formationOverlapSampleTimes = (cues, slotCues) => {
    const ends = [
        ...new Set([
            0,
    ...
    const inside = Math.max(0, Math.floor((FORMATION_OVERLAP_SAMPLE_LIMIT - ends.length) / gaps));
    return [
        ...new Set(ends.flatMap((time, index) => {
            const next = ends[index + 1];
            return next === undefined
                ? [time]
                : [
                    time,
                    ...Array.from({ length: inside }, (_, step) => time + ((next - time) * (step + 1)) / (inside + 1)),
                ];
        })),
    ];
};
 ...
 */
const formationOverlapClearance = (left, right, lift) => {
        ...
        for (const far of right.tiers) {
            ...
            for (const one of near)
                for (const other of far)
                    if (Math.max(one.bottom, other.bottom + lift) <
 ...
 */
export const validateAutoMovieFormationOverlap = (contract, value) => {
    const runtimes = new Map(value.models.map((model) => [model.id, model]));
    const units = value.formations.flatMap((formation, index) => {
    ...
    const cell = 2 *
        Math.max(...units.flatMap((unit) => unit.tiers.flatMap((columns) => columns.map((column) => column.radius))));
    const found = new Map();
        ...
        for (const unit of units) {
            ...
            for (const member of unit.members) {
                const placed = placeFormationSlot({
                    ...
                    unit: motion,
                    member: sampleFormationSlotMotion(slotCues, unit.formation.id, member.slot, time),
                });
                if (placed.present === false)
                    continue;
                const here = {
                ...
                for (let across = -1; across <= 1; ++across)
                    for (let along = -1; along <= 1; ++along)
                        for (const other of grid.get(`${column + across}:${row + along}`) ??
                            ...
                            if (found.has(pair))
                                continue;
                            const clearance = formationOverlapClearance(other.unit, unit, here.point.y - other.point.y);
                            const apart = Math.hypot(here.point.x - other.point.x, here.point.z - other.point.z);
                            if (apart >= clearance)
                                continue;
                            found.set(pair, {
                                time,
                ...
                if (neighbours === undefined)
                    ...
                    neighbours.push(here);
            }
        }
    }
    return [...found.values()].map((overlap) => engineDiagnostic(contract.id, `formation:${overlap.left.unit.formation.id}`, 
    ...
    `must not stand a member where another body already is, but at ${round(overlap.time)}s ${overlap.left.unit === overlap.right.unit
        ? `its slots ${overlap.left.slot} and ${overlap.right.slot}`
        : `its slot ${overlap.left.slot} and slot ${overlap.right.slot} of "${overlap.right.unit.formation.id}"`} stand ${round(overlap.apart)}m apart at (${round((overlap.left.point.x + overlap.right.point.x) / 2)}, ${round((overlap.left.point.z + overlap.right.point.z) / 2)}), inside the ${round(overlap.clearance)}m their bodies fill`));
};
/** Validate bounded source-authored formation cues against one compiled shot. */
export const validateAutoMovieFormationMotions = (contract, value) => {
    const diagnostics = [];
    ...
    if (value.formationMotions.length > 256)
        fail("formationMotions", "must contain at most 256 compact cues rather than per-member curves");
    const ids = new Set();
    const participating = new Set(contract.participants.flatMap((participant) => participant.kind === "formation" ? [participant.id] : []));
    const priorByFormation = new Map();
    for (const cue of [...value.formationMotions].sort((left, right) => compareCodeUnits(left.formation, right.formation) ||
        left.start - right.start ||
        compareCodeUnits(left.id, right.id))) {
        if (cue.id.trim().length === 0 || ids.has(cue.id))
            fail(`formationMotion:${cue.id || "(blank)"}`, "must have one non-blank id unique inside the shot");
        ids.add(cue.id);
        if (participating.has(cue.formation) === false ||
            value.formations.some((formation) => formation.id === cue.formation) ===
                false)
            fail(`formationMotion:${cue.id}.formation`, `must reference participating compiled formation "${cue.formation}"`);
        if (Number.isFinite(cue.start) === false ||
            Number.isFinite(cue.end) === false ||
            cue.start < 0 ||
            cue.end <= cue.start ||
            cue.end > contract.durationSeconds)
            fail(`formationMotion:${cue.id}.time`, `must be one positive interval inside 0..${contract.durationSeconds}s`);
        for (const [name, state] of [
            ["from", cue.from],
            ["to", cue.to],
        ]) {
            if ([state.translation.x, state.translation.y, state.translation.z].some((number) => Number.isFinite(number) === false ||
                Math.abs(number) > 1_000_000_000) ||
                Number.isFinite(state.facingOffsetDeg) === false ||
                Math.abs(state.facingOffsetDeg) > 360_000)
                fail(`formationMotion:${cue.id}.${name}`, "must keep translation inside +/-1000000000m and facing inside +/-360000 degrees");
            if ([state.spacingScale.lateral, state.spacingScale.depth].some((number) => Number.isFinite(number) === false || number < 0.25 || number > 4))
                fail(`formationMotion:${cue.id}.${name}.spacingScale`, "must stay inside the bounded 0.25..4 envelope");
        }
        const prior = priorByFormation.get(cue.formation);
        if (prior !== undefined && cue.start < prior.end)
            fail(`formationMotion:${cue.id}.start`, `must not overlap prior cue "${prior.id}" ending at ${prior.end}s`);
 ...
 */
export const validateAutoMovieFormationSlotMotions = (contract, value) => {
    const diagnostics = [];
    const fail = (field, expectation) => {
        diagnostics.push(engineDiagnostic(contract.id, field, expectation));
    };
    ...
    if (cues.length > 256)
        fail("formationSlotMotions", "must contain at most 256 sparse per-member cues");
    const named = cues.reduce((sum, cue) => sum + cue.slots.length, 0);
    if (named > FORMATION_SLOT_EXCEPTION_LIMIT)
        fail("formationSlotMotions", `must single out at most ${FORMATION_SLOT_EXCEPTION_LIMIT} members in one shot rather than author a curve per member`);
    const ids = new Set();
    const participating = new Set(contract.participants.flatMap((participant) => participant.kind === "formation" ? [participant.id] : []));
    const compiledById = new Map(value.formations.map((formation) => [formation.id, formation]));
    ...
    for (const cue of [...cues].sort((left, right) => compareCodeUnits(left.formation, right.formation) ||
        left.start - right.start ||
        compareCodeUnits(left.id, right.id))) {
        if (cue.id.trim().length === 0 || ids.has(cue.id))
            fail(`formationSlotMotion:${cue.id || "(blank)"}`, "must have one non-blank id unique inside the shot");
        ids.add(cue.id);
        const compiled = compiledById.get(cue.formation);
        if (participating.has(cue.formation) === false || compiled === undefined)
            fail(`formationSlotMotion:${cue.id}.formation`, `must reference participating compiled formation "${cue.formation}"`);
        if (Number.isFinite(cue.start) === false ||
            Number.isFinite(cue.end) === false ||
            cue.start < 0 ||
            cue.end <= cue.start ||
            cue.end > contract.durationSeconds)
            fail(`formationSlotMotion:${cue.id}.time`, `must be one positive interval inside 0..${contract.durationSeconds}s`);
        if (cue.slots.length === 0 ||
            new Set(cue.slots).size !== cue.slots.length ||
            cue.slots.some((slot) => Number.isSafeInteger(slot) === false ||
                slot < 0 ||
                (compiled !== undefined && slot >= compiled.count)))
            fail(`formationSlotMotion:${cue.id}.slots`, `must name at least one unique slot inside 0..${(compiled?.count ?? 0) - 1}`);
        ...
        // wrote last.
        const heroes = (compiled?.heroes ?? []).filter((hero) => cue.slots.includes(hero.slot));
        if (heroes.length !== 0)
            fail(`formationSlotMotion:${cue.id}.slots`, `must not name slots promoted to named actors (${heroes
        ...
        for (const [name, state] of [
            ...
            if ([state.offset.x, state.offset.y, state.offset.z].some((number) => Number.isFinite(number) === false ||
                Math.abs(number) > 1_000_000_000) ||
                Number.isFinite(state.facingOffsetDeg) === false ||
                Math.abs(state.facingOffsetDeg) > 360_000)
                fail(`formationSlotMotion:${cue.id}.${name}`, "must keep offset inside +/-1000000000m and facing inside +/-360000 degrees");
        }
        const priorSlots = priorBySlot.get(cue.formation) ??
            new Map();
        priorBySlot.set(cue.formation, priorSlots);
        for (const slot of cue.slots) {
            const prior = priorSlots.get(slot);
            if (prior !== undefined && cue.start < prior.end)
                fail(`formationSlotMotion:${cue.id}.start`, `must not overlap prior cue "${prior.id}" on slot ${slot} ending at ${prior.end}s`);
...
export const validateAutoMovieEffects = (contract, value) => {
    const diagnostics = [];
    const fail = (field, expectation) => {
        diagnostics.push(engineDiagnostic(contract.id, field, expectation));
    };
    ...
    const ids = new Set();
    const priorByZone = new Map();
    const events = new Map(contract.events.map((event) => [event.id, event]));
    const samples = new Map(value.eventSamples.map((sample) => [sample.id, sample.time]));
    for (const cue of [...cues].sort((left, right) => compareCodeUnits(left.zone, right.zone) ||
        left.start - right.start ||
        compareCodeUnits(left.id, right.id))) {
        ...
        if (compiled === undefined || compiled.zone !== cue.zone)
            fail(`${field}.zone`, `must reference one current compiler-materialized world zone "${cue.zone}"`);
        if (Number.isFinite(cue.start) === false ||
            Number.isFinite(cue.end) === false ||
            cue.start < 0 ||
            cue.end <= cue.start ||
            cue.end > contract.durationSeconds)
            fail(`${field}.time`, `must be one positive interval inside 0..${contract.durationSeconds}s`);
        if ([cue.intensity.from, cue.intensity.to].some((intensity) => Number.isFinite(intensity) === false ||
            intensity < 0 ||
            intensity > 1))
        ...
        if (cue.event !== undefined) {
            ...
            if (event === undefined ||
                ...
                sample < cue.start ||
                sample >= cue.end)
                fail(`${field}.event`, `must name one compiled event realized inside [${cue.start}, ${cue.end})`);
        }
        const prior = priorByZone.get(cue.zone);
        if (prior !== undefined && cue.start < prior.end)
            fail(`${field}.start`, `must not overlap prior zone cue "${prior.id}" ending at ${prior.end}s`);
        priorByZone.set(cue.zone, cue);
    }
    if (value.effects.length !== cues.length ||
        value.effects.some((effect) => ids.has(effect.id) === false))
        fail("effects", "must contain exactly one compiler-owned stream for every source cue");
    return diagnostics;
};
const appendValidation = (diagnostics, id, validation) => {
    if (validation.success === false)
...
const engineDiagnostic = (id, field, expectation) => ({
    code: "engine-validation-failed",
    category: "error",
    phase: "compile",
    target: `shot:${id}`,
...
const missingDesignDiagnostics = (project, graph) => {
    ...
    if (graph.production === null)
        diagnostics.push({
            ...
            message: "Production design is missing. Create the tracked production design record.",
        });
    if (graph.world === null)
        diagnostics.push({
            code: "design-missing",
            ...
            message: "World design is missing. Create the tracked world design record.",
        });
    if (graph.shots.size === 0)
        diagnostics.push({
            ...
            message: "No shot contract exists. Create the first tracked shot contract record.",
        });
    return diagnostics;
};
const sourcePathDiagnostic = (id, sourcePath, error) => {
    const message = errorMessage(error);
    return {
        code: error instanceof AutoMovieProductionSourcePathError &&
...
const filmSourcePathDiagnostic = (error) => ({
    code: error instanceof AutoMovieProductionSourcePathError &&
        error.reason === "outside-root"
        ? "source-path-outside-root"
        : "source-path-missing",
...
const isTypeScriptSourcePath = (file) => [".ts", ".tsx", ".mts", ".cts"].includes(path.extname(file).toLowerCase());
const compilerAssetInventory = (manifestPath, inputs, productionId, graph, archetypes) => {
    ...
    const diagnostics = [];
    const diagnostic = (code, target, message) => {
        diagnostics.push({
            code,
            category: "error",
            phase: "project",
    ...
    if (manifestInput?.bytes === null || manifestInput === undefined) {
        diagnostic("asset-manifest-missing", "asset-manifest", `Production manifest declares "${manifestPath}", but that physical provenance ledger is absent. Restore it before compiling asset references.`);
        return {
            assets: [],
            records: [],
            externalModels: new Map(),
    ...
    try {
        ...
        diagnostic("asset-manifest-invalid", "asset-manifest", `Asset manifest is not valid JSON: ${errorMessage(error)}. Restore one IAutoMovieAssetManifest before compiling.`);
        return {
            assets: [],
            records: [],
            externalModels: new Map(),
    ...
    if (validation.success === false) {
        diagnostic("asset-manifest-invalid", "asset-manifest", `Asset manifest does not satisfy IAutoMovieAssetManifest: ${validation.errors
    ...
    const assets = validation.data.assets
        .filter((asset) => asset.uses.some((use) => use.production === productionId))
        .map((asset) => asset.path);
    const orderedPaths = validation.data.assets.map((asset) => asset.path);
    if (orderedPaths.some((asset, index) => index !== 0 && compareCodeUnits(orderedPaths[index - 1], asset) >= 0))
        diagnostic("asset-manifest-order", "asset-manifest", "Asset entries must be in unique canonical path order. Sort them by code unit and remove duplicates before compiling.");
    const activeConsumerAssets = new Map();
    const consumedModelResources = new Set();
    ...
    for (const asset of validation.data.assets) {
        ...
        if (isCanonicalAssetPath(asset.path) === false ||
            (prior !== undefined && prior !== asset.path))
            diagnostic("asset-path-invalid", asset.path, `Asset path "${asset.path}" is not one canonical, portable project-relative identity. Keep one spelling inside declared content roots.`);
        paths.set(folded, asset.path);
        const input = content.get(asset.path);
        if (input?.bytes === null || input === undefined || input.render === false)
            diagnostic("asset-bytes-missing", asset.path, `Manifest asset "${asset.path}" is not a physical file inside contentRoots/contentFiles. Restore and declare the exact bytes before compiling.`);
        else if (asset.digest !== digestAutoMovieBytes(input.bytes))
            diagnostic("asset-digest-mismatch", asset.path, `Manifest digest ${asset.digest} does not match current bytes ${digestAutoMovieBytes(input.bytes)}. Restore the licensed bytes or update provenance from the verified source.`);
        if (isSha256Digest(asset.digest) === false ||
            ...
            diagnostic("asset-provenance-incomplete", asset.path, `Asset "${asset.path}" lacks a full source URL, original/current SHA-256, license, processing identity, or reasoned use. Complete the distribution ledger before compiling.`);
        if (asset.processing.length === 0 && asset.digest !== asset.original.digest)
            diagnostic("asset-processing-missing", asset.path, `Asset "${asset.path}" differs from its original digest but records no processing steps. Record the reproducible transformation chain before compiling.`);
        if (isExternalModelAsset(asset.path) &&
            (asset.model === undefined ||
                asset.model.ingestProfile.trim().length === 0 ||
                asset.model.lod.length === 0))
            diagnostic("asset-model-provenance-missing", asset.path, `External model "${asset.path}" must declare its ingest profile, explicit LOD ledger, collision proxy and measurement proxy before compiling.`);
    }
    for (const asset of validation.data.assets) {
        const activeUses = asset.uses.filter((use) => use.production === productionId);
        ...
        for (const use of activeUses) {
            ...
            const exclusive = use.consumer.kind !== "model-resource" &&
                use.consumer.kind !== "model-proxy";
            if (seenUses.has(key) || (exclusive && priorAsset !== undefined))
                diagnostic("asset-use-duplicate", asset.path, `Asset "${asset.path}" repeats active consumer ${use.consumer.kind} "${use.consumer.id}"${priorAsset === undefined ? "" : ` already owned by "${priorAsset}"`}. Keep one reasoned use per production consumer.`);
            seenUses.add(key);
            if (exclusive)
                activeConsumerAssets.set(key, asset.path);
            if (assetConsumerExists(graph, validation.data.assets, asset.path, use.consumer) === false)
                diagnostic("asset-use-dangling", asset.path, `Asset "${asset.path}" cites missing ${use.consumer.kind} "${use.consumer.id}" in production "${productionId}". Correct the typed consumer or remove the stale use.`);
        }
        ...
        for (const lod of asset.model.lod) {
            ...
            if (levels.has(lod.level) ||
                ...
                diagnostic("asset-model-lod-dangling", asset.path, `Model asset "${asset.path}" has duplicate/out-of-order LOD "${lod.level}" or points to non-model manifest asset "${lod.asset}". Keep unique hero/near/far levels in order and ground each in model bytes.`);
            if (lod.level !== "hero" &&
                (target === undefined ||
                    hasActiveAssetUse(validation.data.assets.find((candidate) => candidate.path === target), productionId, "model-resource", asset.path) === false))
                diagnostic("asset-model-resource-unbound", asset.path, `LOD "${lod.level}" asset "${lod.asset}" is not authorized as a model-resource of "${asset.path}" in production "${productionId}".`);
        ...
        if (asset.model.lod[0]?.level !== "hero" ||
            asset.model.lod[0]?.asset !== asset.path)
            diagnostic("asset-model-lod-dangling", asset.path, `Model asset "${asset.path}" must bind its own exact bytes as the first hero LOD. Keep optional near/far members after that owned hero identity.`);
            ...
            try {
                inspection = inspectAutoMovieExternalModelBytes({
                    ...
                    resolveResource: (uri) => {
                        ...
                        const resourceRecord = validation.data.assets.find((candidate) => candidate.path === resource);
                        ...
                        if (resourceRecord === undefined ||
                            ...
                            diagnostic("asset-model-resource-unbound", asset.path, `External model "${asset.path}" references sidecar "${uri}", but resolved project asset "${resource}" is absent or byte-stale in the manifest.`);
                            return null;
                        }
                        if (hasActiveAssetUse(resourceRecord, productionId, "model-resource", asset.path) === false)
                            diagnostic("asset-model-resource-unbound", asset.path, `External model sidecar "${resource}" is not authorized as a model-resource of "${asset.path}" in production "${productionId}".`);
                        else
                            consumedModelResources.add(`${asset.path}\0${resource}`);
                ...
                diagnostic("asset-model-ingest-invalid", asset.path, `External model "${asset.path}" cannot be ingested with profile "${asset.model.ingestProfile}": ${errorMessage(error)} Restore valid fixed bytes or select the correct supported profile.`);
            }
        ...
        for (const reference of [
            ...
            if (reference.kind === "asset") {
                const proxyRecord = validation.data.assets.find((candidate) => candidate.path === reference.asset);
    ...
    for (const resource of validation.data.assets)
        for (const use of resource.uses)
            if (use.production === productionId &&
                use.consumer.kind === "model-resource" &&
                consumedModelResources.has(`${use.consumer.id}\0${resource.path}`) ===
                    false)
                diagnostic("asset-use-dangling", resource.path, `Asset "${resource.path}" is authorized as a model-resource of "${use.consumer.id}" but is not an actual LOD, buffer, or image dependency of that model.`);
    const externalModels = new Map();
    for (const [id, model] of graph.models) {
        if (model.asset === undefined)
            continue;
        const record = validation.data.assets.find((asset) => asset.path === model.asset);
        if (isExternalModelAsset(model.asset) === false ||
            record === undefined ||
            record.model === undefined ||
            record.uses.some((use) => use.production === productionId &&
                use.consumer.kind === "model-recipe" &&
                use.consumer.id === id) === false)
            diagnostic("asset-use-missing", model.asset, `Model recipe "${id}" consumes "${model.asset}" without external-model provenance and one matching typed use for production "${productionId}". Register the exact model bytes, model decisions and model-recipe use.`);
        const external = externalByAsset.get(model.asset);
        if (record !== undefined &&
            record.model !== undefined &&
            external !== undefined) {
            const levels = record.model.lod.flatMap((lod) => {
                const levelRecord = validation.data.assets.find((candidate) => candidate.path === lod.asset);
            ...
            if (levels.length !== record.model.lod.length ||
                levelProfiles.size !== 1 ||
                levelProfiles.has(external.profile) === false)
                diagnostic("asset-model-lod-incompatible", model.asset, `Model recipe "${id}" requires every declared LOD to pass the same fixed ingest profile as its hero asset.`);
            else if ((external.profile === "gltf-static-v1") === generatedHasSkeleton)
                diagnostic("asset-model-rig-incompatible", model.asset, `Model recipe "${id}" and ingest profile "${external.profile}" disagree on whether the runtime is articulated. Bind static assets only to skeleton-free recipes and humanoid assets only to articulated recipes.`);
            else if (generatedHasSkeleton &&
                levels.some((level) => recipeBones.some((bone) => level.humanoidBones.some((mapping) => mapping.bone === bone && mapping.weighted) === false)))
                diagnostic("asset-model-rig-incompatible", model.asset, `Model recipe "${id}" requires normalized, visibly weighted skeleton bones that at least one ingested LOD does not prove.`);
            else {
                const assets = new Map(levels.flatMap((level) => externalByAsset
                    .get(level.asset)
                    .assets.map((entry) => [entry.path, entry.digest])));
...
const resolveExternalCollisionProxy = (props) => {
    const proxy = props.reference.kind === "generated"
        ? props.reference
    ...
    if (proxy === null ||
        (proxy.recipe === "capsule-v1"
            ...
            : positiveFiniteValues(proxy.parameters, ["width", "height", "depth"]) ===
                false)) {
        props.diagnostic("asset-model-proxy-dangling", props.owner, `Model asset "${props.owner}" has no byte-grounded collision proxy with the exact positive parameters required by capsule-v1 or box-v1. Correct the explicit proxy decision; mesh inference is not a fallback.`);
        return null;
    }
    return proxy;
};
const resolveExternalMeasurementProxy = (props) => {
    ...
    if (proxy === null ||
        ...
        props.diagnostic("asset-model-proxy-dangling", props.owner, `Model asset "${props.owner}" has no byte-grounded measurement proxy with the exact positive parameters required by box-v1 or humanoid-landmarks-v1. Correct the explicit proxy decision; mesh inference is not a fallback.`);
        return null;
    }
    return proxy;
};
const readExternalProxyAsset = (props, kind) => {
    const record = props.records.find((candidate) => candidate.path === props.reference.asset);
    const input = props.content.get(props.reference.asset);
    if (path.posix.extname(props.reference.asset).toLowerCase() !== ".json" ||
        record === undefined ||
        input?.bytes === null ||
        input === undefined ||
        record.digest !== digestAutoMovieBytes(input.bytes) ||
        hasActiveAssetUse(record, props.productionId, "model-proxy", props.owner) === false)
        return null;
    try {
        const validation = typia.validateEquals(JSON.parse(Buffer.from(input.bytes).toString("utf8")));
        if (validation.success === false)
            return null;
...
const positiveFiniteValues = (values, keys) => Object.keys(values).length === keys.length &&
    keys.every((key) => Object.prototype.hasOwnProperty.call(values, key) &&
        Number.isFinite(values[key]) &&
        values[key] > 0);
const externalModelResourcePath = (modelPath, uri) => {
    if (uri.includes("?") ||
        uri.includes("#") ||
        /^[A-Za-z][A-Za-z0-9+.-]*:/u.test(uri) ||
        uri.startsWith("//"))
        throw new Error(`Sidecar URI "${uri}" must be a plain project-relative asset path.`);
    let decoded;
    try {
    ...
    if (decoded.startsWith("/") ||
        ...
        throw new Error(`Sidecar URI "${uri}" must decode to one plain relative asset path.`);
    const resolved = path.posix.join(path.posix.dirname(modelPath), decoded);
    if (isCanonicalAssetPath(resolved) === false)
        throw new Error(`Sidecar URI "${uri}" escapes or aliases the project asset namespace.`);
    return resolved;
};
const assetUseIncomplete = (use) => use.production.trim().length === 0 ||
    use.consumer.id.trim().length === 0 ||
    use.reason.trim().length === 0;
const assetProcessingStepIncomplete = (step) => step.tool.trim().length === 0 || step.command.trim().length === 0;
const assetConsumerExists = (graph, records, assetPath, consumer) => {
    switch (consumer.kind) {
        ...
        case "model-proxy": {
            const owner = records.find((record) => record.path === consumer.id);
            return (owner?.model !== undefined &&
                [owner.model.collisionProxy, owner.model.measurementProxy].some((reference) => reference.kind === "asset" && reference.asset === assetPath));
        }
        case "rendition-reference":
            return graph.shots.has(consumer.id);
    }
};
const hasActiveAssetUse = (record, productionId, kind, owner) => record?.uses.some((use) => use.production === productionId &&
    use.consumer.kind === kind &&
    use.consumer.id === owner) === true;
/**
 ...
 * humanoid one.
 */
const requiredRecipeBones = (model, archetypes) => archetypes.get(model.archetype)?.bones ?? [];
const refuseUnsupportedExternalInstancing = (graph, externalModels, diagnostic) => {
    for (const formation of graph.formations.values()) {
        ...
        if (recipes.some((recipe) => externalModels.has(recipe)))
            diagnostic("asset-model-instancing-unsupported", formation.id, `Formation "${formation.id}" selects a registered external model for anonymous members, but imported-mesh instancing is not yet supported. Use generated anonymous tiers or named hero nodes.`);
    }
    for (const instanceSet of graph.world?.instanceSets ?? [])
        if (externalModels.has(instanceSet.modelRecipe))
            diagnostic("asset-model-instancing-unsupported", instanceSet.id, `Instance set "${instanceSet.id}" selects registered external model "${instanceSet.modelRecipe}", but imported-mesh instancing is not yet supported. Use a generated recipe or named nodes.`);
};
const validateCompiledAssetUses = (productionId, records, edit) => {
    const diagnostics = [];
    const audioByConsumer = new Map();
    ...
    for (const [id, asset] of audioByConsumer)
        if (actual.get(id) !== asset)
            diagnostics.push(filmDiagnostic("asset-use-stale", `Asset ledger assigns "${asset}" to audio cue "${id}", but the active film does not contain that exact reference. Correct the production use or film cue.`));
    for (const [id, asset] of actual)
        if (audioByConsumer.get(id) !== asset)
            diagnostics.push(filmDiagnostic("asset-use-missing", `Audio cue "${id}" consumes "${asset}" without one matching typed use for production "${productionId}". Add the exact production/audio-cue ledger entry.`));
    return diagnostics;
};
const isCanonicalAssetPath = (value) => path.posix.isAbsolute(value) === false &&
    /^[A-Za-z]:/.test(value) === false &&
    value.includes("\\") === false &&
    value !== "." &&
    path.posix.normalize(value) === value &&
    value.split("/").every((segment) => segment.length > 0 && segment !== "..");
const isSha256Digest = (value) => /^sha256:[0-9a-f]{64}$/.test(value);
...
const isExternalModelAsset = (value) => [".gltf", ".glb", ".vrm"].includes(path.extname(value).toLowerCase());
const contentFingerprintFields = (inputs) => inputs.map((content) => ({
    role: `content:${content.path}`,
    kind: content.bytes === null ? "absent" : "file",
    payload: content.bytes === null
        ? new Uint8Array()
        : content.source && isTypeScriptSourcePath(content.path)
            ? normalizeAutoMovieSource(content.bytes)
            : content.bytes,
}));
const productionCompilerInputFingerprint = (productionId, graph, sourceFields, contentFields) => fingerprintAutoMovieFields([
    {
        role: "protocol",
        kind: "compile-input",
        payload: Buffer.from(`${AUTOMOVIE_COMPILE_FINGERPRINT_PROTOCOL}\0${AUTOMOVIE_PRODUCTION_COMPILER_PROTOCOL}\0${AUTOMOVIE_PRODUCTION_COMPILER_VERSION}`, "utf8"),
    },
    {
        role: "production",
        kind: "namespace",
 ...
 * intend to publish.
 */
export const currentAutoMovieProductionCompilerInputFingerprint = (project, scope) => {
    try {
        ...
        for (const [id, contract] of graph.shots) {
            ...
            try {
                sourceFields.push({
                    role: `source:${id}`,
                    kind: "typescript",
                    payload: normalizeAutoMovieSource(project.readSource(contract.source.module)),
                });
            }
            catch {
                sourceFields.push({
        ...
        if (scope === "design")
            ...
            try {
                sourceFields.push({
                    role: "source:film",
                    kind: "typescript",
                    payload: normalizeAutoMovieSource(project.readSource(FILM_SOURCE_PATH)),
                });
            }
            catch {
                sourceFields.push({
        ...
        if (scope !== "design")
            try {
                contentFields.push(...contentFingerprintFields(project.contentInputs()));
            }
            catch {
                contentFields.push({
                    role: "content:inventory",
        ...
        return productionCompilerInputFingerprint(project.productionId, graph, sourceFields, contentFields);
    }
    catch {
        return null;
    }
};
const designFingerprintFields = (graph) => {
    const fields = [];
    const add = (role, value) => {
        fields.push({
            role,
            kind: value === null ? "absent" : "canonical-json",
            payload: value === null ? new Uint8Array() : canonicalAutoMovieJsonBytes(value),
        });
    };
    add("design:production", graph.production);
    for (const [id, value] of graph.models)
        add(`design:model:${id}`, value);
    ...
    for (const [id, value] of graph.shots)
        add(`design:shot:${id}`, value);
    for (const [id, value] of graph.acceptance)
        add(`design:acceptance:${id}`, value);
    return fields;
};
const materializeGeneratedFiles = (productionId, graph, runtimeModels, compiled, realizations, film, inputFingerprint) => {
    const files = new Map();
    const put = (file, value) => {
        files.set(file, Buffer.concat([
...
const materializeFilmArtifacts = (draft, sourceDigest, inputFingerprint) => ({
    edit: {
        version: 1,
        compiler: AUTOMOVIE_PRODUCTION_COMPILER_PROTOCOL,
        inputFingerprint,
...
const statusesOf = (project, files) => {
...
const sourceTargetsOf = (file, graph) => {
    ...
    for (const [id] of graph.models)
        if (file === `models/${encodeAutoMoviePathSegment(id)}.json` ||
...
const screenplayCoverageDiagnostics = (props) => {
    if (props.screenplay === null)
        return [];
    const realized = new Set();
    for (const [id, contract] of props.contracts) {
        if (props.realizations.get(id) === undefined)
            continue;
    ...
    const active = props.screenplay.screenplay.scenes.filter((scene) => scene.status === "active" && scene.disposition?.phase !== "production");
    ...
    const unobserved = props.scope === "review" || props.scope === "final"
    ...
    if (unobserved.length !== 0)
        diagnostics.push({
            ...
            message: `Active ${unobserved.length === 1 ? "scene" : "scenes"} ${unobserved
                .map((scene) => `"${scene.id}"`)
                .join(", ")} ${unobserved.length === 1 ? "has" : "have"} no required acceptance scenario citing them. A compiled realization is not an observation, so nothing here was ever looked at. Author a required acceptance scenario citing the scene, or record a phase-local disposition, then compile again.`,
        });
    ...
    return [
        ...diagnostics,
        {
            ...
            message: `Active ${uncovered.length === 1 ? "scene" : "scenes"} ${uncovered
                .map((scene) => `"${scene.id}"`)
                .join(", ")} ${uncovered.length === 1 ? "has" : "have"} no shot with a passing compiled realization.${props.scope === "source"
 ...
 */
const screenplayResidencyDiagnostics = (props) => props.screenplay !== null || props.contracts.size === 0
    ? []
    : [
        {
...
const reviewGateDiagnostics = (queue) => queue.entries.flatMap((entry) => entry.state === "complete"
    ? []
    : [
        {
            code: entry.target.kind === "asset"
            ...
            path: null,
            message: entry.target.kind === "asset"
...
const finalDeliverableDiagnostics = (project, production, inputFingerprint, reviews) => {
    if (production === null)
        return [];
    let bytes;
    try {
        ...
        return [
            {
                ...
                message: "Required deliverables have no current render manifest. Run the project render command before final compilation.",
            },
        ];
    const manifestDigest = digestAutoMovieBytes(bytes);
    let receipt = null;
    try {
        const receiptBytes = project.readTrackedStateFile("render-manifest-receipt.json");
        if (receiptBytes !== null) {
            const validation = typia.validateEquals(JSON.parse(Buffer.from(receiptBytes).toString("utf8")));
    ...
    if (receipt === null ||
        ...
        return [
            renderDeliverableDiagnostic("render-deliverable-unowned", production.id, "The aggregate render manifest lacks the matching renderer-owned receipt. Recreate it through the production render command instead of editing tracked state directly."),
        ];
    let manifest;
    try {
        ...
        if (validation.success === false)
            return [
                renderDeliverableDiagnostic("render-deliverable-invalid", production.id, `The active production render manifest does not satisfy the aggregate render-ledger schema: ${validation.errors
        ...
        return [
            renderDeliverableDiagnostic("render-deliverable-invalid", production.id, "The aggregate render manifest is not valid JSON. Recreate it through the production render command."),
        ];
    }
    if (manifest.compileFingerprint !== inputFingerprint)
        return [
            renderDeliverableDiagnostic("render-deliverable-stale", production.id, "Required deliverables are not bound to the current compile fingerprint. Re-render the current production and replace the aggregate render manifest."),
        ];
    const diagnostics = [];
    const declared = new Map(production.deliverables.map((deliverable) => [deliverable.id, deliverable]));
    const resident = new Map();
    const filePaths = new Set();
    const receiptByPath = new Map(receipt.files.map((file) => [
        normalizeSlash(file.path).toLowerCase(),
        file,
    ]));
    if (receiptByPath.size !== receipt.files.length)
        diagnostics.push(renderDeliverableDiagnostic("render-deliverable-unowned", production.id, "The renderer-owned receipt repeats a physical file path. Recreate it through the production render command."));
    const witnessedReceiptPaths = new Set();
    for (const deliverable of manifest.deliverables) {
        if (resident.has(deliverable.id))
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-invalid", deliverable.id, `Deliverable "${deliverable.id}" is duplicated in the aggregate render manifest. Keep one byte-exact record.`));
        else
            resident.set(deliverable.id, deliverable);
        const contract = declared.get(deliverable.id);
        if (contract === undefined || contract.kind !== deliverable.kind)
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-invalid", deliverable.id, `Deliverable "${deliverable.id}" kind "${deliverable.kind}" does not match current production design. Remove it or restore the exact declared id and kind.`));
        if (deliverable.files.length === 0)
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-incomplete", deliverable.id, `Deliverable "${deliverable.id}" has no output file. Render at least one owned byte artifact and record its digest and size.`));
        const probes = [];
        for (const file of deliverable.files) {
            const portablePath = normalizeSlash(file.path).toLowerCase();
            if (filePaths.has(portablePath))
                diagnostics.push(renderDeliverableDiagnostic("render-deliverable-invalid", deliverable.id, `Render file "${file.path}" is claimed more than once. Give each owned output one deliverable owner.`, file.path));
            filePaths.add(portablePath);
            witnessedReceiptPaths.add(portablePath);
            if (Number.isInteger(file.bytes) === false ||
                file.bytes <= 0 ||
                file.mediaType.trim().length === 0) {
                diagnostics.push(renderDeliverableDiagnostic("render-deliverable-invalid", deliverable.id, `Render file "${file.path}" needs a positive integer byte size and non-empty media type. Rebuild its ledger entry.`, file.path));
                continue;
            }
            try {
                const actual = project.readRenderFile(file.path);
                if (actual.length !== file.bytes ||
                    digestAutoMovieBytes(actual) !== file.digest)
                    diagnostics.push(renderDeliverableDiagnostic("render-deliverable-stale", deliverable.id, `Render file "${file.path}" bytes do not match its recorded size and digest. Re-render the current deliverable.`, file.path));
                ...
                if (receiptFile === undefined ||
                    ...
                    diagnostics.push(renderDeliverableDiagnostic("render-deliverable-unowned", deliverable.id, `Render file "${file.path}" lacks one exact renderer-owned byte and media-probe receipt. Recreate the aggregate manifest through the production render command.`, file.path));
                else {
                    let probe;
                    try {
                        ...
                        diagnostics.push(renderDeliverableDiagnostic("render-deliverable-invalid", deliverable.id, `Render file "${file.path}" failed current media probing: ${errorMessage(error)} Re-render a valid declared medium.`, file.path));
                        continue;
                    }
                    if (canonicalizeProbe(probe) !== canonicalizeProbe(receiptFile.probe))
                        diagnostics.push(renderDeliverableDiagnostic("render-deliverable-unowned", deliverable.id, `Render file "${file.path}" current media facts differ from its renderer-owned receipt. Recreate the aggregate manifest.`, file.path));
                    else
                        probes.push(probe);
                }
            }
            catch (error) {
                diagnostics.push(renderDeliverableDiagnostic("render-deliverable-missing", deliverable.id, `${errorMessage(error)} Re-render the missing owned output.`, file.path));
            }
        }
        appendDeliverableTimelineDiagnostics(diagnostics, production, deliverable, probes);
        appendRenditionDeliveryDiagnostics(diagnostics, project, production, inputFingerprint, reviews, deliverable);
    }
    for (const file of receipt.files)
        if (witnessedReceiptPaths.has(normalizeSlash(file.path).toLowerCase()) ===
            false)
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-unowned", file.deliverable, `Renderer receipt file "${file.path}" is not owned by the current aggregate manifest. Recreate the manifest and receipt together.`, file.path));
    for (const deliverable of production.deliverables)
        if (deliverable.required && resident.has(deliverable.id) === false)
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-missing", deliverable.id, `Required ${deliverable.kind} deliverable "${deliverable.id}" is absent from the aggregate render manifest. Render and record it before final compilation.`));
    return diagnostics;
};
const appendRenditionDeliveryDiagnostics = (diagnostics, project, production, inputFingerprint, reviews, deliverable) => {
    if (deliverable.kind !== "feature") {
        if (deliverable.rendition !== undefined)
            diagnostics.push(renderDeliverableDiagnostic("render-rendition-provenance-invalid", deliverable.id, "Only feature delivery may claim repaint rendition provenance."));
        return;
    }
    if (production.visualDelivery !== "repainted") {
        if (deliverable.rendition !== undefined)
            diagnostics.push(renderDeliverableDiagnostic("render-rendition-provenance-invalid", deliverable.id, "Deterministic feature delivery must not claim repaint rendition provenance."));
        return;
    }
    try {
        ...
        const receipts = new Map(project
            .verifiedRepaintRenditions(shots)
            .map((receipt) => [receipt.shot, receipt]));
        const currentReview = (target) => {
            const entry = reviews.entries.find((candidate) => reviewTargetKey(candidate.target) === reviewTargetKey(target));
            ...
            if (entry === undefined ||
                ...
                throw new Error(`Review "${reviewTargetKey(target)}" is not current and complete.`);
        ...
        const expected = {
            ...
            shots: shots.map((shot) => {
                ...
                if (receipt === undefined)
                    throw new Error(`Shot "${shot}" has no current verified repaint receipt.`);
                const sourceReviewFingerprint = currentReview({
                    kind: "shot",
                    id: shot,
                });
                if (receipt.sourceReviewFingerprint !== sourceReviewFingerprint)
                    throw new Error(`Shot "${shot}" repaint receipt cites a different source review.`);
                return {
                    shot,
            ...
            aggregateReviews: reviews.entries
                .flatMap((entry) => entry.target.kind === "sequence" || entry.target.kind === "film"
                ? [
                    {
        ...
        if (deliverable.rendition === undefined ||
            Buffer.from(canonicalAutoMovieJsonBytes(deliverable.rendition)).equals(Buffer.from(canonicalAutoMovieJsonBytes(expected))) === false)
            throw new Error("Feature manifest does not exactly cite the current receipt and review chain.");
        const feature = deliverable.files.find((file) => file.mediaType === "video/mp4");
        ...
        diagnostics.push(renderDeliverableDiagnostic("render-rendition-provenance-invalid", deliverable.id, `${errorMessage(error)} Re-finalize from current reviewed repaint receipts; deterministic fallback is not accepted for repainted delivery.`));
    }
};
const appendDeliverableTimelineDiagnostics = (diagnostics, production, deliverable, probes) => {
    const timed = ["feature", "guide-pass", "captions", "audio-mix"].includes(deliverable.kind);
    const framed = deliverable.kind === "feature" || deliverable.kind === "guide-pass";
    const encoded = deliverable.kind === "feature" ||
        deliverable.kind === "guide-pass" ||
        deliverable.kind === "audio-mix";
    const expectedFrames = Math.round(production.targetRuntimeSeconds * production.frameFormat.fps);
    if ((timed && deliverable.runtimeSeconds !== production.targetRuntimeSeconds) ||
        (!timed &&
        ...
        diagnostics.push(renderDeliverableDiagnostic("render-deliverable-incomplete", deliverable.id, `Deliverable "${deliverable.id}" has incomplete runtime, frame-count, or codec evidence for kind "${deliverable.kind}". Match the ${production.targetRuntimeSeconds}s production clock and ${expectedFrames} frames where applicable.`));
    if (deliverable.kind === "feature" || deliverable.kind === "guide-pass") {
        ...
        const controls = deliverable.kind === "guide-pass"
            ? probes.filter((probe) => probe.kind === "png")
            : [];
        const guideContract = production.deliverables.find((candidate) => candidate.id === deliverable.id && candidate.kind === "guide-pass");
        const guidePass = deliverable.kind === "guide-pass"
            ? (guideContract?.pass ?? "pose")
            : null;
        const controlPaths = guidePass === null
            ? []
            : deliverable.files
                .filter((file) => file.mediaType === "image/png")
                .map((file) => normalizeSlash(file.path));
        const expectedControlPath = (frame) => `frames/${guidePass}/frame_${String(frame).padStart(8, "0")}.png`;
        if (video?.kind !== "video" ||
            (deliverable.kind === "feature" && probes.length !== 1) ||
            (deliverable.kind === "guide-pass" &&
                (controls.length !== probes.length - 1 ||
                    controlPaths.length !== expectedFrames ||
                    controlPaths.some((control, index) => control !== expectedControlPath(index) &&
                        control.endsWith(`/${expectedControlPath(index)}`) === false) ||
                    controls.some((probe) => probe.width !== production.frameFormat.width ||
            ...
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-media-mismatch", deliverable.id, `Deliverable "${deliverable.id}" must own one parsed ${production.frameFormat.width}x${production.frameFormat.height} H.264 MP4 at ${production.frameFormat.fps}fps with ${expectedFrames} resident samples and ${production.targetRuntimeSeconds}s runtime${deliverable.kind === "guide-pass" ? `, plus exactly ${expectedFrames} continuous same-raster "${guidePass}" PNG controls` : ""}. Manifest strings cannot substitute for parser-derived media facts.`));
    }
    else if (deliverable.kind === "preview") {
        if (probes.length !== deliverable.files.length ||
            probes.some((probe) => probe.kind !== "png" ||
                probe.width !== production.frameFormat.width ||
                probe.height !== production.frameFormat.height))
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-media-mismatch", deliverable.id, `Preview deliverable "${deliverable.id}" must contain decoded PNGs at the exact ${production.frameFormat.width}x${production.frameFormat.height} production raster.`));
    }
    else if (deliverable.kind === "captions") {
        if (probes.length !== deliverable.files.length ||
            probes.some((probe) => probe.kind !== "webvtt" ||
                probe.lastCueSeconds > production.targetRuntimeSeconds))
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-media-mismatch", deliverable.id, `Caption deliverable "${deliverable.id}" must contain parser-verified, ordered, non-empty WebVTT cues wholly inside the ${production.targetRuntimeSeconds}s production timeline.`));
    }
    else {
        ...
        if (audio.length !== 1 ||
            rasters.length !== 2 ||
            evidence.length !== 1 ||
            frameClockClose(audio[0].runtimeSeconds, production.targetRuntimeSeconds) === false ||
            ...
            diagnostics.push(renderDeliverableDiagnostic("render-deliverable-media-mismatch", deliverable.id, `Audio deliverable "${deliverable.id}" must own one exact-runtime parsed audio/mp4 track, waveform and spectrogram PNGs, and parser-verified zero-clipping event-alignment evidence.`));
    }
};
const canonicalizeProbe = (probe) => Buffer.from(canonicalAutoMovieJsonBytes(probe)).toString("utf8");
const frameClockClose = (left, right) => Math.abs(left - right) <=
    Number.EPSILON * 64 * Math.max(1, Math.abs(left), Math.abs(right));
const renderDeliverableDiagnostic = (code, target, message, renderPath = null) => ({
    code,
    category: "error",
    phase: "render",
    target,
...
const compareDiagnostics = (left, right) => compareCodeUnits(left.phase, right.phase) ||
    compareCodeUnits(left.path ?? "", right.path ?? "") ||
    compareCodeUnits(left.code, right.code) ||
    compareCodeUnits(left.message, right.message);
const normalizeSlash = (value) => value.split(path.sep).join("/");
//# sourceMappingURL=AutoMovieProductionCompiler.js.map