...
const COORDINATION_ROOT = path.join(currentUser.homedir, ".automovie-root-locks");
const coordinatePath = (kind, namespace) => {
...
const ensureCoordinationRoot = () => {
    ...
    if (linked.isSymbolicLink() || linked.isDirectory() === false)
        throw new Error(`AutoMovie root-lock coordination path "${COORDINATION_ROOT}" is not a physical directory.`);
...
const acquireCoordinate = (lockPath) => ({
    path: lockPath,
    token: acquireCommitLock(lockPath),
});
const releaseCoordinate = (lease) => {
    releaseCommitLock(lease.path, lease.token);
};
const acquireCoordinates = (paths) => {
    const leases = [];
    try {
        ...
        for (const lease of leases.reverse())
            releaseCoordinate(lease);
        throw error;
    }
};
const releaseCoordinates = (leases) => {
    for (const lease of [...leases].reverse())
        releaseCoordinate(lease);
};
const creationCoordinates = (parentReal, childName, parentIdentity) => {
    return [
        coordinatePath("create-path", path.join(parentReal, childName)),
        coordinatePath("create-id", `${parentIdentity.dev}\0${parentIdentity.ino}\0${childName.toLowerCase()}`),
    ];
};
const acquireCreationCoordinates = (parentReal, childName) => {
    const parentIdentity = fs.statSync(parentReal, { bigint: true });
    const locks = acquireCoordinates(creationCoordinates(parentReal, childName, parentIdentity));
    try {
        const current = physicalDirectoryIdentityOrNull(parentReal);
        if (current === null ||
            current.dev !== parentIdentity.dev ||
            current.ino !== parentIdentity.ino)
            throw new Error(`Production project parent "${parentReal}" changed physical identity during namespace acquisition. No child was created.`);
        return locks;
    }
    catch (error) {
...
const ensureDirectory = (directory) => {
    ...
    if (linked !== null) {
        if (fs.statSync(directory).isDirectory() === false)
            throw new Error(`Production project parent "${directory}" is not a directory.`);
        return fs.realpathSync(directory);
    }
    const parent = path.dirname(directory);
    if (parent === directory)
        throw new Error(`Production project parent "${directory}" does not exist as a physical directory.`);
    const parentReal = ensureDirectory(parent);
    const physical = path.join(parentReal, path.basename(directory));
    const locks = acquireCreationCoordinates(parentReal, path.basename(directory));
    try {
        ...
        if (current === null)
            ...
            throw new Error(`Production project parent "${directory}" is not a physical directory.`);
...
const acquireExistingRoot = (rootDirectory, productionId) => {
    const linked = lstatOrNull(rootDirectory);
    if (linked === null ||
        linked.isSymbolicLink() ||
        linked.isDirectory() === false)
        throw new Error(`Production project root "${rootDirectory}" is not a physical directory.`);
    ...
    const locks = productionId === undefined
        ? acquireCoordinates([
        ...
        : acquireCoordinates([
            ...
            coordinatePath("production-id", `${device}\0${inode}\0${productionId}`),
        ]);
    const lease = {
        root,
        locks,
 ...
 */
export const acquireProductionRootNamespace = (rootDirectory, productionId) => acquireExistingRoot(path.resolve(rootDirectory), productionId);
/**
 ...
 */
export const acquireOrCreateProductionRootNamespace = (rootDirectory) => {
    const root = path.resolve(rootDirectory);
    const linked = lstatOrNull(root);
    if (linked !== null)
        return acquireExistingRoot(root);
    ...
    try {
        ...
        if (current === null)
            ...
            throw new Error(`Production project root "${root}" is not a physical directory.`);
...
export const assertProductionRootNamespaceLease = (lease) => {
    ...
    if (current === null ||
        ...
        throw new Error(`Production project root identity or namespace fence changed while "${lease.root}" was held. No unfenced project mutation is allowed.`);
};
/** Release one held physical-root namespace reservation. */
export const releaseProductionRootNamespace = (lease) => {
    releaseCoordinates(lease.locks);
};
const assertRequestedRootIdentity = (requestedRoot, lease) => {
    const current = physicalDirectoryIdentityOrNull(requestedRoot);
    if (current === null ||
        current.dev.toString() !== lease.device ||
        current.ino.toString() !== lease.inode)
        throw new Error(`Requested production root "${requestedRoot}" changed physical identity during namespace acquisition. No project state was initialized.`);
};
...
//# sourceMappingURL=rootNamespaceLock.js.map