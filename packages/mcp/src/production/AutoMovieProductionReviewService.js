...
import { AUTOMOVIE_PRODUCTION_COMPILER_PROTOCOL, AUTOMOVIE_PRODUCTION_COMPILER_VERSION, AutoMovieProductionCompiler, currentAutoMovieProductionCompilerInputFingerprint, } from "./AutoMovieProductionCompiler";
...
import { AUTOMOVIE_REVIEW_FINGERPRINT_PROTOCOL, canonicalAutoMovieJsonBytes, canonicalizeAutoMovieJson, compareCodeUnits, digestAutoMovieBytes, encodeAutoMoviePathSegment, fingerprintAutoMovieFields, normalizeAutoMovieSource, } from "./contentIdentity";
import { parseAutoMovieFilmTimeline, selectAutoMovieFilmReviewFrames, } from "./filmTimeline";
...
export class AutoMovieProductionReviewService {
    project;
    compileStatus;
    constructor(project, compileStatus = () => new AutoMovieProductionCompiler(project).lint({ scope: "source" })) {
        this.project = project;
        this.compileStatus = compileStatus;
    }
    /** Prepare current selectors, frames and required criteria for one target. */
    prepare(input) {
        const compileBound = input.target.kind === "asset" ||
        ...
        const visual = input.target.kind === "asset" ||
            input.target.kind === "shot" ||
            input.target.kind === "rendition" ||
            input.target.kind === "sequence" ||
            input.target.kind === "film";
    ...
    prepareWithStatus(input, compileStatus, context) {
        ...
        if (targetValue === null)
            diagnostics.push({
                ...
                message: "The review target does not exist in current project bytes. Inspect the project and prepare a current target.",
            });
        if (input.target.kind === "source" && compileStatus !== null) {
            const sourcePath = input.target.path;
            const boundShots = new Set([...graph.shots]
                .filter(([, shot]) => shot.source.module === sourcePath)
                .map(([id]) => `shot:${id}`));
            diagnostics.push(...compileStatus.diagnostics.filter((diagnostic) => diagnostic.category === "error" &&
                (diagnostic.path === sourcePath ||
                    ((diagnostic.phase === "source" ||
                        diagnostic.phase === "compile") &&
                        boundShots.has(diagnostic.target)))));
            if (compileStatus.diagnostics.some((diagnostic) => diagnostic.category === "error" && diagnostic.phase === "design"))
                diagnostics.push({
                    ...
                    target: reviewTargetKey(input.target),
                    path: sourcePath,
                    message: "Design or model materialization errors prevented a trustworthy execution of this source. Correct the upstream compile blockers, then prepare this source review again.",
                });
        }
        const frames = currentFrames(this.project, input.target, diagnostics, compileStatus, context);
        const renditions = currentRenditions(this.project, input.target, diagnostics, context);
        if (input.target.kind === "film" &&
            graph.production?.visualDelivery === "repainted" &&
            compileStatus !== null)
            appendRenditionDeliveryReviewDiagnostic(diagnostics, this.project, input.target.id, compileStatus, renditions, context);
        if (input.target.kind === "rendition" && compileStatus !== null) {
            const sourceTarget = {
                kind: "shot",
                id: input.target.id,
            };
            const currentSourceFingerprint = reviewFingerprint(this.project, sourceTarget, compileStatus, context);
        ...
        const outcomes = currentAcceptanceOutcomes(this.project, input.target, diagnostics, compileStatus, context);
        if (visualReviewTarget(input.target) &&
            input.target.kind !== "rendition" &&
            frames.length === 0 &&
            diagnostics.some((diagnostic) => diagnostic.code === "review-evidence-missing") === false)
            diagnostics.push({
                ...
                target: reviewTargetKey(input.target),
                path: null,
                message: "This visual target has no verified current PNG frame. Capture every required current view and pass before submitReview. Correction feedback does not authorize deleting the artifact.",
            });
        const quotable = input.target.kind === "design" && targetValue !== null
            ...
            : input.target.kind === "asset"
                ...
                : input.target.kind === "source"
                    ...
                    : input.target.kind === "shot"
                        ? shotSourceSelectors(this.project, graph.shots.get(input.target.id)?.source.module, diagnostics)
        ...
        return {
            target: input.target,
            fingerprint: reviewFingerprint(this.project, input.target, compileStatus, context),
    ...
    submit(input) {
        ...
        const diagnostics = [
            ...prepared.diagnostics.filter((diagnostic) => diagnostic.category === "error"),
            ...validateWorksheet(this.project, input, prepared),
        ];
        if (diagnostics.some((diagnostic) => diagnostic.category === "error"))
            return refused(this.project, input.target, prepared.fingerprint, diagnostics);
        const finalCompileStatus = visualReviewTarget(input.target)
            ? this.compileStatus()
            : null;
        const fingerprint = reviewFingerprint(this.project, input.target, finalCompileStatus);
            ...
            return refused(this.project, input.target, fingerprint, [
                {
                    ...
                    target: reviewTargetKey(input.target),
                    path: targetPath(this.project, input.target),
                    message: "Target bytes changed while review evidence was validated. Run prepareReview again against the current target.",
                },
            ]);
        const stored = {
            version: 1,
        ...
        try {
            this.project.commitReview(stored, () => reviewFingerprint(this.project, input.target, finalCompileStatus, undefined, input.target.kind === "sequence" || input.target.kind === "film"
                ? currentAutoMovieProductionCompilerInputFingerprint(this.project, "source")
     ...
     */
    queue(currentCompileStatus = this.compileStatus(), snapshot) {
        ...
        const entries = reviewTargets(this.project, context).map((target) => {
            const currentFingerprint = reviewFingerprint(this.project, target, compileStatus, context);
            const stored = this.project.review(target);
            const prepared = stored === null || stored.fingerprint !== currentFingerprint
                ? null
                : this.prepareWithStatus({ target }, target.kind === "asset" ||
                    target.kind === "source" ||
                    target.kind === "shot" ||
                    ...
                    ? compileStatus
                    : null, context);
            const storedDiagnostics = stored === null || prepared === null
                ? []
                : [
                    ...prepared.diagnostics.filter((diagnostic) => diagnostic.category === "error"),
            ...
            return {
                ...
                state: stored === null
                    ...
                    : stored.fingerprint !== currentFingerprint
                        ? "stale"
                        : storedDiagnostics.some((diagnostic) => diagnostic.category === "error")
                            ? "incomplete"
                            : reviewState(stored),
                currentFingerprint,
...
const validateWorksheet = (project, input, prepared) => {
    const diagnostics = [];
    ...
    if (input.observations.trim().length === 0)
        add("review-observation-empty", "Overall observations are blank. Record concrete current-target observations before submitReview.");
    if (input.completionBasis.trim().length === 0)
        add("review-completion-basis-empty", "Completion basis is blank. Cite the decisive checks and evidence before the complete field.");
    ...
    if (actual.length !== expected.length ||
        ...
        add("review-checklist-incomplete", `Criteria must appear exactly once in canonical order: ${expected.join(", ")}. Rewrite checks before submitReview.`);
    const copies = new Set();
    const reusedEvidence = new Set();
    for (const check of input.checks) {
        if (check.criterion !== "acceptance-scenarios" &&
            (check.acceptanceScenarios?.length ?? 0) !== 0)
            add("review-acceptance-coverage-misplaced", `Criterion "${check.criterion}" cannot claim acceptance scenario ids. Put them only on acceptance-scenarios.`);
        if (check.observation.trim().length === 0)
            add("review-observation-empty", `Criterion "${check.criterion}" has a blank observation. Record a criterion-specific observation.`);
        if (check.evidence.length === 0)
            add("review-evidence-missing", `Criterion "${check.criterion}" has no evidence. Quote current design, source, frame, or diagnostic evidence.`);
        const copyKey = `${check.observation.trim()}\0${canonicalizeAutoMovieJson(check.evidence)}`;
        if (copies.has(copyKey))
            add("review-observation-copied", `Criterion "${check.criterion}" duplicates another observation and evidence set. Record what this criterion independently establishes.`);
        copies.add(copyKey);
        for (const evidence of check.evidence) {
            const evidenceKey = reviewEvidenceIdentity(evidence);
            if ((input.target.kind === "design" || input.target.kind === "source") &&
                reusedEvidence.has(evidenceKey))
                add("review-evidence-reused", `Criterion "${check.criterion}" reuses the same evidence item as another criterion. Inspect and cite a distinct current selector for each design or source concern.`);
            reusedEvidence.add(evidenceKey);
            diagnostics.push(...validateEvidence(project, input.target, evidence, prepared, check));
        }
    }
    if (input.complete) {
        for (const check of input.checks)
            if (check.verdict !== "pass")
                add("review-required-criterion-not-passed", `Required criterion "${check.criterion}" must pass before complete can be true; not-applicable cannot discharge a required checklist entry.`);
        if (input.checks.some((check) => check.verdict === "revise"))
            add("review-self-contradiction", "complete is true while at least one criterion says revise. Correct the target or set complete false.");
        if (input.corrections.length !== 0)
            add("review-self-contradiction", "complete is true while corrections remain. Apply them or set complete false.");
        if (visualReviewTarget(input.target) &&
            input.target.kind !== "rendition" &&
            input.checks.some((check) => check.evidence.some((evidence) => evidence.kind === "frame")) === false)
            add("review-evidence-missing", "A visual target cannot complete without a verified current frame. Capture and cite one required frame.");
        if (input.target.kind === "asset") {
            const targetId = input.target.id;
            const cited = new Set(input.checks.flatMap((check) => check.evidence.flatMap((evidence) => evidence.kind === "frame" &&
            ...
            if (missing.length !== 0)
                add("review-asset-view-coverage-incomplete", `A completed asset review must cite every required current view digest. Missing: ${missing.join(", ")}.`);
        }
        if (prepared.renditions.length !== 0) {
            const requiredShots = [
                ...new Set(prepared.renditions.map((rendition) => rendition.shot)),
            ];
            const citedShots = new Set(input.checks.flatMap((check) => check.evidence.flatMap((evidence) => evidence.kind === "rendition" ? [evidence.shot] : [])));
            const missing = requiredShots.filter((shot) => citedShots.has(shot) === false);
            if (missing.length !== 0)
                add("review-rendition-coverage-incomplete", `A completed repainted-delivery review must cite one current receipt-bound rendition for every addressed shot. Missing: ${missing.join(", ")}.`);
        }
        for (const criterion of highRiskCriteria(input.target)) {
            if (input.completionBasis.includes(criterion) === false)
                add("review-completion-basis-incomplete", `Completion basis does not name high-risk criterion "${criterion}". Reconfirm it with current evidence.`);
            if (input.checks.find((check) => check.criterion === criterion)?.verdict !==
                "pass")
                add("review-high-risk-not-passed", `High-risk criterion "${criterion}" must pass before complete can be true. Use not-applicable only on genuinely optional criteria.`);
        }
        diagnostics.push(...validateAcceptanceCoverage(project, input, prepared));
    }
    else if (input.corrections.length === 0 &&
        input.checks.some((check) => check.verdict === "revise") === false)
        add("review-self-contradiction", "complete is false but no revise verdict or correction explains the next round. Add an actionable correction.");
    for (const correction of input.corrections)
        if (correction.target.trim().length === 0 ||
            correction.problem.trim().length === 0 ||
            correction.expected.trim().length === 0)
            add("review-correction-empty", "Every correction requires target, problem, and expected state. Complete the correction before submitReview.");
    return diagnostics;
};
/**
 ...
 */
const reviewEvidenceIdentity = (evidence) => {
...
const validateAcceptanceCoverage = (project, input, prepared) => {
    ...
    const sequenceShots = target.kind === "sequence"
    ...
    const scenarios = [...graph.acceptance.values()]
        ...
        (target.kind === "film" ||
            (target.kind === "sequence" &&
                [...(sequenceShots ?? [])].some((shot) => acceptanceAddressesShot(scenario, shot))) ||
            acceptanceAddressesShot(scenario, target.id)))
        .filter((scenario) => target.kind === "shot" ||
        filmTimeline === null ||
        acceptanceBelongsToFilm(graph, scenario, filmTimeline, currentAcceptanceEventTime(project, scenario)))
        .sort((left, right) => compareCodeUnits(left.id, right.id));
    const check = input.checks.find((item) => item.criterion === "acceptance-scenarios");
    const actual = [...(check?.acceptanceScenarios ?? [])].sort(compareCodeUnits);
    const expected = scenarios.map((scenario) => scenario.id);
    const add = (scenario, message) => {
        diagnostics.push({
    ...
    if (sameStringSet(actual, expected) === false)
        add(expected.join(", ") || "(none)", `the acceptance-scenarios check must list the exact current required ids: ${expected.join(", ") || "(none)"}.`);
    for (const scenario of scenarios) {
        if (check?.evidence.some((evidence) => evidence.kind === "acceptance" &&
            evidence.scenario === scenario.id &&
            canonicalizeAutoMovieJson(evidence.exactValue) ===
                canonicalizeAutoMovieJson(scenario)) !== true)
            add(scenario.id, "cite its exact current acceptance contract in the acceptance-scenarios check.");
        ...
        if (criterion.kind === "frame") {
            ...
            if (shot === undefined ||
                check?.evidence.some((evidence) => evidence.kind === "frame" &&
                    ...
                    prepared.frames.some((frame) => canonicalizeAutoMovieJson(frame.target) ===
                        ...
                        frame.digest === evidence.digest)) !== true)
                add(scenario.id, `cite the exact current frame "${criterion.frame}" pass "${criterion.pass}" from shot "${String(shot)}".`);
        }
        else {
            const outcome = prepared.outcomes.find((candidate) => candidate.scenario === scenario.id);
            if (outcome === undefined ||
                outcome.passed === false ||
                check?.evidence.some((evidence) => evidence.kind === "outcome" &&
                    evidence.scenario === scenario.id &&
                    canonicalizeAutoMovieJson(evidence.exactValue) ===
                        canonicalizeAutoMovieJson(outcome)) !== true)
                add(scenario.id, criterion.kind === "event"
...
const validateEvidence = (project, target, evidence, prepared, check) => {
    const diagnostics = [];
    ...
    if (evidence.kind === "design") {
        if (!((target.kind === "design" &&
            ...
            fail("review-evidence-target-mismatch", "design evidence addresses a different target. Quote the prepared target.");
        else {
            ...
            if (resolved.found === false)
                fail("review-evidence-selector-invalid", `JSON pointer "${evidence.pointer}" does not exist. Use a selector returned by prepareReview.`);
            else if (canonicalizeAutoMovieJson(resolved.value) !==
                canonicalizeAutoMovieJson(evidence.exactValue))
                fail("review-evidence-stale", `JSON pointer "${evidence.pointer}" no longer equals exactValue. Prepare the review again.`);
        }
    }
    else if (evidence.kind === "source") {
        const allowed = prepared.quotable.some((selector) => selector.kind === "source" &&
            selector.path === evidence.path &&
            selector.line === evidence.line);
        if (allowed === false)
            fail("review-evidence-selector-invalid", `Source ${evidence.path}:${evidence.line} is not a prepared selector. Quote a current owned line.`);
        else if (evidence.exactText.trim().length === 0)
            fail("review-evidence-empty", "source exactText is blank. Quote concrete non-whitespace source.");
        else {
            ...
            if (selector?.kind !== "source" ||
                ...
                fail("review-evidence-stale", `Source ${evidence.path}:${evidence.line} no longer exactly equals exactText. Prepare the review again.`);
        }
    }
    else if (evidence.kind === "frame") {
        const frame = prepared.frames.find((item) => canonicalizeAutoMovieJson(item.target) ===
            canonicalizeAutoMovieJson(evidence.target) &&
        ...
        if (frame === undefined)
            fail("review-evidence-stale", "frame evidence is not in the current verified bundle inventory. Capture and prepare the current frame.");
        else if (evidence.region !== undefined &&
            ...
            fail("review-evidence-region-invalid", "frame region must be a non-empty integer rectangle inside the current image.");
    }
    else if (evidence.kind === "rendition") {
        const { kind: _kind, ...submitted } = evidence;
        if (prepared.renditions.some((rendition) => canonicalizeAutoMovieJson(rendition) ===
            canonicalizeAutoMovieJson(submitted)) === false)
            fail("review-evidence-stale", "rendition evidence is not in the current byte- and receipt-verified inventory. Prepare and inspect the current repainted output.");
    }
    else if (evidence.kind === "diagnostic") {
        const diagnostic = prepared.diagnostics.find((item) => item.code === evidence.code && (item.path ?? "") === evidence.path);
        if (diagnostic === undefined)
            fail("review-evidence-stale", "diagnostic evidence is not in the current prepare snapshot.");
        else if (canonicalizeAutoMovieJson(evidence.actual) !==
            canonicalizeAutoMovieJson(diagnostic.message))
            fail("review-evidence-stale", "diagnostic actual must exactly equal the current diagnostic message.");
    }
    else if (evidence.kind === "outcome") {
        const current = prepared.outcomes.find((outcome) => outcome.scenario === evidence.scenario);
        if (current === undefined ||
            canonicalizeAutoMovieJson(current) !==
                canonicalizeAutoMovieJson(evidence.exactValue))
            fail("review-evidence-stale", `acceptance outcome "${evidence.scenario}" is absent or no longer equals exactValue. Prepare the current compiler/oracle outcome again.`);
    }
    else {
        ...
        if (current === undefined ||
            ...
            (target.kind === "sequence" &&
                sequenceShotIds(project, target.id).every((shot) => acceptanceAddressesShot(current, shot) === false)) ||
            (target.kind !== "shot" &&
                target.kind !== "sequence" &&
                target.kind !== "film") ||
            canonicalizeAutoMovieJson(current) !==
                canonicalizeAutoMovieJson(evidence.exactValue))
            fail("review-evidence-stale", `acceptance scenario "${evidence.scenario}" is not a current required scenario for this visual target or exactValue is stale.`);
    }
    return diagnostics;
};
const currentSourceLine = (project, sourcePath, line) => {
    try {
        const source = Buffer.from(normalizeAutoMovieSource(project.readSource(sourcePath))).toString("utf8");
        return source.split("\n")[line - 1] ?? "";
    }
    catch {
        return "";
...
const targetValueOf = (project, target) => {
    if (target.kind === "asset")
        return project.graph().models.get(target.id) ?? null;
    if (target.kind === "design")
        return project.design(target.design);
    if (target.kind === "source") {
        try {
            return Buffer.from(normalizeAutoMovieSource(project.readSource(target.path))).toString("utf8");
        }
        catch {
            return null;
        }
...
const reviewFingerprint = (project, target, compileStatus, context, currentCompileInput) => {
    ...
    if (target.kind === "asset") {
        ...
        try {
            addJson("compiled-model", JSON.parse(Buffer.from(currentGeneratedFile(project, `models/${encodeAutoMoviePathSegment(target.id)}.json`, context)).toString("utf8")));
        }
        catch {
            addJson("compiled-model", null);
        }
        for (const frame of currentFrames(project, target, [], compileStatus, context))
            addJson(`frame:${canonicalizeAutoMovieJson(frame.target)}:${frame.pass}`, frame);
        fields.push(compilerField());
    }
    else if (target.kind === "design") {
        addJson("design", project.design(target.design));
        const addModelGraph = (role, modelId, seen = new Set()) => {
            if (seen.has(modelId))
                return;
            seen.add(modelId);
            ...
            if (model !== null)
                for (const lod of model.lod)
                    if (lod.recipe !== modelId)
                        addModelGraph(role, lod.recipe, seen);
        };
        ...
        if (shot !== null)
            addSourceField(fields, project, shot.source.module);
        ...
        addJson("render-target", generated === null
            ? null
            : currentRenderTargetFingerprint(project, generated, target, context));
        for (const frame of currentFrames(project, target, [], compileStatus, context))
            addJson(`frame:${frame.bundle}:${frame.frame}:${frame.pass}`, frame);
        for (const outcome of currentAcceptanceOutcomes(project, target, [], compileStatus, context))
        ...
        fields.push({
            ...
            kind: "digest",
            payload: Buffer.from(reviewFingerprint(project, shotTarget, compileStatus, context), "utf8"),
        });
        ...
        for (const id of shotIds) {
            ...
            if (graph.production?.visualDelivery === "repainted") {
                ...
                fields.push({
                    ...
                    kind: "digest",
                    payload: Buffer.from(reviewFingerprint(project, renditionTarget, compileStatus, context), "utf8"),
                });
        ...
        for (const rendition of currentRenditions(project, target, [], context))
            addJson(`rendition:${rendition.path}`, rendition);
        for (const frame of currentFrames(project, target, [], compileStatus, context))
            addJson(`frame:${canonicalizeAutoMovieJson(frame.target)}:${frame.frame}:${frame.pass}`, frame);
        fields.push(compilerField());
    }
    else {
        ...
        addJson("compile-current", currentCompileInput === undefined
            ...
            : currentCompileInput);
        try {
            addJson("film-timeline", JSON.parse(Buffer.from(currentGeneratedFile(project, "film-timeline.json", context)).toString("utf8")));
        }
        catch {
        ...
        for (const sequence of project.screenplayIndex()?.treatment.sequences ??
            ...
            fields.push({
                ...
                kind: "digest",
                payload: Buffer.from(reviewFingerprint(project, sequenceTarget, compileStatus, context), "utf8"),
            });
...
const addSourceField = (fields, project, sourcePath) => {
    try {
        fields.push({
...
const reviewTargets = (project, context) => {
    const graph = project.graph();
    const targets = [];
...
const consumedModelIds = (project, context) => {
    const generated = currentGeneratedManifest(project, context);
    const graph = project.graph();
    ...
    const addModel = (id) => {
        ...
        if (recipe === undefined)
            return;
        models.add(id);
        for (const lod of recipe.lod)
            addModel(lod.recipe);
    };
    for (const shot of graph.shots.values())
        for (const participant of shot.participants)
            if (participant.kind === "actor")
                addModel(participant.id);
            else {
                const formation = graph.formations.get(participant.id);
                if (formation !== undefined)
                    addModel(formation.modelRecipe);
            }
    if (generated !== null)
        for (const entry of generated.files)
            if (entry.path.startsWith("shots/"))
                try {
                    const validation = typia.validateEquals(JSON.parse(Buffer.from(currentGeneratedFile(project, entry.path, context)).toString("utf8")));
                    if (validation.success)
                        for (const model of validation.data.models)
                            addModel(model.id);
                }
                catch {
...
const sequenceShotIds = (project, sequenceId) => {
    const index = project.screenplayIndex();
    const sequence = index?.treatment.sequences.find((candidate) => candidate.id === sequenceId);
    if (index === null || index === undefined || sequence === undefined)
        return [];
    const beats = new Set(sequence.beats.map((beat) => beat.text));
    const scenes = new Set(index.screenplay.scenes
        .filter((scene) => scene.status === "active" &&
        scene.covers.some((coverage) => beats.has(coverage.beat)))
        .map((scene) => scene.id));
    return [...project.graph().shots]
        .filter(([, shot]) => (shot.evidence ?? []).some((evidence) => scenes.has(evidence.scene)))
        .map(([id]) => id)
        .sort(compareCodeUnits);
};
const currentAcceptanceOutcomes = (project, target, diagnostics, compileStatus, context) => {
    if (target.kind !== "shot" &&
        target.kind !== "sequence" &&
        target.kind !== "film")
        return [];
    ...
    if (generated === null ||
        ...
        for (const scenario of scenarios)
            diagnostics.push(outcomeMissingDiagnostic(target, scenario.id, "Compiler-owned output is absent or not current, so no acceptance outcome can be derived. Compile the project, then prepare this review again."));
        return [];
    }
    ...
    const readTimeline = () => {
        if (retainedTimeline !== undefined)
            return retainedTimeline;
        try {
            retainedTimeline = currentFilmTimeline(project, generated.inputFingerprint, context);
        }
        catch {
    ...
    const readCompiled = (shot) => {
        const retained = compiled.get(shot);
        if (retained !== undefined)
            return retained;
        try {
            const validation = typia.validateEquals(JSON.parse(Buffer.from(currentGeneratedFile(project, `shots/${encodeAutoMoviePathSegment(shot)}.json`, context)).toString("utf8")));
    ...
    const readRealization = (shot) => {
        const retained = realizations.get(shot);
        if (retained !== undefined)
            return retained;
        try {
            const validation = typia.validateEquals(JSON.parse(Buffer.from(currentGeneratedFile(project, `realizations/${encodeAutoMoviePathSegment(shot)}.json`, context)).toString("utf8")));
            if (validation.success === false)
                return null;
    ...
    for (const scenario of scenarios) {
        ...
        if (criterion.kind === "event") {
            const shot = criterion.shot ??
                (scenario.target.kind === "shot" ? scenario.target.id : undefined);
            const event = shot === undefined
                ? undefined
                : readRealization(shot)?.events.find((candidate) => candidate.id === criterion.event);
            if (target.kind === "film" &&
                acceptanceBelongsToFilm(graph, scenario, readTimeline(), event?.time) === false)
                continue;
            if (shot === undefined || event === undefined) {
                diagnostics.push(outcomeMissingDiagnostic(target, scenario.id, `Compiler-derived event "${criterion.event}" is absent. Recompile the owning shot before review.`));
                continue;
            }
            outcomes.push({
                kind: "event",
        ...
        const filmFrames = scenario.target.kind === "film" ? readTimeline()?.totalFrames : null;
        const actual = scenario.target.kind === "shot"
            ? readCompiled(scenario.target.id)?.shot.duration
            : filmFrames === null || filmFrames === undefined
                ? null
                : filmFrames / fps;
        if (actual === undefined || actual === null) {
            diagnostics.push(outcomeMissingDiagnostic(target, scenario.id, "Compiler-derived runtime is unavailable. Recompile every addressed shot before review."));
            continue;
        }
        outcomes.push({
            kind: "metric",
...
const outcomeMissingDiagnostic = (target, scenario, message) => ({
...
const acceptanceBelongsToFilm = (graph, scenario, timeline, eventTime) => {
    if (timeline === null)
        return true;
    if (scenario.target.kind === "film" && scenario.criterion.kind === "metric")
        return true;
    const shot = (scenario.criterion.kind === "frame" ||
    ...
    if (shot === null)
        return true;
    const segment = timeline.segments.find((candidate) => candidate.shot === shot);
    if (segment === undefined)
        return false;
    if (scenario.criterion.kind === "frame") {
        const criterion = scenario.criterion;
        const residentShot = graph.shots.get(shot);
        if (residentShot === undefined)
            return false;
        const frame = residentShot.reviewFrames.find((candidate) => candidate.id === criterion.frame);
        if (frame === undefined)
            return true;
...
const currentAcceptanceEventTime = (project, scenario) => {
    if (scenario.criterion.kind !== "event")
        return undefined;
    const shot = scenario.criterion.shot ??
        (scenario.target.kind === "shot" ? scenario.target.id : undefined);
    if (shot === undefined)
        return undefined;
    const eventId = scenario.criterion.event;
    try {
        const validation = typia.validateEquals(JSON.parse(Buffer.from(project.readGeneratedFile(`realizations/${encodeAutoMoviePathSegment(shot)}.json`)).toString("utf8")));
...
const currentAssetFrames = (project, target, diagnostics, compileStatus, context) => {
    const generated = currentGeneratedManifest(project, context);
    const production = project.graph().production;
    if (generated === null || production === null)
        return [];
    if (compileStatus.success === false ||
        ...
        diagnostics.push({
            ...
            target: reviewTargetKey(target),
            path: null,
            message: "Generated model output is not a clean compile of current design and source. Compile the project before capturing asset review evidence.",
        });
        return [];
    }
    let model;
    try {
        const validation = typia.validateEquals(JSON.parse(Buffer.from(currentGeneratedFile(project, `models/${encodeAutoMoviePathSegment(target.id)}.json`, context)).toString("utf8")));
        if (validation.success === false)
        ...
        diagnostics.push({
            ...
            message: `Current compiler-owned model is unavailable: ${error instanceof Error ? error.message : String(error)}. Compile the consumed model before asset review.`,
        });
        return [];
    }
    const required = [
    ...
    const frames = [];
    const inventory = context?.renderInventory ?? collectRenderManifestInventory(project);
    appendInvalidRenderManifestDiagnostics(project, diagnostics, inventory.invalid);
    const entries = inventory.all
        .filter((entry) => entry.manifest.target.kind === "asset" &&
        entry.manifest.target.id === target.id)
        .sort((left, right) => compareCodeUnits(left.path, right.path));
    for (const entry of entries) {
        const manifest = project.verifiedRenderManifest(entry.path);
        if (manifest === null || manifest.target.kind !== "asset")
            continue;
        const manifestTarget = manifest.target;
        const requirement = required.find((item) => item.angleDeg === manifestTarget.angleDeg &&
        ...
        if (manifest.targetFingerprint !==
            currentRenderTargetFingerprint(project, generated, manifest.target, context))
            continue;
        const bundleRoot = path.dirname(entry.path);
        const bundle = normalizeSlash(path.relative(project.root, bundleRoot));
        if (manifest.renderSpec.target !== manifest.target.id ||
            ...
            diagnostics.push({
                ...
                message: "This asset view does not match the current asset, production FPS, and exact production raster, so it cannot discharge review. Capture the required view again without width/height overrides before submitReview.",
            });
            continue;
        }
        const expectedIndex = Math.round((requirement.angleDeg / 30) * production.frameFormat.fps);
        const frame = manifest.frames.find((candidate) => candidate.index === expectedIndex &&
        ...
        try {
            ...
            const bytes = project.readRenderFile(normalizeSlash(path.relative(project.renderRoot(), file)));
            ...
            if (png.width !== production.frameFormat.width ||
                png.height !== production.frameFormat.height ||
                hasVisiblePixelVariance(png) === false ||
                frameClockClose(frame.time, expectedTime) === false)
                throw new Error("production dimensions, visible pixels, or frame clock do not match");
            frames.push({
            ...
            diagnostics.push({
                ...
                message: `${error instanceof Error ? error.message : String(error)}. Recreate this asset view through captureFrame.`,
            });
        }
    }
    ...
    return frames.sort((left, right) => compareCodeUnits(left.reviewFrame, right.reviewFrame));
};
const currentRenditions = (project, target, diagnostics, context) => {
    const graph = project.graph();
    if (graph.production?.visualDelivery !== "repainted" ||
        (target.kind !== "rendition" &&
            target.kind !== "sequence" &&
    ...
    if (target.kind === "film") {
        ...
        if (generated !== null)
            try {
                filmShots = [
                    ...new Set(currentFilmTimeline(project, generated.inputFingerprint, context).segments.map((segment) => segment.shot)),
                ].sort(compareCodeUnits);
            }
            catch { }
    }
    const shotIds = target.kind === "rendition"
        ? [target.id]
        : target.kind === "sequence"
            ? sequenceShotIds(project, target.id)
            : filmShots;
    const required = new Set(shotIds);
    const missingInventory = context === undefined
        ? shotIds
    ...
    const inventory = context === undefined
        ? verified
...
const appendRenditionDeliveryReviewDiagnostic = (diagnostics, project, film, compileStatus, renditions, context) => {
    const generated = currentGeneratedManifest(project, context);
    if (generated === null ||
        compileStatus.success === false ||
        compileStatus.compiler.inputFingerprint !== generated.inputFingerprint)
        return;
    try {
        const timeline = currentFilmTimeline(project, generated.inputFingerprint, context);
        const shots = [
            ...new Set(timeline.segments.map((segment) => segment.shot)),
        ];
        const byShot = new Map(renditions.map((rendition) => [rendition.shot, rendition]));
        if (shots.some((shot) => byShot.has(shot) === false))
            return;
        assertProductionRenditionTimelineDelivery({
            timeline,
            clips: new Map(shots.map((shot) => {
                const rendition = byShot.get(shot);
                return [shot, project.readRenderFile(rendition.path)];
            })),
        });
    }
    catch (error) {
        diagnostics.push({
            ...
            target: `film:${film}`,
            path: null,
            message: `${error instanceof Error ? error.message : String(error)} Correct the film edit or repaint clips before completing film review; final delivery never falls back to deterministic pixels.`,
        });
    }
};
const currentFrames = (project, target, diagnostics, compileStatus, context) => {
    if (target.kind === "asset")
        return currentAssetFrames(project, target, diagnostics, compileStatus, context);
    if (target.kind !== "shot" &&
        target.kind !== "sequence" &&
        target.kind !== "film")
        return [];
    ...
    if (compileStatus.success === false ||
        compileStatus.compiler.inputFingerprint !== generated.inputFingerprint) {
        const generatedManifestPath = normalizeSlash(path.relative(project.root, project.trackedStatePath("generated-manifest.json")));
        diagnostics.push({
            code: "review-evidence-stale",
            category: "error",
            phase: "review",
            ...
            message: "Generated output is not a clean compile of current design and source. Run the scaffold compile command before using any frame as review evidence.",
        });
        return [];
    }
    const frames = [];
    ...
    if (target.kind === "sequence" || target.kind === "film")
        try {
            timeline = currentFilmTimeline(project, generated.inputFingerprint, context);
        }
        catch (error) {
            diagnostics.push({
                code: "review-evidence-stale",
                ...
                phase: "review",
                target: reviewTargetKey(target),
                path: normalizeSlash(path.relative(project.root, path.join(project.generatedRoot(), "film-timeline.json"))),
                message: `${error instanceof Error ? error.message : String(error)} Recompile before preparing ${target.kind} review.`,
            });
            return [];
        }
    const targetShots = target.kind === "sequence"
        ? new Set(sequenceShotIds(project, target.id))
        : undefined;
    const filmShots = new Set((timeline?.segments ?? [])
        .map((segment) => segment.shot)
        .filter((shot) => targetShots === undefined || targetShots.has(shot)));
    const required = requiredReviewFrames(graph, target, timeline, target.kind === "sequence" ? sequenceShotIds(project, target.id) : []);
    const covered = new Set();
    const inventory = context?.renderInventory ?? collectRenderManifestInventory(project);
    appendInvalidRenderManifestDiagnostics(project, diagnostics, inventory.invalid);
    const legacyEntries = target.kind === "shot"
        ? inventory.legacy.filter((entry) => reviewTargetKey(entry.target) === reviewTargetKey(target))
        : inventory.legacy.filter((entry) => (entry.target.kind === "film" && entry.target.id === target.id) ||
    ...
    for (const entry of legacyEntries)
        diagnostics.push({
            ...
            path: normalizeSlash(path.relative(project.root, entry.path)),
            message: "This legacy v2 render bundle is retained as historical output but is not current review evidence. Recapture required frames through captureFrame; a current v3 bundle supersedes this warning without deleting history.",
        });
    const manifestEntries = target.kind === "shot"
        ? (inventory.byTarget.get(reviewTargetKey(target)) ?? [])
        : inventory.all.filter((entry) => (entry.manifest?.target.kind === "film" &&
            entry.manifest.target.id === target.id) ||
    ...
    for (const entry of manifestEntries) {
        ...
        if (manifest === null) {
            diagnostics.push({
                ...
                message: "This manifest is not at the canonical content-addressed path or lacks the matching MCP render receipt. Recreate it through captureFrame.",
            });
            continue;
        }
        if (manifest.targetFingerprint !==
        ...
        if (manifest.renderSpec.target !== manifest.target.id ||
            ...
            diagnostics.push({
                ...
                target: bundle,
                path: normalizeSlash(path.relative(project.root, manifestPath)),
                message: "This iteration frame does not match the current shot, production FPS, and exact production raster, so it cannot discharge review. Small preview thumbnails remain usable for diagnosis; capture each required frame again without width/height overrides before submitReview.",
            });
            continue;
        }
        for (const frame of manifest.frames) {
            ...
            try {
                ...
                if (digest !== frame.digest)
                    throw new Error("frame bytes changed after renderer ownership verification");
                const png = PNG.sync.read(Buffer.from(bytes));
                const expectedTime = frame.index / manifest.renderSpec.frameFormat.fps;
                if (frame.width !== manifest.renderSpec.frameFormat.width ||
                    frame.height !== manifest.renderSpec.frameFormat.height ||
                    hasVisiblePixelVariance(png) === false ||
                    isProductionFrameTime(frame.time, manifest.renderSpec.frameFormat.fps) === false ||
                    frameClockClose(frame.time, expectedTime) === false)
                    throw new Error("production dimensions, visible pixels, or frame clock do not match");
                const requirements = manifest.target.kind === "shot"
                    ? required.filter((item) => item.shot === manifest.target.id &&
                ...
                for (const requirement of requirements) {
                    ...
                    covered.add(reviewFrameKey(requirement.shot, requirement.frame, frame.index, frame.pass));
                }
            }
            catch (error) {
                diagnostics.push({
                    ...
                    target: `${bundle}:${frame.index}:${frame.pass}`,
                    path: normalizeSlash(path.relative(project.root, file)),
                    message: `${error instanceof Error ? error.message : String(error)}. Recreate this frame through captureFrame.`,
                });
            }
        }
    }
    for (const requirement of required)
        if (covered.has(reviewFrameKey(requirement.shot, requirement.frame, requirement.index, requirement.pass)) === false)
            diagnostics.push({
                code: "review-evidence-missing",
                category: "error",
                phase: "review",
    ...
    return frames.sort((left, right) => compareCodeUnits(left.bundle, right.bundle) ||
        left.frame - right.frame ||
        compareCodeUnits(left.pass, right.pass) ||
        compareCodeUnits(left.reviewFrame, right.reviewFrame));
};
const requiredReviewFrames = (graph, target, timeline, sequenceShots = []) => {
    // currentFrames reaches this helper only after a clean compile, whose design
    // gate requires production metadata.
    const fps = graph.production.frameFormat.fps;
    const segments = new Map(timeline?.segments.map((segment) => [segment.shot, segment]) ?? []);
    const shots = target.kind === "shot"
        ? [[target.id, graph.shots.get(target.id)]]
        : [...segments.keys()]
            .filter((shot) => target.kind !== "sequence" || sequenceShots.includes(shot))
            .map((shot) => [shot, graph.shots.get(shot)]);
    return shots.flatMap(([shotId, shot]) => (shot === undefined
        ? []
        : target.kind === "film" || target.kind === "sequence"
            ...
            }))).flatMap((frame) => frame.passes.map((pass) => ({
        shot: shotId,
        frame: frame.id,
        time: frame.time,
        index: frame.index,
    ...
    const visit = (current, pointer) => {
        ...
        if (Array.isArray(current))
            ...
            for (const key of Object.keys(current).sort(compareCodeUnits))
                visit(current[key], `${pointer}/${key.replace(/~/g, "~0").replace(/\//g, "~1")}`);
    };
    visit(value, "");
    return pointers.map((pointer) => ({ kind: "design", target, pointer }));
};
const sourceSelectors = (project, sourcePath, diagnostics) => {
    try {
        const lines = Buffer.from(normalizeAutoMovieSource(project.readSource(sourcePath)))
            .toString("utf8")
            .split("\n");
        const selectors = lines.flatMap((line, index) => line.trim().length === 0
            ? []
            : [{ kind: "source", path: sourcePath, line: index + 1 }]);
        if (selectors.length > 512)
            diagnostics.push({
                ...
                path: sourcePath,
                message: "Source has more than 512 non-blank lines; prepareReview returns the first 512 selectors. Split the module or review a narrower source target.",
            });
...
const shotSourceSelectors = (project, sourcePath, diagnostics) => sourcePath === undefined
    ? []
...
const criteriaOf = (target) => AUTOMOVIE_REVIEW_CRITERIA[target.kind];
const highRiskCriteria = (target) => {
    switch (target.kind) {
        case "asset":
...
const reviewState = (review) => review.complete
    ? "complete"
    : review.checks.some((check) => check.verdict === "revise")
        ? "revise"
        : "incomplete";
const refused = (project, target, currentFingerprint, diagnostics) => {
...
const appendReviewCorrectionSafety = (diagnostic) => diagnostic.message.includes("Correction feedback does not authorize deleting the artifact.")
    ? diagnostic
    : {
        ...diagnostic,
        message: `${diagnostic.message} Correction feedback does not authorize deleting the artifact.`,
    };
const sameDesignTarget = (left, right) => left.kind === right.kind &&
    ("id" in left
...
const targetPath = (project, target) => {
    ...
    const directory = target.design.kind === "acceptance"
        ? "acceptance"
        : `${target.design.kind}s`;
    const scope = target.design.kind === "model" || target.design.kind === "formation"
        ? "shared"
        : production;
    return `.automovie/design/${scope}/${directory}/${encodeAutoMoviePathSegment(target.design.id)}.json`;
};
const visualReviewTarget = (target) => target.kind === "asset" ||
    target.kind === "shot" ||
    target.kind === "rendition" ||
    target.kind === "sequence" ||
    target.kind === "film";
const readJsonIfPresent = (project, renderRoot, file) => {
    try {
        return JSON.parse(Buffer.from(project.readRenderFile(normalizeSlash(path.relative(renderRoot, file)))).toString("utf8"));
    }
    catch (error) {
        return { invalidJson: String(error) };
    }
};
const appendInvalidRenderManifestDiagnostics = (project, diagnostics, entries) => {
    for (const entry of entries) {
        const relativePath = normalizeSlash(path.relative(project.root, entry.path));
        diagnostics.push({
 ...
 */
const collectRenderManifestInventory = (project) => {
    const invalid = [];
    const legacy = [];
    const all = [];
    ...
    for (const manifestPath of listNamedFiles(renderRoot, "manifest.json")) {
        ...
        const legacyValidation = typia.validateEquals(value);
...
const currentRenderTargetFingerprint = (project, generated, target, context) => {
    if (context === undefined)
    ...
    const fingerprint = productionRenderTargetFingerprint(project, generated, target, context.renderContentInputs);
    context.renderTargetFingerprints.set(key, fingerprint);
    return fingerprint;
};
const currentGeneratedManifest = (project, context) => context?.generatedManifest ?? project.generatedManifest();
const currentGeneratedFile = (project, relativePath, context) => {
...
const currentFilmTimeline = (project, fingerprint, context) => parseAutoMovieFilmTimeline({
    manifest: currentGeneratedManifest(project, context),
    fingerprint,
    read: (file) => currentGeneratedFile(project, file, context),
});
const renderDirectoryIdentity = (rootReal, directory) => {
    const linked = fs.lstatSync(directory);
    if (linked.isSymbolicLink())
        throw new Error(`Render inventory directory "${directory}" is not a physical directory.`);
    if (linked.isDirectory() === false)
        throw new Error(`Render inventory directory "${directory}" is not a directory.`);
    const real = fs.realpathSync(directory);
    if (isInside(rootReal, real) === false)
        throw new Error(`Render inventory directory "${directory}" escapes the render root.`);
    const status = fs.statSync(real, { bigint: true });
    return {
        key: [real, status.dev.toString(), status.ino.toString()].join("\0"),
    };
};
const assertRenderDirectoryIdentity = (rootReal, directory, expected) => {
    const current = renderDirectoryIdentity(rootReal, directory);
    if (current.key !== expected.key)
        throw new Error(`Render inventory directory "${directory}" changed physical identity during review evidence discovery.`);
};
    ...
    const visit = (directory) => {
        ...
        for (const entry of entries) {
            ...
            if (isInside(rootReal, realChild) === false)
                throw new Error(`Render inventory path "${child}" escapes the render root.`);
            if (status.isDirectory())
                visit(child);
            else if (status.isFile() && entry.name === name)
                output.push(child);
            assertRenderDirectoryIdentity(rootReal, directory, identity);
        }
        assertRenderDirectoryIdentity(rootReal, directory, identity);
...
const hasVisiblePixelVariance = (png) => {
    ...
    for (let offset = 4; offset < png.data.length; offset += 4) {
        ...
        if (png.data[offset] * currentAlpha !== first[0] ||
...
//# sourceMappingURL=AutoMovieProductionReviewService.js.map