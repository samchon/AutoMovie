import { Quaternion, formationSlot, mixSeed, productionRuntimeModelId, productionRuntimeSkeletonId, seededValue, } from "@automovie/engine";
import { canonicalAutoMovieJsonBytes, compareCodeUnits, digestAutoMovieBytes, } from "./contentIdentity";
...
export const AUTOMOVIE_FORMATION_MATRIX_BYTES = 16 * Float32Array.BYTES_PER_ELEMENT;
 ...
 */
export const materializeProductionModels = (recipes, externalModels = new Map(), archetypes = AUTOMOVIE_REGISTERED_ARCHETYPES) => new Map([...recipes]
...
export const materializeFormationSlots = (formation) => Array.from({ length: formation.count }, (_, slot) => materializeFormationSlot(formation, slot));
/** Regenerate one exact formation slot in constant memory. */
export const materializeFormationSlot = (formation, slot) => formationSlot(formation, slot);
/** Compiler-owned formation inventory passed to deterministic shot source. */
export const materializeFormationInventory = (formations, surfaces = []) => Object.fromEntries([...formations]
    .sort(([left], [right]) => compareCodeUnits(left, right))
    .map(([id, formation]) => [
    id,
    materializeFormationSlots(groundFormation(formation, surfaces).formation),
]));
/** Compile every formation into bounded chunks rather than anonymous nodes. */
export const materializeCompiledFormationInventory = (formations, recipes, externalModels = new Map(), surfaces = [], archetypes = AUTOMOVIE_REGISTERED_ARCHETYPES) => Object.fromEntries([...formations]
    .sort(([left], [right]) => compareCodeUnits(left, right))
    .map(([id, formation]) => [
    id,
    materializeCompiledFormation(formation, recipes, externalModels, surfaces, archetypes),
]));
/** Compile one formation into independently regenerable chunk metadata. */
export const materializeCompiledFormation = (formation, recipes = new Map(), externalModels = new Map(), surfaces = [], archetypes = AUTOMOVIE_REGISTERED_ARCHETYPES) => {
    ...
    const chunks = Array.from({
        ...
        const count = Math.min(AUTOMOVIE_FORMATION_CHUNK_SIZE, formation.count - start);
    ...
    const summary = grounded.footprint ??
    ...
    const lod = (anonymousLod.length === 0
        ? [
            {
                tier: "near",
                maxDistance: null,
    ...
    const core = {
        ...
        seed: formation.seed,
        ...summary,
        projectionRadius: Math.max(0.01, ...lod.map((item) => recipeProjectionRadius(recipes.get(item.recipe), externalModels.get(item.recipe), archetypes) ??
            recipeProjectionRadius(recipe, externalModels.get(formation.modelRecipe), archetypes) ??
            0.5)),
        chunks,
...
export const materializeInstanceSlot = (instanceSet, world, slot) => {
    if (Number.isSafeInteger(slot) === false ||
        slot < 0 ||
        slot >= instanceSet.count)
        throw new RangeError(`Instance set "${instanceSet.id}" slot ${slot} is outside 0..${instanceSet.count - 1}.`);
    ...
    const scale = stableInterpolate(instanceSet.variation.scale.min, instanceSet.variation.scale.max, scaleSample);
    const paletteIndex = Math.min(instanceSet.variation.palette.length - 1, Math.floor(seededValue(instanceSet.seed, slot, 0x70616c65) *
        instanceSet.variation.palette.length));
    const position = instanceSet.layout.kind === "along-route"
        ? {
            x: point.x,
    ...
    const traits = Object.fromEntries(instanceSet.variation.traits.map((trait, index) => [
        trait.name,
        stableInterpolate(trait.min, trait.max, seededValue(instanceSet.seed, slot, index, 0x74726169)),
    ]));
    const palette = instanceSet.variation.palette[paletteIndex];
    if ([position.x, position.y, position.z, scale, ...Object.values(traits)].some((value) => Number.isFinite(value) === false) ||
        palette === undefined)
        throw new RangeError(`Instance set "${instanceSet.id}" slot ${slot} derived non-finite variation or an empty palette.`);
    return {
        slot,
...
export const materializeInstanceSlots = (instanceSet, world) => Array.from({ length: instanceSet.count }, (_, slot) => materializeInstanceSlot(instanceSet, world, slot));
/** Compile every world instance set into bounded regenerable chunks. */
export const materializeCompiledInstanceSetInventory = (world, recipes, externalModels = new Map(), archetypes = AUTOMOVIE_REGISTERED_ARCHETYPES) => Object.fromEntries([...(world.instanceSets ?? [])]
    .sort((left, right) => compareCodeUnits(left.id, right.id))
    .map((instanceSet) => [
    instanceSet.id,
    materializeCompiledInstanceSet(instanceSet, world, recipes, externalModels, archetypes),
]));
/** Compile one world instance set without expanding its full slot inventory. */
export const materializeCompiledInstanceSet = (instanceSet, world, recipes = new Map(), externalModels = new Map(), archetypes = AUTOMOVIE_REGISTERED_ARCHETYPES) => {
    const chunks = Array.from({
        length: Math.ceil(instanceSet.count / AUTOMOVIE_INSTANCE_CHUNK_SIZE),
    }, (_, index) => {
        const start = index * AUTOMOVIE_INSTANCE_CHUNK_SIZE;
        const count = Math.min(AUTOMOVIE_INSTANCE_CHUNK_SIZE, instanceSet.count - start);
        return {
            index,
            start,
            count,
    ...
    const core = {
        ...
        route: layout.kind === "along-route"
            ? structuredClone(world.routes.find((route) => route.id === layout.route) ?? null)
            : null,
        ...
        ...summary,
        projectionRadius: Math.max(0.01, ...lod.map((item) => recipeProjectionRadius(recipes.get(item.recipe), externalModels.get(item.recipe), archetypes) ??
            recipeProjectionRadius(recipe, externalModels.get(instanceSet.modelRecipe), archetypes) ??
            0.5)),
        chunks,
        lod,
    };
...
export const materializeCompiledShot = (props) => {
    ...
    for (const participant of props.contract.participants) {
        ...
        if (formation === undefined)
            continue;
        const compiled = props.formationRuntime?.[participant.id] ??
            materializeCompiledFormation(formation, props.modelRecipes, undefined, 
            ...
            props.world?.surfaces, props.archetypes);
        ...
        for (const node of source.scene.nodes) {
            if (node.id.startsWith(ordinaryPrefix) === false)
                continue;
    ...
    const modelByRuntimeId = new Map([...props.runtimeModels.values()].map((model) => [model.id, model]));
    const models = [
        ...new Set([
            ...source.scene.nodes.map((node) => node.model),
            ...formations.flatMap((formation) => formation.lod.map((lod) => lod.model)),
            ...Object.values(props.instanceSetRuntime ?? {}).flatMap((instanceSet) => instanceSet.lod.map((lod) => lod.model)),
        ]),
    ]
    ...
    return {
        value: {
            ...
            models,
            formations,
            instanceSets: Object.values(props.instanceSetRuntime ?? {}).sort((left, right) => compareCodeUnits(left.id, right.id)),
        },
        collisions,
    };
};
...
export const materializeCompiledEffects = (props) => {
    if (props.world === undefined)
        return [];
    const recipes = new Map(props.world.effectRecipes.map((recipe) => [recipe.id, recipe]));
    ...
    return [...props.cues]
        ...
        if (zone === undefined || recipe === undefined)
            return [];
        const seedDigest = digestAutoMovieBytes(canonicalAutoMovieJsonBytes({
 ...
 * extra work at all.
 */
const groundFormation = (formation, surfaces) => {
    const flat = { ...formation, ground: [] };
    const footprint = summarizeFormationRange(flat, 0, formation.count);
    const ground = surfaces.filter((surface) => reachesFootprint(surface.polygon, footprint.bounds));
...
const reachesFootprint = (polygon, bounds) => {
    const xs = polygon.map((point) => point.x);
    const zs = polygon.map((point) => point.z);
    return (Math.min(...xs) <= bounds.max.x &&
        Math.max(...xs) >= bounds.min.x &&
        Math.min(...zs) <= bounds.max.z &&
        Math.max(...zs) >= bounds.min.z);
};
const summarizeFormationRange = (formation, start, count) => {
    const min = {
...
const localInstancePoint = (instanceSet, world, slot) => {
    const layout = instanceSet.layout;
    if (layout.kind === "grid") {
    ...
    if (layout.kind === "scatter") {
        const radius = Math.sqrt(seededValue(instanceSet.seed, slot, 0x72616469)) *
            layout.radius;
    ...
    if (route === undefined || route.waypoints.length < 2)
        throw new Error(`Instance set "${instanceSet.id}" references unavailable route "${layout.route}".`);
    ...
    if (Number.isFinite(total) === false || total <= 0)
        throw new RangeError(`Instance set "${instanceSet.id}" route "${layout.route}" must have finite non-zero length.`);
    ...
    const jitter = (seededValue(instanceSet.seed, slot, 0x6a697474) * 2 - 1) *
...
const summarizeInstanceRange = (instanceSet, world, start, count) => {
    const min = {
        x: Number.POSITIVE_INFINITY,
        y: Number.POSITIVE_INFINITY,
        z: Number.POSITIVE_INFINITY,
...
const stableMeanStep = (mean, value, count) => mean * ((count - 1) / count) + value / count;
const stableInterpolate = (from, to, ratio) => from * (1 - ratio) + to * ratio;
/**
 ...
 */
const lodRecipeDigest = (recipes, id) => {
    const recipe = recipes.get(id);
    if (recipe === undefined)
        return digestAutoMovieBytes(canonicalAutoMovieJsonBytes({ id, missing: true }));
    try {
        return digestAutoMovieBytes(canonicalAutoMovieJsonBytes(recipe));
    }
    catch {
        return digestAutoMovieBytes(canonicalAutoMovieJsonBytes({ id, unencodable: true }));
    }
};
const recipeProjectionRadius = (recipe, external, archetypes) => {
    if (external !== undefined)
        return external.measurement.recipe === "box-v1"
            ? Math.hypot(external.measurement.parameters.width, external.measurement.parameters.height, external.measurement.parameters.depth) / 2
            : Math.hypot(Math.max(external.measurement.parameters.shoulderWidth, external.measurement.parameters.hipWidth), external.measurement.parameters.height) / 2;
    if (recipe === undefined)
        return null;
...
const materializeModel = (recipe, external, archetypes) => {
    const generated = materializeGeneratedModel(recipe, archetypes);
    if (external === undefined)
        return generated;
    const shape = external.collision.recipe === "capsule-v1"
        ? {
            type: "capsule",
...
const materializeGeneratedModel = (recipe, archetypes) => {
    const archetype = archetypes.get(recipe.archetype);
    if (archetype === undefined)
        throw new Error(`Model recipe "${recipe.id}" names archetype "${recipe.archetype}", which is not registered with this compiler. Register a builder for it, or name a registered archetype in the tracked model recipe record; the design gate refuses an unregistered archetype before compilation reaches geometry.`);
...
const materialOf = (recipe) => {
    const [name, hex] = Object.entries(recipe.palette).sort(([left], [right]) => compareCodeUnits(left, right))[0];
...
//# sourceMappingURL=materializeProduction.js.map