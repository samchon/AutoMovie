...
export const trimProductionAudioPresentation = (props) => {
    ...
    if (values.some((value) => Number.isSafeInteger(value) === false || value <= 0) ||
    ...
    const segmentDuration = (props.presentationSamples * props.movieTimescale) / props.mediaTimescale;
    if (Number.isSafeInteger(segmentDuration) === false)
        throw new Error("Audio presentation duration does not land on the movie timescale.");
    ...
    const movieHeader = props.file.getBox("mvhd");
...
//# sourceMappingURL=trimProductionAudioPresentation.js.map