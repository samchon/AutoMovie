...
import { canonicalAutoMovieJsonBytes, canonicalizeAutoMovieJson, compareCodeUnits, digestAutoMovieBytes, encodeAutoMoviePathSegment, fingerprintAutoMovieFields, } from "./contentIdentity";
/** Validate and canonicalize one host repaint/model identity. */
export const canonicalAutoMovieRepaintRuntimeIdentity = (identity) => {
    if (identity.protocolVersion !== "automovie.repaint-runtime.v1" ||
        ...
        throw new Error("Repaint runtime identity requires protocol v1 plus non-blank provider, model, version, and a supported execution boundary.");
    return canonicalizeAutoMovieJson(identity);
};
/** Fingerprint a verified deterministic render manifest and frame digests. */
export const productionSourceRenderFingerprint = (props) => fingerprintAutoMovieFields([
    {
        role: "protocol",
        kind: "utf8",
...
export const productionRepaintStructuralControls = (manifest) => [...new Set(manifest.frames.map((frame) => frame.pass))]
    .filter((pass) => pass !== "beauty")
...
export const productionRepaintOutputPath = (props) => {
    const request = digestAutoMovieBytes(canonicalAutoMovieJsonBytes({
...
export const productionRepaintReceiptPath = (outputPath) => path.posix.join("renditions", `${digestAutoMovieBytes(Buffer.from(outputPath, "utf8")).slice(7)}.json`);
/** Tracked pointer selecting one current rendition receipt for a shot. */
export const productionRepaintActiveReceiptPath = (shot) => path.posix.join("renditions", "active", `${encodeAutoMoviePathSegment(shot)}.json`);
//# sourceMappingURL=renditionIdentity.js.map