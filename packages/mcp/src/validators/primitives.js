import { asArray, isRecord, pushViolation, validateObjectArtifact, validateRange, validateVectorArtifact, } from "@automovie/engine";
/**
 ...
 */
export { isRecord, pushViolation, validateArrayArtifact, validateNonEmptyId, validateObjectArtifact, validateRange, validateUniqueBy, validateUniqueIds, validateVectorArtifact, } from "@automovie/engine";
 ...
 */
export const appendValidation = (violations, validation) => {
    if (validation.success === false)
        violations.push(...validation.violations);
};
export const validateNonEmptyText = (text, path, label, violations) => {
    if (typeof text !== "string") {
        pushViolation(violations, "type", path, `${label} must be a string`, text);
        return;
    }
    if (text.trim().length === 0)
        pushViolation(violations, "type", path, `${label} must be non-empty text`, text);
};
export const validateTransformArtifact = (transform, path, label, violations) => {
    if (!validateObjectArtifact(transform, path, label, violations))
        return;
    validateVectorArtifact(transform.translation, `${path}.translation`, `${label} translation`, violations);
    validateQuaternionArtifact(transform.rotation, `${path}.rotation`, `${label} rotation`, violations);
    validateVectorArtifact(transform.scale, `${path}.scale`, `${label} scale`, violations);
    ...
    for (const axis of ["x", "y", "z"]) {
        ...
        if (typeof value === "number" && value <= 0)
            pushViolation(violations, "range", `${path}.scale.${axis}`, `${label} scale component must be > 0, but was ${value}`, value);
    }
};
const validateQuaternionArtifact = (quaternion, path, label, violations) => {
    if (!validateObjectArtifact(quaternion, path, label, violations))
        return;
    for (const axis of ["x", "y", "z", "w"])
        if (!Number.isFinite(quaternion[axis]))
            pushViolation(violations, "range", `${path}.${axis}`, `${label} component must be finite, but was ${quaternion[axis]}`, quaternion[axis]);
    ...
    if (Number.isFinite(length) && Math.abs(length - 1) > UNIT_QUATERNION_EPSILON)
        pushViolation(violations, "range", path, `${label} must be a unit quaternion (length 1), but length was ${length}`, quaternion);
};
export const validateColorArtifact = (color, path, violations) => {
    if (!validateObjectArtifact(color, path, "color", violations))
        return;
    for (const channel of ["r", "g", "b"])
        validateRange(color[channel], `${path}.${channel}`, 0, 1, channel, violations);
    if (color.a !== null)
 ...
 * argument.
 */
export const validateBeatIdCaseCollisions = (beats, path, violations) => {
    const byLower = new Map();
    asArray(beats).forEach((beat, index) => {
        if (!isRecord(beat) || typeof beat.id !== "string")
            return;
        const lower = beat.id.toLowerCase();
        const prior = byLower.get(lower);
        if (prior !== undefined && prior.id !== beat.id)
            pushViolation(violations, "type", `${path}[${index}].id`, `beat id "${beat.id}" collides case-insensitively with "${prior.id}" (${path}[${prior.index}].id); their per-beat slice files would clobber on a case-insensitive filesystem, rename one beat`, beat.id);
        if (prior === undefined)
            byLower.set(lower, { id: beat.id, index });
    });
};
