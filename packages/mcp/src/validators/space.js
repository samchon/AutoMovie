import { pushViolation, validateArrayArtifact, validateObjectArtifact, } from "./primitives";
/**
 ...
 * @author Samchon
 */
export const validateSpaceShape = (space, path, violations) => {
    const before = violations.length;
    if (!validateObjectArtifact(space, path, "space", violations))
        return false;
    requireStringField(space.id, `${path}.id`, "space id", violations);
    if (validateArrayArtifact(space.surfaces, `${path}.surfaces`, "space surfaces", violations))
        space.surfaces.forEach((surface, index) => {
            validateSurfaceShape(surface, `${path}.surfaces[${index}]`, violations);
        });
    if (validateArrayArtifact(space.walkable, `${path}.walkable`, "space walkable ids", violations))
        space.walkable.forEach((id, index) => {
            requireStringField(id, `${path}.walkable[${index}]`, "walkable surface id", violations);
        });
    return violations.length === before;
};
const validateSurfaceShape = (surface, path, violations) => {
    ...
    if (validateArrayArtifact(surface.polygon, `${path}.polygon`, "surface footprint", violations))
        surface.polygon.forEach((point, index) => {
            validateObjectArtifact(point, `${path}.polygon[${index}]`, "footprint point", violations);
        });
    validateObjectArtifact(surface.anchor, `${path}.anchor`, "surface anchor", violations);
    ...
    if (surface.rampTo !== null)
        validateObjectArtifact(surface.rampTo, `${path}.rampTo`, "surface ramp anchor (null when flat)", violations);
};
const requireStringField = (value, path, label, violations) => {
