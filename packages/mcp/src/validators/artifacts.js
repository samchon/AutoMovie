import { AUTO_MOVIE_LIGHT_TYPES, asArray, isAutoMovieLightType, isRecord, pushViolation, toValidation, validateArrayArtifact, validateNonEmptyId, validateObjectArtifact, validateRange, validateShotArtifact as validateShotStructure, validateSpace, validateUniqueIds, } from "@automovie/engine";
import { appendValidation, validateColorArtifact, validateTransformArtifact, } from "./primitives";
 ...
 */
export const validateSceneArtifact = (scene, models) => {
    const violations = [];
    ...
    asArray(models).forEach((model, i) => {
        const path = `$models[${i}]`;
        if (!validateObjectArtifact(model, path, "model", violations))
            return;
        validateNonEmptyId(model.id, `${path}.id`, "model id", violations);
    });
    const modelIds = new Set(asArray(models)
        .filter(isRecord)
        .map((model) => model.id)
        .filter((id) => typeof id === "string"));
    asArray(scene.nodes).forEach((node, i) => {
        const path = `$input.nodes[${i}]`;
        if (!validateObjectArtifact(node, path, "scene node", violations))
            return;
        validateNonEmptyId(node.id, `${path}.id`, "scene node id", violations);
        validateNonEmptyId(node.model, `${path}.model`, "scene node model", violations);
        if (typeof node.model === "string" && !modelIds.has(node.model))
            pushViolation(violations, "type", `${path}.model`, `scene node model "${node.model}" must reference an available model id`, node.model);
        validateTransformArtifact(node.transform, `${path}.transform`, "scene node transform", violations);
    });
    asArray(scene.cameras).forEach((camera, i) => {
        const path = `$input.cameras[${i}]`;
        if (!validateObjectArtifact(camera, path, "camera", violations))
            return;
        validateNonEmptyId(camera.id, `${path}.id`, "camera id", violations);
        validateTransformArtifact(camera.transform, `${path}.transform`, "camera transform", violations);
        validateRange(camera.fovY, `${path}.fovY`, 0, 180, "camera fovY", violations, false);
        validateRange(camera.near, `${path}.near`, 0, Infinity, "camera near", violations, false);
        ...
        if (!Number.isFinite(far) || far <= near)
            pushViolation(violations, "range", `${path}.far`, `camera far must be finite and greater than near (${camera.near}), but was ${camera.far}`, camera.far);
    });
    asArray(scene.lights).forEach((light, i) => {
        const path = `$input.lights[${i}]`;
        if (!validateObjectArtifact(light, path, "light", violations))
            return;
        validateNonEmptyId(light.id, `${path}.id`, "light id", violations);
        validateTransformArtifact(light.transform, `${path}.transform`, "light transform", violations);
        validateColorArtifact(light.color, `${path}.color`, violations);
        validateRange(light.intensity, `${path}.intensity`, 0, Infinity, "light intensity", violations);
        const lightType = light.type;
        const validLightType = isAutoMovieLightType(lightType);
        if (!validLightType)
            pushViolation(violations, "type", `${path}.type`, `light type must be one of ${[...AUTO_MOVIE_LIGHT_TYPES].join(", ")}`, lightType);
        if (validLightType && (lightType === "point" || lightType === "spot"))
            validateRange(light.range, `${path}.range`, 0, Infinity, "light range", violations);
        if (validLightType && lightType === "spot")
            validateRange(light.coneAngle, `${path}.coneAngle`, 0, 90, "spot coneAngle", violations, false);
    });
    ...
    if (space !== null && validateSpaceShape(space, "$input.space", violations))
        appendValidation(violations, remapSpaceViolations(validateSpace({ space: space })));
    ...
    if (fog !== null &&
        ...
        validateRange(fog.density, "$input.fog.density", 0, Infinity, "fog density", violations);
...
const remapSpaceViolations = (validation) => validation.success === false
    ? {
        success: false,
 ...
 */
export const validateShotArtifact = (shot, scene, motions) => {
    const violations = [];
    ...
    const motionIds = (() => {
        if (motions === undefined)
            return null;
        if (!isRecord(motions)) {
            pushViolation(violations, "type", "$motions", "motion registry must be a JSON object", motions);
            return new Set();
        }
        const ids = new Set();
        Object.entries(motions).forEach(([key, motion]) => {
            const path = `$motions.${key}`;
            if (!validateObjectArtifact(motion, path, "motion registry entry", violations))
                return;
            validateNonEmptyId(motion.id, `${path}.id`, "motion id", violations);
            if (typeof motion.id === "string")
                ids.add(motion.id);
        });
        return ids;
    })();
...
export const validateSequenceArtifact = (sequence, shots) => {
    const violations = [];
    ...
    validateRange(sequence.fps, "$input.fps", 0, Infinity, "sequence fps", violations, false);
    validateUniqueIds(shots, "$shots", "shot id", violations);
    asArray(shots).forEach((shot, i) => {
        const path = `$shots[${i}]`;
        if (!validateObjectArtifact(shot, path, "shot", violations))
            return;
        ...
        if (typeof shot.duration !== "number" ||
            ...
            shot.duration <= 0)
            pushViolation(violations, "range", `${path}.duration`, `referenced shot "${String(shot.id)}" duration must be a finite number > 0 seconds, but was ${String(shot.duration)}`, shot.duration);
    });
    const shotsById = new Map(asArray(shots)
        .filter(isRecord)
    ...
    if (validateArrayArtifact(sequence.shots, "$input.shots", "sequence shots", violations) &&
        sequence.shots.length === 0)
        pushViolation(violations, "type", "$input.shots", "a film must contain at least one shot", sequence.shots);
    ...
    entries.forEach((entry, i) => {
        ...
        const shot = typeof entry.shot === "string" ? shotsById.get(entry.shot) : undefined;
        if (shot === undefined)
            pushViolation(violations, "type", `${path}.shot`, `sequence shot "${entry.shot}" must reference an available shot`, entry.shot);
        if (entry.trim === undefined)
            pushViolation(violations, "type", `${path}.trim`, "sequence trim must be null or an object", entry.trim);
        else if (entry.trim !== null)
            validateTrim(entry.trim, shot?.duration ?? Infinity, `${path}.trim`, violations);
        if (entry.transition === undefined) {
            pushViolation(violations, "type", `${path}.transition`, "sequence transition must be null or an object", entry.transition);
        }
        else if (entry.transition !== null) {
            if (i === 0)
                pushViolation(violations, "temporal", `${path}.transition`, "the first sequence entry cannot have an incoming transition", entry.transition);
            validateTransition(entry.transition, `${path}.transition`, violations);
            const current = entryDuration(entry, shot);
            const previousEntry = entries[i - 1];
            const previous = !isRecord(previousEntry) || typeof previousEntry.shot !== "string"
                ? null
                : entryDuration(previousEntry, shotsById.get(previousEntry.shot));
            const transitionDuration = isRecord(entry.transition) &&
            ...
            if (previous !== null &&
                ...
                if (transitionDuration > Math.min(previous, current))
                    pushViolation(violations, "temporal", `${path}.transition.duration`, `transition duration must fit adjacent entries, but was ${transitionDuration}`, transitionDuration);
                else if (previousIncoming + transitionDuration > previous)
                    pushViolation(violations, "temporal", `${path}.transition.duration`, `adjacent transitions (${previousIncoming}s + ${transitionDuration}s) must not overlap inside the previous entry's played span (${previous}s)`, transitionDuration);
                else
...
const validateTrim = (trim, shotDuration, path, violations) => {
    if (!validateObjectArtifact(trim, path, "sequence trim", violations))
        return;
    validateRange(trim.start, `${path}.start`, 0, Infinity, "trim start", violations);
    validateRange(trim.duration, `${path}.duration`, 0, Infinity, "trim duration", violations, false);
    const start = typeof trim.start === "number" ? trim.start : NaN;
    const duration = typeof trim.duration === "number" ? trim.duration : NaN;
    if (Number.isFinite(shotDuration) &&
        Number.isFinite(start) &&
        Number.isFinite(duration) &&
        start + duration > shotDuration)
        pushViolation(violations, "temporal", path, `trim start + duration must fit within shot duration ${shotDuration}`, trim);
};
const validateTransition = (transition, path, violations) => {
    if (!validateObjectArtifact(transition, path, "sequence transition", violations))
        return;
    validateRange(transition.duration, `${path}.duration`, 0, Infinity, "transition duration", violations, false);
};
...
const entryDuration = (entry, shot) => {
    if (shot === undefined)
        return null;
