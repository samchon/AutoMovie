...
export class AutoMovieApplication {
    context;
    guides = new AutoMovieProductionGuideService();
    repaintService;
    constructor(props) {
        this.context = new AutoMovieProductionContext(props?.capture, props?.projectRoot, props?.productionId, props?.archetypes);
     ...
     */
    async captureFrame(props) {
        this.requireGuides("captureFrame");
        if (props.target.productionId !== undefined &&
            ...
            return {
                ...
                frame: null,
                diagnostics: [
                    diagnostic("capture-production-invalid", props.target.id, "captureFrame productionId must be a trimmed non-empty production namespace."),
                ],
            };
        let services;
        try {
            ...
            return {
                ...
                frame: null,
                diagnostics: [
                    diagnostic("capture-production-unregistered", props.target.id, error instanceof Error ? error.message : String(error)),
                ],
            };
        }
        ...
        try {
            ...
            return failure("capture-registry-unavailable", error instanceof Error ? error.message : String(error));
        }
        const registered = props.target.kind === "shot"
            ? registry.shots.some((target) => target.id === props.target.id)
            : registry.assets.some((target) => target.id === props.target.id);
        if (registered === false)
            return failure("capture-target-missing", `Target "${props.target.kind}:${props.target.id}" is absent from compiler registry ${registry.inputFingerprint}. Correct its registration or compile current source before capture.`);
        const preview = props.target.kind === "shot"
        ...
        const manifestPath = path.join(services.project.root, preview.renderBundle, "manifest.json");
        ...
        try {
            ...
            const relativeFrame = path
                .relative(path.dirname(manifestPath), path.join(services.project.root, frame.path))
                .split(path.sep)
                .join("/");
            const targetMatches = props.target.kind === "shot"
            ...
            current =
                manifest !== null &&
                    ...
                    targetMatches &&
                    manifest.frames.some((candidate) => candidate.path === relativeFrame &&
        ...
        if (current === false || manifest === null)
            return failure("capture-receipt-invalid", "Captured pixels do not reopen through their atomic current render receipt. Discard them, correct the capture host, and retry.");
        const target = props.target.kind === "shot"
            ? {
                kind: "shot",
                productionId: services.project.productionId,
                id: props.target.id,
        ...
        return {
            captured: true,
            productionId: services.project.productionId,
            reviewTarget: props.target.kind === "shot"
     ...
     */
    async repaintShot(props) {
        this.requireGuides("repaintShot");
        if (props.productionId.trim().length === 0 ||
            ...
            return {
                ...
                receipt: null,
                diagnostics: [
                    diagnostic("repaint-production-invalid", props.shot, "repaintShot productionId must be a trimmed non-empty production namespace."),
                ],
            };
        let services;
        try {
            ...
            return {
                ...
                receipt: null,
                diagnostics: [
                    diagnostic("repaint-production-unregistered", props.shot, error instanceof Error ? error.message : String(error)),
                ],
            };
        }
        if (services.project.graph().production?.visualDelivery !== "repainted")
            return {
                ...
                receipt: null,
                diagnostics: [
                    diagnostic("repaint-delivery-disabled", props.shot, 'The current production design declares visualDelivery "deterministic". Change that tracked contract to "repainted", recompile current source, then read DIFFUSION_ENHANCE before requesting a rendition.'),
                ],
            };
    ...
    requireGuides(tool, targetGuide) {
        const required = [
            ...AUTOMOVIE_TOOL_GUIDES[tool],
            ...(targetGuide === undefined ? [] : [targetGuide]),
        ];
        const missing = required.filter((guide) => this.context.hasGuide(guide) === false);
        if (missing.length === 0)
            return;
        throw new Error(`${tool} is knowledge-gated: ${required.length - missing.length}/${required.length} required guides have session credit. Recover in order:\n${missing
            .map((guide, index) => `${index + 1}. getGuideDocument({ name: "${guide}" })`)
...
export const AUTOMOVIE_REPAINT_GUIDE = "DIFFUSION_ENHANCE";
...
const reviewGuide = (target) => AUTOMOVIE_REVIEW_GUIDES[target.kind];
...
//# sourceMappingURL=AutoMovieApplication.js.map