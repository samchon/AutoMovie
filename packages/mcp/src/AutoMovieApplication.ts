...
export class AutoMovieApplication {
  ...
  /**
   * Activate or initialize one production namespace after the overall guide is
   * read. The project root is durable shared memory for source and reusable
   * design assets; production design, reviews, generated artifacts and renders
   * remain isolated by the selected production id. Omit `productionId` only for
   * a one-production project. This boundary does not author shots, compile,
   * render or review. A host-fixed root still refuses a different filesystem
   * root.
   */
  public openProject(
    ...
    const activation = this.context.activate(props.root, props.productionId);
    return {
      project: {
  ...
  /**
   * Validate and atomically replace the active production design: film
  ...
  /**
   * Validate and atomically replace the project-shared named world after
  ...
  /**
   * Erase the active production namespace after `PRODUCTION_DESIGN` is read.
   * The operation records the non-empty reason, removes only that production's
   * design, reviews, generated bytes and renders, and preserves project-shared
   * source, model/world/formation design and every sibling production. The
   * deleted handle becomes unusable; call `openProject` with a remaining
   * production id before continuing. It never deletes shared assets or source,
   * never cascades into a sibling namespace, and refuses a blank audit reason.
   * This explicit destructive boundary exists so agents do not approximate a
   * production deletion by individually erasing records and leaving stale
   * review or render bytes behind.
   */
  public eraseProduction(
    props: IAutoMovieEraseProduction.IProps,
  ): IAutoMovieEraseProduction {
    if (props.reason.trim().length === 0)
      throw new Error(
        "eraseProduction requires a non-empty audit reason before changing tracked state.",
      );
    this.context.requireGuide("PRODUCTION_DESIGN", "eraseProduction");
    return this.context
      .require("eraseProduction")
      .project.eraseProduction(props.reason);
  }
