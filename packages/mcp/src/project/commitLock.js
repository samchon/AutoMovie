 ...
 */
const closeCommitLockDescriptor = (descriptor, failure, lockPath) => {
    try {
...
const lockIdentity = (status) => `${status.dev}\0${status.ino}`;
const readCommitLockSnapshot = (lockPath) => {
...
const restoreQuarantinedLock = (quarantine, lockPath) => {
    try {
 ...
 */
const reclaimCommitLock = (lockPath, token) => {
    try {
 ...
 */
export const acquireCommitLock = (lockPath) => {
    ...
    for (;;) {
        try {
            ...
            if (Date.now() > deadline)
                throw new Error(`the project commit lock is held by another session ("${lockPath}"); retry shortly, or if a crashed process left it behind, verify that no AutoMovie commit is running and remove that lock file manually`);
            waitForRelease(2);
        }
    }
};
 ...
 */
export const releaseCommitLock = (lockPath, token, options = {}) => {
    const current = held.get(lockPath);
    let owned = false;
    if (current !== undefined && current.token === token) {
        if (options.retire !== true && --current.depth !== 0)
            return;
        held.delete(lockPath);
        owned = true;
    }
    if (options.retire === true || options.unlink === false)
        return;
    try {
        const observed = readCommitLockSnapshot(lockPath);
        if (observed === null || observed.token !== token)
            return;
        const quarantine = path.join(path.dirname(lockPath), `.automovie-lock-release-${process.pid}-${randomUUID()}`);
        try {
            ...
            if (owned)
                reclaimCommitLock(lockPath, token);
            return;
        }
        try {
...
//# sourceMappingURL=commitLock.js.map