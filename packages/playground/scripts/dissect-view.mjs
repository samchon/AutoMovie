...
const { photo, photoYaw, matched, matchedYaw } = await withBrowserPage(
  ...
  async (page) => {
    ...
    if (photoYaw != null) {
      ...
      if (best) {
        ...
        fs.writeFileSync(
          path.join(outDir, `dissect-${hero}-${view}.png`),
          shot,
        );
      }
    }

    return { photo, photoYaw, matched, matchedYaw };
  },
);
...
if (rp && rm) {
  ...
  rows.sort(
    (a, b) => Math.abs(b.relErr ?? b.absErr) - Math.abs(a.relErr ?? a.absErr),
  );
}
console.log(
  JSON.stringify(
