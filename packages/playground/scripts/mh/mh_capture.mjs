...
const outDir = path.join(
  root,
  ".shots",
  PAGE === "mhfull.html" ? "mh-full" : "mh-render",
  MODEL,
);
const CHROME =
  process.env.CHROME ??
...
await withBrowserPage(
  ...
  async (page) => {
    ...
    await page
      .waitForFunction(() => window.__mhReady === true, { timeout: 20000 })
      .catch(() => {});
    const settle = () =>
      page.evaluate(
        () =>
...
console.log(
  `wrote ${path.relative(root, outDir)} (${views.length} views)` +
