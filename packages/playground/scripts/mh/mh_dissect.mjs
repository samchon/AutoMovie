...
const result = await withBrowserPage(
  ...
  async (page) => {
    ...
    await page
      .goto(`${BASE}/mhhead.html`, { waitUntil: "load" })
      .catch(() => {});
    ...
    const result = await page.evaluate(async () => {
      ...
      const detect = async (src) => {
        ...
        return {
          lm: r.faceLandmarks[0].map((p) => [p.x, p.y]),
          w: im.naturalWidth,
          h: im.naturalHeight,
        };
      };
      return {
        model: await detect("/mh/_cmp/model.png"),
        ref: await detect("/mh/_cmp/ref.png"),
      };
    });
    return result;
  },
...
const ratios = (det) => {
  ...
  const L = det.lm,
    P = (i) => [L[i][0] * det.w, L[i][1] * det.h],
    d = (a, b) => Math.hypot(a[0] - b[0], a[1] - b[1]);
  const faceWidth = d(P(234), P(454)),
    faceHeight = d(P(10), P(152));
  const eyeWidthL = d(P(33), P(133)),
    eyeWidthR = d(P(263), P(362));
  const eyeHeightL = d(P(159), P(145)),
    eyeHeightR = d(P(386), P(374));
  const eyeWidth = (eyeWidthL + eyeWidthR) / 2,
    eyeHeight = (eyeHeightL + eyeHeightR) / 2;
  const irisSpacing = d(P(468), P(473)),
    noseWidth = d(P(129), P(358)),
    mouthWidth = d(P(61), P(291));
  const upperLip = Math.abs(P(0)[1] - P(13)[1]),
    lowerLip = Math.abs(P(14)[1] - P(17)[1]);
  const lipThick = Math.abs(P(0)[1] - P(17)[1]);
  const eyeY = (P(33)[1] + P(263)[1]) / 2,
    noseTipY = P(1)[1],
    mouthY = (P(61)[1] + P(291)[1]) / 2,
    chinY = P(152)[1];
...
const rm = ratios(result.model),
  rp = ratios(result.ref);
