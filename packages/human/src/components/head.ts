...
import { subdivideControlMesh } from "../geometry/subdivideControlMesh";
import {
...
export function buildPortraitHead(
  ...
  anatomy: { cranium?: IPortraitCraniumShape; neck?: IPortraitNeckShape } = {},
) {
  ...
  const collar = appendPortraitCranium(cage, anatomy.cranium);
  const neckCrop = appendPortraitNeck(cage, collar, anatomy.neck);
