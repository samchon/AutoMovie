...
This directory holds what this production authors. Every branch below is production-owned: its facts, its research, its designs, its prose.

The contracts those hosts answer are not here. `@automovie/template` publishes `docs/discovery`, `docs/principles`, and `docs/obligations`, and the graph resolves them from the installed package, so an author cites them by their evidence roots (`discovery/...`, `principles/...`, `obligations/...`) and never edits them. Discovery states the open searches a population must run, principles state per-file review questions, and obligations state the duties a unit owes and the roles a whole layer allocates. Upgrading the package delivers new contracts; nothing in this project has to be copied to follow them.
