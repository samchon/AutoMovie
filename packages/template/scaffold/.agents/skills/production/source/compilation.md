...
A clean source compile is the precondition for evidence, not a substitute for it: `captureProductionTurntable` and `captureProductionFrame` refuse against a stale compile, and the evidence citation reports evidence as missing until the frames exist. Compile, then look, then record the verdict.
