...
Read the complete production declaration in `productionEvidence.mjs`, its `lint.config.mjs` graph consumer, every target and host involved, and the applicable authoring phase document before changing a stage or evidence statement. `@automovie/evidence` owns reusable graph mechanics; the generated project has no second production-evidence declaration.
