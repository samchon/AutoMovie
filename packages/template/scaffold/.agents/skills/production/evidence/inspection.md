...
Use subject inspection after compilation to ask what one authored thing is and what it looks like, on its own, without staging a shot around it. Routes answer different halves of that. The numbers come from ordinary `@automovie/engine` queries over an `IAutoMovieSubjectArtifact`, which is one `IAutoMovieCompiledShotSource` paired with the revision you read it at. The pictures come from `inspectSubject`, the project scripts this document gates.
...
This ground is divided, and no guide on it substitutes for another. This one answers what a compiled subject is, how two compiled artifacts differ, and what one subject looks like from an eye the inspection chose. `VISUAL_CHANGE_REPORT` answers which already-rendered views moved between two revisions, as digests. the review this target owes is where a verdict is produced, and no question answered here produces one.
...
That revision string is your label for the exact bytes you read. the evidence citation mints its own label from the compiled shot file it opened, so the two strings are not comparable; join an inspection to a review through the subject id, never through the revision.
...
- `review-subject-viewpoint-unsupported` means the subject carries no measured extent to frame, which is the honest answer for a formation. Report that range as unsupported, not as observed. Note that the evidence citation returns this same code as a standing warning about something else entirely; the review this target owes says what it means there.
