...
Every active authored population includes the selected language discovery route. Its `language/principles/common.md` targets join the per-unit checklist and its `language/obligations/common.md` targets join the post-draft population account; neither route permits another language pack to remain as residue.
...
| Shot and acceptance exports to their exact screenplay scene or brief shot H3 | Error, without review freshness | Warning, with current review freshness |
| Film sources to screenplay sequence or brief delivery H2 units | Error, without review freshness | Warning, with current review freshness |
...
`obligations/design/models.md#model-review-set` defines the finite review plan, not a completed image observation. Write its views, neutral background, scale, and comparison criteria before rendering. Its account-to-obligation and account-to-population references both remain errors, as do model construction, determinism, and unsupported-fidelity boundaries. A local authored obligation has the same strict account form; neither its entire document nor its account is downgraded because its prose mentions a view or image.
...
The per-unit checklist includes `principles/core/defaults.md` and `language/principles/common.md` alongside the common and layer-specific principles. Population accounts likewise include `obligations/core/defaults.md` and `language/obligations/common.md` only after the complete selected population exists.
...
A production-local principle uses the selected authored-unit hosts, H2 targets, `checklist: true`, no exclusion, and the host layer's stage and review requirement. A production-local obligation uses one dedicated `docs/accounts/<layer>` H2 host that owns exactly one target H2 and selects the complete authored H2 population as a no-exclusion checklist; it does not restore obligation references on each authored unit. An independent target declares the population and relationship its distinct evidence behavior requires. Declare these mechanics in the added typed claim, not in target prose. Create no empty target or family, and never treat the automatic discovery-host claim as the claim that enforces the adopted rule.

Declare a local authored obligation with `createAutoMovieProductionObligationClaim` from `@automovie/evidence` in `lint.config.ts`. Pass `name`, one `document` under the flat `contracts` inventory, one `account` path such as `accounts/models/local-obligations.md`, and the owning `layer`, `stage`, and `populationScope` from the same production declaration. `documentRoot` defaults to `docs`; `docs/contracts` with a bare contract filename is also supported. Create one account H2 for each H2 in that contract document. Different contract documents need different account files, and shared account filenames and `accounts/settings/story-subjects.md` remain reserved.

The helper derives the complete authored file selector from `layer` and `populationScope`; callers provide neither `files` nor `symbol` for an obligation. Film pilot script and screenplay accounts compare their exact selected delivery group, treatments remain flat, and complete production or reset selects all groups. A library pilot compares its whole existing selected branch. A declaration remains disabled in `disabled` and `draft`, activates in `evidence`, and requires current review fingerprints on both references in `review`. Both native account references explicitly remain `error`. Preserve the helper's omitted claim-level severity: canonical admission refuses claim-level overrides, including `off` or `0` that would disable the whole native claim, and refuses a rewritten weaker reference. `inapplicable: true` is an explicit first-pilot audit only; it cannot exempt a complete production or reset from a required role.
