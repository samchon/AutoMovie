...
AutoMovie source is ordinary tracked TypeScript compiled in a deterministic no-I/O sandbox. Types and JSDoc are the primary payload textbook. Let the builder reject invalid structure early; do not defeat it with casts, `any`, ignored diagnostics, or copied generated JSON.
...
Do not place that loader inside a shot or film build function; it performs Node I/O and is deliberately outside the deterministic builder VM.
