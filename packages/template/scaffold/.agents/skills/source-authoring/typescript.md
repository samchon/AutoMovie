...
Browser code, such as a viewer page or `viewer/preview.ts`, runs without Node built-ins. Take its calculations from `@automovie/engine`, `@automovie/render`, `@automovie/viewer` and `@automovie/interface`, whose entry points reach no Node module. `@automovie/production` and the project scripts are Node services for files, currentness and publication: a browser import of either, directly or through a helper module, reaches Node modules the browser does not have, and Vite does not polyfill them. A type-only import is erased and stays safe.
