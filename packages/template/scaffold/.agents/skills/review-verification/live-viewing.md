...
All viewer servers, including the internal capture and inspection servers, exclude reserved output directories before opening file-watch handles. These include `generated`, `renders`, `reports`, `artifacts`, build output, caches, logs, and AutoMovie runtime state. Authored source and `public` assets remain watched. Keep source outside reserved output directories; a Git ignore is not a source-watch policy. Waiting for a write to settle delays source notifications and cannot protect a watcher that already opened a busy output file.

Record review observations at the authored owner. Imported source and assets remain preview inputs; output writes must not restart the source compiler or reset the current camera.

## Compiled output

`npm run viewer` starts a fresh `ttsx` execution of `src/scripts/build.ts`, then repeats it when authored source, design documents, assets, scripts, viewer code, root configuration, or the tracked AutoMovie design and derivation inputs change. All JavaScript and TypeScript helpers live under `src`. Saves are coalesced and builder executions are serial. An edit during a run queues another run; the intermediate result is not served as current. Generated output, renders, capture state, receipts, logs, reports, and caches do not trigger that loop. Restart the server after changing installed dependencies or inputs outside the project root.
