...
## Derived-result failures

Trace an incorrect derived value to its producer and explicit inputs. Recompute it from the current source and verify the same value reaches measurements, preview, and delivery. Do not alter an output or introduce a second state record to make its identity agree.
