...
A project created before the layout migration learned to read the project rather than its registration may be missing the tracked `renders/README.md` the scaffold ships. That project's first mutable command staged the whole render root into `renders/<production>/`, and the ignore rules cover that path, so version control reports the document deleted while the copy that moved is invisible. Restore `renders/README.md` from version control and delete the stray `renders/<production>/README.md`: the document is tracked, so the history holds it. Do not write a replacement from memory, and do not reach for `npm run sync`, which replaces the generated instruction surface and owns no tracked document.
