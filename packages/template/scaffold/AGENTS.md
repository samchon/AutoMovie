...
Read docs/README.md and every applicable shared discovery target, principle, and obligation before drafting. Audit the applicable discovery searches into the production's flat `docs/contracts` surface first. Every subject a later layer stages, animates, voices, or observes has a settings owner first, and in a film that owner settles what docs/obligations/story/subjects.md requires. Film treatments are flat numbered event files with H2 units only and no delivery group. Scripts independently partition those events into grouped H2/H3/H4 delivery units, and screenplays preserve the script filenames, anchors, nesting, and order exactly while both lower layers cite every treatment event they realize. A brief uses anchored H2 delivery, H3 shot, and H4 observation units without inventing dramatic causality. Outside an optional H1 title, settings, research, design, and treatment hosts use only H2; script, screenplay, and brief hosts use only H2/H3/H4. No other heading depth may hide authored work.
