...
The complete production kind, stage declarations, and custom claims live in lint.config.ts. Reusable graph mechanics and populations come from `@automovie/evidence`; there is no second project-local configuration file. Shared targets are published by `@automovie/template` and resolved from the installed package, so they are cited by their evidence roots and never edited here. Named authoring branches under docs hold production hosts, while flat `docs/contracts/*.md` files hold every production-only target. Never put production facts or host-side evidence tags in a shared target.
