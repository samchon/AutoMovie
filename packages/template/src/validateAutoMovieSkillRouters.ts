...
export const validateAutoMovieSkillRouterLinks = (
  ...
  for (const [file, markdown] of routers) {
    ...
    for (const match of markdown.matchAll(/\[[^\]]*\]\(([^)]+)\)/gu)) {
      ...
      if (/^[A-Za-z][A-Za-z0-9+.-]*:/u.test(destination)) continue;
      ...
      const resolved = path.posix.normalize(
        path.posix.join(path.posix.dirname(file), route),
      );
      if (resolved === ".." || resolved.startsWith("../") || path.posix.isAbsolute(route))
        throw new Error(`${file}: SKILL.md route escapes its project root: ${destination}.`);
      ...
      if (target === undefined && !directory)
        throw new Error(`${file}: SKILL.md routes to missing target ${destination}.`);
      if (anchor !== undefined && anchor !== "") {
        if (target === undefined)
          throw new Error(`${file}: SKILL.md anchor targets a directory: ${destination}.`);
        const anchors = new Set(
          [...target.matchAll(/\{#([^{}\s]+)\}/gu)].map((entry) => entry[1]!),
        );
        if (!anchors.has(anchor))
          throw new Error(`${file}: SKILL.md routes to missing anchor ${destination}.`);
      }
    }
  }
};
