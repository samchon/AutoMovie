...
try {
  const { createPreview } = await import("../createPreview");
  ...
  const isForm = (target: EventTarget | null): boolean =>
    ...
    target.closest("input, textarea, select, button, [contenteditable]") !==
      null;
  // Flight uses yaw/pitch mouse look, world-up height and a lens zoom.
  // Mouse look rotates the eye without translating its position.
