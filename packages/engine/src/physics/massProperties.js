 ...
 * @author Samchon
 */
export const primitiveVolume = (shape) => {
    switch (shape.type) {
        case "box":
 ...
 * @author Samchon
 */
export const primitiveCentroid = (shape) => {
    if (shape.type === "cone")
        return { x: 0, y: shape.height / 4, z: 0 };
    return { x: 0, y: 0, z: 0 };
};
/**
 ...
 * @author Samchon
 */
export const deriveCenterOfMass = (model) => {
    let total = 0;
    ...
    for (const part of model.parts) {
        ...
        const volume = primitiveVolume(part.geometry.shape) * scaleVolume(transform);
        if (volume <= 0)
            continue;
        const centroid = applyTransform(transform, primitiveCentroid(part.geometry.shape));
 ...
 * @author Samchon
 */
export const bodyCenterOfMass = (model) => model.body?.centerOfMass ?? deriveCenterOfMass(model);
const IDENTITY = {
...
const scaleVolume = (t) => t.scale.x * t.scale.y * t.scale.z;
/** Apply a full TRS transform to a local point. */
const applyTransform = (t, p) => Vector3.add(t.translation, Quaternion.rotateVector(t.rotation, {
    x: p.x * t.scale.x,
    y: p.y * t.scale.y,
    z: p.z * t.scale.z,
}));
