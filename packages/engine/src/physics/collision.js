 ...
 * @author Samchon
 */
export const segmentSphere = (a, b, c, radius) => {
    if (!Number.isFinite(radius))
        throw new RangeError(`segment sphere radius must be finite, but was ${radius}`);
    if (!(radius > 0))
        throw new RangeError(`segment sphere radius must be > 0, but was ${radius}`);
    ...
    if (A === 0)
        return C <= 0 ? 0 : null; // a == b: point test
    ...
    if (s1 >= 0 && s1 <= 1)
        return s1; // enters within the segment
    if (s1 < 0 && s2 >= 0)
        return 0; // a is inside the sphere
 ...
 * @author Samchon
 */
export const projectileSphereHit = (projectile, sphere, tMax, steps = 120) => {
    ...
    if (!Number.isInteger(steps))
        throw new RangeError(`projectile hit steps must be an integer, but was ${steps}`);
    if (steps < 1)
