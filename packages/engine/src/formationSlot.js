...
export const IDENTITY_FORMATION_SLOT_STATE = {
    present: true,
    offset: { x: 0, y: 0, z: 0 },
    facingOffsetDeg: 0,
};
 ...
 */
export const sampleFormationSlotMotion = (motions, formation, slot, time) => {
    const cues = motions
 ...
 */
export const rotateFormationLocalOffset = (offset, headingDeg) => {
...
export const placeFormationSlot = (props) => {
    const placed = transformFormationPoint(props.position, props.anchor, props.unit, props.baseFacingDeg);
    const offset = rotateFormationLocalOffset(props.member.offset, props.baseFacingDeg + props.unit.facingOffsetDeg);
    return {
        ...
        facingDeg: props.facingDeg +
            props.unit.facingOffsetDeg +
            props.member.facingOffsetDeg,
    };
};
//# sourceMappingURL=formationSlot.js.map