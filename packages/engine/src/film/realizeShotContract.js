...
import { DEFAULT_SUBJECT_HEIGHT, computeModelRestExtentY, computeRestHeight, } from "./cameraMove";
import { intersectsPerspectiveFrustumBox, resolveCameraAt, } from "./cameraProjection";
import { formationMemberExtent, formationSubjectBox, pointSubjectBox, } from "./subjectExtent";
...
export const realizeShotContract = (props) => {
    ...
    for (const node of props.collisions)
        fail(`formation slot "${node}"`, "collides with a coding-agent scene node; ordinary formation slots are compiler-owned");
    const opening = props.contract.opening.map((state) => {
        const predicates = state.predicates.map((predicate) => evaluatePredicate(props, predicate, 0));
        const passed = predicates.every((predicate) => predicate.passed);
        if (passed === false)
            fail(`opening state "${state.id}"`, "must satisfy every typed predicate at time 0");
        return { id: state.id, predicates, passed };
    });
    const closing = props.contract.closing.map((state) => {
        const predicates = state.predicates.map((predicate) => evaluatePredicate(props, predicate, props.contract.durationSeconds));
        const passed = predicates.every((predicate) => predicate.passed);
        if (passed === false)
            fail(`closing state "${state.id}"`, `must satisfy every typed predicate at ${props.contract.durationSeconds}s`);
        return { id: state.id, predicates, passed };
    });
    const samples = new Map(props.compiled.eventSamples.map((sample) => [sample.id, sample]));
    if (samples.size !== props.compiled.eventSamples.length ||
    ...
    const events = props.contract.events.map((event) => {
        ...
        const subjectsResolved = event.subjects.every((subject) => eventSubjectResolves(props, subject));
        const predicates = event.predicates.map((predicate) => evaluatePredicate(props, predicate, time));
        const passed = sample !== undefined &&
            subjectsResolved &&
        ...
        if (passed === false)
            fail(`event "${event.id}"`, `must resolve every declared subject and have one finite source sample inside ${event.window.from}..${event.window.to}s whose typed predicates all pass`);
    ...
    for (const outcome of camera)
        if (outcome.passed === false)
            fail(`camera at ${outcome.time}s`, "must resolve and project every required subject root inside current camera depth and frame bounds");
    const formations = props.contract.participants.flatMap((participant) => {
        ...
        const formation = (props.compiled.formations ?? []).find((candidate) => candidate.id === participant.id);
        const nodes = new Map(props.compiled.scene.nodes.map((node) => [node.id, node]));
        const heroes = formation?.heroes.flatMap((hero) => {
            const node = nodes.get(hero.actor);
            return node === undefined ? [] : [{ hero, node }];
        }) ?? [];
        const compactPassed = design !== undefined &&
            formation !== undefined &&
            ...
            formation.chunks.reduce((sum, chunk) => sum + chunk.count, 0) ===
                design.count &&
            formation.chunks.every((chunk, index) => chunk.index === index &&
                chunk.start ===
                    formation.chunks
                        .slice(0, index)
        ...
        const materializedHeroesPassed = props.production === null ||
            (design !== undefined &&
                formation !== undefined &&
                heroes.length === formation.heroes.length &&
                heroes.every(({ hero, node }) => {
                    return (vectorClose(node.transform.translation, hero.transform.translation) &&
        ...
        if (passed === false)
            fail(`formation "${participant.id}"`, `must materialize one compact ${design?.count ?? 0}-slot runtime with contiguous chunks, designed bounds, and exact promoted hero nodes`);
        return [
            {
                id: participant.id,
    ...
    for (const participant of props.contract.participants)
        if (participant.kind === "actor") {
            const node = props.compiled.scene.nodes.find((candidate) => candidate.id === participant.id);
            const performance = props.compiled.shot.performances.find((candidate) => candidate.node === participant.id);
            if (node === undefined || performance === undefined)
                fail(`actor "${participant.id}"`, "must be a current performed scene node");
        }
    return {
        realization: {
            version: 1,
...
const eventSubjectResolves = (props, subject) => {
...
const evaluatePredicate = (props, predicate, time) => {
    let actual = null;
    try {
        if (predicate.kind === "joint-angle") {
            const node = props.compiled.scene.nodes.find((candidate) => candidate.id === predicate.actor);
            if (node === undefined)
            ...
            if (skeleton === null ||
                ...
                throw new Error(`node "${predicate.actor}" has no bone "${predicate.bone}"`);
    ...
    return {
        ...
        actual,
        passed: actual !== null &&
            scalarPass(actual, predicate.operator, predicate.value, predicate.tolerance),
    };
};
const scalarPass = (actual, operator, expected, tolerance) => operator === "=="
    ? Math.abs(actual - expected) <= tolerance
    : operator === "<="
        ? actual <= expected + tolerance
        : actual >= expected - tolerance;
const resolveSpatial = (props, selector, time) => {
    if (selector.kind === "point")
        return selector.position;
    if (selector.kind === "landmark") {
        const landmark = props.world?.landmarks.find((candidate) => candidate.id === selector.id);
    ...
    const formation = (props.compiled.formations ?? []).find((candidate) => candidate.id === selector.id);
    if (formation === undefined)
        throw new Error(`formation "${selector.id}" has no slots`);
    return transformFormationPoint(formation.centroid, formation.anchor, sampleFormationMotion(props.compiled.formationMotions ?? [], formation.id, time), formation.facingDeg);
};
/**
 ...
 */
const framedSubjectHeight = (props, subject) => {
    const node = props.compiled.scene.nodes.find((candidate) => candidate.id === subject);
    const model = node === undefined
        ? undefined
        : (props.compiled.models ?? []).find((candidate) => candidate.id === node.model);
 ...
 */
const framedSubjectBox = (props, subject, point, time) => {
    const formation = props.formations.has(subject)
        ? (props.compiled.formations ?? []).find((candidate) => candidate.id === subject)
        : undefined;
...
const cameraOutcome = (props, time) => {
    const camera = props.compiled.scene.cameras.find((candidate) => candidate.id === props.compiled.shot.camera);
    ...
    const resolvedCamera = resolveCameraAt(camera.transform, props.compiled.shot.cameraMotion, camera.id, time);
    ...
    for (const subject of props.contract.camera.requiredSubjects)
        try {
            ...
            if (intersectsPerspectiveFrustumBox({
    ...
    return {
        ...
        resolvedSubjects,
        readableSubjects,
        passed: resolvedSubjects === props.contract.camera.requiredSubjects.length &&
            readableSubjects === props.contract.camera.requiredSubjects.length,
    };
};
const actorPoseAt = (props, node, time) => {
    const compiled = props.compiled;
    const performance = compiled.shot.performances.find((candidate) => candidate.node === node.id);
    const skeleton = skeletonOf(props, node);
    if (skeleton === null)
        throw new Error(`node "${node.id}" has no skeleton`);
    ...
    const motion = compiled.motions.find((candidate) => candidate.id === motionId);
    if (motion === undefined)
        throw new Error(`motion "${motionId}" is absent`);
    return sampleMotion(motion, performance === undefined
        ? time
        : Math.max(0, time - performance.startOffset)).pose;
};
const actorTransformAt = (props, actor, time) => {
    const compiled = props.compiled;
    const node = compiled.scene.nodes.find((candidate) => candidate.id === actor);
    if (node === undefined)
        throw new Error(`node "${actor}" is absent`);
    const skeleton = skeletonOf(props, node);
...
const skeletonOf = (props, node) => {
    const model = (props.compiled.models ?? []).find((candidate) => candidate.id === node.model);
...
const vectorClose = (left, right) => Math.abs(left.x - right.x) <= 1e-9 &&
    Math.abs(left.y - right.y) <= 1e-9 &&
    Math.abs(left.z - right.z) <= 1e-9;
const quaternionClose = (left, right) => Math.abs(Math.abs(left.x * right.x + left.y * right.y + left.z * right.z + left.w * right.w) - 1) <= 1e-9;
//# sourceMappingURL=realizeShotContract.js.map