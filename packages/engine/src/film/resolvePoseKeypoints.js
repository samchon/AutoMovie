 ...
 */
export const DEFAULT_KEYPOINT_BONES = [
    "hips",
    "spine",
    "chest",
    "neck",
...
export const resolvePoseKeypoints = (props) => {
    const rigByBone = new Map(resolvePose(props.pose, props.skeleton).map((b) => [
        b.bone,
        b.worldPosition,
    ]));
    const cam = resolveCameraAt(props.camera.transform, props.cameraMotion ?? null, props.camera.id, props.time ?? 0);
    ...
    for (const bone of joints) {
        ...
        const inFrame = depth >= props.camera.near &&
...
const toSceneWorld = (transform, point) => Vector3.add(transform.translation, Quaternion.rotateVector(transform.rotation, {
    x: transform.scale.x * point.x,
    y: transform.scale.y * point.y,
    z: transform.scale.z * point.z,
}));
//# sourceMappingURL=resolvePoseKeypoints.js.map