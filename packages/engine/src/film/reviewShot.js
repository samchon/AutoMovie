 ...
 */
export const reviewShot = (script, review) => {
    ...
    if (!beatById.has(review.beat))
        out.push("type", "$input.beat", `beat "${review.beat}" must be one of the script's beats`, review.beat);
    if (review.verdict === "revise" && review.notes.length === 0)
        out.push("type", "$input.notes", "a revise verdict must carry at least one note, the next round needs something to fix", review.notes);
    if (review.verdict === "pass" && review.notes.length > 0)
        out.push("type", "$input.notes", "a pass verdict must carry no open notes, passing declares the backlog empty", review.notes);
    review.notes.forEach((note, i) => {
        validateNonEmptyId(note.beat, `$input.notes[${i}].beat`, "review note beat id");
        if (note.beat !== review.beat)
            out.push("type", `$input.notes[${i}].beat`, `note filed on "${note.beat}" but this review judges "${review.beat}"`, note.beat);
    });
...
//# sourceMappingURL=reviewShot.js.map