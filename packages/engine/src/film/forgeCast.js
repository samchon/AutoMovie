 ...
 */
export const forgeCast = (script, forge) => {
    ...
    forge.entries.forEach((entry, i) => {
        ...
        if (found === undefined) {
            out.push("type", `${ep}.node`, `entry must name a script cast node, but "${entry.node}" is not in the cast`, entry.node);
            return;
        }
        const { member } = found;
        if (member.modelRef !== null)
            out.push("type", `${ep}.node`, `cast node "${entry.node}" already has modelRef "${member.modelRef}" (an imported asset) and must not be forged`, entry.node);
        if (seen.has(entry.node))
            out.push("type", `${ep}.node`, `cast node "${entry.node}" is forged more than once`, entry.node);
        seen.add(entry.node);
        if (entry.model.id !== entry.node)
            out.push("type", `${ep}.model.id`, `model id must equal the cast node "${entry.node}" (the staged scene joins on it), but was "${entry.model.id}"`, entry.model.id);
        if (entry.model.origin !== "generated")
            out.push("type", `${ep}.model.origin`, `a forged stand-in's origin must be "generated", but was "${entry.model.origin}"`, entry.model.origin);
        if (entry.model.skeleton === null)
            out.push("type", `${ep}.model.skeleton`, "a stand-in performer needs a skeleton: a boneless model cannot be posed", entry.model.skeleton);
    ...
    script.cast.forEach((member, i) => {
        if (member.modelRef === null && !seen.has(member.node))
            out.push("type", "$input.entries", `cast node "${member.node}" (cast[${i}]) has no modelRef and must be forged`, member.node);
    });
    if (out.items.length > 0)
        return { success: false, violations: out.items };
    const models = {};
    for (const entry of forge.entries)
        models[entry.node] = entry.model;
    return { success: true, models };
};
//# sourceMappingURL=forgeCast.js.map