...
import { POSITIONAL_TARGET_SHAPE, positionalTargetFault, } from "../perform/positionalTargetFault";
...
import { DEFAULT_SUBJECT_HEIGHT, compileCameraCoverage, compileCameraMove, computeModelRestExtentY, computeRestHeight, } from "./cameraMove";
...
import { formationMemberExtent, formationSubjectBox, framedBoxOf, pointSubjectBox, unionSubjectBoxes, } from "./subjectExtent";
/**
 ...
 * same facing).
 */
const animatedBaseAt = (base, facing, motion) => (seconds) => Vector3.add(base, Quaternion.rotateVector(facing, sampleMotion(motion, seconds).pose.root?.translation ?? {
    x: 0,
    y: 0,
    z: 0,
}));
const isRecord = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
const isFiniteVector3 = (vector) => [vector.x, vector.y, vector.z].every((coordinate) => Number.isFinite(coordinate));
const isFiniteQuaternion = (rotation) => [rotation.x, rotation.y, rotation.z, rotation.w].every((component) => Number.isFinite(component));
const actionActors = (action) => typeof action.actor === "string"
    ? [action.actor]
...
const orderEvents = (events) => [...events].sort((a, b) => a.time - b.time ||
...
export const performShot = (props) => {
    const { script, staged, performance, synthesize, skeleton, hasActorContext, jointAxes, restFrames, targetAt: resolveLiveTarget, gaits, blocking, previous, } = props;
    ...
    const formationById = new Map((props.formations ?? []).map((formation) => [formation.id, formation]));
     ...
     */
    const frameAspect = props.frameFormat === undefined ||
    ...
    const synthesizeOnce = (action, actor) => {
        const cachedByActor = synthesisCache.get(action);
        if (cachedByActor?.has(actor) === true)
            return cachedByActor.get(actor);
        const motion = synthesize(action, actor, previous);
        const byActor = cachedByActor ?? new Map();
    ...
    script.beats.forEach((beat, index) => {
        ...
        if (existing !== undefined) {
            out.push("type", `$script.beats[${index}].id`, `script beat id "${beat.id}" is duplicated; first declared at $script.beats[${existing.index}].id`, beat.id);
            return;
        }
        beatById.set(beat.id, { beat, index });
    });
    const validateNonEmptyId = (id, path, label) => {
        if (typeof id !== "string") {
    ...
    const validateTargetNodeIds = (target, path, label) => {
        if (!isRecord(target)) {
        ...
        if (target.kind === "bone") {
            ...
            if (typeof target.node === "string" && typeof target.bone === "string") {
                if (!nodeIds.has(target.node))
                    out.push("type", `${path}.node`, `bone target actor "${target.node}" must be a staged scene node`, target.node);
                else if (hasActorContext !== undefined && !hasActorContext(target.node))
                    out.push("type", `${path}.node`, `staged bone target actor "${target.node}" has no actor context`, target.node);
                else {
                    const rig = skeleton(target.node);
                    if (rig === null)
                        out.push("type", `${path}.bone`, `staged bone target actor "${target.node}" has no rig`, target.bone);
                    else if (!rig.bones.some((bone) => bone.bone === target.bone))
                        out.push("type", `${path}.bone`, `bone "${target.bone}" is not carried by rigged staged actor "${target.node}"`, target.bone);
                }
            }
            return true;
        }
        if (target.kind === "group") {
            if (!Array.isArray(target.nodes)) {
                out.push("type", `${path}.nodes`, `${label} group nodes must be an array`, target.nodes);
                return false;
            }
            target.nodes.forEach((node, j) => validateNonEmptyId(node, `${path}.nodes[${j}]`, `${label} group node id`));
            if (target.formations !== undefined) {
                if (!Array.isArray(target.formations)) {
                    out.push("type", `${path}.formations`, `${label} group formations must be an array`, target.formations);
                    return false;
                }
                ...
                target.formations.forEach((formation, j) => {
                    validateNonEmptyId(formation, `${path}.formations[${j}]`, `${label} group formation id`);
                    if (typeof formation === "string" &&
                        formation.trim().length > 0 &&
                        !formationById.has(formation))
                        out.push("type", `${path}.formations[${j}]`, `${label} group formation "${formation}" is not one of this shot's compiled formations (${formationById.size === 0
                            ? "none were supplied"
                            : [...formationById.keys()].join(", ")})`, formation);
                });
            }
            return true;
        }
    ...
    if (foundBeat === undefined)
        out.push("type", "$input.beat", `beat "${performance.beat}" must be one of the script's beats`, performance.beat);
    const beat = foundBeat?.beat;
    if (!Number.isFinite(performance.duration) || !(performance.duration > 0))
        out.push("range", "$input.duration", `shot duration must be a finite number > 0 seconds, but was ${performance.duration}`, performance.duration);
    const actions = performance.revise.final ?? performance.draft;
    const base = performance.revise.final !== null ? "$input.revise.final" : "$input.draft";
    ...
    const nodeRotations = new Map(staged.scene.nodes.map((n) => [n.id, n.transform.rotation]));
    /**
     ...
     */
    const formationPoints = new Map([...formationById].map(([id, formation]) => [
        id,
        transformFormationPoint(formation.centroid, formation.anchor, sampleFormationMotion(formationMotions, id, 0), formation.facingDeg),
    ]));
    const resolvePositionalTarget = (target, path, label, subject, seconds = 0, 
    /**
    ...
    formationsFramed = false) => {
        ...
        if (formationsFramed === false && isRecord(target)) {
            ...
            if (named.length > 0) {
                out.push("type", path, `${subject} must resolve to a point (${POSITIONAL_TARGET_SHAPE}), but its group names formation ${named
                    .map((formation) => `"${String(formation)}"`)
                    .join(", ")}: a formation is a mass a camera frames, not one body with one point to aim at; name the staged nodes to aim at instead`, target);
                return null;
            }
        }
        if (!validateTargetNodeIds(target, path, label))
            return null;
        const point = resolveLiveTarget?.(target, seconds) ??
        ...
        if (point === null ||
            ...
            out.push("type", path, `${subject} must resolve to a point (${POSITIONAL_TARGET_SHAPE}), but ${positionalTargetFault(target)}`, target);
            return null;
        }
        return point;
    };
     ...
     */
    const refuseUnbendableArm = (side, path, performers) => {
        for (const performer of performers) {
            ...
            if (fault === null)
                continue;
            out.push("type", path, `"${performer}" cannot perform an arm verb on its ${side} arm: ${fault.reason}`, side);
        }
    };
    let liveCamera = null;
    const stageActions = [];
    ...
    actions.forEach((action, i) => {
        ...
        if (typeof action.actor === "string")
            validateNonEmptyId(action.actor, `${base}[${i}].actor`, "action actor id");
        else if (Array.isArray(action.actor)) {
            if (action.actor.length === 0)
                out.push("type", `${base}[${i}].actor`, "an action actor list must name at least one staged scene node or camera", action.actor);
            const seen = new Set();
            action.actor.forEach((actor, j) => {
                validateNonEmptyId(actor, `${base}[${i}].actor[${j}]`, "action actor id");
                if (seen.has(actor))
                    out.push("type", `${base}[${i}].actor[${j}]`, `actor "${actor}" is duplicated in this action's actor list`, actor);
                seen.add(actor);
            });
        }
        else {
            out.push("type", `${base}[${i}].actor`, "action actor must be a staged scene node id or an array of ids", action.actor);
        }
        actors.forEach((actor) => {
            const isNode = nodeIds.has(actor);
            const isCamera = cameraIds.has(actor);
            if (!isNode && !isCamera)
                out.push("type", `${base}[${i}].actor`, `actor "${actor}" must be a staged scene node or camera`, actor);
            else if (action.verb !== "frame" && isCamera)
                out.push("type", `${base}[${i}].actor`, `a ${action.verb} action's actor must be a staged scene node, but "${actor}" is a camera`, actor);
        });
        const finiteStart = Number.isFinite(action.start);
        if (!finiteStart || action.start < 0 || action.start > performance.duration)
            out.push("range", `${base}[${i}].start`, `action start must be within [0, ${performance.duration}] (the shot), but was ${action.start}`, action.start);
        const finiteDuration = action.duration === "auto" || Number.isFinite(action.duration);
        if (action.duration !== "auto" &&
            (!finiteDuration || !(action.duration > 0)))
            out.push("range", `${base}[${i}].duration`, `action duration must be a finite number > 0 seconds or "auto", but was ${action.duration}`, action.duration);
        if (action.duration !== "auto" &&
            finiteStart &&
            finiteDuration &&
            action.start + action.duration > performance.duration)
            out.push("range", `${base}[${i}].duration`, `action span [${action.start}, ${action.start + action.duration}] must lie inside the shot [0, ${performance.duration}]`, action.duration);
        ...
        if (action.duration === "auto" &&
            ...
            out.push("range", `${base}[${i}].duration`, `an "auto" duration leaves no span when the action starts at the shot end (${performance.duration}s), start earlier`, action.duration);
        if (action.repeat !== undefined &&
            (!Number.isInteger(action.repeat) || action.repeat < 1))
            out.push("range", `${base}[${i}].repeat`, `action repeat must be a positive integer when present, but was ${action.repeat}`, action.repeat);
        if (action.verb === "frame") {
            const camera = typeof action.actor === "string" ? action.actor : "";
            if (!CAMERA_FRAMINGS.has(action.framing))
                out.push("type", `${base}[${i}].framing`, `camera framing must be one of wide, full, medium, close, but was "${String(action.framing)}"`, action.framing);
            if (!CAMERA_MOVES.has(action.move))
                out.push("type", `${base}[${i}].move`, `camera move must be one of static, follow, orbit, push-in, truck, whip, but was "${String(action.move)}"`, action.move);
            const target = resolvePositionalTarget(action.on, `${base}[${i}].on`, "frame target", "a frame subject", 0, true);
            ...
            if (action.focus !== undefined)
                resolvePositionalTarget(action.focus, `${base}[${i}].focus`, "focus target", "a focus subject", 0, true);
            if (action.focalLength !== undefined &&
                (!Number.isFinite(action.focalLength) || !(action.focalLength > 0)))
                out.push("range", `${base}[${i}].focalLength`, `a focal length must be a finite number > 0 mm, but was ${action.focalLength}`, action.focalLength);
            if (typeof action.actor !== "string")
                out.push("type", `${base}[${i}].actor`, `a frame action must name exactly one staged camera, not an actor list`, action.actor);
            if (typeof action.actor === "string" && !cameraIds.has(camera))
                out.push("type", `${base}[${i}].actor`, `a frame action's actor must be a staged camera, but "${camera}" is not`, camera);
            else if (typeof action.actor === "string" && liveCamera === null)
                liveCamera = camera;
            else if (typeof action.actor === "string" && liveCamera !== camera)
                out.push("type", `${base}[${i}].actor`, `one live camera per shot, "${liveCamera}" already frames it`, camera);
            if (target !== null)
            ...
            if (action.verb === "locomote" && gaits !== undefined) {
                ...
                actors.forEach((actor) => {
                    ...
                    if (available !== undefined && !available.includes(action.gait))
                        out.push("type", `${base}[${i}].gait`, `locomote gait "${action.gait}" is not one of actor "${actor}"'s gaits (${available.length === 0 ? "none supplied" : available.join(", ")})`, action.gait);
                });
            }
            if (action.verb === "locomote") {
                const relative = isRecord(action.to) &&
                    (action.to.kind === "direction" || action.to.kind === "offscreen");
                if (!relative) {
                    const destination = resolvePositionalTarget(action.to, `${base}[${i}].to`, "locomote target", "a locomote destination", action.start);
                    if (destination !== null)
                        for (const actor of actors.filter((candidate) => nodeIds.has(candidate))) {
                            ...
                            if (displacement.verticalOnly)
                                out.push("range", `${base}[${i}].to`, `actor "${actor}" cannot locomote to a vertical-only destination (${Math.abs(destination.y - origin.y)} m height change with ${displacement.groundDistance} m XZ travel); choose a walkable ground point with horizontal travel or use another action verb`, action.to);
                        }
                }
            }
                ...
                if (!Number.isFinite(action.speed) || !(action.speed > 0))
                    out.push("range", `${base}[${i}].speed`, `a launch speed must be a finite number > 0 m/s, but was ${action.speed}`, action.speed);
                if (action.onHit !== undefined &&
                    !(action.onHit.force >= 0 && action.onHit.force <= 1))
                    out.push("range", `${base}[${i}].onHit.force`, `reaction force must be within [0, 1], but was ${action.onHit.force}`, action.onHit.force);
                validateNonEmptyId(action.projectile, `${base}[${i}].projectile`, "launch projectile id");
                const target = resolvePositionalTarget(action.at, `${base}[${i}].at`, "launch target", "a launch target");
                const aimNode = isRecord(action.at) &&
                ...
                if (action.onHit !== undefined &&
                    ...
                    out.push("type", `${base}[${i}].at`, `a launch's onHit recoils the id it strikes, but "${aimNode}" is a camera, and a camera performs nothing but frame; drop onHit to shoot at the lens, or aim at a staged scene node`, action.at);
                const stagedProjectile = nodeIds.has(action.projectile);
                if (!stagedProjectile)
                    out.push("type", `${base}[${i}].projectile`, `a launch's projectile "${action.projectile}" must be a staged scene node`, action.projectile);
                if (actors.includes(action.projectile))
                    out.push("type", `${base}[${i}].projectile`, `a launch's projectile "${action.projectile}" cannot also be a launching actor`, action.projectile);
                if (isRecord(action.at) &&
                    action.at.kind === "node" &&
                    action.at.node === action.projectile)
                    out.push("type", `${base}[${i}].at`, `a launch's projectile "${action.projectile}" cannot target itself`, action.at);
            ...
            else if (action.verb === "react" &&
                !(action.force >= 0 && action.force <= 1)) {
                out.push("range", `${base}[${i}].force`, `reaction force must be within [0, 1], but was ${action.force}`, action.force);
            }
            else if (action.verb === "emote" &&
                !(action.intensity >= 0 && action.intensity <= 1)) {
                out.push("range", `${base}[${i}].intensity`, `emote intensity must be within [0, 1], but was ${action.intensity}`, action.intensity);
            }
            else if (action.verb === "lookAt") {
                resolvePositionalTarget(action.to, `${base}[${i}].to`, "lookAt target", "a lookAt target");
            }
            else if (action.verb === "reach") {
                resolvePositionalTarget(action.to, `${base}[${i}].to`, "reach target", "a reach target");
                refuseUnbendableArm(action.hand, `${base}[${i}].hand`, actors);
            }
            else if (action.verb === "gesture" &&
                ...
                if (action.at !== undefined)
                    resolvePositionalTarget(action.at, `${base}[${i}].at`, `${action.kind} gesture target`, `a ${action.kind} gesture target`);
                else
                    out.push("type", `${base}[${i}].at`, `a ${action.kind} gesture target must resolve to a point (${POSITIONAL_TARGET_SHAPE}), but none was given`, action.at);
            }
                ...
                for (const child of actors)
                    if (child === action.parent)
                        out.push("type", `${base}[${i}].actor`, `an attachTo child "${child}" cannot attach to itself`, child);
                validateNonEmptyId(action.parent, `${base}[${i}].parent`, "attach parent id");
                const parentRig = nodeIds.has(action.parent)
                    ? skeleton(action.parent)
                    : null;
                if (!nodeIds.has(action.parent))
                    out.push("type", `${base}[${i}].parent`, `an attachTo parent "${action.parent}" must be a staged scene node`, action.parent);
                else if (parentRig === null)
                    out.push("type", `${base}[${i}].parent`, `an attachTo parent "${action.parent}" must have a rig to attach a bone of`, action.parent);
                else if (!parentRig.bones.some((b) => b.bone === action.bone))
                    out.push("type", `${base}[${i}].bone`, `bone "${action.bone}" is not on ${action.parent}'s skeleton`, action.bone);
                else
                    attachments.push({ action, index: i });
            }
        }
    });
    const spanOf = (action) => [
    ...
    for (let i = 0; i + 1 < frames.length; ++i) {
        ...
        if (next.start <= move.start + 1e-9) {
            out.push("range", `${base}[${frames[i + 1].index}].start`, `frame moves share the same start time ${next.start}s on the live camera; choose one framing for that instant`, next.start);
            continue;
        }
        if (move.duration === "auto")
            continue;
        const end = Math.min(move.start + move.duration, performance.duration);
        if (end > next.start + 1e-9)
            out.push("range", `${base}[${frames[i + 1].index}].start`, `frame moves overlap, the previous move runs until ${end}s, but this one starts at ${frames[i + 1].action.start}s`, frames[i + 1].action.start);
    }
    ...
    for (const [actor, list] of actorActions) {
        ...
        if (out.items.length > 0)
            continue;
        const layered = new Set(list.map(({ action }) => actionRegion(action))).size > 1;
        const carried = new Map();
        const contentOf = (action) => {
            ...
            if (motion !== null)
                for (const keyframe of motion.keyframes) {
                    for (const joint of keyframe.pose.joints)
                        if (allowed.has(joint.bone))
                            content.bones.add(joint.bone);
                    if (keyframe.pose.root !== null &&
        ...
        const sorted = [...list].sort((a, b) => spanOf(a.action)[0] - spanOf(b.action)[0]);
        for (let i = 0; i + 1 < sorted.length; ++i) {
            ...
            for (let j = i + 1; j < sorted.length; ++j) {
                ...
                if (shared.length > 0 && b1 > a0 + 1e-9)
                    out.push("range", `${base}[${b.index}].start`, `${actor} has overlapping ${aRegion} and ${bRegion} actions that both author ${shared.join(", ")} after region masking; move one action, shorten it, or author disjoint content`, b.action.start);
            }
        }
    }
    ...
    if (blocking !== undefined) {
        if (blocking.beat !== performance.beat)
            out.push("type", "$input.beat", `the performance realizes beat "${performance.beat}" but the blocking plans "${blocking.beat}"`, performance.beat);
        if (Math.abs(blocking.duration - performance.duration) > 1e-6)
            out.push("range", "$input.duration", `the blocking fixed this beat at ${blocking.duration}s, but the performance runs ${performance.duration}s`, performance.duration);
        for (const intent of blocking.actors)
            for (const anchor of intent.anchors ?? []) {
                const covered = stageActions.some((action) => {
                    if (!actionActors(action).includes(intent.node))
                        return false;
                    const [from, to] = spanOf(action);
                    return anchor.t >= from - 1e-9 && anchor.t <= to + 1e-9;
                });
                if (!covered)
                    out.push("range", base, `anchor "${anchor.cue}" pins ${intent.node} at t=${anchor.t}s, but no action of that actor covers the instant`, anchor.t);
            }
        const lead = frames[0];
        if (lead === undefined) {
            if (blocking.camera.move !== "static")
                out.push("type", base, `the blocking asks for a "${blocking.camera.move}" camera, but no frame action authors it`, blocking.camera.move);
        }
        else {
            if (lead.action.framing !== blocking.camera.framing)
                out.push("type", `${base}[${lead.index}].framing`, `the blocking frames this beat "${blocking.camera.framing}", but the performance frames "${lead.action.framing}"`, lead.action.framing);
            if (lead.action.move !== blocking.camera.move)
                out.push("type", `${base}[${lead.index}].move`, `the blocking moves the camera "${blocking.camera.move}", but the performance moves "${lead.action.move}"`, lead.action.move);
        }
    }
    if (liveCamera === null) {
        const first = staged.scene.cameras[0];
        if (first === undefined)
            out.push("type", "$input", "the staged scene has no camera, so the shot cannot be framed", performance.beat);
        else {
            ...
            validateNonEmptyId(first.id, "$staged.scene.cameras[0].id", "the fallback camera id");
            liveCamera = first.id;
        }
    }
    ...
    (blocking?.coverage ?? []).forEach((intent, i) => {
        ...
        if (camera === undefined)
            out.push("type", `${path}.camera`, `a coverage camera must be a staged camera, but "${intent.camera}" is not`, intent.camera);
        else if (intent.camera === liveCamera)
            out.push("type", `${path}.camera`, `coverage plays ANOTHER angle of the beat, but "${intent.camera}" is already this shot's live camera`, intent.camera);
        const first = coveredCameras.get(intent.camera);
        if (first !== undefined)
            out.push("type", `${path}.camera`, `coverage camera id "${intent.camera}" is duplicated; first declared at $blocking.coverage[${first}].camera`, intent.camera);
        else
            coveredCameras.set(intent.camera, i);
        const framed = CAMERA_FRAMINGS.has(intent.framing);
        if (!framed)
            out.push("type", `${path}.framing`, `camera framing must be one of wide, full, medium, close, but was "${String(intent.framing)}"`, intent.framing);
        const moved = CAMERA_MOVES.has(intent.move);
        if (!moved)
            out.push("type", `${path}.move`, `camera move must be one of static, follow, orbit, push-in, truck, whip, but was "${String(intent.move)}"`, intent.move);
        const subject = resolvePositionalTarget(intent.on, `${path}.on`, "coverage target", "a coverage subject", 0, true);
        if (camera !== undefined &&
    ...
    const validateFramingCamera = (camera, path, 
    /**
    ...
    readsRotation) => {
        if (!(camera.fovY > 0 && camera.fovY < 180))
            out.push("range", `${path}.fovY`, `a camera that frames must have a vertical field of view within (0, 180)°, but "${camera.id}" has ${camera.fovY}`, camera.fovY);
        if (!isFiniteVector3(camera.transform.translation))
            out.push("range", `${path}.transform.translation`, `a camera that frames must be placed at a finite point, but "${camera.id}" is not`, camera.transform.translation);
        if (readsRotation && !isFiniteQuaternion(camera.transform.rotation))
            out.push("range", `${path}.transform.rotation`, `a "whip" pans from the camera's staged orientation, so "${camera.id}" must carry a finite rotation`, camera.transform.rotation);
    };
    ...
    for (const job of launches) {
        ...
        let targetAt;
        const targetsNode = (action) => job.targetNode !== null && actionActors(action).includes(job.targetNode);
        if (job.action.at.kind === "bone" && resolveLiveTarget !== undefined)
            targetAt = (time) => resolveLiveTarget(job.action.at, time) ?? job.target;
        else if (job.targetNode !== null &&
            stageActions.some((action) => action.verb === "locomote" && targetsNode(action)))
            targetAt = animatedBaseAt(nodePositions.get(job.targetNode), nodeRotations.get(job.targetNode), 
            ...
            compilePerformance(stageActions.filter((action) => targetsNode(action)), synthesizeOnce).performances[job.targetNode]);
        ...
        if (result === null) {
            out.push("range", `${base}[${job.index}].speed`, `the launch cannot reach its target at ${job.action.speed} m/s, raise the speed or move the shooter closer`, job.action.speed);
            continue;
        }
        ...
        if (hitAt > performance.duration + 1e-9) {
            out.push("range", `${base}[${job.index}].speed`, `the launch lands at ${hitAt}s, outside the shot ending at ${performance.duration}s, fire earlier, raise the speed, or lengthen the shot`, job.action.speed);
            continue;
        }
        ...
        objectMotions.push({
            ...result.clip,
            id: flightCount === 1 ? result.clip.id : `${result.clip.id}:${flightCount}`,
    ...
    const plants = [];
    const previousByNode = new Map((previous?.actors ?? []).map((actor) => [actor.node, actor]));
    ...
    for (const [actor, motion] of Object.entries(motions).sort(([x], [y]) => compareCodeUnits(x, y))) {
        ...
        if (rig === null || node === undefined)
            continue;
        const inverse = Quaternion.inverse(node.transform.rotation);
        const toModelPoint = (point) => Quaternion.rotateVector(inverse, Vector3.subtract(point, node.transform.translation));
        const toWorldPoint = (point) => Vector3.add(node.transform.translation, Quaternion.rotateVector(node.transform.rotation, point));
        const modelGround = worldGround === null
            ? null
            : (x, z) => {
    ...
    for (const drop of compiled.masked) {
        ...
        out.push("type", path, `${drop.actor}'s clip authors ${lost}, which the "${drop.region}" body region does not carry, so the performance would drop that content; set region to one that owns it ("fullBody" owns every bone and the root, only "face" carries an expression), or move the content to its own action`, drop.region);
    }
     ...
     */
    const groupSubjectBox = (on, seconds) => unionSubjectBoxes([
    ...
    const framedSubject = (on) => {
        ...
        if (on.kind === "group") {
            ...
            if (box !== null) {
                ...
                const cued = (on.formations ?? []).some((id) => formationMotions.some((cue) => cue.formation === id));
                return {
                    base: framed.base,
        ...
        const point = resolveLiveTarget?.(on, 0) ??
            resolveTargetPoint(on, nodePositions, formationPoints);
        ...
        const vertical = node === null
            ? { min: 0, max: DEFAULT_SUBJECT_HEIGHT }
            : nodeVerticalExtent(node);
        const floor = vertical.min;
        const measured = vertical.max - vertical.min;
        const onFloor = (value) => floor === 0 ? value : { x: value.x, y: value.y + floor, z: value.z };
        const motion = node === null ? undefined : motions[node];
        const facing = node === null ? undefined : nodeRotations.get(node);
        const objectAt = node === null
            ? null
            : (seconds) => bakedTransformFromClipsAt(objectMotions, node, seconds)
                ?.translation ?? null;
        const hasObjectMotion = node !== null &&
            objectMotions.some((clip) => clip.tracks.some((track) => track.channel.kind === "node" &&
                track.channel.node === node &&
                track.channel.path === "translation"));
        const animated = on.kind === "bone" && resolveLiveTarget !== undefined
    ...
    const entries = frames.map(({ action }) => ({
        action,
    ...
    const coverage = coverageJobs.map(({ intent, camera }) => compileCameraCoverage({
        camera,
        ...
        cameraIntent: frames.map(({ action }) => ({
            ...
            focus: action.focus === undefined
                ...
                : (resolveLiveTarget?.(action.focus, 0) ??
                    resolveTargetPoint(action.focus, nodePositions, formationPoints)),
    ...
    const assembled = validateShotArtifact(shot, staged.scene, new Set(Object.values(motions).map((motion) => motion.id)));
    if (assembled.success === false)
        throw new Error(`performShot produced a shot that violates the shot artifact contract, which is an engine defect rather than an authoring fault: ${assembled.violations
            .slice(0, 5)
            .map((item) => `${item.kind} at ${item.path}: ${item.expected}`)
            .join("; ")}`);
    return { success: true, shot, motions, plants };
};
//# sourceMappingURL=performShot.js.map