 ...
 */
export const forgeProp = (spec) => {
    const out = new ViolationCollector();
    if (spec.node.trim().length === 0)
        out.push("type", "$input.node", "prop node must be a non-empty scene node id", spec.node);
    if (spec.model.id !== spec.node)
        out.push("type", "$input.model.id", `model id must equal the prop node "${spec.node}" (the staged scene joins on it), but was "${spec.model.id}"`, spec.model.id);
    if (spec.model.origin !== "generated")
        out.push("type", "$input.model.origin", `a forged prop's origin must be "generated", but was "${spec.model.origin}"`, spec.model.origin);
    if (spec.model.skeleton !== null)
        out.push("type", "$input.model.skeleton", "a prop must be skeleton-less, riggable actors go through forgeCast; moving parts are articulation nodes", spec.model.skeleton.id);
    ...
    if (spec.articulation !== null)
        gateArticulation(spec.articulation, out);
    if (out.items.length > 0)
        return { success: false, violations: out.items };
    return { success: true, prop: spec };
};
const gateArticulation = (articulation, out) => {
    const path = "$input.articulation";
    if (articulation.nodes.length === 0)
        out.push("type", `${path}.nodes`, "an articulation needs at least one joint node", articulation.nodes);
    const byId = new Map();
    articulation.nodes.forEach((node, i) => {
        const np = `${path}.nodes[${i}]`;
        if (node.id.trim().length === 0)
            out.push("type", `${np}.id`, "articulation node id must be non-empty", node.id);
        const existing = byId.get(node.id);
        if (existing !== undefined) {
            out.push("type", `${np}.id`, `articulation node id "${node.id}" is duplicated; first declared at ${path}.nodes[${existing.index}].id`, node.id);
            return;
        }
    ...
    articulation.nodes.forEach((node, i) => {
        if (node.parent !== null && !byId.has(node.parent))
            out.push("type", `${path}.nodes[${i}].parent`, `parent "${node.parent}" is not a declared articulation node`, node.parent);
    });
    for (const [id, entry] of byId) {
        ...
        while (parent !== null) {
            if (trail.has(parent)) {
                out.push("type", `${path}.nodes[${entry.index}].parent`, `articulation parent chain of "${id}" is cyclic at "${parent}"`, parent);
                break;
            }
            trail.add(parent);
    ...
    if (binding.profile !== profile.id)
        out.push("type", `${path}.binding.profile`, `binding targets profile "${binding.profile}" but the declared profile is "${profile.id}"`, binding.profile);
    for (const [key, mapped] of Object.entries(binding.boneMap)) {
        ...
        if (!byId.has(mapped))
            out.push("type", bp, `boneMap maps "${key}" to "${mapped}", which is not a declared articulation node`, mapped);
    }
    for (const key of profileSemanticKeys(profile))
        if (binding.boneMap[key] === undefined)
            out.push("type", `${path}.binding.boneMap`, `profile "${profile.id}" references "${key}" but the binding does not map it`, key);
};
//# sourceMappingURL=forgeProp.js.map