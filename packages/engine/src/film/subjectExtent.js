 ...
 */
export const formationMemberExtent = (formation, models) => {
...
export const formationSubjectBox = (props) => {
    const moved = transformFormationBounds(props.formation.bounds, props.formation.anchor, sampleFormationMotion(props.motions, props.formation.id, Math.max(0, props.seconds)), props.formation.facingDeg);
 ...
 */
export const pointSubjectBox = (point, extent) => ({
...
export const unionSubjectBoxes = (boxes) => boxes.length === 0
    ? null
    : {
        min: {
...
//# sourceMappingURL=subjectExtent.js.map