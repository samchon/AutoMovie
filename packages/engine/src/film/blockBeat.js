 ...
 */
export const blockBeat = (script, staged, blocking, previous) => {
    const out = new ViolationCollector();
    const beatIds = new Map();
    script.beats.forEach((beat, index) => {
        const existing = beatIds.get(beat.id);
        if (existing !== undefined) {
            out.push("type", `$script.beats[${index}].id`, `script beat id "${beat.id}" is duplicated; first declared at $script.beats[${existing}].id`, beat.id);
            return;
        }
    ...
    if (!beatIds.has(blocking.beat))
        out.push("type", "$input.beat", `beat "${blocking.beat}" must be one of the script's beats`, blocking.beat);
    if (!Number.isFinite(blocking.duration) || !(blocking.duration > 0))
        out.push("range", "$input.duration", `beat duration must be a finite number > 0 seconds, but was ${blocking.duration}`, blocking.duration);
    const nodeIds = new Set(staged.scene.nodes.map((n) => n.id));
    blocking.actors.forEach((intent, i) => {
        validateNonEmptyId(intent.node, `$input.actors[${i}].node`, "actor node id");
        if (!nodeIds.has(intent.node))
            out.push("type", `$input.actors[${i}].node`, `intent must belong to a staged actor, but "${intent.node}" is not placed`, intent.node);
        ...
        (intent.anchors ?? []).forEach((anchor, j) => {
            ...
            if (!finiteAnchorTime || anchor.t < 0 || anchor.t > blocking.duration)
                out.push("range", `$input.actors[${i}].anchors[${j}].t`, `anchor "${anchor.cue}" must land within [0, ${blocking.duration}] (the beat), but was ${anchor.t}`, anchor.t);
            if (finiteAnchorTime && anchor.t < previous)
                out.push("range", `$input.actors[${i}].anchors[${j}].t`, `anchor "${anchor.cue}" (t=${anchor.t}) runs before its predecessor (t=${previous}): the list order is the causal order`, anchor.t);
            if (finiteAnchorTime)
                previous = anchor.t;
        });
    });
    ...
    if (blocking.camera.on.kind === "node" &&
        ...
        out.push("type", "$input.camera.on.node", `the camera must favour something staged (an actor, a set piece, or another camera), but "${blocking.camera.on.node}" is not placed`, blocking.camera.on.node);
    if (blocking.camera.on.kind === "node")
        validateNonEmptyId(blocking.camera.on.node, "$input.camera.on.node", "camera target node id");
    ...
    (blocking.coverage ?? []).forEach((intent, i) => {
        validateNonEmptyId(intent.camera, `$input.coverage[${i}].camera`, "coverage camera id");
        if (!cameraIds.has(intent.camera))
            out.push("type", `$input.coverage[${i}].camera`, `a coverage camera must be a staged camera, but "${intent.camera}" is not`, intent.camera);
        const first = covered.get(intent.camera);
        if (first !== undefined)
            out.push("type", `$input.coverage[${i}].camera`, `coverage camera id "${intent.camera}" is duplicated; first declared at $input.coverage[${first}].camera`, intent.camera);
        else
            covered.set(intent.camera, i);
        if (!CAMERA_FRAMINGS.has(intent.framing))
            out.push("type", `$input.coverage[${i}].framing`, `camera framing must be one of wide, full, medium, close, but was "${String(intent.framing)}"`, intent.framing);
        if (!CAMERA_MOVES.has(intent.move))
            out.push("type", `$input.coverage[${i}].move`, `camera move must be one of static, follow, orbit, push-in, truck, whip, but was "${String(intent.move)}"`, intent.move);
        if (intent.on.kind === "node" && !placedIds.has(intent.on.node))
            out.push("type", `$input.coverage[${i}].on.node`, `a coverage camera must favour something staged (an actor, a set piece, or another camera), but "${intent.on.node}" is not placed`, intent.on.node);
        if (intent.on.kind === "node")
            validateNonEmptyId(intent.on.node, `$input.coverage[${i}].on.node`, "coverage target node id");
    });
    if (previous !== undefined) {
        const carried = new Map();
        previous.actors.forEach((actor, i) => {
            validateNonEmptyId(actor.node, `$previous.actors[${i}].node`, "previous beat-end actor node id");
            const first = carried.get(actor.node);
            if (first !== undefined)
                out.push("type", `$previous.actors[${i}].node`, `previous beat-end actor "${actor.node}" is duplicated; first declared at $previous.actors[${first}].node`, actor.node);
            else
                carried.set(actor.node, i);
            if (!nodeIds.has(actor.node))
                out.push("type", `$previous.actors[${i}].node`, `previous beat-end actor "${actor.node}" is not a staged scene node: a carried state must resume placed actors`, actor.node);
        });
    }
...
//# sourceMappingURL=blockBeat.js.map