 ...
 */
const wrapTime = (seconds, duration) => seconds % duration;
const toMatrix = (transform) => Matrix4.compose(transform.translation, transform.rotation, transform.scale);
/** Fold a sampled pose root into a staged base placement, in world space. */
export const foldRoot = (base, root) => {
    if (root === null)
        return base;
 ...
 */
const velocityOver = (node, clip, t0, t1) => {
    const span = t1 - t0;
    if (span <= 0)
        return { x: 0, y: 0, z: 0 };
 ...
 */
export const gaitPhaseOf = (clip, localTime) => {
    ...
    if (cycle !== null) {
        if (!Number.isFinite(cycle.period) || cycle.period <= 0)
            return null;
        if (!Number.isFinite(cycle.phaseAt))
            return null;
 ...
 * velocity.
 */
export const rootVelocityOf = (node, clip, localTime) => {
    if (clip.loop && clip.duration > 0) {
        ...
        if (phase > 0)
            return velocityOver(node, clip, 0, phase);
        const clamped = { ...clip, loop: false };
 ...
 */
export const plantsAtEnd = (plants, localTime) => {
    if (plants === undefined)
        return null;
    const byFoot = new Map();
    for (const plant of plants) {
        if (plant.start > localTime || plant.end < localTime)
            continue;
...
//# sourceMappingURL=beatEndSim.js.map