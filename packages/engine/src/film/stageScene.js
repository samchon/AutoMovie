...
import { AUTO_MOVIE_LIGHT_TYPES, isAutoMovieLightType, } from "../resolve/lightChannel";
 ...
 */
const validateLightPlacementShape = (light, path, out) => {
    const type = lightTypeOf(light);
    if (type === null) {
        out.push("type", `${path}.type`, `light type must be one of ${[...AUTO_MOVIE_LIGHT_TYPES].join(", ")}`, light.type);
        return;
    }
    ...
    if (light.direction === undefined) {
        if (aimed)
            out.push("type", `${path}.direction`, `a ${type} light is aimed and needs a direction`, light.direction);
    }
    else if (!aimed)
        out.push("type", `${path}.direction`, `a point light radiates in every direction and takes no direction`, light.direction);
    else if (!isFiniteVector3(light.direction) ||
        Vector3.length(light.direction) === 0)
        out.push("range", `${path}.direction`, `direction must be a finite non-zero vector`, light.direction);
    if (light.position === undefined) {
        if (positioned)
            out.push("type", `${path}.position`, `a ${type} light falls off with distance and needs a position`, light.position);
    }
    else if (!positioned)
        out.push("type", `${path}.position`, `a directional light is infinitely distant and takes no position`, light.position);
    else if (!isFiniteVector3(light.position))
        out.push("range", `${path}.position`, `position must be a finite vector`, light.position);
    if (light.range !== undefined) {
        if (!positioned)
            out.push("type", `${path}.range`, `a directional light has no distance falloff and takes no range`, light.range);
        else if (!Number.isFinite(light.range) || light.range < 0)
            out.push("range", `${path}.range`, `light range must be a finite number >= 0 (0 = infinite), but was ${light.range}`, light.range);
    }
    if (light.coneAngle !== undefined) {
        if (type !== "spot")
            out.push("type", `${path}.coneAngle`, `only a spot light has a cone; a ${type} light takes no coneAngle`, light.coneAngle);
        else if (!Number.isFinite(light.coneAngle) ||
            light.coneAngle <= 0 ||
            light.coneAngle > 90)
            out.push("range", `${path}.coneAngle`, `spot coneAngle must be a finite number within (0, 90], but was ${light.coneAngle}`, light.coneAngle);
    }
    if (light.color !== undefined)
        validateLightColor(light.color, path, out);
};
/**
 ...
 * elsewhere.
 */
const validateLightColor = (color, path, out) => {
    if (!isRecord(color)) {
        out.push("type", `${path}.color`, "light color must be a JSON object", color);
        return;
    }
    for (const key of ["r", "g", "b"])
        unitComponent(color[key], `${path}.color.${key}`, `light color ${key}`, out);
 ...
 * two rungs later.
 */
const validateFogPlacement = (fog, path, out) => {
    if (!isRecord(fog)) {
    ...
    if (typeof fog.density !== "number" ||
        ...
        fog.density < 0)
        out.push("range", `${path}.density`, `fog density must be a finite number >= 0, but was ${String(fog.density)}`, fog.density);
    if (!isRecord(fog.color)) {
        out.push("type", `${path}.color`, "fog color must be a JSON object", fog.color);
        return;
    }
    for (const key of ["r", "g", "b"])
        unitComponent(fog.color[key], `${path}.color.${key}`, `fog color ${key}`, out);
};
/**
 ...
 */
const unitComponent = (value, path, label, out) => {
    if (typeof value === "number" &&
        Number.isFinite(value) &&
        value >= 0 &&
        value <= 1)
        return;
    out.push("range", path, `${label} must be a finite number within [0, 1], but was ${String(value)}`, value);
};
/**
 ...
 */
const lowerLightPlacement = (light) => {
    const type = lightTypeOf(light);
    const base = {
        id: light.node,
        transform: {
            translation: light.position ?? { x: 0, y: 0, z: 0 },
            rotation: light.direction === undefined
                ? IDENTITY_ROTATION
    ...
    if (type === "point")
        return { ...base, type, range: light.range ?? 0 };
    if (type === "spot")
        return {
            ...base,
            type,
 ...
 */
export const stageScene = (script, staging) => {
    const out = new ViolationCollector();
    const cast = new Map();
    script.cast.forEach((member, index) => {
        const existing = cast.get(member.node);
        if (existing !== undefined) {
            out.push("type", `$script.cast[${index}].node`, `script cast node "${member.node}" is duplicated; first declared at $script.cast[${existing.index}].node`, member.node);
            return;
        }
    ...
    const placedPoints = new Map([
        ...staging.cameras.map((camera) => [camera.node, camera.position]),
        ...staging.actors.map((a) => [a.node, a.position]),
        ...(staging.set ?? []).map((piece) => [piece.node, piece.position]),
    ]);
    ...
    script.cast.forEach((member, i) => {
        if (member.modelRef !== null)
            validateNonEmptyId(member.modelRef, `$script.cast[${i}].modelRef`, "cast model reference");
        if (!placed.has(member.node))
            out.push("type", `$input.actors`, `cast node "${member.node}" (cast[${i}]) must be placed by staging`, member.node);
    });
    const ids = new Set();
    const claim = (id, path, label) => {
    ...
    staging.actors.forEach((placement, i) => {
        ...
        if (!cast.has(placement.node))
            out.push("type", `$input.actors[${i}].node`, `placement must name a script cast node, but "${placement.node}" is not in the cast`, placement.node);
        if (!isFiniteVector3(placement.position))
            out.push("range", `$input.actors[${i}].position`, "actor position must be a finite vector", placement.position);
        if (!Number.isFinite(placement.facingDeg))
            out.push("range", `$input.actors[${i}].facingDeg`, `actor facingDeg must be finite, but was ${placement.facingDeg}`, placement.facingDeg);
        if (placement.attach !== undefined) {
            if (placement.attach.parent === placement.node)
                out.push("type", `$input.actors[${i}].attach.parent`, `a node cannot ride itself`, placement.attach.parent);
            else if (!placed.has(placement.attach.parent))
                out.push("type", `$input.actors[${i}].attach.parent`, `mount parent "${placement.attach.parent}" must be a placed actor`, placement.attach.parent);
        }
    });
    (staging.set ?? []).forEach((piece, i) => {
        ...
        if (!isFiniteVector3(piece.position))
            out.push("range", `$input.set[${i}].position`, "set position must be a finite vector", piece.position);
        if (piece.facingDeg !== undefined && !Number.isFinite(piece.facingDeg))
            out.push("range", `$input.set[${i}].facingDeg`, `set facingDeg must be finite when present, but was ${piece.facingDeg}`, piece.facingDeg);
        if (piece.scale !== undefined) {
            ...
            if (![scale.x, scale.y, scale.z].every((axis) => Number.isFinite(axis) && axis > 0))
                out.push("range", `$input.set[${i}].scale`, "set scale must be finite and greater than zero on every axis", piece.scale);
        }
    });
    ...
    staging.cameras.forEach((camera, i) => {
        ...
        if (!positionFinite)
            out.push("range", `$input.cameras[${i}].position`, "camera position must be a finite vector", camera.position);
        if (!(camera.fovDeg > 0 && camera.fovDeg < 180))
            out.push("range", `$input.cameras[${i}].fovDeg`, `vertical field of view must be within (0, 180)°, but was ${camera.fovDeg}`, camera.fovDeg);
        if (camera.lookAt.kind === "node" && !placedPoints.has(camera.lookAt.node))
            out.push("type", `$input.cameras[${i}].lookAt.node`, `camera target "${camera.lookAt.node}" must be a placed actor, set piece, or camera`, camera.lookAt.node);
        if (camera.lookAt.kind === "point" && !isFiniteVector3(camera.lookAt.point))
            out.push("range", `$input.cameras[${i}].lookAt.point`, "camera point target must be a finite vector", camera.lookAt.point);
        const target = camera.lookAt.kind === "node"
        ...
        if (target !== undefined &&
            ...
            out.push("range", `$input.cameras[${i}].lookAt`, "camera lookAt must not equal the camera position", camera.lookAt);
    });
    staging.lights.forEach((light, i) => {
        ...
        if (!Number.isFinite(light.intensity) || light.intensity < 0)
            out.push("range", `${path}.intensity`, `intensity must be a finite number >= 0, but was ${light.intensity}`, light.intensity);
        validateLightPlacementShape(light, path, out);
    });
    if (out.items.length > 0)
        return { success: false, violations: out.items };
    const nodes = [
        ...staging.actors.map((placement) => ({
            id: placement.node,
            model: cast.get(placement.node).member.modelRef ?? placement.node,
            transform: {
                translation: placement.position,
                rotation: Quaternion.fromAxisAngle({ x: 0, y: 1, z: 0 }, placement.facingDeg),
                scale: { x: 1, y: 1, z: 1 },
            },
            motion: null,
            pose: null,
        ...
        ...(staging.set ?? []).map((piece) => ({
            id: piece.node,
            model: piece.model,
            transform: {
                translation: piece.position,
                rotation: Quaternion.fromAxisAngle({ x: 0, y: 1, z: 0 }, piece.facingDeg ?? 0),
    ...
    const lights = staging.lights.map(lowerLightPlacement);
    const mounts = staging.actors
