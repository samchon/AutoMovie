const findUniqueOrNull = (props) => {
    ...
    for (let index = 0; index < props.items.length; index++) {
        ...
        if (!props.matches(item))
            continue;
        if (found !== null)
            throw new Error(`${props.label} "${props.key}" is duplicated at ${props.path(index)}; first declared at ${props.path(found.index)}`);
        found = { item, index };
    }
 ...
 */
export const readSlateContext = (slate, request) => {
    switch (request.type) {
        case "getScript":
            return slate.script;
        case "getScene":
...
//# sourceMappingURL=readSlateContext.js.map