...
const indexShots = (shots) => {
    const byId = new Map();
    shots.forEach((shot, index) => {
        const existing = byId.get(shot.id);
        if (existing !== undefined)
            throw new Error(`shot id "${shot.id}" is duplicated at shots[${index}].id; first declared at shots[${existing.index}].id`);
 ...
 */
export const sequenceTimeline = (sequence, shots) => {
    ...
    sequence.shots.forEach((entry, i) => {
        if (i === 0 && entry.transition !== null)
            throw new Error("sequence.shots[0].transition has nothing to transition from");
        const found = byId.get(entry.shot);
        if (found === undefined)
            throw new Error(`sequence shot "${entry.shot}" at sequence.shots[${i}].shot was not provided`);
 ...
 */
const sampleAt = (sequence, entries, live, seconds) => {
 ...
 */
export const resolveFromTimeline = (sequence, timeline, seconds) => {
 ...
 */
export const playbackCursor = (sequence, timeline) => {
    const entries = timeline.entries;
    let liveIdx = 0;
    return (seconds) => {
        while (liveIdx + 1 < entries.length &&
 ...
 */
export const resolveSequencePlayback = (sequence, shots, seconds) => {
    const timeline = sequenceTimeline(sequence, shots);
    if (seconds < 0 || seconds >= timeline.runtime)
        return null;
 ...
 */
export const sequenceEventTimeline = (sequence, shots) => {
    const timeline = sequenceTimeline(sequence, shots);
    const byId = indexShots(shots);
    const events = [];
    const entriesByShot = new Map();
    for (const entry of timeline.entries) {
    ...
    for (const entry of timeline.entries) {
        ...
        const ownedElsewhere = (time) => entriesByShot
            .get(entry.shot)
            .some((other) => Math.abs(other.offset - time) <= 1e-9 &&
            Math.abs(other.start - globalEnd) <= 1e-9);
        for (const event of shot.events ?? []) {
            if (event.time < from - 1e-9)
                continue;
            if (atShotEnd
                ? event.time > to + 1e-9
    ...
    return events.sort((a, b) => a.globalTime - b.globalTime ||
        a.entry - b.entry ||
 ...
 */
export const playbackFrameSamples = (sequence, shots) => {
    if (!Number.isFinite(sequence.fps) || !(sequence.fps > 0))
        throw new Error(`sequence fps must be a finite number > 0, but was ${sequence.fps}`);
    const timeline = sequenceTimeline(sequence, shots);
