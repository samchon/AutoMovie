...
export const compileAttach = (props) => {
    const { child, bone, parentTransform, parentSkeleton, parentMotion, start, duration, shotDuration, } = props;
    ...
    if (!(duration > 0))
        throw new RangeError(`attach follow duration must be > 0 seconds, but was ${duration}`);
    const restPose = {
    ...
    for (let i = 0; i <= count; ++i) {
        ...
        const pose = parentMotion === undefined
            ? restPose
            : sampleMotion(parentMotion, t).pose;
        const local = resolveAttachment(pose, parentSkeleton, attachment, props.jointAxes, props.restFrames);
        const parentWorld = props.parentTransformAt?.(t) ?? parentTransform;
        const worldPos = Vector3.add(parentWorld.translation, Quaternion.rotateVector(parentWorld.rotation, local.translation));
...
//# sourceMappingURL=compileAttach.js.map