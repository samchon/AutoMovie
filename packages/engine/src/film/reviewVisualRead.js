...
import { grammarDiagnosticsToReviewNotes, } from "./filmGrammar";
...
export const reviewVisualRead = (props) => {
    ...
    // convention).
    const worldRootAt = (node, motion, startOffset, t) => foldRoot(node.transform, sampleMotion(motion, Math.max(0, t - startOffset)).pose.root).translation;
    ...
    if (camera !== undefined &&
        ...
        const silhouetteRadius = props.silhouetteRadius ?? DEFAULT_SILHOUETTE_RADIUS;
        const halfY = Math.tan((camera.fovY * Math.PI) / 360);
        const times = sampleTimes(props.shot.duration, props.sampleRate ?? DEFAULT_SAMPLE_RATE);
        const camAt = (time) => resolveCameraAt(camera.transform, props.shot.cameraMotion, camera.id, time);
        ...
        const performers = props.shot.performances
            ...
            const motion = performance.motion === null
                ? undefined
        ...
        for (const actor of performers) {
            const offAt = times.find((time) => !inFrame(camAt(time), worldRootAt(actor.node, actor.motion, actor.offset, time), camera.near, camera.far, halfY, aspect));
            if (offAt !== undefined)
                notes.push({
                    beat: props.beat,
                    tier: "visual",
        ...
        for (let i = 0; i < performers.length; ++i)
            for (let j = i + 1; j < performers.length; ++j) {
                ...
                const mergeAt = times.find((time) => {
                    ...
                    const va = Vector3.subtract(worldRootAt(a.node, a.motion, a.offset, time), camPos);
                    const vb = Vector3.subtract(worldRootAt(b.node, b.motion, b.offset, time), camPos);
                    ...
                    if (Math.min(da, db) < camera.near)
                        return false;
                    const cos = Math.min(1, Math.max(-1, Vector3.dot(va, vb) / (da * db)));
    ...
    const performanceByNode = new Map(props.shot.performances.map((p) => [p.node, p]));
    for (const event of props.shot.events ?? []) {
        if ((event.kind !== "hit" && event.kind !== "contact") ||
            event.point === null ||
            event.target === null)
            continue;
        ...
        const motion = performance.motion === null
            ? undefined
            : motionById.get(performance.motion);
        if (node === undefined || motion === undefined)
            continue;
        const world = worldRootAt(node, motion, performance.startOffset, event.time);
 ...
 */
const inFrame = (cam, point, near, far, halfY, aspect) => {
...
//# sourceMappingURL=reviewVisualRead.js.map