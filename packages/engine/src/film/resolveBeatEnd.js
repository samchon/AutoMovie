...
import { VELOCITY_DT, foldRoot, gaitPhaseOf, plantsAtEnd, rootVelocityOf, } from "./beatEndSim";
 ...
 */
const bakedFollowVelocity = (clips, node, t) => {
    if (t <= 0)
        return ZERO;
    const t1 = t;
    const t0 = Math.max(0, t1 - VELOCITY_DT);
    const p1 = sampleClipSequence(clips, t1).get(`node:${node}:translation`)?.value;
    const p0 = sampleClipSequence(clips, t0).get(`node:${node}:translation`)?.value;
 ...
 */
export const resolveBeatEnd = (props) => resolveSnapshot(props, props.shot.duration);
/**
 ...
 */
export const resolveBeatOpening = (props) => resolveSnapshot(props, 0);
/** Shared body: resolve every scene actor's snapshot at a shot-local instant. */
const resolveSnapshot = (props, instant) => {
    const motionById = new Map();
    props.motions.forEach((motion, index) => {
        const existing = motionById.get(motion.id);
        if (existing !== undefined)
            throw new Error(`motion "${motion.id}" is duplicated at props.motions[${index}].id; first declared at props.motions[${existing.index}].id`);
    ...
    props.shot.performances.forEach((performance, index) => {
        ...
        if (existing !== undefined)
            throw new Error(`performance for node "${performance.node}" is duplicated at props.shot.performances[${index}].node; first declared at props.shot.performances[${existing.index}].node`);
    ...
    (props.mounts ?? []).forEach((mount, index) => {
        if (mountByNode.has(mount.node))
            throw new Error(`mount for node "${mount.node}" is duplicated at props.mounts[${index}].node`);
        mountByNode.set(mount.node, { binding: mount.binding });
    });
    const plantsByNode = new Map();
    (props.plants ?? []).forEach((entry, index) => {
        if (plantsByNode.has(entry.node))
            throw new Error(`plants for node "${entry.node}" are duplicated at props.plants[${index}].node`);
...
const endActorOf = (context, node) => {
    const performed = context.performanceByNode.get(node.id);
    const motionId = performed === undefined ? node.motion : performed.performance.motion;
    const localTime = performed === undefined
        ? context.instant
    ...
    const bakedTransform = bakedTransformFromClipsAt(context.objectMotions, node.id, localTime);
    const world = bakedTransform === null
        ? null
        : {
            transform: bakedTransform,
            rootVelocity: bakedFollowVelocity(context.objectMotions, node.id, localTime),
        };
    if (motionId === null)
        return actorState({
            node,
...
const actorState = (props) => {
    ...
    const transform = props.world !== null
...
//# sourceMappingURL=resolveBeatEnd.js.map