 ...
 * correction round.
 */
export const scriptAncestors = (tree, nodeId) => {
    const byId = new Map(tree.map((node) => [node.id, node]));
    const start = byId.get(nodeId);
    if (start === undefined)
        throw new Error(`scriptAncestors node "${nodeId}" is not in the screenplay tree`);
    const chain = [];
    const visited = new Set([nodeId]);
    let parent = start.parent;
    while (parent !== null) {
        if (visited.has(parent))
            throw new Error(`scriptAncestors parent chain of "${nodeId}" is cyclic at "${parent}"`);
        const node = byId.get(parent);
        if (node === undefined)
            throw new Error(`scriptAncestors parent "${parent}" is not in the screenplay tree`);
        visited.add(parent);
        chain.push(parent);
        parent = node.parent;
    }
 ...
 */
export const beatNodeOf = (tree, beatId) => {
    if (tree === null || tree === undefined)
        return null;
    for (const node of tree)
        if (node.kind === "beat" && node.payload.beat === beatId)
            return node.id;
    return null;
};
/**
 ...
 */
export const locateOnBeat = (violations, tree, beatId) => {
    const node = beatNodeOf(tree, beatId);
    if (node === null)
        return [...violations];
    return violations.map((violation) => ({ ...violation, node }));
};
//# sourceMappingURL=scriptGraph.js.map