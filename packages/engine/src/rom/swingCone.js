 ...
 * @author Samchon
 */
export const swingConeAngle = (flexion, abduction) => (Math.acos(Math.cos((flexion * Math.PI) / 360) * Math.cos((abduction * Math.PI) / 360)) *
    360) /
    Math.PI;
/**
 ...
 * @author Samchon
 */
export const swingConeBlend = (flexion, abduction, anchorFlexion, anchorAbduction, swingDeg) => {
    if (swingConeAngle(flexion, abduction) <= swingDeg)
        return 1;
    const at = (t) => swingConeAngle(anchorFlexion + (flexion - anchorFlexion) * t, anchorAbduction + (abduction - anchorAbduction) * t);
    let lo = 0;
    let hi = 1;
...
//# sourceMappingURL=swingCone.js.map