...
const clampAxis = (angle, allowed) => {
    if (angle === null)
        return null;
    if (allowed === null)
        return 0; // immobile axis: forced back to neutral
 ...
 * @author Samchon
 */
export const clampJointRom = (joint, constraint) => {
    ...
    if (typeof constraint.swingDeg === "number") {
        ...
        const anchorA = abduction === null ? 0 : nearestNeutral(constraint.abduction);
        const t = swingConeBlend(f, a, anchorF, anchorA, constraint.swingDeg);
        const blend = (value, anchor) => anchor + (value - anchor) * t;
        return {
            bone: joint.bone,
 ...
 */
const nearestNeutral = (allowed) => allowed === null
    ? 0
    : allowed.min > 0
        ? allowed.min
        : allowed.max < 0
 ...
 * @author Samchon
 */
export const clampJointToSkeleton = (joint, skeleton) => {
 ...
 * @author Samchon
 */
export const clampPose = (pose, skeleton) => ({
    skeleton: pose.skeleton,
    root: pose.root,
    joints: pose.joints.map((joint) => clampJointToSkeleton(joint, skeleton)),
});
//# sourceMappingURL=clampPose.js.map