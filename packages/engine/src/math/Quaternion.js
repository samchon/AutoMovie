...
(function (Quaternion) {
     ...
     */
    Quaternion.inverse = (q) => Quaternion.normalize({ x: -q.x, y: -q.y, z: -q.z, w: q.w });
    /** Rotation of `angleDeg` degrees about a (not necessarily unit) `axis`. */
    Quaternion.fromAxisAngle = (axis, angleDeg) => {
        const len = Math.sqrt(axis.x * axis.x + axis.y * axis.y + axis.z * axis.z);
        if (len === 0)
            return Quaternion.identity();
        const half = (angleDeg * Quaternion.DEG2RAD) / 2;
    ...
    Quaternion.fromEuler = (euler) => {
        const axisOf = (c) => c === "X"
            ...
            : c === "Y"
                ? { x: 0, y: 1, z: 0 }
                : { x: 0, y: 0, z: 1 };
        const angleOf = (c) => c === "X" ? euler.x : c === "Y" ? euler.y : euler.z;
        return euler.order
            .split("")
            .map((c) => Quaternion.fromAxisAngle(axisOf(c), angleOf(c)))
            .reduce((acc, q) => Quaternion.multiply(acc, q), Quaternion.identity());
    };
