 ...
 * @author Samchon
 */
export const convexHull2D = (points) => {
 ...
 */
export const pointInHull = (point, hull) => {
    if (hull.length < 3)
        return false;
 ...
 */
export const pointHullDistance = (point, hull) => {
    if (hull.length === 0)
        return Infinity;
    if (hull.length === 1)
        return distanceXZ(point, hull[0]);
    if (pointInHull(point, hull))
        return 0;
 ...
 * edge at that vertex.
 */
export const nearestHullEdge = (point, hull) => {
    if (hull.length === 1)
        return {
            start: hull[0],
            end: hull[0],
    ...
    for (let i = 0; i < edgeCount; i++) {
        ...
        const distance = distanceXZ(point, closestPointOnSegmentXZ(point, start, end));
...
export const closestPointOnSegmentXZ = (point, start, end) => {
