...
export const validatePose = (props) => {
    ...
    if (props.pose.skeleton !== props.skeleton.id)
        collector.push("type", `${path}.skeleton`, `pose skeleton "${props.pose.skeleton}" does not match target skeleton "${props.skeleton.id}"`, props.pose.skeleton);
    ...
    props.pose.joints.forEach((joint, i) => {
        ...
        if (bone === undefined) {
            collector.push("type", `${jointPath}.bone`, `bone "${joint.bone}" is not present in the target skeleton`, joint.bone);
            return;
        }
        if (seen.has(joint.bone))
            collector.push("type", `${jointPath}.bone`, `bone "${joint.bone}" is articulated more than once in this pose`, joint.bone);
        seen.add(joint.bone);
...
export const validatePoseResult = (pose, skeleton) => validatePose({ pose, skeleton }).toValidation();
//# sourceMappingURL=validatePose.js.map