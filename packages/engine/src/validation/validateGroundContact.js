...
const DEFAULT_FOOT_BONES = ["leftFoot", "rightFoot"];
/**
...
export const validateGroundContact = (props) => {
    ...
    const capsuleValid = capsules.map((capsule, capsuleIndex) => validateCapsule(capsule, `${path}.capsules[${capsuleIndex}]`, skeletonBones, reachableBones, collector));
    ...
    if (badTolerance)
        collector.push("range", `${path}.tolerance`, `tolerance must be a finite number, but was ${tolerance}`, tolerance);
    if (badRate || badTolerance)
        return collector.toValidation();
    sampleTimes(props.motion.duration, sampleRate).forEach((time, index) => {
        const resolved = new Map(resolvePose(sampleMotion(props.motion, time).pose, props.skeleton, props.jointAxes, props.restFrames, topology).map((bone) => [bone.bone, bone]));
        for (const bone of footBones) {
            ...
            if (y < minY && !suppressed)
                collector.warn("physics", `${path}.samples[${index}].${bone}.worldPosition.y`, `${bone} world y must stay >= ${minY} at t=${round(time)}s (ground ${ground}, tolerance ${tolerance}; a foot usually should not pass through the ground; mark physicsIntent if it is deliberate)`, y, minY - y);
        }
        capsules.forEach((capsule, capsuleIndex) => {
            if (!capsuleValid[capsuleIndex])
                return;
            ...
            const worst = [capsule.from, capsule.to].reduce((deepest, endpoint) => {
            ...
            if (worst.depth > 0 && !suppressed)
                collector.warn("physics", `${path}.samples[${index}].capsules[${capsuleIndex}].lowestY`, `capsule (${capsule.from} to ${capsule.to}) lowest surface y must stay >= ${round(worst.minY)} at t=${round(time)}s (radius ${capsule.radius}, tolerance ${tolerance}; a body part usually should not pass through the ground; mark physicsIntent if it is deliberate)`, worst.surfaceY, worst.depth);
        });
    });
    return collector.toValidation();
};
const round = (value) => Math.round(value * 1_000) / 1_000;
//# sourceMappingURL=validateGroundContact.js.map