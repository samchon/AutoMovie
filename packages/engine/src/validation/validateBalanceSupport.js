...
const CENTER_BONE_REACHABLE = "center bone is declared but not reachable from a root bone via forward kinematics";
const MARGIN_EXPECTED = "margin must be a finite number >= 0";
const SUPPORT_BONES_EXPECTED = "supportBones must contain at least one contact bone";
const SUPPORT_BONE_EXPECTED = "support bone must exist in the target skeleton";
const SUPPORT_BONE_REACHABLE = "support bone is declared but not reachable from a root bone via forward kinematics";
/**
...
export const validateBalanceSupport = (props) => {
    ...
    for (const [supportIndex, support] of props.supports.entries()) {
        ...
        const centerMissing = centerBone !== undefined && !skeletonBones.has(centerBone);
        ...
        // declaration.
        const centerUnreachable = centerBone !== undefined &&
        ...
        const temporalInvalid = !Number.isFinite(support.start) ||
            !Number.isFinite(support.end) ||
            support.end <= support.start;
        const marginInvalid = !Number.isFinite(margin) || margin < 0;
        if (centerMissing)
            collector.push("type", `${sp}.centerBone`, CENTER_BONE_EXPECTED, centerBone);
        if (centerUnreachable)
            collector.push("type", `${sp}.centerBone`, CENTER_BONE_REACHABLE, centerBone);
        if (emptySupport)
            collector.push("type", `${sp}.supportBones`, SUPPORT_BONES_EXPECTED, support.supportBones);
        for (const bone of support.supportBones) {
            if (!skeletonBones.has(bone)) {
                missingSupport = true;
                collector.push("type", `${sp}.supportBones`, SUPPORT_BONE_EXPECTED, bone);
            }
            else if (!reachableBones.has(bone)) {
                unreachableSupport = true;
                collector.push("type", `${sp}.supportBones`, SUPPORT_BONE_REACHABLE, bone);
            }
        }
        if (temporalInvalid)
            collector.push("temporal", sp, `support window must have finite start/end with end > start, but was [${support.start}, ${support.end}]`, { start: support.start, end: support.end });
        if (marginInvalid)
        ...
        for (let sampleIndex = 0; sampleIndex < times.length; sampleIndex++) {
            ...
            const pose = resolvePose(sampleMotion(props.motion, time).pose, props.skeleton, props.jointAxes, props.restFrames, topology);
            for (const bone of pose)
                resolved.set(bone.bone, bone.worldPosition);
            const centerPosition = centerBone === undefined
                ? weightedCenterOfMass(resolved)
                : resolved.get(centerBone);
            ...
            const hull = convexHull2D(support.supportBones.map((bone) => resolved.get(bone)));
 ...
 * weight is positive.
 */
const weightedCenterOfMass = (resolved) => {
    let mass = 0;
    let x = 0;
    let y = 0;
    let z = 0;
...
//# sourceMappingURL=validateBalanceSupport.js.map