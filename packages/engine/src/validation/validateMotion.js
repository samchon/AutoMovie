...
export const validateMotion = (props) => {
    ...
    if (motion.id.trim().length === 0)
        collector.push("type", `${path}.id`, "motion id must be a non-empty id", motion.id);
    if (motion.skeleton !== skeleton.id)
        collector.push("type", `${path}.skeleton`, `motion skeleton "${motion.skeleton}" does not match target skeleton "${skeleton.id}"`, motion.skeleton);
    if (!Number.isFinite(motion.duration) || !(motion.duration > 0))
        collector.push("temporal", `${path}.duration`, `motion duration must be a finite number > 0 seconds, but was ${motion.duration}`, motion.duration);
    if (typeof motion.loop !== "boolean")
        collector.push("type", `${path}.loop`, `motion loop must be boolean, but was ${String(motion.loop)}`, motion.loop);
    if (motion.keyframes.length < 2)
        collector.push("temporal", `${path}.keyframes`, `motion must have at least two keyframes, but had ${motion.keyframes.length}`, motion.keyframes.length);
    ...
    motion.keyframes.forEach((kf, i) => {
        ...
        if (!finiteTime || kf.time < 0 || kf.time > motion.duration)
            collector.push("temporal", `${kp}.time`, `keyframe time must be a finite number within [0, ${motion.duration}]s, but was ${kf.time}`, kf.time);
        if (finiteTime && kf.time <= previousTime)
            collector.push("temporal", `${kp}.time`, `keyframe times must strictly increase; ${kf.time} is not greater than the previous ${previousTime}`, kf.time);
        if (!KEYFRAME_EASINGS.has(kf.easing))
            collector.push("type", `${kp}.easing`, `unknown keyframe easing "${String(kf.easing)}"`, kf.easing);
        ...
        if (i > 0)
            checkAngularSpeed(motion, i, kp, collector);
        if (finiteTime)
    ...
    for (const j of cur.pose.joints) {
        ...
        for (const axis of axes) {
            ...
            if (speed > MAX_ANGULAR_SPEED_DEG_PER_S)
                collector.push("temporal", `${kp}.pose`, `${j.bone} ${axis} changes ${delta.toFixed(0)}° in ${dt.toFixed(2)}s (${speed.toFixed(0)}°/s), exceeding ${MAX_ANGULAR_SPEED_DEG_PER_S}°/s`, speed);
        }
    }
};
/** Enforce the easing/control-point pairing documented on IAutoMovieKeyframe. */
const validateKeyframeBezier = (kf, kp, collector) => {
    if (kf.easing !== "cubicBezier") {
        if (kf.bezier !== null)
            collector.push("type", `${kp}.bezier`, "keyframe bezier controls are only valid for cubicBezier easing", kf.bezier);
        return;
    }
    if (!isFiniteBezierTuple(kf.bezier)) {
        collector.push("type", `${kp}.bezier`, "cubicBezier keyframes must carry four finite bezier control values", kf.bezier);
        return;
    }
    ...
    if (x1 < 0 || x1 > 1 || x2 < 0 || x2 > 1)
        collector.push("range", `${kp}.bezier`, `cubicBezier control x's (x1, x2) must be within [0, 1] to keep the eased curve monotone, but were x1=${x1}, x2=${x2}`, kf.bezier);
};
const isFiniteBezierTuple = (value) => Array.isArray(value) &&
    value.length === 4 &&
...
//# sourceMappingURL=validateMotion.js.map