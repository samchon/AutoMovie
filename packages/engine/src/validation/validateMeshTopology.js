...
export const validateMeshTopology = (props) => {
    ...
    appendMeshTopology(props.mesh, props.path ?? "$input", collector, props.expectClosed ?? false);
 ...
 */
export const appendMeshTopology = (mesh, path, collector, expectClosed) => {
    const vertexCount = mesh.positions.length / 3;
    if (vertexCount === 0 || !Number.isInteger(vertexCount))
        return;
    const indices = mesh.indices ?? Array.from({ length: vertexCount }, (_, i) => i);
    if (indices.length % 3 !== 0)
        return;
    if (indices.some((index) => !Number.isInteger(index) || index < 0 || index >= vertexCount))
        return;
    const keyOf = (vertex) => [0, 1, 2]
        .map((axis) => Math.round(mesh.positions[vertex * 3 + axis] * WELD_GRID) || 0)
        .join(",");
    ...
    for (const [edge, count] of undirected)
        if (count > 2)
            collector.push("topology", `${path}.indices`, `a 2-manifold mesh edge is shared by at most 2 triangles, but the edge (${edge}) is shared by ${count}`, count);
    for (const [edge, count] of directed)
        if (count > 1)
            collector.push("topology", `${path}.indices`, `triangles adjacent on an edge must wind in opposite directions, but the directed edge (${edge}) appears ${count} times (a flipped triangle)`, count);
    if (expectClosed)
        for (const [edge, count] of undirected)
            if (count === 1)
                collector.push("topology", `${path}.indices`, `a closed mesh has every edge shared by 2 triangles, but the edge (${edge}) is a boundary (open) edge`, edge);
};
//# sourceMappingURL=validateMeshTopology.js.map