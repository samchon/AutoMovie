...
export const validateScriptTree = (props) => {
    ...
    tree.forEach((node, i) => {
        ...
        if (existing !== undefined) {
            out.push("type", `${np}.id`, `node id "${node.id}" is duplicated; first declared at ${path}[${existing.index}].id`, node.id);
            return;
        }
    ...
    if (roots.length !== 1)
        out.push("type", path, `the refinement tree needs exactly one root (parent: null), but found ${roots.length}`, roots.map((node) => node.id));
    for (const root of roots)
        if (root.kind !== "intent")
            out.push("type", `${path}[${tree.indexOf(root)}].kind`, `the root must be the "intent" the tree refines, but "${root.id}" is a "${root.kind}"`, root.kind);
    tree.forEach((node, i) => {
        const np = `${path}[${i}]`;
        if (node.parent !== null) {
            const parent = byId.get(node.parent);
            if (parent === undefined) {
                out.push("type", `${np}.parent`, `parent "${node.parent}" is not a node of this tree`, node.parent);
            }
            else if (node.kind === "intent") {
                out.push("type", `${np}.parent`, `an "intent" is the root thought and cannot refine another node`, node.parent);
            }
            else if (!ALLOWED_PARENTS[node.kind].includes(parent.node.kind)) {
                out.push("type", `${np}.parent`, `a "${node.kind}" cannot refine a "${parent.node.kind}" (allowed: ${ALLOWED_PARENTS[node.kind].join(", ")})`, node.parent);
            }
        }
        if (node.temporal !== null) {
            if (node.temporal === node.id)
                out.push("type", `${np}.temporal`, "a node cannot temporally follow itself", node.temporal);
            else if (!byId.has(node.temporal))
                out.push("type", `${np}.temporal`, `temporal predecessor "${node.temporal}" is not a node of this tree`, node.temporal);
        }
        node.interactsWith.forEach((other, j) => {
            if (other === node.id)
                out.push("type", `${np}.interactsWith[${j}]`, "a node cannot interact with itself", other);
            else if (!byId.has(other))
                out.push("type", `${np}.interactsWith[${j}]`, `interaction target "${other}" is not a node of this tree`, other);
        });
    });
    for (const [id, entry] of byId) {
        ...
        while (parent !== null) {
            if (trail.has(parent)) {
                out.push("type", `${path}[${entry.index}].parent`, `refinement chain of "${id}" is cyclic at "${parent}"`, parent);
                break;
            }
            trail.add(parent);
    ...
    tree.forEach((node, i) => {
        ...
        if (!beatIds.has(node.payload.beat)) {
            out.push("type", `${np}.beat`, `beat node "${node.id}" refines beat "${node.payload.beat}", which is not in the script's beats`, node.payload.beat);
        }
        else {
            const first = claimed.get(node.payload.beat);
            if (first !== undefined)
                out.push("type", `${np}.beat`, `beat "${node.payload.beat}" is already refined by ${path}[${first}]; each beat joins exactly one node`, node.payload.beat);
            else
                claimed.set(node.payload.beat, i);
        }
        node.payload.dialogue.forEach((line, j) => {
            if (line.anchor !== null &&
                (!Number.isFinite(line.anchor) || line.anchor < 0))
                out.push("range", `${np}.dialogue[${j}].anchor`, `dialogue anchor must be a finite number >= 0 or null, but was ${line.anchor}`, line.anchor);
        });
    });
    beats.forEach((beat, i) => {
        if (!claimed.has(beat.id))
            out.push("type", path, `beat "${beat.id}" ($input.beats[${i}]) has no beat node; a tree must refine every beat`, beat.id);
    });
    return out.toValidation();
};
//# sourceMappingURL=validateScriptTree.js.map