...
export const validateFace = (props) => {
    ...
    for (const trait of flattenFace(props.face))
        collector.range(`${path}.${trait.path}`, trait.weight, -FACE_PARAMETER_LIMIT, FACE_PARAMETER_LIMIT, "weight");
    return collector;
};
/** Convenience wrapper returning a finished {@link IAutoMovieValidation}. */
export const validateFaceResult = (face) => validateFace({ face }).toValidation();
//# sourceMappingURL=validateFace.js.map