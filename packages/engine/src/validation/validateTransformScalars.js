...
export const validateTransformScalars = (props) => {
    ...
    for (const [fieldPath, value] of finiteFields)
        if (!Number.isFinite(value))
            collector.push("range", fieldPath, `${label} component must be finite, but was ${value}`, value);
    const rotationLength = Math.hypot(transform.rotation.x, transform.rotation.y, transform.rotation.z, transform.rotation.w);
    if (Number.isFinite(rotationLength) &&
        Math.abs(rotationLength - 1) > UNIT_QUATERNION_EPSILON)
        collector.push("range", `${path}.rotation`, `${label} rotation must be a unit quaternion (length 1), but length was ${rotationLength}`, transform.rotation);
    const scaleFields = [
        [`${path}.scale.x`, transform.scale.x],
        [`${path}.scale.y`, transform.scale.y],
        [`${path}.scale.z`, transform.scale.z],
    ...
    for (const [fieldPath, value] of scaleFields)
        if (value <= 0)
            collector.push("range", fieldPath, `${label} scale component must be > 0, but was ${value}`, value);
};
//# sourceMappingURL=validateTransformScalars.js.map