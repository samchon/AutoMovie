 ...
 * @author Samchon
 */
export const validateCapsule = (capsule, path, skeletonBones, reachableBones, collector) => {
    let valid = true;
    const endpoint = (bone, at) => {
        if (!skeletonBones.has(bone)) {
            valid = false;
            collector.push("type", `${path}.${at}`, `capsule endpoint "${bone}" must exist in the target skeleton`, bone);
        }
        else if (!reachableBones.has(bone)) {
            valid = false;
            collector.push("type", `${path}.${at}`, `capsule endpoint "${bone}" is declared but not reachable from a root bone via forward kinematics`, bone);
        }
    };
    ...
    if (capsule.from === capsule.to) {
        valid = false;
        collector.push("type", path, "capsule endpoints must be two distinct bones", { from: capsule.from, to: capsule.to });
    }
    if (!Number.isFinite(capsule.radius) || capsule.radius <= 0) {
        valid = false;
        collector.push("range", `${path}.radius`, `capsule radius must be a finite number > 0, but was ${capsule.radius}`, capsule.radius);
    }
    return valid;
};
//# sourceMappingURL=capsuleProxy.js.map