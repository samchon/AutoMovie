...
const SURFACE_KINDS = ["floor", "platform", "ramp"];
...
export const validateSpace = (props) => {
    ...
    if (space.id.trim().length === 0)
        collector.push("type", `${path}.id`, "space id must be non-empty", space.id);
    const ids = new Set();
    space.surfaces.forEach((surface, i) => {
        const sp = `${path}.surfaces[${i}]`;
        if (surface.id.trim().length === 0)
            collector.push("type", `${sp}.id`, "surface id must be non-empty", surface.id);
        if (ids.has(surface.id))
            collector.push("type", `${sp}.id`, `surface id "${surface.id}" must be unique within the space`, surface.id);
        ids.add(surface.id);
    ...
    space.walkable.forEach((id, i) => {
        ...
        if (!ids.has(id))
            collector.push("type", wp, `walkable id "${id}" does not resolve to any surface of this space`, id);
        if (walked.has(id))
...
const validateSurface = (surface, path, collector) => {
    if (!SURFACE_KINDS.includes(surface.kind))
        collector.push("type", `${path}.kind`, `unknown surface kind "${String(surface.kind)}"`, surface.kind);
    if (surface.polygon.length < 3)
        collector.push("type", `${path}.polygon`, `a surface footprint needs at least 3 points, but had ${surface.polygon.length}`, surface.polygon.length);
    let planFinite = true;
    surface.polygon.forEach((point, i) => {
        for (const axis of ["x", "z"])
            if (!Number.isFinite(point[axis])) {
                planFinite = false;
                collector.push("range", `${path}.polygon[${i}].${axis}`, `polygon ${axis} must be finite, but was ${point[axis]}`, point[axis]);
            }
    });
    if (planFinite && surface.polygon.length >= 3) {
        const hull = convexHull2D(surface.polygon);
        if (hull.length < 3)
            collector.push("type", `${path}.polygon`, "surface footprint points are collinear: they enclose no area", surface.polygon);
        ...
        else if (surface.polygon.some((point) => nearestHullEdge(point, hull).distance > CONVEX_TOLERANCE))
            collector.push("type", `${path}.polygon`, "surface footprint is concave: a vertex sits inside its convex hull, which the ground query would fill; footprints must be convex", surface.polygon);
    }
    ...
    if (surface.rampTo !== null) {
        ...
        if (Number.isFinite(ax) &&
            ...
            collector.push("range", `${path}.rampTo`, "ramp axis is degenerate: rampTo must sit at a different (x, z) than anchor", surface.rampTo);
    }
};
const validateAnchor = (anchor, path, collector) => {
    for (const axis of ["x", "y", "z"])
        if (!Number.isFinite(anchor[axis]))
            collector.push("range", `${path}.${axis}`, `anchor ${axis} must be finite, but was ${anchor[axis]}`, anchor[axis]);
};
//# sourceMappingURL=validateSpace.js.map