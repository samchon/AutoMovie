import { resolveBeatEnd, resolveBeatOpening, } from "../film/resolveBeatEnd";
...
export const validateContinuity = (props) => {
    ...
    const tolerances = readTolerances(props.positionTolerance, props.facingToleranceDeg, "$input", collector);
    if (tolerances === null)
        return collector.toValidation();
    compareBoundary(props.previous, props.opening, "$input", tolerances, collector);
    return collector.toValidation();
};
/**
...
export const validateFilmContinuity = (props) => {
    ...
    for (let i = 1; i < snapshots.length; ++i)
        compareBoundary(snapshots[i - 1].end, snapshots[i].opening, `$input.beats[${i}]`, tolerances, collector);
    return collector.toValidation();
};
/**
 ...
 */
const readTolerances = (position, facingDeg, root, collector) => {
    ...
    if (badPos)
        collector.push("range", `${root}.positionTolerance`, "a finite position tolerance >= 0 metres", position);
    if (badFacing)
        collector.push("range", `${root}.facingToleranceDeg`, "a finite facing tolerance in [0, 180] degrees", facingDeg);
    if (badPos || badFacing)
        return null;
    return { position: pos, facingDeg: facing };
};
/** Compare one incoming opening against the prior end, pushing drift warnings. */
const compareBoundary = (previous, opening, root, tolerances, collector) => {
    const openingByNode = new Map(opening.actors.map((actor) => [actor.node, actor]));
    previous.actors.forEach((prev) => {
        const open = openingByNode.get(prev.node);
        if (open === undefined) {
            collector.warn("physics", `${root}.opening.actors`, `actor "${prev.node}" ended the previous beat but is absent from the incoming beat's opening: continuity cannot be verified`, prev.node);
            return;
        }
        compareActor(prev, open, `${root}.opening.actors[node=${prev.node}]`, tolerances, collector);
    });
};
/** Position, facing, and mount drift for one actor carried across a cut. */
const compareActor = (prev, open, path, tolerances, collector) => {
    const drift = Vector3.length(Vector3.subtract(open.transform.translation, prev.transform.translation));
    if (drift > tolerances.position)
        collector.warn("physics", `${path}.transform.translation`, `resume within ${tolerances.position} m of where the previous beat ended`, open.transform.translation, drift - tolerances.position);
    const facingDeg = angleBetweenDeg(prev.facing, open.facing);
    if (facingDeg > tolerances.facingDeg)
        collector.warn("physics", `${path}.facing`, `resume within ${tolerances.facingDeg} deg of the previous beat's facing`, open.facing, facingDeg - tolerances.facingDeg);
    if (prev.mount !== null && !sameMount(prev.mount, open.mount))
        collector.warn("physics", `${path}.mount`, `keep riding "${prev.mount.parent}"'s "${prev.mount.bone}" the rider ended the previous beat mounted on`, open.mount);
};
/**
...
const sameMount = (prev, open) => open !== null && open.parent === prev.parent && open.bone === prev.bone;
//# sourceMappingURL=validateContinuity.js.map