...
export const tessellate = (shape) => {
    switch (shape.type) {
        ...
        case "capsule":
            ...
            // radii.
            return cylinder(shape.radius, shape.radius, shape.height + 2 * shape.radius, 16);
        default: {
            const unknown = shape;
...
export const tessellateToMesh = (shape) => {
...
const sphere = (radius, segments, rings) => {
    const positions = [];
    const normals = [];
    const indices = [];
...
const cylinder = (radiusTop, radiusBottom, height, segments) => {
    const positions = [];
    const normals = [];
    const indices = [];
    ...
    const cap = (radius, y, up) => {
        if (radius <= 0)
            return; // a cone's apex end is a point, not a disk
