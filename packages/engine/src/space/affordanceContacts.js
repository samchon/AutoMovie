...
export const affordanceSupportContacts = (props) => {
    ...
    if (affordance.kind !== "stack-top")
        throw new Error(`affordanceSupportContacts: "${affordance.id}" is a "${affordance.kind}", not a "stack-top"`);
    if (affordance.extent === null)
        throw new Error(`affordanceSupportContacts: stack-top "${affordance.id}" has no extent`);
    const frame = affordance.frame;
    return affordance.extent.map((corner) => {
        const inModel = Vector3.add(frame.translation, Quaternion.rotateVector(frame.rotation, {
            x: corner.x,
            y: 0,
            z: corner.z,
        }));
        return Vector3.add(parentWorld.translation, Quaternion.rotateVector(parentWorld.rotation, inModel));
    });
};
//# sourceMappingURL=affordanceContacts.js.map