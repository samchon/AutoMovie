...
import { classifyLocomoteGroundDisplacement, locomoteMotion, } from "../motion/locomote";
...
const staticActorWorldFrame = (context) => ({
...
const resumeActorContext = (context, previous) => {
    if (previous === undefined)
        return context;
    const carriedSpeed = previous.rootVelocity === null
        ? 0
        : Math.hypot(previous.rootVelocity.x, previous.rootVelocity.z);
    return {
        ...context,
        position: previous.transform.translation,
        facingDeg: (Math.atan2(previous.facing.x, previous.facing.z) * 180) / Math.PI,
...
const resumedRuntime = (contexts, placements, previous) => {
    ...
    const liveContexts = new Map([...contexts].map(([node, context]) => [node, resumeActorContext(context, states.get(node))]));
...
export const resolveActorWorldFrame = (context, motion, seconds) => {
    if (motion === undefined)
        return null;
    ...
    return {
        position: Vector3.add(staged.position, Quaternion.rotateVector(staged.rotation, root.translation)),
        rotation,
        facingDeg: (Math.atan2(forward.x, forward.z) * 180) / Math.PI,
    };
};
/** Drop a world point into an actor's model space (undo its live frame). */
const toModelSpace = (world, frame) => Quaternion.rotateVector(Quaternion.inverse(frame.rotation), Vector3.subtract(world, frame.position));
/**
 ...
 */
const aimPointsOf = (contexts, nodes) => new Map([...nodes].map(([id, point]) => {
...
const extendHoldClip = (id, skeleton, pose, duration) => {
    const rest = { skeleton, root: null, joints: [] };
    const key = (time, p) => ({
        time,
        pose: p,
        expression: null,
...
const boneTargetTimes = (duration, landmarks = []) => {
    const count = Math.max(2, Math.ceil(duration * BONE_TARGET_HZ) + 1);
    return [
        ...new Set([
            ...Array.from({ length: count }, (_, index) => (duration * index) / (count - 1)),
            ...landmarks,
        ]),
    ].sort((a, b) => a - b);
};
...
const weightedArmPose = (skeleton, pose, weight) => ({
    skeleton,
    root: null,
    joints: weight === 0
        ? []
...
const dynamicExtendHoldClip = (props) => {
    ...
    return dynamicPoseClip({
        ...
        poseAt: (time) => {
            ...
            if (pose === null)
                return null;
            const weight = time >= extendedAt ? 1 : ease("easeInOut", time / extendedAt);
...
export const resolveBoneTarget = (target, contexts, motions, seconds) => {
    if (target.kind !== "bone")
        return null;
    ...
    const pose = motion === undefined
        ? context.restPose
        : sampleMotion(motion, seconds).pose;
    const resolved = resolvePose(pose, context.rig, HUMANOID_JOINT_AXES, context.restFrames).find((entry) => entry.bone === target.bone);
    if (resolved === undefined)
        return null;
    const facing = Quaternion.fromAxisAngle({ x: 0, y: 1, z: 0 }, context.facingDeg);
    return Vector3.add(context.position, 
...
const jabClip = (id, skeleton, pose, duration) => {
    const rest = { skeleton, root: null, joints: [] };
    const key = (time, p, easing) => ({
        time,
        pose: p,
        expression: null,
        easing,
 ...
 * @author Samchon
 */
export const makeActorSynthesizer = (contexts, placements, boneTargetAt, actorFrameAt) => {
    for (const [actor, ctx] of contexts)
        assertUniqueActorGaits(actor, ctx.gaits);
    return (action, actor, previous) => {
        ...
        const frameAt = (seconds) => actorFrameAt?.(actor, seconds) ?? staticFrame;
        const resolveTarget = (target, table, seconds) => target.kind === "bone"
        ...
        if (action.verb === "locomote") {
            ...
            if (gait === undefined)
                return null;
            const cycle = seedRestArticulation(gaitMotion(`${actor}:${action.gait}`, ctx.skeleton, gait, GAIT_SAMPLES, ctx.gaitPhase ?? 0), ctx.restPose);
            ...
            if (dest === null)
                return fitDeclaredSpan(cycle, action.duration); // relative/unresolved → step in place
            ...
            const travelSpeed = ctx.speed * (displacement.travelDistance / displacement.groundDistance);
            return fitDeclaredSpan(locomoteMotion(`${actor}:${action.gait}:travel`, cycle, displacement.travelDistance, travelSpeed, local, action.faceTravel === true), action.duration);
        }
        if (action.verb === "hold")
            return holdMotion(`${actor}:hold`, ctx.skeleton, ctx.restPose, action.duration);
        if (action.verb === "lookAt") {
            ...
            const poseAt = (time) => {
                ...
                const eye = Vector3.add(frame.position, Quaternion.rotateVector(frame.rotation, {
                    x: 0,
                    y: ctx.eyeHeight,
                    z: 0,
                }));
        ...
        if (action.verb === "gesture") {
            ...
            if (action.kind === "point" && ctx.rig !== undefined) {
                const world = action.at === undefined
                    ? null
                ...
                if (action.at?.kind === "bone" ||
                    ...
                    return dynamicExtendHoldClip({
                        ...
                        duration,
                        poseAt: (time) => {
                            const point = resolveTarget(action.at, nodes, action.start + time);
                            return point === null
                                ? null
                                : reachPose(ctx.rig, "right", toModelSpace(point, frameAt(action.start + time)), ctx.restFrames);
                        },
                    });
                const pose = reachPose(ctx.rig, "right", toModelSpace(world, staticFrame), ctx.restFrames);
                return pose === null
                    ? null
            ...
            return gestureMotion(`${actor}:${action.kind}`, ctx.skeleton, action.kind, duration);
        }
        if (action.verb === "reach") {
            ...
            if (ctx.rig === undefined)
                return null;
            const world = resolveTarget(action.to, nodes, action.start);
            if (world === null)
                return null; // relative target: no point to reach
            ...
            if (action.to.kind === "bone" ||
                ...
                return dynamicExtendHoldClip({
                    ...
                    poseAt: (time) => {
                        ...
                        return point === null
                            ? null
                            : reachPose(ctx.rig, action.hand, toModelSpace(point, frameAt(action.start + time)), ctx.restFrames);
                    },
                });
            const reach = reachPose(ctx.rig, action.hand, toModelSpace(world, staticFrame), ctx.restFrames);
            if (reach === null)
                return null;
        ...
        if (action.verb === "react") {
            ...
            const magnitude = REACT_MAX_DEFLECTION *
            ...
            if (source === null)
                ...
                const dir = Vector3.length(away) < 1e-6 ? forward : Vector3.normalize(away);
                push = {
                    flexion: Vector3.dot(dir, forward) * magnitude,
                    abduction: Vector3.dot(dir, right) * magnitude,
                };
            }
            return reactMotion(`${actor}:react`, ctx.rig, push, [...REACT_CHAIN], duration);
        }
        return null;
    };
};
//# sourceMappingURL=makeActorSynthesizer.js.map