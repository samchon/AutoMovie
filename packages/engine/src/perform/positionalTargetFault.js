...
const targetKindName = (target) => isRecord(target) && typeof target.kind === "string"
    ? target.kind
    : "malformed";
/**
 ...
 * @author Samchon
 */
export const POSITIONAL_TARGET_SHAPE = "a node/bone/point/group, whose ids name placed actors, set pieces, or cameras, and whose group may also name the formations a camera frames";
/**
 ...
 * @author Samchon
 */
export const positionalTargetFault = (target) => {
    if (isRecord(target)) {
        if (target.kind === "node")
        ...
        if (target.kind === "group") {
            const members = [
                ...(Array.isArray(target.nodes) ? target.nodes : []).map((node) => `"${String(node)}"`),
                ...(Array.isArray(target.formations) ? target.formations : []).map((formation) => `formation "${String(formation)}"`),
            ];
...
//# sourceMappingURL=positionalTargetFault.js.map