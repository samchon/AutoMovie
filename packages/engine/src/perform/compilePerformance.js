import { arrangeMotion } from "../motion/arrange";
...
const ROOT_REGIONS = new Set(["lowerBody", "fullBody"]);
/**
...
const maskedAnything = (masked) => masked.bones.length > 0 || masked.root || masked.expression;
const maskMotionToRegion = (motion, region, keepRoot) => {
    ...
    const keyframes = motion.keyframes.map((keyframe) => {
        if (!keepRoot && keyframe.pose.root !== null)
            droppedRoot = true;
...
const layerClips = (id, clips) => {
    ...
    for (const { first, last } of envelopes) {
        ...
        if (latest - last > BOUNDARY_EPSILON)
            timeSet.add(last + BOUNDARY_EPSILON);
    }
    ...
    const keyframes = times.map((time) => {
        ...
        for (const { clip, first, last } of envelopes) {
            if (time < first - 1e-9)
                continue; // not started: no claim yet
 ...
 */
const padRestLeadIn = (motion) => {
    const first = motion.keyframes[0].time;
    if (first <= 1e-9)
        return motion;
    const rest = (time, easing) => ({
        time,
 ...
 */
export const compilePerformance = (actions, synthesize) => {
    // 1. fan each action to every actor that performs it, grouped by body region
    const byActor = new Map();
    actions.forEach((action, index) => {
        const actors = typeof action.actor === "string" ? [action.actor] : action.actor;
        for (const actor of actors) {
            const base = synthesize(action, actor);
            if (base === null)
                continue; // no motion for this actor, skip
            // 2. repeat: concatenate the base cycle N times within the action's span
            const cycles = action.repeat !== undefined && action.repeat > 1 ? action.repeat : 1;
            const motion = cycles > 1
                ? sequenceMotion(`${base.id}:x${cycles}`, Array.from({ length: cycles }, () => base))
                : base;
            const region = actionRegion(action);
            const regions = byActor.get(actor) ??
                new Map();
    ...
    for (const [actor, regions] of byActor) {
        ...
        const regionClips = [...regions.entries()].flatMap(([region, placements]) => {
            const keepRoot = !layered || ROOT_REGIONS.has(region);
            const lanes = [];
            for (const placement of [...placements].sort((a, b) => a.start - b.start || a.action - b.action)) {
                ...
                const lane = lanes.find((candidate) => candidate.end <= item.start + 1e-9);
                if (lane === undefined)
                    lanes.push({
            ...
            return lanes.map((lane, index) => arrangeMotion(`perform:${actor}:${region}:${index}`, lane.placements));
        });
        performances[actor] = padRestLeadIn(regionClips.length === 1
    ...
    masked.sort((a, b) => a.action - b.action || compareCodeUnits(a.actor, b.actor));
    return { performances, masked };
};
//# sourceMappingURL=compilePerformance.js.map