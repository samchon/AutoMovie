 ...
 * @author Samchon
 */
export const scenePlacements = (scene) => new Map([
    ...scene.cameras.map((camera) => [camera.id, camera.transform.translation]),
    ...scene.nodes.map((node) => [node.id, node.transform.translation]),
]);
//# sourceMappingURL=scenePlacements.js.map