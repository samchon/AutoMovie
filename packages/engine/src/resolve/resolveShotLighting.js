import { LIGHT_CHANNEL_PROPERTIES, applyLightOverride, parseLightPointer, } from "./lightChannel";
 ...
 * @author Samchon
 */
export const resolveShotLighting = (input) => {
    ...
    for (const clip of input.clips)
        for (const { channel, value } of sampleClip(clip, input.seconds).values()) {
            const target = channel.kind === "pointer" ? parseLightPointer(channel.pointer) : null;
            if (target === null)
                throw new Error(`light clip "${clip.id}" track must address /lights/<id>/<property>`);
            const light = staged.get(target.light);
            if (light === undefined)
                throw new Error(`light clip "${clip.id}" addresses missing light "${target.light}"`);
            const property = LIGHT_CHANNEL_PROPERTIES[target.property];
            if (!property.carries(light.type))
                throw new Error(`light clip "${clip.id}" addresses "${target.property}", which a ${light.type} light does not carry`);
            const carried = overrides.get(target.light);
            const override = carried ?? {};
            if (carried === undefined)
                overrides.set(target.light, override);
...
//# sourceMappingURL=resolveShotLighting.js.map