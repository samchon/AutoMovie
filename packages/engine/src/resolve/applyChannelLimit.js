 ...
 * @author Samchon
 */
export const applyChannelLimit = (value, limit) => {
...
const validateFiniteValues = (value, key) => {
    for (let i = 0; i < value.length; ++i)
        if (!Number.isFinite(value[i]))
            throw new Error(`channel limit "${key}" value[${i}] must be finite, but was ${value[i]}`);
};
const validateFiniteBounds = (bounds, key, side, width) => {
    if (bounds === null)
        return;
    for (let i = 0; i < bounds.length; ++i) {
        ...
        if (bound === null)
            continue;
        if (i >= width)
            throw new Error(`channel limit "${key}" ${side}[${i}] is outside value width ${width}, but was ${bound}`);
        if (!Number.isFinite(bound))
            throw new Error(`channel limit "${key}" ${side}[${i}] must be finite, but was ${bound}`);
    }
};
const validateOrderedBounds = (min, max, key) => {
    ...
    for (let i = 0; i < length; ++i) {
        ...
        if (lo !== null && hi !== null && lo > hi)
            throw new Error(`channel limit "${key}" min[${i}] must be <= max[${i}], but was ${lo} > ${hi}`);
    }
};
//# sourceMappingURL=applyChannelLimit.js.map