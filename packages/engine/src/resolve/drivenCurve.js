 ...
 * @author Samchon
 */
export const evaluateDrivenCurve = (source, curve) => {
    validateDrivenCurve(curve);
    const points = curve.points;
    const x = source ?? points[0].source;
    if (x <= points[0].source)
...
const validateDrivenCurve = (curve) => {
    ...
    if (curve.points.length === 0)
        throw new Error("driven driver curve.points must contain at least one point");
    let previousSource = null;
    for (let i = 0; i < curve.points.length; ++i) {
        const point = curve.points[i];
        validateDrivenCurvePoint(i, point);
        if (previousSource !== null && point.source <= previousSource)
            throw new Error(`driven driver curve.points source values must be strictly increasing, but point ${i} was ${point.source} after ${previousSource}`);
        previousSource = point.source;
    }
};
const validateDrivenCurvePoint = (index, point) => {
    if (typeof point !== "object" || point === null || Array.isArray(point))
        throw new Error(`driven driver curve.points[${index}] point must be an object`);
...
//# sourceMappingURL=drivenCurve.js.map