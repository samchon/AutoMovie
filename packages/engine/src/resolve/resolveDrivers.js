...
const VALUE_DRIVER_TYPES = new Set(["copy", "driven"]);
const DEFERRED_DRIVER_TYPES = new Set([
    "aim",
    "ik",
    "parent",
    "spring",
 ...
 * @author Samchon
 */
export const resolveDrivers = (drivers, sampled, nodesById) => {
    const value = [];
    const deferred = [];
    for (const d of drivers) {
        const type = d.type;
        if (VALUE_DRIVER_TYPES.has(type))
            value.push(d);
        else if (DEFERRED_DRIVER_TYPES.has(type))
            deferred.push(d);
        else
            throw new Error(`unknown driver type "${String(type)}"`);
    }
    for (const d of topoSort(value))
        if (d.type === "copy")
            applyCopy(d, sampled, nodesById);
        else
            applyDriven(d, sampled);
    return deferred;
};
// dependency ordering
const trsKeys = (node, flags) => {
    const keys = [];
    if (flags.translation)
        keys.push(`node:${node}:translation`);
    if (flags.rotation)
        keys.push(`node:${node}:rotation`);
    if (flags.scale)
        keys.push(`node:${node}:scale`);
    return keys;
};
const outputsOf = (d) => d.type === "copy" ? trsKeys(d.owner, d) : [channelKey(d.output)];
const inputsOf = (d) => d.type === "copy" ? trsKeys(d.source, d) : [channelKey(d.source)];
/**
 ...
 */
const topoSort = (drivers) => {
    const outs = drivers.map(outputsOf);
    const ins = drivers.map(inputsOf);
    const deps = drivers.map((_, j) => drivers
        .map((__, i) => i)
        .filter((i) => i !== j && outs[i].some((k) => ins[j].includes(k))));
    const color = new Array(drivers.length).fill(0); // 0 white, 1 gray, 2 black
    const order = [];
    const visit = (j) => {
        if (color[j] === 2)
            return;
        if (color[j] === 1)
            throw new Error("driver dependency cycle");
        color[j] = 1;
        for (const i of deps[j])
            visit(i);
        color[j] = 2;
...
// copy
const applyCopy = (d, sampled, nodesById) => {
    ...
    if (d.translation)
        writeBlend(d, "translation", false, sampled, nodesById);
    if (d.rotation)
        writeBlend(d, "rotation", true, sampled, nodesById);
    if (d.scale)
        writeBlend(d, "scale", false, sampled, nodesById);
};
const validateCopyFlag = (value, label) => {
    if (typeof value !== "boolean")
        throw new Error(`copy driver ${label} flag must be boolean, but was ${value}`);
};
const validateCopyInfluence = (influence) => {
    if (!Number.isFinite(influence))
        throw new Error(`copy driver influence must be finite, but was ${influence}`);
    if (influence < 0)
        throw new Error(`copy driver influence must be between 0 and 1, but was ${influence}`);
    if (influence > 1)
        throw new Error(`copy driver influence must be between 0 and 1, but was ${influence}`);
};
const writeBlend = (d, path, isRotation, sampled, nodesById) => {
    ...
    setChannel(sampled, `node:${d.owner}:${path}`, { kind: "node", node: d.owner, path }, result);
};
/** Current value of a node TRS channel: the sampled override, else rest pose. */
const readTRS = (node, role, path, sampled, nodesById) => {
...
const validateCopySampledValue = (role, path, value) => {
    const expected = COPY_SAMPLE_WIDTHS[path];
    if (!Array.isArray(value))
        throw new Error(`copy driver ${role} ${path} value must be an array`);
    if (value.length !== expected)
        throw new Error(`copy driver ${role} ${path} value must contain exactly ${expected} entries, but had ${value.length}`);
};
...
// driven
const applyDriven = (d, sampled) => {
    validateDrivenOutputChannel(d.output, sampled);
    const src = sampled.get(channelKey(d.source));
    const source = src !== undefined ? readDrivenSourceValue(src.value) : undefined;
    if (source !== undefined)
        validateDrivenFinite("source value", source);
    const y = d.curve !== null && d.curve !== undefined
 ...
 */
const validateDrivenOutputChannel = (output, sampled) => {
    if (output.kind === "node") {
        if (output.path !== "weights")
            throw new Error(`driven driver output must be a scalar channel, but addressed node "${output.node}" ${output.path}; a width-1 write into a TRS channel would NaN-poison the composed world matrices`);
        const existing = sampled.get(channelKey(output));
        if (existing !== undefined && existing.value.length !== 1)
            throw new Error(`driven driver output "node:${output.node}:weights" must stay scalar-width, but the sampled weights carry ${existing.value.length} morph targets: a scalar write would silently narrow them`);
        return;
    }
    if (output.valueType !== "scalar")
        throw new Error(`driven driver output must be a scalar channel, but pointer "${output.pointer}" has valueType "${output.valueType}"`);
};
const readDrivenSourceValue = (value) => {
    validateDrivenSourceValue(value);
    return value[0];
};
const validateDrivenSourceValue = (value) => {
    if (!Array.isArray(value))
        throw new Error("driven driver source value must be an array");
    if (value.length !== 1)
        throw new Error(`driven driver source value must contain exactly 1 entry, but had ${value.length}`);
};
const validateDrivenClamp = (clamp) => {
    if (typeof clamp !== "boolean")
        throw new Error(`driven driver clamp must be boolean, but was ${clamp}`);
};
const validateDrivenRange = (inRange, outRange) => {
...
const validateDrivenRangeTuple = (label, range) => {
    if (!Array.isArray(range))
        throw new Error(`driven driver ${label} must be an array`);
    if (range.length !== 2)
        throw new Error(`driven driver ${label} must contain exactly 2 entries, but had ${range.length}`);
};
const validateDrivenFinite = (label, value) => {
    if (!Number.isFinite(value))
        throw new Error(`driven driver ${label} must be finite, but was ${value}`);
};
const remapDriven = (d, source) => {
    const { inRange, outRange } = d;
    if (inRange === undefined || outRange === undefined)
        throw new Error("driven driver without a curve requires inRange and outRange");
    validateDrivenRange(inRange, outRange);
    if (d.clamp !== undefined)
        validateDrivenClamp(d.clamp);
    return remap(source ?? inRange[0], inRange, outRange, d.clamp ?? false);
};
const remap = (x, [i0, i1], [o0, o1], clamp) => {
    const t = i1 === i0 ? 0 : (x - i0) / (i1 - i0);
    const tc = clamp ? Math.min(1, Math.max(0, t)) : t;
    return o0 + (o1 - o0) * tc;
};
// shared
const setChannel = (sampled, key, channel, value) => {
    const existing = sampled.get(key);
    if (existing !== undefined)
        existing.value = value;
    else
        sampled.set(key, { channel, value });
};
const lerpArray = (a, b, t) => a.map((v, i) => v + (b[i] - v) * t);
const slerpArray = (a, b, t) => {
    const q = Quaternion.slerp({ x: a[0], y: a[1], z: a[2], w: a[3] }, { x: b[0], y: b[1], z: b[2], w: b[3] }, t);
    return [q.x, q.y, q.z, q.w];
};
//# sourceMappingURL=resolveDrivers.js.map