...
import { clipDurationFault, clipLoopFault, clipTrackShapeFaults, } from "../validation/clipTrackShape";
 ...
 * @author Samchon
 */
export const sampleClip = (clip, seconds) => {
    ...
    for (const track of clip.tracks) {
        ...
        if (out.has(key))
            throw new Error(`duplicate track channel "${key}"`);
        out.set(key, {
 ...
 * @author Samchon
 */
export const sampleClipSequence = (clips, seconds) => {
    if (!Number.isFinite(seconds))
        throw new Error(`sampleClipSequence seconds must be finite, but was ${seconds}`);
    const sampledByClip = new Map(clips.map((clip) => [clip, sampleClip(clip, seconds)]));
 ...
 * tangents.
 */
const sampleTrack = (track, time, duration) => {
    ...
    if (interpolation === "step")
        return localT >= 1 ? valueAt(hi) : valueAt(lo);
    if (cubic)
        return cubicHermite(track, lo, hi, span, localT, width, stride);
 ...
 * sphere).
 */
const cubicHermite = (track, lo, hi, span, t, width, stride) => {
 ...
 * would reach it (#1353).
 */
const validateSampleTime = (seconds, duration, loop) => {
    if (!Number.isFinite(seconds))
    ...
    if (fault !== null)
        throw new Error(`sampleClip clip ${fault.message}`);
};
/**
 ...
 */
const validateTrackShape = (track, key, duration) => {
    const fault = clipTrackShapeFaults(track, duration)[0];
    if (fault !== undefined)
        throw new Error(`track "${key}" ${fault.message}`);
};
...
//# sourceMappingURL=sampleClip.js.map