...
export const sceneToNodes = (props) => {
    ...
    for (const placement of scene.nodes) {
        ...
        if (models === undefined && propSpecs === undefined)
            continue;
        const spec = propSpecs?.[placement.model];
        const model = models?.[placement.model];
        if (spec !== undefined && model !== undefined)
            throw new Error(`sceneToNodes scene "${scene.id}" resolves model "${placement.model}" at node "${placement.id}" in BOTH the props and models registries: the registries contradict (a forged prop carries its own model)`);
        if (spec !== undefined) {
            if (spec.articulation !== null)
                nodes.push(...lowerArticulationNodes(spec.articulation, `${placement.id}/`, placement.id));
            continue;
        }
        if (model === undefined && allowPartialModels)
            continue;
        if (model === undefined)
            throw new Error(`sceneToNodes scene "${scene.id}" places model "${placement.model}" at node "${placement.id}" but the registry has no such model`);
        if (model.skeleton === null)
            continue;
        nodes.push(...lowerSkeletonNodes({
 ...
 */
const lowerArticulationNodes = (articulation, prefix, rootParent) => articulation.nodes.map((node) => ({
    ...node,
    id: `${prefix}${node.id}`,
    parent: node.parent === null ? rootParent : `${prefix}${node.parent}`,
}));
//# sourceMappingURL=sceneToNodes.js.map