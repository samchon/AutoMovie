 ...
 * hurting determinism.
 */
const FALLBACK_DIR = { x: 1, y: 0, z: 0 };
/**
 ...
 * @author Samchon
 */
export const applyIterativeIK = (d, world, localById, childrenById) => {
    validateInfluence("iterative IK", d.influence);
    const iterations = readIterations(d.iterations);
    if (d.chain.length < 2)
        throw new Error(`world driver iterative IK chain needs at least 2 nodes, but had ${d.chain.length}`);
    const matrices = d.chain.map((id) => readWorld(world, id, "iterative IK chain"));
    ...
    for (let i = 0; i + 1 < before.length; ++i) {
        ...
        if (!(length > 0))
            throw new Error(`world driver iterative IK segment ${i} length must be > 0, but was ${length}`);
        lengths.push(length);
    }
    const goal = Matrix4.position(readWorld(world, d.goal, "iterative IK goal"));
    const after = d.solver === "ccd"
...
const readIterations = (iterations) => {
    if (iterations === null)
        return DEFAULT_ITERATIONS;
    if (!Number.isInteger(iterations) || iterations <= 0)
        throw new Error(`world driver iterative IK iterations must be a positive integer, but was ${iterations}`);
    return iterations;
};
/**
 ...
 */
const solveCcd = (before, goal, iterations) => {
    ...
    for (let it = 0; it < iterations; ++it) {
        ...
        for (let j = tip - 1; j >= 0; --j) {
            ...
            const rot = rotationBetween(Vector3.normalize(Vector3.subtract(p[tip], p[j])), Vector3.normalize(Vector3.subtract(goal, p[j])));
            for (let k = j + 1; k <= tip; ++k)
                p[k] = Vector3.add(p[j], Quaternion.rotateVector(rot, Vector3.subtract(p[k], p[j])));
        }
    }
    return p;
};
 ...
 * cannot touch".
 */
const solveFabrik = (before, lengths, goal, iterations) => {
    const p = before.map((v) => ({ ...v }));
    const tip = p.length - 1;
    const root = { ...p[0] };
    ...
    for (let it = 0; it < iterations; ++it) {
        ...
        p[tip] = { ...goal };
        for (let j = tip - 1; j >= 0; --j)
            p[j] = Vector3.add(p[j + 1], Vector3.scale(unitOr(Vector3.subtract(p[j], p[j + 1])), lengths[j]));
        // forward: pin the root home, walk to the tip
        p[0] = { ...root };
        for (let j = 1; j <= tip; ++j)
            p[j] = Vector3.add(p[j - 1], Vector3.scale(unitOr(Vector3.subtract(p[j], p[j - 1])), lengths[j - 1]));
    }
    return p;
};
/** Unit vector of `v`, or the deterministic fallback when `v` is degenerate. */
const unitOr = (v) => Vector3.length(v) < 1e-12 ? FALLBACK_DIR : Vector3.normalize(v);
 ...
 */
const writeChain = (d, world, matrices, before, after, childrenById, localById) => {
    const t = d.influence;
    const tip = d.chain.length - 1;
    for (let j = 0; j <= tip; ++j) {
        const dec = Matrix4.decompose(matrices[j]);
        const rotation = j < tip
            ? Quaternion.slerp(dec.rotation, Quaternion.multiply(rotationBetween(unitOr(Vector3.subtract(before[j + 1], before[j])), unitOr(Vector3.subtract(after[j + 1], after[j]))), dec.rotation), t)
            : dec.rotation;
        world.set(d.chain[j], Matrix4.compose(blendVec(before[j], after[j], t), rotation, dec.scale));
    }
    recompose(d.chain[tip], world, localById, childrenById);
};
//# sourceMappingURL=iterativeIK.js.map