...
const indexNodes = (nodes) => {
    ...
    nodes.forEach((node, index) => {
        ...
        if (existing !== undefined)
            throw new Error(`node id "${node.id}" is duplicated at nodes[${index}].id; first declared at nodes[${existing.index}].id`);
        byId.set(node.id, { node, index });
    });
    nodes.forEach((node, index) => {
        if (node.parent !== null && !byId.has(node.parent))
            throw new Error(`node "${node.id}" references missing parent "${node.parent}" at nodes[${index}].parent`);
    });
    return byId;
};
/**
 ...
 * @author Samchon
 */
export const composeScene = (nodes, localOverrides) => {
...
//# sourceMappingURL=composeScene.js.map