...
export const twoBoneChainArticulation = (props) => {
    ...
    const upperDir = Quaternion.rotateVector(Quaternion.fromAxisAngle(normal, lift), axis);
    ...
    const upper = Quaternion.multiply(Quaternion.inverse(rsu), Quaternion.multiply(du, rsu));
    ...
    const localGoal = Quaternion.rotateVector(rslInv, Quaternion.rotateVector(Quaternion.inverse(du), foreDir));
    const lower = aimRotation(localFore, localGoal);
    return { upper, lower };
};
//# sourceMappingURL=twoBoneChainArticulation.js.map