...
export const gazeChainJoints = (props) => {
    const declared = (bone) => props.rig?.bones.find((entry) => entry.bone === bone) ?? null;
    const headBone = declared(HEAD);
    const neckBone = declared(NECK);
    const onlyHead = (flexion, twist) => [
...
//# sourceMappingURL=gazeChain.js.map