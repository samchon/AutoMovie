...
import { toRigAngle } from "../rom/restFrame";
const JOINT_AXES = ["flexion", "abduction", "twist"];
...
export const DEFAULT_JOINT_AXES = {
    flexion: { x: 1, y: 0, z: 0 },
    abduction: { x: 0, y: 0, z: 1 },
    twist: { x: 0, y: 1, z: 0 },
};
const readAngle = (joint, axis, frame) => {
    const value = joint[axis];
    if (value !== null && !Number.isFinite(value))
        throw new Error(`jointToQuaternion ${axis} must be finite or null, but was ${value}`);
 ...
 * @author Samchon
 */
export const validateJointAxesBasis = (axes, path) => {
    const issues = [];
 ...
 * @author Samchon
 */
export const normalizeJointAxes = (axes, path) => {
 ...
 * @author Samchon
 */
export const jointToQuaternion = (joint, axes = DEFAULT_JOINT_AXES, frame) => {
    const basis = normalizeJointAxes(axes, "jointToQuaternion axes");
    const qFlexion = Quaternion.fromAxisAngle(basis.flexion, readAngle(joint, "flexion", frame?.flexion));
    const qAbduction = Quaternion.fromAxisAngle(basis.abduction, readAngle(joint, "abduction", frame?.abduction));
    const qTwist = Quaternion.fromAxisAngle(basis.twist, readAngle(joint, "twist", frame?.twist));
    return Quaternion.multiply(qTwist, Quaternion.multiply(qAbduction, qFlexion));
};
//# sourceMappingURL=jointToQuaternion.js.map