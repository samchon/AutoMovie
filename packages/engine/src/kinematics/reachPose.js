...
import { clinicalDeviation, hingedArmArticulation, jointRomOvershoot, } from "./hingedArmArticulation";
 ...
 * @author Samchon
 */
export const reachPose = (skeleton, side, target, restFrames = HUMANOID_REST_FRAME) => {
    ...
    if (armChainFault(skeleton, side) !== null)
        return null;
    ...
    const jointsOf = (upperDelta, lowerDelta) => [
        {
            bone: upperName,
    ...
    const articulation = hingedArmArticulation({
        ...
        score: (upperDelta, lowerDelta) => {
            ...
            return {
                overshoot: jointRomOvershoot(upperJoint, upperConstraint) +
                    jointRomOvershoot(lowerJoint, lowerConstraint),
                deviation: clinicalDeviation(upperJoint) + clinicalDeviation(lowerJoint),
            };
        },
    });
...
//# sourceMappingURL=reachPose.js.map