 ...
 * @author Samchon
 */
export const armChainFault = (skeleton, side, jointAxes = HUMANOID_JOINT_AXES) => {
    ...
    if (hingeLength < 1e-6)
        return null;
    const sine = Vector3.length(Vector3.cross(Vector3.scale(flexion, 1 / hingeLength), Vector3.scale(fore, 1 / foreLength)));
    if (sine >= PARALLEL_SINE)
        return null;
    return {
        side,
        bone: lowerName,
        reason: `${lowerName} cannot bend this arm: its flexion axis (${axisText(flexion)}) is parallel to the ` +
...
const axisText = (axis) => `${trim(axis.x)}, ${trim(axis.y)}, ${trim(axis.z)}`;
/** Six significant digits, trailing zeros dropped: a readable, stable number. */
const trim = (value) => String(Number(value.toPrecision(6)));
//# sourceMappingURL=armChainFault.js.map