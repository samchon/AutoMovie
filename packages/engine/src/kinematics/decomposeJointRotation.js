...
import { toClinicalAngle } from "../rom/restFrame";
import { DEFAULT_JOINT_AXES, normalizeJointAxes, } from "./jointToQuaternion";
const RAD2DEG = 180 / Math.PI;
const QUATERNION_AXES = ["x", "y", "z", "w"];
const assertFiniteQuaternion = (q) => {
    for (const axis of QUATERNION_AXES) {
        const value = q[axis];
        if (!Number.isFinite(value))
            throw new Error(`decomposeJointRotation quaternion.${axis} must be finite, but was ${value}`);
    }
};
/**
 ...
 * @author Samchon
 */
export const decomposeJointRotation = (q, axes = DEFAULT_JOINT_AXES, frame) => {
    ...
    const handed = Vector3.dot(basis.twist, Vector3.cross(basis.flexion, basis.abduction)) >= 0
        ? 1
        : -1;
...
//# sourceMappingURL=decomposeJointRotation.js.map