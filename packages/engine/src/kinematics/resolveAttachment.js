 ...
 * @author Samchon
 */
export const resolveAttachment = (parentPose, parentSkeleton, attachment, jointAxes, restFrames) => {
    const resolved = resolvePose(parentPose, parentSkeleton, jointAxes, restFrames);
    const seat = resolved.find((r) => r.bone === attachment.parentBone);
    if (seat === undefined)
        throw new Error(`resolveAttachment: parent bone "${attachment.parentBone}" is not in the skeleton`);
    const off = attachment.offset;
    return {
        translation: Vector3.add(seat.worldPosition, Quaternion.rotateVector(seat.worldRotation, off.translation)),
        rotation: Quaternion.multiply(seat.worldRotation, off.rotation),
        scale: off.scale,
    };
};
//# sourceMappingURL=resolveAttachment.js.map