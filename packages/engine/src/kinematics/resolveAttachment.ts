 ...
 *
 * - `translation = boneWorldPos + boneWorldRot 쨌 offset.translation`
 * - `rotation = boneWorldRot ??offset.rotation`
 *
 * So the child inherits both where the bone is and how it is turned: as a horse
 * rears and its chest pitches back, the seat ??and the rider locked to it ?? * pitch with it, exactly as a physics fixed-joint would carry one body on
