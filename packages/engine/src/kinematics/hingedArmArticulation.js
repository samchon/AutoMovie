...
export const hingedArmArticulation = (props) => {
    ...
    const rotated = (vector) => Quaternion.rotateVector(props.midRest, vector);
    const k = Vector3.dot(midOffset, midOffset) +
    ...
    const branches = ratio >= 1
        ? [phase]
        : ratio <= -1
            ? [phase + Math.PI]
            : [phase + Math.acos(ratio), phase - Math.acos(ratio)];
    const goal = Quaternion.rotateVector(Quaternion.inverse(props.upper.worldRotation), reach);
    let best = null;
    for (const theta of branches) {
        const lower = Quaternion.fromAxisAngle(hinge, theta * RAD2DEG);
        const mid = Vector3.add(midOffset, rotated(Quaternion.rotateVector(lower, endOffset)));
        const aim = aimRotation(mid, goal);
        const swivelAxis = Vector3.normalize(mid);
        for (let step = 0; step * SWIVEL_STEP < 360; ++step) {
            const upper = Quaternion.multiply(aim, Quaternion.fromAxisAngle(swivelAxis, step * SWIVEL_STEP));
 ...
 * @author Samchon
 */
export const jointRomOvershoot = (joint, constraint) => {
    if (constraint === null)
        return 0;
    ...
    (total, item) => total + (item.overshoot ?? Number.POSITIVE_INFINITY), 0);
};
/** Squared clinical deviation from the anatomical neutral, in degrees². */
export const clinicalDeviation = (joint) => joint.flexion * joint.flexion +
    joint.abduction * joint.abduction +
    joint.twist * joint.twist;
//# sourceMappingURL=hingedArmArticulation.js.map