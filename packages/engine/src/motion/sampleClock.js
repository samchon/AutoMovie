...
export const windowSampleTimes = (start, end, sampleRate) => {
    const frames = Math.max(1, Math.ceil((end - start) * sampleRate));
    const times = Array.from({ length: frames + 1 }, (_, index) => Math.min(end, start + index / sampleRate));
...
export const sampleTimes = (duration, sampleRate) => windowSampleTimes(0, duration, sampleRate);
//# sourceMappingURL=sampleClock.js.map