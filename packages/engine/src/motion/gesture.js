...
function jumpPose(skeleton, rootY, legFlex) {
 ...
 * @author Samchon
 */
export const gestureMotion = (id, skeleton, kind, duration) => {
    if (kind === "jump") {
        const keyframes = JUMP_STOPS.map(([fraction, rootY, legFlex]) => ({
    ...
    const keyframes = shape.map(([fraction, joints]) => {
        const pose = { skeleton, root: null, joints };
        return {
            time: fraction * duration,
            pose,
            expression: null,
