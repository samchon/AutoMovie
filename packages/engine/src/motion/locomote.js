...
export const classifyLocomoteGroundDisplacement = (displacement) => {
    ...
    return {
        ...
        travelDistance,
        alreadyThere,
        verticalOnly: alreadyThere === false && groundDistance < LOCOMOTE_GROUND_EPSILON,
    };
};
const assertFiniteVector = (label, vector) => {
 ...
 * @author Samchon
 */
export const locomoteMotion = (id, gait, distance, speed, direction, faceTravel = false) => {
    ...
    const facing = faceTravel
        ? Quaternion.fromAxisAngle({ x: 0, y: 1, z: 0 }, (Math.atan2(heading.x, heading.z) * 180) / Math.PI)
        : undefined;
    return travelMotion(id, base, cycles, velocity, facing);
};
//# sourceMappingURL=locomote.js.map