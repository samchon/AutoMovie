 ...
 * @author Samchon
 */
export const sequenceMotion = (id, parts, loop = false) => {
    if (parts.length === 0)
        throw new Error("sequence parts must not be empty");
    ...
    const continuous = first !== null &&
        parts.every((part) => (part.gaitCycle ?? null) !== null &&
            part.gaitCycle.period === first.period &&
            part.gaitCycle.phaseAt === first.phaseAt &&
            Math.abs(part.duration / first.period -
                Math.round(part.duration / first.period)) < 1e-9);
    return {
        id,
        skeleton,
        duration: offset,
