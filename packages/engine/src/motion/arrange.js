 ...
 * @author Samchon
 */
export const holdMotion = (id, skeleton, pose, duration) => {
 ...
 * @author Samchon
 */
export const arrangeMotion = (id, placements) => {
    for (const placement of placements) {
        ...
        if (placement.motion.keyframes.length === 0)
            throw new Error(`motion placement "${placement.motion.id}" must have keyframes`);
        if (!Number.isFinite(placement.motion.duration) ||
            placement.motion.duration <= 0)
            throw new Error(`motion placement "${placement.motion.id}" duration must be finite and positive`);
    }
    const sorted = [...placements].sort((a, b) => a.start - b.start);
    const keyframes = [];
    const push = (k) => {
        const last = keyframes[keyframes.length - 1];
        if (last !== undefined && k.time <= last.time)
    ...
    const gaitCycle = lastCycle === null
        ? null
        : {
            period: lastCycle.period,
            phaseAt: addPositiveModulo(lastCycle.phaseAt, -last.start, lastCycle.period),
        };
    return {
        id,
