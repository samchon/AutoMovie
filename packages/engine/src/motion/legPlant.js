...
import { DEFAULT_JOINT_AXES, jointToQuaternion, normalizeJointAxes, } from "../kinematics/jointToQuaternion";
import { indexSkeletonTopology, resolvePose, } from "../kinematics/resolvePose";
...
export const HUMANOID_LEG_CHAINS = [
...
export const rekeyPlantedFeet = (props) => {
    ...
    props.times.forEach((time, index) => {
        ...
        const pose = {
            ...
            joints: plantedJoints(props.skeleton, sampled.pose, props.legs, props.targets[index], topology, prior, props.jointAxes, props.restFrames),
        };
        keyframes.push({
            time,
            pose,
...
export const assemblePlantedFeet = (motion, keyframes, plants) => ({
    motion: {
        ...motion,
        keyframes,
    },
 ...
 */
export const resolveBoneMap = (skeleton, pose, topology = indexSkeletonTopology(skeleton), jointAxes, restFrames) => new Map(resolvePose(pose, skeleton, jointAxes, restFrames, topology).map((bone) => [
    bone.bone,
    bone,
]));
/**
...
export const fitChainToTarget = (props) => {
    ...
    if (prepared === null)
        return props.pose;
    const solve = (bendNormal) => solvePreparedChainPlant({
        prepared,
        target: props.target,
        bendNormal,
    ...
    if (pole === null)
        return props.pose;
    const authored = resolveBoneMap(props.skeleton, props.pose, props.topology, props.jointAxes, props.restFrames).get(props.chain.effector).worldPosition;
    const distance = (position) => Vector3.length(Vector3.subtract(position, props.target));
    const reference = props.referencePose ?? props.pose;
    const referenceAngles = new Map(reference.joints.map((joint) => [joint.bone, joint]));
    const authoredAngles = new Map(props.pose.joints.map((joint) => [joint.bone, joint]));
    const rotationDistance = (bone, candidate) => jointRotationDistance(candidate ?? {
        bone,
        flexion: null,
        abduction: null,
        twist: null,
    }, referenceAngles.get(bone), props.jointAxes?.[bone], props.restFrames?.[bone]);
    let best = {
        pose: props.pose,
        residual: distance(authored),
        continuity: props.referencePose === undefined
            ? 0
            : rotationDistance(props.chain.upper, authoredAngles.get(props.chain.upper)) +
                rotationDistance(props.chain.lower, authoredAngles.get(props.chain.lower)),
    };
    const consider = (solved) => {
        const upper = clampJointToSkeleton(solved.upper, props.skeleton);
        const lower = clampJointToSkeleton(solved.lower, props.skeleton);
        const candidate = {
            ...props.pose,
            joints: [
                ...props.pose.joints.filter((joint) => joint.bone !== props.chain.upper &&
                    joint.bone !== props.chain.lower),
                upper,
                lower,
            ],
        };
        const candidateResidual = distance(resolvedPreparedEffector({
            prepared,
            upper,
            lower,
            jointAxes: props.jointAxes,
        ...
        const continuity = jointRotationDistance(upper, referenceAngles.get(upper.bone), props.jointAxes?.[upper.bone], props.restFrames?.[upper.bone]) +
            jointRotationDistance(lower, referenceAngles.get(lower.bone), props.jointAxes?.[lower.bone], props.restFrames?.[lower.bone]);
    ...
    const canFinish = () => plantResidualBucket(best.residual) === 0 &&
    ...
    const reachAxis = Vector3.normalize(Vector3.subtract(props.target, upper.worldPosition));
    let primary = Vector3.subtract(pole.hinge, Vector3.scale(reachAxis, Vector3.dot(pole.hinge, reachAxis)));
    ...
    const normalAt = (angle) => Vector3.add(Vector3.scale(primary, Math.cos(angle)), Vector3.scale(secondary, Math.sin(angle)));
    if (props.referencePose !== undefined) {
        const referenceJoints = [props.chain.upper, props.chain.lower].map((bone) => referenceAngles.get(bone) ?? {
            bone,
            flexion: null,
            abduction: null,
            twist: null,
        });
        const referenceLower = resolveBoneMap(props.skeleton, {
            ...props.pose,
            joints: [
                ...props.pose.joints.filter((joint) => joint.bone !== props.chain.upper &&
                    joint.bone !== props.chain.lower),
        ...
        }, props.topology, props.jointAxes, props.restFrames).get(props.chain.lower);
        const referenceNormal = Vector3.cross(reachAxis, Vector3.subtract(referenceLower.worldPosition, upper.worldPosition));
    ...
    for (const minimum of minima) {
        ...
        for (let iteration = 0; iteration < 10; ++iteration) {
            ...
            for (const angle of [center.angle - step, center.angle + step]) {
                ...
                if (comparePlantCandidate(candidate, center) < 0)
                    center = candidate;
                if (canFinish())
                    return best.pose;
            }
...
const plantResidualBucket = (residual) => Math.floor(residual / PLANT_RESIDUAL_EPSILON);
/** Residual buckets are ordered first; continuity breaks equivalent pins. */
const comparePlantCandidate = (left, right) => {
    const residualOrder = plantResidualBucket(left.residual) - plantResidualBucket(right.residual);
    if (residualOrder !== 0)
...
const jointRotationDistance = (joint, reference, axes, restFrame) => {
    const candidate = Quaternion.normalize(jointToQuaternion(joint, axes, restFrame));
    const prior = Quaternion.normalize(jointToQuaternion(reference ?? {
        bone: joint.bone,
        flexion: null,
        abduction: null,
        twist: null,
    }, axes, restFrame));
    const dot = Math.min(1, Math.abs(candidate.x * prior.x +
        candidate.y * prior.y +
        candidate.z * prior.z +
        candidate.w * prior.w));
 ...
 * through unchanged.
 */
const plantedJoints = (skeleton, pose, legs, targets, topology, referencePose, jointAxes, restFrames) => {
...
const prepareChainPlant = (props) => {
    ...
    const zeroed = {
        ...
        joints: props.pose.joints.filter((j) => j.bone !== chain.upper && j.bone !== chain.lower),
    };
    const map = resolveBoneMap(props.skeleton, zeroed, props.topology, props.jointAxes, props.restFrames);
    ...
    if (upper === undefined || lower === undefined || effector === undefined)
        return null;
    if (isDescendant(props.topology, chain.upper, chain.lower) === false ||
    ...
    return {
        ...
        hinge: Quaternion.rotateVector(lower.worldRotation, normalizeJointAxes(props.jointAxes?.[chain.lower] ?? DEFAULT_JOINT_AXES, "solveChainPlant axes").flexion),
        lowerOffset: Quaternion.rotateVector(upperInverse, Vector3.subtract(lower.worldPosition, upper.worldPosition)),
        lowerRotation: Quaternion.multiply(upperInverse, lower.worldRotation),
        effectorOffset: Quaternion.rotateVector(lowerInverse, Vector3.subtract(effector.worldPosition, lower.worldPosition)),
    };
};
/** Whether a reachable bone sits strictly below another in the rig tree. */
const isDescendant = (topology, ancestor, descendant) => (topology.childrenByParent.get(ancestor) ?? []).some((child) => child.bone === descendant ||
...
const solvePreparedChainPlant = (props) => {
    ...
    return {
        ...
        upper: {
            bone: chain.upper,
            ...decomposeJointRotation(articulation.upper, props.jointAxes?.[chain.upper], props.restFrames?.[chain.upper]),
        },
        lower: {
            bone: chain.lower,
            ...decomposeJointRotation(articulation.lower, props.jointAxes?.[chain.lower], props.restFrames?.[chain.lower]),
        },
    };
};
/** FK only the prepared chain after its two candidate joints are clamped. */
const resolvedPreparedEffector = (props) => {
    const upperRotation = Quaternion.multiply(props.prepared.upper.worldRotation, jointToQuaternion(props.upper, props.jointAxes?.[props.prepared.chain.upper], props.restFrames?.[props.prepared.chain.upper]));
    const lowerPosition = Vector3.add(props.prepared.upper.worldPosition, Quaternion.rotateVector(upperRotation, props.prepared.lowerOffset));
    const lowerRotation = Quaternion.multiply(Quaternion.multiply(upperRotation, props.prepared.lowerRotation), jointToQuaternion(props.lower, props.jointAxes?.[props.prepared.chain.lower], props.restFrames?.[props.prepared.chain.lower]));
    return Vector3.add(lowerPosition, Quaternion.rotateVector(lowerRotation, props.prepared.effectorOffset));
};
//# sourceMappingURL=legPlant.js.map