 ...
 */
export const formationSlot = (formation, slot) => {
    const actor = formation.heroOverrides.find((hero) => hero.slot === slot)?.actor ?? null;
    return {
        slot,
        node: actor ??
 ...
 */
export const formationSlotPosition = (formation, slot) => {
    if (Number.isSafeInteger(slot) === false ||
        slot < 0 ||
        slot >= formation.count)
        throw new RangeError(`Formation "${formation.id}" slot ${slot} is outside 0..${formation.count - 1}.`);
 ...
 */
export const formationGroundRelief = (formation, point) => {
 ...
 */
const formationGroundDatum = (formation, surfaces) => {
 ...
 */
export const selectFormationLod = (input) => {
    ...
    const matchedIndex = input.lod.findIndex((lod) => lod.maxDistance === null || effectiveDistance <= lod.maxDistance);
    const desiredIndex = matchedIndex < 0 ? input.lod.length - 1 : matchedIndex;
    const previousIndex = input.lod.findIndex((lod) => lod.tier === input.previous);
...
export const sampleFormationMotion = (motions, formation, time) => {
    const cues = motions
    ...
    for (const cue of cues) {
        if (time < cue.start)
            return retained;
        if (time < cue.end) {
            const progress = easingProgress(cue.easing, Math.max(0, Math.min(1, (time - cue.start) / (cue.end - cue.start))));
            return {
                translation: {
                ...
                facingOffsetDeg: lerp(cue.from.facingOffsetDeg, cue.to.facingOffsetDeg, progress),
                spacingScale: {
                    lateral: lerp(cue.from.spacingScale.lateral, cue.to.spacingScale.lateral, progress),
                    depth: lerp(cue.from.spacingScale.depth, cue.to.spacingScale.depth, progress),
                },
            };
        }
        retained = cue.to;
...
export const transformFormationPoint = (point, anchor, motion, baseFacingDeg = 0) => {
    ...
    const localX = (deltaX * baseCosine - deltaZ * baseSine) * motion.spacingScale.lateral;
    const localZ = (deltaX * baseSine + deltaZ * baseCosine) * motion.spacingScale.depth;
 ...
 */
export const transformFormationBounds = (bounds, anchor, motion, baseFacingDeg = 0) => {
    const corners = [bounds.min.x, bounds.max.x].flatMap((x) => [bounds.min.y, bounds.max.y].flatMap((y) => [bounds.min.z, bounds.max.z].map((z) => transformFormationPoint({ x, y, z }, anchor, motion, baseFacingDeg))));
    return {
        min: {
 ...
 * source-owned.
 */
export const composeFormationHeroTransform = (base, source, anchor, motion, baseFacingDeg = 0) => ({
    translation: Vector3.add(transformFormationPoint(base.translation, anchor, motion, baseFacingDeg), Vector3.subtract(source.translation, base.translation)),
    rotation: Quaternion.multiply(Quaternion.fromAxisAngle({ x: 0, y: 1, z: 0 }, baseFacingDeg + motion.facingOffsetDeg), Quaternion.multiply(Quaternion.inverse(base.rotation), source.rotation)),
    scale: { ...source.scale },
});
/**
 ...
 */
const dressedFormationPoint = (formation, slot, point) => {
    const dressing = "dressing" in formation.layout ? formation.layout.dressing : undefined;
    if (dressing === undefined)
        return point;
    const deviation = (salt, bound) => bound === 0 ? 0 : (seededValue(formation.seed, slot, salt) * 2 - 1) * bound;
    return {
        x: point.x + deviation(0x64726573, dressing.lateral),
        z: point.z + deviation(0x73646570, dressing.depth),
    };
};
const localFormationPoint = (formation, slot) => {
    const layout = formation.layout;
    if (layout.kind === "line" || layout.kind === "column") {
        const rank = layout.kind === "line"
            ? Math.floor(slot / layout.files)
            : slot % layout.ranks;
        const file = layout.kind === "line"
            ? slot % layout.files
    ...
    const radius = Math.sqrt(seededValue(formation.seed, layout.seed, slot, 0)) *
        layout.radius;
...
export const lerp = (from, to, progress) => from * (1 - progress) + to * progress;
/**
 ...
 */
export const easingProgress = (easing, progress) => {
    switch (easing) {
        case "linear":
            return progress;
        case "easeIn":
