...
| `IAutoMovieBenchmarkSubmissionDraft` / `sealAutoMovieBenchmarkSubmission` | run 산출물을 검증·정렬하고 내용 주소 `runId`를 붙여 **깊게 freeze**한다. scorer가 archive를 고치면 조용히 재채점되는 대신 실패한다. |
