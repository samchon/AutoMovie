...
The observer performs no automatic retry or resume and starts no subagents of its own. Interrupting it or losing a log/receipt writer stops only its directly owned child handle and prevents successful completion. It does not find or terminate unrelated processes; independently detached descendants remain subject to the run's ownership and cleanup procedure. Keep the output directory within the run's declared privacy and retention boundary.
