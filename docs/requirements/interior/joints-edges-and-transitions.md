...
Movement, expansion, settlement와 operable separation을 허용하는 joint는 initial gap, allowed direction와 range, open·closed state, seal 또는 cover와 adjacent fastening을 가져야 한다. 주변 assembly가 허용 motion을 막거나 movement range에서 방수·음향 continuity가 끊기면 named finding을 남겨야 한다.
...
검증은 nominal dimension, fabrication·installation tolerance, authored gap variation와 current motion state를 구분하고 joint path의 discontinuity, flipped profile, corner clash와 pattern cut consequence를 실제 resolved geometry에서 보고해야 한다.
