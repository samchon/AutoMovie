...
Client 등록 계획은 기존 `.mcp.json`과 `.codex/config.toml`을 입력으로 받아 publication할 후보만 계산한다. Claude의 `mcpServers.automovie_reference`는 `type: stdio`와 정확한 installed-bin/root 관계만 가진 생성 schema일 때만 갱신한다. 기록된 Node executable은 그 등록을 publication한 machine의 사실이므로 소유 판정에 쓰지 않고 현재 값으로 갱신한다. 다른 option을 추가하거나 args 구조를 바꾸거나 args의 첫 항목이 기록된 root 아래의 installed bin이 아닌 entry는 충돌이다. 기록된 root가 옮겨진 것은 갱신 사유다. Codex의 `mcp_servers.automovie_reference`는 같은 args 구조에 `cwd`를 root와 같게 결속하고 digest가 유지된 managed block만 갱신하며, 그 block이 기록한 command도 현재 값으로 갱신한다. Marker는 이 toolchain이 그 span을 썼다는 증거이므로 marker가 없는 entry는 갱신하지 않는다. 기록된 Node executable을 제외하고 동일한 entry는 원문을 보존하며, 그 밖이 다른 entry만 거부한다. 유지되는 거부의 message는 실행 가능한 복구를 함께 말한다. 독립적으로 해석한 block과 전체 문서의 소유 entry가 같고 block을 제거하면 그 entry도 사라지는지 확인하여 사용자 문자열 안의 marker를 편집 영역으로 오인하지 않는다. 관련 없는 JSON 값과 TOML 원문 bytes는 보존한다. 유효한 TOML table에 등록을 추가할 수 없는 충돌은 기존 문서의 parse 오류와 구별한다. 소유권 충돌의 해소와 기록된 Node executable의 갱신은 참조 읽기의 부수 효과가 아니라 명시적인 등록 publication에서만 일어난다.
