...
<!-- @evidence requirements/agent-authoring/mcp-boundary.md#agent-mcp-host-evidence 이 출력이 actual project와 target 실행에서 나온 digest-bound receipt를 요구한다. -->
...
<!-- @evidence requirements/agent-authoring/mcp-boundary.md#agent-mcp-authoring-api-refusal 이 불변식이 tool call로 source 밖 장면 상태를 누적 편집하지 못하게 한다. -->
...
<!-- @evidence requirements/agent-authoring/mcp-boundary.md#agent-mcp-content-supply-refusal 이 불변식이 host가 시대별 완성 콘텐츠를 대신 선택해 주지 못하게 한다. -->
<!-- @evidence requirements/agent-authoring/mcp-boundary.md#agent-mcp-no-surprise-external-effects 이 불변식이 knowledge와 evidence 요청만으로 외부 upload나 generation을 시작하지 못하게 한다. -->
