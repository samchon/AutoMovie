...
Cue는 stable identity, caption·subtitle·non-speech·speaker·song·chapter role, canonical language, exact start-inclusive·end-exclusive film range, Unicode text, source utterance 또는 sound event, speaker, region, lines, direction과 semantic style을 가진다. 다른 목적을 한 track으로 합치지 않고, missing glyph·invalid encoding·mixed language를 replacement character나 default language로 숨기지 않는다.
...
Language version은 canonical language와 필요한 script·region variation, audience·selection role, source content revision, translation·adaptation revision, approval과 fallback policy를 가진다. Source text, translator 또는 provider, translated text, terminology, pronunciation, cultural adaptation, reviewer와 approval을 구분하고 translation이 original을 덮어쓰지 않는다. Unknown 또는 mixed language를 default로 자동 분류하지 않는다.
