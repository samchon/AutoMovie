...
Product는 purpose, camera·view, frame schedule, dimensions, pass, channel schema, data type, background, color treatment와 dependency closure를 가진 독립 identity다. Beauty는 appearance를, structural product는 geometry, depth, normal, pose 또는 semantic identity 같은 선언된 사실을 표현하고 beauty-only light·tone·atmosphere가 structural 값에 섞이지 않는다. 임의 channel은 component order, unit, coordinate space, precision, missing value와 valid region을 명시한다.
