...
Timed ownership resolver는 evidence가 존재하면서 `manifest.kind`가 `null`이면 종류 선택 오류를 던진다. Production open과 design emitter는 이 검사를 project store 생성보다 먼저 실행하고, 생성 lint도 선언을 읽고 검사한 뒤 read-only store를 연다. Evidence 인자를 생략한 호출만 기존 `legacy-film` 호환 경로를 유지한다. 빈 프로젝트의 TypeScript·authored-evidence source lint는 별도로 허용한다.
