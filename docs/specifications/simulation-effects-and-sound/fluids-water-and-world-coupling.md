...
Object interaction 입력은 같은 tick의 world proxy sweep, displacement 또는 authored disturbance와 interaction mode다. 기본은 object가 fluid에 disturbance를 주고 fluid는 splash request, wetness state, drag proxy 또는 sound emission을 출력하는 단방향 coupling이다. 양방향 force feedback은 별도 coupled model, iteration order와 combined budget이 있을 때만 허용한다.
...
Checkpoint는 domain revision, grid order, solver revision, initial field, source/drain schedule, world dependency digest와 absolute tick을 key로 하고 모든 보존 state를 담는다. Seek는 호환 checkpoint 또는 initial field에서 정방향으로만 계산한다. Surface mesh나 spray particle만으로 solver state를 역구성할 수 없다.
...
World snapshot은 film tick, coordinate basis, subject poses, collider proxies, surface/material traits, space/opening state, wind와 gravity-like inputs를 하나의 immutable revision으로 묶는다. One-way consumer는 snapshot을 바꾸지 않고 consequence event를 다음 위상에 출력한다. Iterative coupling은 참여 domain, convergence criterion, maximum iterations, stable order와 failure state를 별도로 선언한다.
...
Snapshot 구성 요소나 interaction mode가 바뀌면 dependent solve, checkpoint와 presentation consequence가 stale이다. Missing basis/unit, open edge와 wall의 중복, 음수 depth, source/drain의 미해결 identity, feedback cycle, 수렴 상한 없음, budget 초과는 domain과 tick을 지목한 실패다. 실패 뒤의 state나 wetnessㆍsound consequence는 complete로 게시하지 않는다.
