---
name: multi-agent
description: Review Cycle, Discussion, Research Review Round. Read the Briefing subagents rule before delegating to any subagent; read in full only when the user asks for a named mode.
---

# Multi-Agent Workflows

Use only when the user explicitly asks for one of the named modes: Review Cycle, Discussion, or Research Review Round. Each mode composes the shared building blocks below; do not invent unnamed combinations. The reference study that precedes a major `interface`/`engine` design (rig model, constraint/driver taxonomy, motion model) runs as a Research Review Round or a Discussion.
...
Subagents start blind: they carry no conversation history and do not auto-load `AGENTS.md` or the skills. Give each a self-contained brief with the objective, the constraints, the context it needs, the output format, which `AGENTS.md` sections (at least `## Attitude`) and `.codex/skills/*/SKILL.md` to read, and which `.wiki/` or `.references/` material is relevant. State the evidence and the constraints, not a pre-chosen answer; a leading hypothesis steers the agent to a shallow conclusion. A subagent runs its brief directly and does not re-delegate.

### Reference materials

Downloaded specs, example models, and motion datasets used as study evidence live under `.references/<source>/` (gitignored). Durable conclusions drawn from them are promoted into `.wiki/04-domain-research/` or `.wiki/06-architecture/`, with the source cited; the raw material stays in `.references/`.
...
Open-ended topic exploration without changing code. No proposals, no validation. Use for reference study and architecture exploration before committing a design.

1. Create the topic directory. Form the team; each builds its knowledge base from `.references/`, `.wiki/`, and external sources.
2. Run the three transcript rounds directly under the topic directory.
3. After `round3.md`, the lead writes agreed conclusions and major open points into `summary.md`, promotes durable findings into `.wiki/`, reports to the user, and waits.
