...
- `test/` (`@motica/test`): the `@nestia/e2e` `DynamicExecutor` program; one scenario per file under `test/src/features/<domain>/`, builders under `features/internal/`.
...
pnpm --filter @motica/test start          # run the test suite
pnpm --filter @motica/test coverage       # run tests under the c8 100% gate
```
