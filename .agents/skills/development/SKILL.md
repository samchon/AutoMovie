...
The measured set is every authored executable TypeScript source in the package, build, configuration, scaffold, playground-script, and typed repository-tool roots declared by `test/src/coverage/measureCoverage.ts`. The measure runs with `--all`, so a source no test imports is reported rather than silently absent; test scenarios and fixtures, generated outputs, declarations, and the established `index.ts` and `bin.ts` entry barrels are the closed exclusions. Measure with `pnpm --filter @automovie/test coverage`: this single typed command runs the c8 suite, prints the inherited gap report, and then refuses any base-to-final touched source file whose complete statements, branches, functions, and lines are not 100% or whose exact source snapshot was not instrumented. c8 writes only under `node_modules/.cache/`; an absolute `/tmp` path silently measured nothing on Windows. Never leave `coverage/` or `.nyc_output/` in the tree, and never paper over them with `.gitignore`.
