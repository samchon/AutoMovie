...
- **A solver lands with the consumer that calls it.** A validated, fully covered fold no product path reaches is a public surface with maintenance cost and no effect on any frame; one cycle shipped three of them past every gate. Wire the producer to its consumer in the same change, register it on a reviewed package README or MCP authoring surface, or mark a deliberately early API with `@publicUnconsumed <planned consumer>: <reason>`. That tag is the declared form, so the next reader is not left parsing prose: the planned consumer names a concrete future component and the reason explains why the API must land before it, while `none`, `unknown`, `TBD`, and equivalent placeholders are invalid. A test proves behavior but remains test reach rather than a product consumer, so test-only reach never counts as wiring. `pnpm run consumer:check` resolves callable package exports to their TypeScript declarations and refuses an export with no repository reference, reviewed authoring document, or valid early-API declaration; it also refuses malformed and stale declarations. That automated zero-reference floor does not turn a test-only reference into product wiring, so review still adjudicates the population listed by `node internals/public-api-consumer-gate.mjs --list-test-only`.
...
- **A configured check is not a running check until it has been made to fail.** A guard you disabled and restored is armed by definition; a guard configured by a selector may never have been armed at all, and it reports the same green either way. `packages/cli/scaffold/lint.config.ts` carried a claim binding every shot to the script scene it realizes, with `symbol: "function"` — and a shot is `export const opening = defineShot(...)`, a `const` initialized with a call, which `@ttsc/evidence` classifies as a `property`. The claim selected no host, a claim with an empty host population is dropped before its references are read, and deleting **every** citation it was supposed to require still reported PASS. It had enforced nothing since the day it was written. The same shape has already cost this repository twice more: a lint probe with no `package.json` produced no diagnostics for anything, and a CI workflow reported success having run zero steps. So when you add or inherit a lint rule, an evidence claim, a coverage threshold, or a CI job, delete the thing it is supposed to catch and watch it go red before you believe the green — and where the check has a population, count what it selected rather than trusting that it selected anything.
