...
- Create worktrees only for parallel implementation batches and their integrated cleanup. Solo implementation, ordinary pull requests, and every Self-Review use the current checkout and one topic branch.
...
The user's phase boundary controls the topology. A multi-agent issue campaign uses parallel discovery and parallel implementation by default. Switch only its implementation to the solo workflow when the user explicitly requests parallel discovery with solo implementation.
