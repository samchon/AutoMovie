...
**Which identifying signal exists at all is a property of the harness.** On the Codex leg a resume turn carries its session UUID on the command line and a first turn does not; on the Claude Code leg, `claude -p --resume <uuid>` carries **neither the working directory nor the sandbox nor the UUID** — a machine running seven `claude.exe` had not one of them naming any sandbox. The anchor that survives there is the product's own MCP server: the `node.exe` whose command line holds the **absolute** sandbox path, walked **up** to its root, with the UUID-named transcript as independent identity evidence. Establish which signal your harness leaves before you need it, rather than discovering the absence during a cleanup.
...
**A convention can manufacture a false absence in two different ways, and both look like a finding.** The case above searched a file the convention keeps empty. The other searches a line the convention splits: this repository writes prose one clause per line, so a grep for a sentence that wraps returns zero on a document that plainly contains it. A driver hit that, read it as a documentation regression, and was one step from filing it — the text was present in all four copies, including the commit it suspected. **Search prose multiline, always**, and when a grep of source returns zero, ask whether the convention puts the value somewhere else before you ask whether the value is gone.

**A verdict about a different process is not a verdict about yours.** `claude mcp list` proved unreliable in both directions in one hour: it reported `Failed to connect` for a configuration that worked in the session that followed, and `Pending approval` for three servers that launched fine in the very next session. Its health check launches its own probe, from its own working directory, without the injection a real session performs. It is a good diagnostic — its warning text is what identified the mechanism above — and it is not a gate. The gate is the session's own per-server tool count, backed by ancestry: an MCP server process that is a direct child of your pinned root is evidence about **this** session, which nothing external can supply.
...
Splitting the axis beats reporting it lost. That driver kept **production legibility** as measurable — the agent was recovering state from the work root's own decision records, source, generated output and guides, which is the question — while recording **blindness to being in an experiment** as spent, and noting that the ceilings question had narrowed rather than died. Three answers where a single verdict would have thrown away two of them.
