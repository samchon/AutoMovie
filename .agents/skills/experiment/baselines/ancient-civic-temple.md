...
(Codex `codex exec`, `gpt-5.6-sol`, `xhigh`). 이 사이클의 전체 기록은 `.wiki/`에 있고
**`.wiki/`는 gitignore된다.** 그래서 다음 실행에 필요한 것만 여기에 옮겨 적는다.
...
그 세션은 도구를 스스로 찾아 두 번 불렀고, 두 번 다
`{"Err":"user cancelled MCP tool call"}`로 서버에 닿기 전에 거절당했으며, **같은 턴에**
...
못한다.** 그것을 선호로 읽는 캠페인은 이 문서가 스스로 단 단서를 지나쳐 읽는 것이다.
대체 결론을 세우지 않는다. 근거만 철회한다.

### 방법 — 0을 읽기 전에 채널의 생존을 먼저 확정한다

MCP 호출 수를 세는 모든 실행에 적용된다. **숫자보다 방법이 오래 간다.**

**MCP 서버는 codex 프로세스의 자식으로 뜬다.** 0이 관측된 **그 순간에** 그 자식이
있었는지가 *죽은 채널*과 *조용한 에이전트*를 가른다.

2026-08-18 실측: 거절이 나던 턴에 서버가 살아 있었다 — PID 13648
(`…/ancient-civic-temple/scripts/mcp.ts`)이 codex PID 10980의 자식. 그래서 이 사이클은
"서버 실패"·"등록 실패"를 배제하고 **"승인 게이트에서 dispatch 전에 거절"**까지 좁힐
수 있었다. `mcp_tool_call_end`의 `duration` 0초가 그것과 일치한다.

`codex mcp list`에 서버가 없는 것은 정상이다(호출 단위 등록은 전역에 쓰지 않는다).
**전역 목록의 부재를 채널의 부재로 읽지 않는다.** 프로세스 표를 본다.

### 죽은 후보 넷 — 다시 시험하지 말 것

2026-08-18 세션에서 거절이 **4회 재현**됐다. 형제 세션(`modern-suburban-house`)은
**같은 머신에서 MCP 호출이 완료**됐고 `user cancelled` 0건이었다. 두 세션이 서로 볼 수
있는 모든 축이 같은데 결과가 갈렸다.

| 후보 | 상태 | 근거 |
| --- | --- | --- |
| `turn_context.approvals_reviewer` 값 차이 | **죽음** | 양쪽 다 `user` |
| `-c sandbox_workspace_write.network_access=true` 유무 | **죽음** | 양쪽 다 넘김 |
| 첫 `codex exec` 호출 vs `resume` | **죽음** | 양쪽 다 첫 호출 |
| `-c approval_policy="on-request"`로 전환 | **죽음** | 여전히 거절 |
| (남은 것) 서버 등록 방식 — bundled vs 호출 단위 | **미검증** | 쫓지 않았다 |

**원인은 `unverified`다.** 이 사이클에서 실제로 고친 것은
`-c approvals_reviewer="auto_review"` 하나이고, **왜 고쳐지는지는 모른다.**
...
| | 이전 실행 (이 문서 본문) | 2026-08-18 사이클 |
| --- | --- | --- |
| 두 턴 ~90분 뒤 기하 | **0** | — |
| 턴 1이 **끝나기 전** `src/world/` | — | **타입 소스 모듈 17개** |
| 표면 분해 | 실행 절반이 지나서야 도달 | **턴 1 안에** 도착 |

명명된 공간마다 모듈 하나(`Shrine`, `Courtyard`, `Colonnade`, `CommunalOffering`,
`Records`, `Administration`, `OfferingStorage`, `ServiceYard`, `Entrance`), **네 개 입면이
각각 독립 소유자**, 그리고 `Dimensions`·`Circulation`·`Ownership`이 분리되어 있다.
브리프가 **S1 종료 시점에** 요구한 분해가 턴 1 안에 나타났다.

이전 실행의 두 턴을 통째로 먹은 것은 스타터 걷어내기였다. **그 벽이 이 사이클에서는
같은 방식으로 먹지 않았다.** 사이클이 무엇을 샀는가에 대한 캠페인 자신의 답이다.
