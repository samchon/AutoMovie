...
- Anything added to a `src` or `docs` population selected by `lint.config.ts` immediately owes its citation, so a new selected file lands with its upstream contract or it does not land.
