...
- `packages/benchmark` (`@automovie/benchmark`): the executable benchmark contracts. Versioned movie task law, sealed immutable run submissions, the nine-gate lifecycle, an append-only oracle trace, the deterministic judge and its verdict taxonomy, calibration anchors, and the registered short/medium/long scenario corpus. Pure law and scoring; it runs no agent and owns no filesystem.
- `packages/benchmark-runner` (`@automovie/benchmark-runner`): the external-workspace and agent-process execution layer for registered benchmark scenarios. It archives task, brief, project, transcripts, sealed submission, trace, taxonomy verdict, aggregate report, and measured MCP tool-inventory comparison under a content-addressed `.benchmarks/<campaign>/<run-id>/` directory outside the source repository.
