...
- `packages/production` (`@automovie/production`): the deterministic production library a generated project runs on: the compiler, the tracked project store, capture, inspection, and the render job. It answers a project's own scripts, not a network surface. The repository hosts no internal LLM or production-action tool server; authoring doctrine remains in the shipped skills, separately from optional Markdown reference navigation.
