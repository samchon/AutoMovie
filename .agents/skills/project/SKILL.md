...
- `packages/cli` (`automovie`): the `automovie` binary that scaffolds and inspects a production repository. `packages/cli/scaffold/` is the blank authoring harness it stamps out; completed regression productions live in repository-only fixtures (see the scaffold skill).
