...
- **A shared tree can type-break.** Root `pnpm test` type-checks the test project before starting the runner; the direct package `start` command does not run a separate type-check. Report another owner's blocking file and error to the main agent rather than editing it, and continue independent work while that owner repairs it.
