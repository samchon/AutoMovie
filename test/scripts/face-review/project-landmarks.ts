...
async function main(): Promise<void> {
  ...
  const profile = JSON.parse(
    await fs.readFile(path.join(preview, "capture-profile.json"), "utf8"),
  );
  const receipt = JSON.parse(
    await fs.readFile(path.join(preview, "capture.json"), "utf8"),
  );
  const bytes = await new NodeIO()
    .registerExtensions(portraitGltfExtensions)
    .writeBinary(portraitDocument(buildReferencePortrait(portraitAssembly)));
  if (
    createHash("sha256").update(bytes).digest("hex") !== receipt.artifact.gltf
  )
    throw new Error(
  ...
  const render = await fs.readFile(path.join(preview, "reference.png"));
  ...
  try {
    ...
    await page
      .locator("main")
      .screenshot({ path: path.join(preview, "landmarks.png") });
  } finally {
    await browser.close();
  }
  await fs.writeFile(
