...
async function main(): Promise<void> {
  ...
  const read = (file: string) => fs.readFile(path.join(preview, file));
  const digest = (bytes: Uint8Array) =>
    createHash("sha256").update(bytes).digest("hex");
  const receipt = JSON.parse((await read("capture.json")).toString());
  const profile = JSON.parse((await read("capture-profile.json")).toString());
  const bytes = await read("reference.png");
  if (
    receipt.artifact.input !== referenceControlNet.inputSha256 ||
    digest(bytes) !==
      receipt.captures.find((c: { name: string }) => c.name === "reference")
        .sha256
  )
    throw new Error("The reference frame and target observation basis differ.");
  ...
  try {
    ...
    const modelBytes = await read("model.json");
    if (digest(modelBytes) !== receipt.artifact.model)
      throw new Error("Named geometry does not belong to the captured frame.");
    ...
    for (const [side, contour] of [
      ...
      if (part?.geometry.type !== "mesh" || part.geometry.mesh.indices === null)
        throw new Error(
    ...
    const measures = (p: number[][]) => {
      ...
      const opening = (a: number, b: number, top: number, bottom: number) => {
        const x = p[b][0] - p[a][0],
          y = p[b][1] - p[a][1];
        return (
          Math.abs(
    ...
    const latest = JSON.parse((await read("capture.json")).toString());
    if (
      latest.artifact.gltf !== receipt.artifact.gltf ||
      latest.artifact.profile !== receipt.artifact.profile
    )
      throw new Error("The preview changed during landmark observation.");
    await fs.writeFile(
