 ...
 * @evidence principles/core/source-units.md#source-evidence-content-conformance The createChorusBreakMotion annotations name the reviewed chorusBreak motion inputs, channels, limits, and terminal state, and the implementation fragment { if (props.id.length === 0 || props.formation.length === 0) throw new Error("CHORUS break ids must be non-empty."); if ( !Number.isFinite(props.start) || is the concrete state or behavior by which this declaration establishes those relationships.
 * @evidenceReview principles/core/source-units.md#source-evidence-content-conformance #b63c520 I reread every citation on createChorusBreakMotion against the whole declaration and its source-local calls; each stated parent, role, refusal, and result is present in code and no annotation claims a different owner.
