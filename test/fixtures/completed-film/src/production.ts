 ...
 * @evidence principles/core/source-units.md#source-evidence-content-conformance The PRODUCTION_BACKGROUND annotations name the settings-owned identity, timing, frame, output, and runtime policy, and the implementation fragment "#182235" is the concrete state or behavior by which this declaration establishes those relationships.
 * @evidenceReview principles/core/source-units.md#source-evidence-content-conformance #b63c520 I reread every citation on PRODUCTION_BACKGROUND against the whole declaration and its source-local calls; each stated parent, role, refusal, and result is present in code and no annotation claims a different owner.
