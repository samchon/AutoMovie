 ...
 * @evidenceReview {@link Component.IPortraitSkinConstraint} Read vertex, XYZ target and reach together in blendPortraitSkin. The fixed map retains requested coordinates while the graph field affects only reachable neighbours; duplicate contradictory targets refuse rather than selecting a component by order.
 ...
 * @evidenceReview {@link Component.IPortraitSkinConstraint.vertex} Checked integral resident-index admission before the fixed target map is updated. The same identity is later retained through subdivision, while inserted vertices receive new indices; negative, fractional and out-of-range IDs are exercised by the attachment scenario.
 ...
 * @evidenceReview {@link Component.portraitFacesInsideLoop} Read the complete wrapper: it passes original host indices and the supplied boundary to selectAutoMovieTriangleRegion. No coordinate projection or new face-centroid classifier is introduced, and returned ordinals belong to that original topology.
 ...
 * @evidenceReview {@link FinalSurface.IPortraitFinalSurfaceHost} Read its four readonly buffers beside the assembler's deeply frozen snapshot. Providers share one object before any result is applied; the final-surface scenario verifies identity and failed writes to positions, indices, groups and normals.
 ...
 * @evidenceReview {@link FinalSurface.IPortraitFinalSurfaceHost.indices} Followed the copied frozen index buffer into the observer and the unchanged original indices in the returned mesh. This stage can prescribe existing vertex positions but does not accept replacement topology in its proposal type.
 ...
 * @evidenceReview {@link FinalSurface.IPortraitFinalSurfaceHost.groups} Read frozen label copying and the nasal/eyelid providers' group-based vertex selection. Labels remain triangle-local even when neighbouring regions share a vertex; they do not permit contradictory owners to overwrite each other.
 ...
 * @evidenceReview {@link FinalSurface.IPortraitFinalSurfaceHost.normals} Traced the single portraitNormals call before provider execution. The buffer is not recomputed after each proposal; the head assembler performs the final common recomputation after positions are accepted and before material extraction.
 ...
 * @evidenceReview {@link FinalSurface.IPortraitFinalSurface} Compared its readonly proposal array with output copying and finite XYZ/resident-index admission. Identical shared requests are compatible; different coordinates at one identity refuse even when both individual requests are finite.
 ...
 * @evidenceReview {@link LowerLid.IPortraitLowerLidProfile} Read the two-to-32 section population, required zero/one endpoints and strict finite progress against the copied resolver. Eye construction chooses the basic path when the optional profile is absent and uses the owned resolver when present; a supplied empty list refuses rather than meaning omission.
 ...
 * @evidenceReview {@link LowerLid.createPortraitLowerLidProfile} Read every admission branch, interval selection and shared smoothstep weight against the independent quarter-interval oracle: weight5/32 gives offset37/32 and projection-3/8. Returned section objects are owned, and the consumer maps the six roles into shared skin rows. Current db7a56b3 and WebGL close inspection still show broad regular pads, so working numerical control is not acceptance of aegyo-sal form.
 ...
 * @evidenceReview {@link NasalLobule.IPortraitNasalLobule} Read the head XYZ/millimetre frame, physical half-extents, core [0,1) domain and optional dz/dx,dz/dy tangent. Separate array members allow asymmetric alae; the section pole is not necessarily the maximum head-Z point when its tangent is inclined. None of these inputs is asserted as a measured cartilage value.
 ...
 * @evidenceReview {@link NasalLobule.createPortraitNasalLobules} Traced the anterior square-root section, affine tangent, cubic identity annulus, normalized overlap and owned datums. The radius-five/radius-three test yields depth four; a half X slope adds 1.5 mm there. The actual component's aperture and lining share the result. The zero-slope be197532 preset was visually rejected for sharp flared alae, so calculation correctness is kept separate from the next inclined preset's pending render.
