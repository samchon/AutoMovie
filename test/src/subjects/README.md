...
These are directly authored AutoMovie model studies. They use `@automovie/interface` model/mesh types, general `@automovie/engine` geometry operations and anatomical builders from [`@automovie/human`](../../../packages/human/README.md). Subject measurements, person-specific settings and review decisions stay here, outside the shared capability package. `human-face-documents/` contains independently replayable numerical studies for the nineteen subjects in issue #2469.
