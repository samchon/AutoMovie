...
The active `portraitAssembly` uses the measured component surface, a fitted CC0 nasal patch and grouped upper dentition. Read `configuration.ts`, `model.ts`, `head.ts`, `nasalReference.ts` and `dentalComponent.ts`. The nasal provider admits its fitted source digest and explicit binding. With `preserveSource`, `portraitMeshPatch.ts` inserts that source after host subdivision and surface layers, preserving its native triangles instead of subdividing the completed nose again. `refinePortraitJoin` subdivides only the new shared annulus. Tangent mode places its first interior row in the neighbouring face planes, fixes those targets and calls `fairPortraitSurface` independently on X/Y/Z using one metric. This preserves native core and shared boundaries but has not prevented visible annulus folds. Position mode retains scalar view-ray fairing. The dental component supplies no skin cut and reads final oral anchors; the mouth's legacy crowns are disabled, giving dentition one owner.
