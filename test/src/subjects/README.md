...
The active `portraitAssembly` uses the measured component surface, a fitted CC0 nasal patch and grouped upper dentition. Read `configuration.ts`, `model.ts`, `head.ts`, `nasalReference.ts` and `dentalComponent.ts`. The nasal provider admits its fitted source digest and explicit binding, then `portraitMeshPatch.ts` attaches it through one shared annulus. `fairPortraitSurface` minimizes the joining region's squared-Laplacian energy with the native core, shared boundaries and neighbouring skin fixed. Only join-interior vertices travel along the recorded view ray. The current bridge improvement does not complete the nasal underside or likeness. The dental component supplies no skin cut and reads final oral anchors; the mouth's legacy crowns are disabled, giving dentition one owner.
