...
The active `portraitAssembly` uses the measured component surface, a fitted CC0 nasal patch and grouped upper dentition. Read `configuration.ts`, `model.ts`, `head.ts`, `nasalReference.ts` and `dentalComponent.ts`. The nasal provider admits its fitted source digest and explicit binding. With `preserveSource`, `portraitMeshPatch.ts` inserts that source after host subdivision and surface layers, preserving its native triangles instead of subdividing the completed nose again. `refinePortraitJoin` subdivides only the new shared annulus. Tangent mode admits a positive XY chart and places a local first row in the neighbouring face planes. `fitPortraitJoinReference` uses the complete donor height surface as its shape basis and applies the existing positive-weight skin relaxation to attachment displacement, with both boundaries and first-row targets fixed. This replaces the rejected XYZ and absolute-height fairing trials; source-relative bounded displacement still requires visual acceptance. Position mode retains scalar view-ray fairing. The dental component supplies no skin cut and reads final oral anchors; the mouth's legacy crowns are disabled, giving dentition one owner.

`measuredPortraitAssembly` selects the current basic subject components with legacy dental placement for comparisons. It is not an immutable checkpoint: changes to shared settings affect it. The complete attributed CC0 [anatomical reference study](reference-anatomy/README.md) and `fittedModel.ts` remain unaccepted whole-face alternatives; only the explicitly bound nasal patch supplies active source geometry. No source photograph is projected onto either model as a texture.
...
The optional [nasal lobule sections](generated-korean-girl-01/nasalLobule.ts) prescribe anterior depth through transverse, vertical and depth extents, retained skin datums and optional local tangents. `rimRefinement` preserves an aperture curve, and [rim sections](generated-korean-girl-01/nasalRimSection.ts) supply an exterior band from actual skin normals. Those presets retained pinches or introduced double outlines and were rejected. The measured construction remains available with the recovered basic nose. The next active trial instead uses [a complete CC0 reference nasal patch](generated-korean-girl-01/nasalReference.ts) through [shared patch attachment](portraitMeshPatch.ts). Its fitted-skin digest and boundary identities are recorded in nasalReferenceBinding.json; the provider checks the current source model, target and fitted skin before using them. The old nasal relief is disabled for that replacement. This is a limited adoption from the attributed prior, not a claim of original sculpting or acceptance of its entire face.
