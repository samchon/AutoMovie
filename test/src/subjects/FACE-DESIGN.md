...
For this directional height solve, cotangent lengths and areas come from the head's anatomical X/Y plane, the same domain used by annular triangulation. This parameterization is distinct from the displacement ray. The unknown initial depth must not alter that operator: an arbitrary crease in an unfinished join is not its intrinsic reference metric. A unit-lattice quadratic height has constant Laplacian and must recover the same centre from different initial depths. Supporting triangles collapsed in anatomical X/Y are outside this height-field representation and refuse. The native cavity core remains outside the free population; this rule does not flatten nasal undercuts into a depth facade.
