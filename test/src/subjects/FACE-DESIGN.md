...
The current smooth aperture still pinches the adjacent skin. The next optional rim section introduces an exterior skin band with a declared physical width and crest projection. The original host cut becomes the band's outer attachment; the fitted aperture is a new inner loop shared with the vestibule. One intermediate crest loop joins them. The group derives the band's outward co-normal from the actual ordered aperture and shares its identities with both surfaces. This is a tissue section with connected topology, rather than a free-standing ring. Omission preserves the existing skin-to-lining attachment exactly. The default crest is an authored hypothesis and the assembled colour/clay result remains the acceptance boundary.
