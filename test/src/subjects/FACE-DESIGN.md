...
The next optional nasal construction replaces the tip/alar inflation layer with three explicit ellipsoidal anterior sections on shared skin. Each has a retained datum, an XYZ apex offset, transverse/vertical/depth radii and a normalized inner extent. Inside that extent the section prescribes actual depth; its outer annulus joins the existing host smoothly. Overlapping sections use normalized weights and one combined host transition, so neither declaration order nor stacked inflation owns the join. Exterior and aperture samples evaluate the same surface before fitting their shared lining. Global depth scaling remains the support basis. These authored dimensions are fitting hypotheses; the subsequent full and close renders decide whether to retain them.
