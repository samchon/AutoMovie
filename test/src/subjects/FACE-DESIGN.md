...
The local nasal fits did not resolve the assembled surface. The next study selects the complete nose from the existing attributed and fitted CC0 skin, including its native tip, alae and recessed openings. A subject-owned boundary identifies that source patch, and another identifies the area replaced on the measured host. The engine selects both regions by connectivity. An annular bridge connects the two ordered boundaries in the shared head frame; its first samples must correspond anatomically. Patch and bridge enter the common mesh before refinement and normals. The basic nasal relief must be omitted for this alternative so it does not reapply a correction from the old nasal basis. This study requires an outer-join and multi-angle review and is not a declaration that the prior's entire face is suitable.
