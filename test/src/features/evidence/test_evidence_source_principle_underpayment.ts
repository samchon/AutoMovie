...
const cleanSource = (): string =>
  [
    ...
    '  readonly id: "source-principle-probe";',
    '  readonly logline: "One deterministic source proves its declared delivery.";',
    "  readonly runtimeSeconds: 12;",
    "  readonly frameFormat: { readonly width: 1920; readonly height: 1080; readonly fps: 24 };",
    '  readonly deliveryMode: "prototype";',
    '  readonly deliverables: readonly ["blocking-pass"];',
    '  readonly settingsOwner: "settings/production.md#delivery";',
    "  readonly visualGrammar: {",
    '    readonly palette: readonly ["#20242b", "#d7dde8"];',
    '    readonly silhouette: "single centered subject";',
    '    readonly scale: "meters";',
    '    readonly materialLanguage: "matte blocking surfaces";',
    '    readonly fidelity: "blocking";',
    "  };",
    "}",
    "",
    "/**",
...
export const test_evidence_source_principle_underpayment = (): void => {
  ...
  try {
    ...
    const falseCitation = replaceOnce(
      complete,
      ` * @evidence ${PRINCIPLES[0]} The type owns only the immutable delivery envelope serialized by the production value.`,
      ` * @evidence ${PRINCIPLES[0]} The type declares a six-second runtime although its runtimeSeconds field is twelve.`,
    );
    fs.writeFileSync(
      source,
      replaceOnce(
