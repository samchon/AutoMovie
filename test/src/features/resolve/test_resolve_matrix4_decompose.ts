 ...
 * 2. 180째 about X ??the `r00`-largest branch.
 * 3. 180째 about Y ??the `r11`-largest branch.
 * 4. 180째 about Z ??the `r22`-largest branch.
 * 5. 180째 about a tilted axis where `r00 > r11` but `r00 ??r22` ??exercises the
...
export const test_resolve_matrix4_decompose = (): void => {
  ...
  roundTrip("180째 X", { x: 0, y: 0, z: 0 }, { x: 1, y: 0, z: 0, w: 0 }, one);
  roundTrip("180째 Y", { x: 0, y: 0, z: 0 }, { x: 0, y: 1, z: 0, w: 0 }, one);
  roundTrip("180째 Z", { x: 0, y: 0, z: 0 }, { x: 0, y: 0, z: 1, w: 0 }, one);

  // 5. 180째 about a tilted axis n with nz짼 > nx짼 > ny짼 (q = (n, 0))
  const n = { x: Math.sqrt(0.3), y: Math.sqrt(0.1), z: Math.sqrt(0.6) };
  roundTrip("180째 tilted", { x: 0, y: 0, z: 0 }, { ...n, w: 0 }, one);
