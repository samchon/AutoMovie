 ...
 * Scenarios:
 *
 * 1. Aiming `?뭒` at a target on `+X` rotates the owner so its aim axis points
...
export const test_resolve_world_drivers = (): void => {
  // 1. aim ?뭒 at +X: aim axis ??+X, up stays up
  ...
  // 4. influence 0 ??unchanged (aim axis still at rest ?뭒)
  const w4 = runAim(
    aim({ influence: 0 }),
    { x: 0, y: 0, z: 0 },
    { x: 5, y: 0, z: 0 },
  ...
  TestValidator.predicate(
    "antiparallel (aim x large) flips to ?뭎",
    vclose(aimedDir(wAntiX, { x: 1, y: 0, z: 0 }), { x: -1, y: 0, z: 0 }, 1e-5),
  );

  // 6. subtree recompose: child follows the rotated owner. Aiming ?뭒 up at a
