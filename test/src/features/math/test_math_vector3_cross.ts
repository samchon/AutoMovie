 ...
 * Scenarios:
 *
 * 1. The three cyclic basis products: X횞Y=Z, Y횞Z=X, Z횞X=Y.
 * 2. Anti-commutativity ??reversing the operands negates the result, so Y횞X=?뭒.
...
export const test_math_vector3_cross = (): void => {
  ...
  TestValidator.equals("X횞Y=Z", Vector3.cross(X, Y), Z);
  TestValidator.equals("Y횞Z=X", Vector3.cross(Y, Z), X);
  TestValidator.equals("Z횞X=Y", Vector3.cross(Z, X), Y);
