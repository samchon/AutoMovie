 ...
 * 2. Opposite hemisphere (identity ??Y200, dot = cos 100째 < 0): the implementation
 *    negates `b` to take the shorter arc, so the t=0.5 result is Y(??0째) ??the
 *    short-way midpoint ??not the long-way Y100. Pins that slerp never takes
 *    the long way around. (Endpoints still hold up to sign, since q and ?뭧 are
 *    the same rotation.)
 * 3. Near-parallel (identity ??Y0.4, dot ??0.99999 > 0.9995): slerp falls back to
 *    a normalized lerp to avoid dividing by sin 罐 ??0; the midpoint is still ?? *    Y0.2 to a looser tolerance.
 */
export const test_math_quaternion_slerp = (): void => {
  ...
  // 2. opposite hemisphere ??shorter arc means the midpoint is Y(??0째)
  const far = yaw(200);
  TestValidator.predicate(
  ...
  TestValidator.predicate(
    "opposite: midpoint takes short arc ??Y(??0)",
