 ...
 * 4. One candidate cannot be produced by two analyses, so a reading has exactly
 *    one origin story.
 ...
 * 7. A generated output digest is checked against the current bytes, and the check
 *    is skipped exactly when recorded processing has replaced them.
 * 8. Citations resolve to real documents and real candidates, may deliberately
 *    cite two competing readings at once, and are refused when they dangle,
 *    repeat, or carry no reason.
 * 9. Sixty-one malformed observations are each refused at their own path.
 * 10. The untouched fixture produces no violation path at all.
 */
export const test_architecture_design_reference = (): void => {
  ...
  // 5-7. The generated acquisition ledger.
  TestValidator.equals(
