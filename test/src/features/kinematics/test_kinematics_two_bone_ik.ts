 ...
 * Scenarios:
 *
 * 1. Right-angle reach: equal unit bones to distance √2 ⇒ bend 90°, lift 45°, in
 *    reach.
 * 2. Fully extended at max reach (distance = sum) ⇒ bend 180°, lift 0°, not
 *    clamped (the shell is inclusive).
 * 3. Distance 0 with equal bones ⇒ fully folded (bend 0°), lift short-circuits to
 *    0 (the d===0 guard).
 * 4. Goal beyond reach (distance > sum) ⇒ clamped, bend 180° (extended).
 * 5. Goal inside the inner shell (distance < |upper−lower|) ⇒ clamped, bend 0°
 *    (folded).
 */
