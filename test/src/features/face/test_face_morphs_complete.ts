 ...
 * per `AutoMovieFaceParameterName`, each aligned to the canonical vertex
 * count), so a face asset baked from it answers every legal parameter document
 * and nothing else. Paired features (eyes, brows, cheeks) appear once per
 * side.
 *
 * Scenario: default (canonical) build yields the 25 expected keys with
 * 468·3-long deltas.
 */
export const test_face_morphs_complete = (): void => {
