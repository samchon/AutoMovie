 ...
 */
export const test_production_legacy_import = isolatedFileSystemTest(
  (fileSystem): void => {
    ...
    try {
      ...
      const planNestedPath = path.join(
        fixture.root,
        "actors/archive/README.txt",
      );
    ...
    try {
      ...
      fileSystem.rmSync = ((target: fs.PathLike, ...args: unknown[]): void => {
        ...
        if (
          path
            ...
            .startsWith("automovie-legacy-import-")
        )
      ...
      const belongsToPlanningTemporary = (file: fs.PathLike): boolean =>
        path
          ...
          .split(path.sep)
          .some((component) =>
            component.startsWith("automovie-legacy-import-"),
          );
      fileSystem.writeFileSync = ((
        file: fs.PathOrFileDescriptor,
        ...args: unknown[]
    ...
    try {
      const publicationFailure = new Error(
        "injected import publication failure",
      );
    ...
    let missingPreexistingSourceFailure:
      | ILegacyImportFixtureFailure
      | undefined;
    try {
    ...
    try {
      ...
      let deniedImportStateLstatFailure:
        | ILegacyImportFixtureFailure
        | undefined;
      try {
        TestValidator.equals(
    ...
    try {
      ...
      let caught: unknown;
      let restorationCleanupHookFailure:
        | ILegacyImportFixtureFailure
        | undefined;
      try {
    ...
    try {
      ...
      fileSystem.renameSync = ((
        ...
        if (
          path
            .basename(oldPath.toString())
            .startsWith(".automovie-rollback-") &&
          path.resolve(newPath.toString()) === stateRoot
        )
          return;
        nativeRename(oldPath, newPath);
      }) as typeof fs.renameSync;
      let preservedQuarantineHookFailure:
        | ILegacyImportFixtureFailure
        | undefined;
      try {
        TestValidator.predicate(
          ...
          TestValidator.equals(
            ...
            namedFacts([
              ...
              [
                ...
                () =>
                  fs.existsSync(
                    path.join(replacementTarget, "revision.lock"),
                  ) === false,
              ],
    ...
    try {
      fs.writeFileSync(
        path.join(collidingCase.root, "actors/Officer.txt"),
        "A",
      );
      fs.writeFileSync(
        path.join(collidingCase.root, "actors/officer.txt"),
