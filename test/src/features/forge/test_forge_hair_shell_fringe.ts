 ...
 * narrows the face opening so a mid-side strand (yaw ??50째) falls with the back
 * instead of stopping at fringe height. The strand grid is column-major (yaw 횞
 * rows), so each column's last row is its tip.
 *
 * Scenario: front-column tip lower at bangs 1 than bangs 0; the yaw??0째
