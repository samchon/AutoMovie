 ...
 * lifting one corner to z = 1 and hand-running one 貫|關 iteration at factor 0.6
 * gives: 貫 ??v0 1??.7, v1/v3 0??.15, v2 0??.1; 關 ??v0 0.7 +
 * (??.318)((0.15+0.1+0.15)/3 ??0.7) = 0.8802. Oracle by hand.
 *
...
export const test_forge_taubin_boundary = (): void => {
  ...
  TestValidator.predicate("corner 貫|關 step", nclose(smoothed[2]!, 0.8802));
};
