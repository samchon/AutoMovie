...
export const test_mcp_production_compiler = async (): Promise<void> => {
  ...
  try {
    ...
    const writeOutputVerificationReadGenerated = project.readGeneratedFile;
    let writeOutputRaceInjected = false;
    project.readGeneratedFile = ((relativePath: string) => {
      if (
        writeOutputRaceInjected === false &&
        new Error("write output verification stack").stack?.includes(
          "assertGeneratedOutputCurrent",
        )
      ) {
        fs.writeFileSync(generatedShot, "{}\n");
        writeOutputRaceInjected = true;
      }
      return writeOutputVerificationReadGenerated.call(project, relativePath);
    }) as typeof project.readGeneratedFile;
    fs.writeFileSync(sourcePath, `${original}\n// guarded output race\n`);
    const writeOutputRace = compiler.compile({ scope: "source" });
    project.readGeneratedFile = writeOutputVerificationReadGenerated;
    fs.writeFileSync(sourcePath, original);
    TestValidator.predicate(
