...
/**
 * Every production-oracle lifecycle whose protected cleanup restores exactly
 * one injected project method.
 *
 ...
 */
export const productionOracleSingleHookContract = (text: string): unknown => {
  ...
  const singleResource = (block: ts.Block): boolean => {
    ...
    if (
      ...
      return false;
    return /^\{resource:"[^"]+",cleanup:\(\)=>\{project\.[A-Za-z]+=[\s\S]*;\},\}$/u.test(
  ...
  return {
    ...
    rawFinalizers: [
      ...text.matchAll(/finally\s*\{\s*project\.verifiedRenderManifest\s*=/gu),
    ].length,
  };
};
...
export const test_mcp_production_oracle_single_hook_cleanup = (): void => {
  const primaryFailure = { phase: "production oracle regression" };
  const restorationFailure = { phase: "production oracle restoration" };
  ...
  TestValidator.equals(
    ...
    productionOracleSingleHookContract(
      fs.readFileSync(
  ...
  lifecycles: [
    {
      ...
      finallyBodies: [
        'preserveProductionOracleHookCleanup(forgedLedgerFailure,[{resource:"forgedledgermanifestoverride",cleanup:()=>{project.verifiedRenderManifest=residentVerifiedRenderManifestForForgery;},},]);',
      ],
      index: 16,
      prefixes: [
