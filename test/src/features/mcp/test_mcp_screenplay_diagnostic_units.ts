 ...
 * type checking, and avoids depending on a pre-existing `packages/mcp/lib`
 * build in a clean checkout.
 */
const proseUnits = require(
  path.resolve(
    __dirname,
    "../../../../packages/mcp/src/production/screenplayProseDiagnostics.ts",
  ),
) as {
...
const timingUnits = require(
  path.resolve(
    __dirname,
    "../../../../packages/mcp/src/production/screenplayTimingDiagnostics.ts",
  ),
) as {
 ...
 * disagreement.
 */
export const test_mcp_screenplay_diagnostic_units = (): void => {
