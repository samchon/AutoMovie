...
export const test_mcp_project_manifest_race_cleanup = (): void => {
  ...
  TestValidator.equals(
    ...
    ),
    {
      ...
      policy: {
        bodies: [
          '{constcleanupFailures:Array<{error:unknown;resource:string}>=[];for(constresourceofresources)try{resource.cleanup();}catch(error){cleanupFailures.push({error,resource:resource.resource});}if(cleanupFailures.length===1&&failure===undefined)throwcleanupFailures[0]!.error;if(cleanupFailures.length!==0)thrownewProjectManifestRaceCleanupError([...(failure===undefined?[]:[failure.error]),...cleanupFailures.map((entry)=>entry.error),],`Project-manifestracecleanupfailed${failure===undefined?"":"aftertheracefailed"}:${cleanupFailures.map((entry)=>entry.resource).join(",")}.`,);}',
        ],
