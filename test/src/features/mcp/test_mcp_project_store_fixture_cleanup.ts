...
export const test_mcp_project_store_fixture_cleanup = (): void => {
  ...
  TestValidator.equals(
    ...
    ),
    {
      ...
      policy: {
        bodies: [
          "{try{cleanup();}catch(cleanupFailure){if(failure===undefined)throwcleanupFailure;thrownewProjectStoreFixtureCleanupError([failure.error,cleanupFailure],`Project-store$" +
