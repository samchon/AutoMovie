...
export const test_mcp_project_store_plumbing_fixture_cleanup = (): void => {
  ...
  TestValidator.equals(
    ...
    ),
    {
      ...
      policy: {
        bodies: [
          "{try{cleanup();}catch(cleanupFailure){if(failure===undefined)throwcleanupFailure;thrownewProjectStorePlumbingFixtureCleanupError([failure.error,cleanupFailure],`Project-store$" +
            "{resource}fixturecleanupfailedafterthetestfailed.`,);}}",
        ],
