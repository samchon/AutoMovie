...
/**
 * Every project-state lifecycle whose protected cleanup restores exactly one
 * injected resource, including an override installed on a shared prototype.
 *
 ...
 */
export const projectStateSingleHookContract = (text: string): unknown => {
  ...
  const singleResource = (block: ts.Block): boolean => {
    ...
    if (
      ...
      return false;
    return /^\{resource:"[^"]+",cleanup:\(\)=>\{[\s\S]+;\},\}$/u.test(
  ...
  return {
    ...
    rawFinalizers: [
      ...text.matchAll(
        /finally\s*\{\s*AutoMovieProductionProject\.prototype\./gu,
      ),
    ].length,
  };
};
...
export const test_cli_project_state_single_hook_cleanup = (): void => {
  const primaryFailure = { phase: "project state regression" };
  const restorationFailure = { phase: "project state restoration" };
  ...
  TestValidator.equals(
    ...
    projectStateSingleHookContract(
      fs.readFileSync(
  ...
  lifecycles: [
    {
      ...
      finallyBodies: [
        'preserveProjectStateHarnessCleanup(invalidManifestFailure,[{resource:"invalidgenerated-manifestprototypeoverride",cleanup:()=>{AutoMovieProductionProject.prototype.generatedManifest=generatedManifestMethod;},},]);',
      ],
      index: 57,
      prefixes: [
