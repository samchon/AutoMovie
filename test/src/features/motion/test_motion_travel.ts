 ...
 * linearly with elapsed time (`velocity 쨌 t`) is added, so the figure glides
 ...
 *    keyframes, not 6) and whose root Z is exactly velocity쨌time at every
 ...
 *    45째-yaw base with a further 90째 yaw lands the root at 135째 yaw.
 */
export const test_motion_travel = (): void => {
  ...
  // times are 0, 1, 2, 3 and root Z = 0.5 횞 time at each
  ...
  TestValidator.predicate(
    "root Z = velocity 횞 time at every keyframe",
  ...
  const bob: IautomovieTransform = {
    ...
    rotation: { x: 0, y: 0.3826834, z: 0, w: 0.9238795 }, // ~45째 yaw
    scale: { x: 1, y: 1, z: 1 },
  };
