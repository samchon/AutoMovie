 ...
 * Pinned by hand-computed semi-implicit Euler: `v += (k쨌(target?뭯) ??c쨌v)쨌dt; x
 * += v쨌dt`.
 *
 * Scenarios:
 *
 ...
 *    (k=20, x=0?뭪arget 10, dt=0.1 ??v=20, x=2).
...
export const test_motion_damped_spring = (): void => {
  ...
  TestValidator.predicate("velocity = k쨌?쨌dt", nclose(b.velocity, 20));
  TestValidator.predicate("value = v쨌dt", nclose(b.value, 2));
