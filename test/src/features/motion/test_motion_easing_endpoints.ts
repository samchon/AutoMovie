...
/**
 * Every continuous easing curve must map 0?? and 1?? and clamp inputs outside
 ...
 * at t=0 and 1 at t=1, and clamps a below-range input (??) to 0 and an
 * above-range input (2) to 1.
 */
export const test_motion_easing_endpoints = (): void => {
  const curves: automovieEasing[] = ["linear", "easeIn", "easeOut", "easeInOut"];
